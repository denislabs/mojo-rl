...
struct NStepSARSAAgent(TabularAgent, Copyable, Movable, ImplicitlyCopyable):
    ...
    fn _update_at_tau(mut self, tau: Int):
        ...
        if tau + self.n < self.T:
            ...
            if bootstrap_idx < len(self.states) and bootstrap_idx < len(self.actions):
                var bootstrap_state = self.states[bootstrap_idx]
                var bootstrap_action = self.actions[bootstrap_idx]
                var power = self.n
                var discount = self._power(self.discount_factor, power)
                G += discount * self.q_table.get(bootstrap_state, bootstrap_action)
    ...
    ) -> Tuple[NStepSARSAAgent, TrainingMetrics]:
        """Train an N-Step SARSA agent on the given environment.
