"""MuJoCo-style Generalized Coordinates types - Model/Data separation.
...
# =============================================================================
# Helper for array sizing (avoid zero-size arrays)
# =============================================================================
...
struct ModelGC[
    ...
    var body_half_length: InlineArray[Scalar[Self.DTYPE], Self.NBODY]  # For capsules
    # Box half-extents
    ...
    fn __init__(
        ...
        for i in range(Self.NBODY * 3):
            self.body_inertia[i] = Scalar[Self.DTYPE](0.004)  # Default sphere inertia
            self.body_inv_inertia[i] = Scalar[Self.DTYPE](250.0)

        # Initialize joints
        self.joints = InlineArray[JointDef[Self.DTYPE], _max_one[Self.NJOINT]()](
            uninitialized=True
        )
    ...
    fn set_body(
        ...
        inertia: Tuple[Scalar[Self.DTYPE], Scalar[Self.DTYPE], Scalar[Self.DTYPE]],
        ...
        self.body_inv_inertia[body_id * 3 + 0] = Scalar[Self.DTYPE](1.0) / inertia[0]
        self.body_inv_inertia[body_id * 3 + 1] = Scalar[Self.DTYPE](1.0) / inertia[1]
        self.body_inv_inertia[body_id * 3 + 2] = Scalar[Self.DTYPE](1.0) / inertia[2]
...
struct DataGC[
    ...
    var qfrc: InlineArray[Scalar[Self.DTYPE], _max_one[Self.NV]()]  # Applied forces
    ...
    # Contacts
    var contacts: InlineArray[ContactInfoGC[Self.DTYPE], _max_one[Self.MAX_CONTACTS]()]
