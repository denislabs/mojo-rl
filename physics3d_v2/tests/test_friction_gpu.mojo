...
Run with:
    cd mojo-rl && pixi run -e apple mojo run physics3d_v2/tests/test_friction_gpu.mojo
"""
...
fn test_sphere_sliding_to_stop_gpu() raises -> Bool:
    ...
    # Get results
    var vel = get_body_velocity[DTYPE, NUM_BODIES, MAX_CONTACTS](host_state, env=0, body=0)
...
fn test_cpu_gpu_friction_comparison() raises -> Bool:
    ...
    var model_cpu = Model[DTYPE_CPU, NUM_BODIES, MAX_CONTACTS](
        gravity_z=-9.81, timestep=dt, ground_z=0.0, restitution=0.0, friction=friction_coef
    )
    ...
    var gpu_vel = get_body_velocity[DTYPE_GPU, NUM_BODIES, MAX_CONTACTS](host_state, env=0, body=0)
    var gpu_pos = get_body_position[DTYPE_GPU, NUM_BODIES, MAX_CONTACTS](host_state, env=0, body=0)
    ...
    print("  CPU: pos=(", cpu_pos[0], ",", cpu_pos[1], ",", cpu_pos[2], "), speed=", cpu_speed)
    print("  GPU: pos=(", gpu_pos[0], ",", gpu_pos[1], ",", gpu_pos[2], "), speed=", gpu_speed)
