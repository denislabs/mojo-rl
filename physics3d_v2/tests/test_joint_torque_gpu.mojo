...
fn test_gpu_cpu_torque_parity() raises:
    ...
    for _ in range(100):
        ImpulseIntegrator.step_gpu[DType.float32, NUM_BODIES, MAX_CONTACTS, MAX_JOINTS, BATCH](
            ...
            dt=Float32(0.001), gravity_z=Float32(0.0), ground_z=Float32(-10.0),
...
fn test_gpu_batched_torque() raises:
    ...
    for env in range(BATCH):
        ...
        state_host[j0 + JOINT_IDX_ANCHOR_PZ] = 1.0
        state_host[j0 + JOINT_IDX_AXIS_Y] = 1.0
        state_host[j0 + JOINT_IDX_TARGET_TORQUE] = Float32(env) * 0.5  # 0, 0.5, 1.0, ...
        state_host[j0 + JOINT_IDX_TORQUE_LIMIT] = 100.0
    ...
    for _ in range(50):
        ImpulseIntegrator.step_gpu[DType.float32, NUM_BODIES, MAX_CONTACTS, MAX_JOINTS, BATCH](
            ctx, state_buf, model_buf,
            dt=Float32(0.01), gravity_z=Float32(0.0), ground_z=Float32(-10.0),
            restitution=Float32(0.0), friction=Float32(0.0),
        )
    ctx.synchronize()
    ...
    for env in range(BATCH):
        ...
        if env < 5:
            print("    Env", env, "(torque =", Float32(env) * 0.5, "): wy =", wy)
    ...
    print("  ...")
    print("  All environments show increasing angular velocity:", all_increasing)
