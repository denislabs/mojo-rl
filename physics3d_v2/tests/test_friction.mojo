...
Tests Coulomb friction implementation for the physics3d_v2 engine.
...
Run with:
    cd mojo-rl && pixi run mojo run physics3d_v2/tests/test_friction.mojo
"""
...
fn test_sphere_sliding_to_stop() -> Bool:
    ...
    print("\nResults:")
    print("  Final velocity: (", data.velocities[0], ",", data.velocities[1], ",", data.velocities[2], ") m/s")
    ...
    if passed:
        ...
        if not stopped:
            print("FAILED: Sphere did not stop (final speed =", final_speed, "m/s)")
