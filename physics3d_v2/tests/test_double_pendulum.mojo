...
fn compute_kinetic_energy(
    vx0: Float64, vy0: Float64, vz0: Float64, mass0: Float64,
    vx1: Float64, vy1: Float64, vz1: Float64, mass1: Float64,
) -> Float64:
...
fn compute_potential_energy(z0: Float64, mass0: Float64, z1: Float64, mass1: Float64, g: Float64) -> Float64:
    ...
    for step in range(max_steps):
        ...
        if step % sample_interval == 0 or step == max_steps - 1:
            ...
            if step % (sample_interval * 2) == 0:
                ...
                print(
                    "  t =", t, "s: L1_err =", length_error_1 * 1000.0,
                    "mm, L2_err =", length_error_2 * 1000.0, "mm"
                )
...
fn test_double_pendulum_energy_stability() -> Bool:
    ...
    var initial_KE = compute_kinetic_energy(0.0, 0.0, 0.0, mass, 0.0, 0.0, 0.0, mass)
    ...
    for step in range(max_steps):
        ...
        if step % sample_interval == 0 or step == max_steps - 1:
            ...
            var KE = compute_kinetic_energy(vx0, vy0, vz0, mass, vx1, vy1, vz1, mass)
            ...
            if step % (sample_interval * 4) == 0:
                var t = Float64(step) * dt
                var energy_change_pct = (E - initial_E) / abs_val(initial_E) * 100.0
                print("  t =", t, "s: E =", E, "J (change:", energy_change_pct, "%)")

    _ = max_energy - min_energy  # energy_range - not used but computed for verification
    var max_change = max_val(abs_val(max_energy - initial_E), abs_val(min_energy - initial_E))
...
fn test_double_pendulum_motion() -> Bool:
    ...
    for step in range(max_steps):
        ...
        if (prev_x1 <= 0.0 and curr_x1 > 0.0) or (prev_x1 >= 0.0 and curr_x1 < 0.0):
    ...
    var passed = zero_crossing_count >= 4 and amplitude > 0.01 and amplitude < 5.0
...
fn test_double_pendulum_chaotic_sensitivity() -> Bool:
    ...
    var initial_angle_2 = (initial_angle_deg + 0.1) * PI / 180.0  # +0.1 degree difference
    ...
    var model1 = Model[DTYPE, NUM_BODIES, MAX_CONTACTS, MAX_JOINTS](
        gravity_z=Scalar[DTYPE](-G), timestep=Scalar[DTYPE](dt),
        ground_z=Scalar[DTYPE](-10.0), restitution=Scalar[DTYPE](0.0),
    )
    ...
    data1.set_body_position(0, Scalar[DTYPE](body0_x_1), Scalar[DTYPE](0.0), Scalar[DTYPE](body0_z_1))
    data1.set_body_position(1, Scalar[DTYPE](body1_x_1), Scalar[DTYPE](0.0), Scalar[DTYPE](body1_z_1))
    ...
    data2.set_body_position(0, Scalar[DTYPE](body0_x_2), Scalar[DTYPE](0.0), Scalar[DTYPE](body0_z_2))
    data2.set_body_position(1, Scalar[DTYPE](body1_x_2), Scalar[DTYPE](0.0), Scalar[DTYPE](body1_z_2))
