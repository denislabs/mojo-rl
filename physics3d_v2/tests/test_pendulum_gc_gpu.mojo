...
fn main() raises:
    ...
    var expected_period = Float32(2.0) * Float32(pi) * sqrt(I_pivot / (mass * g * L))
    ...
    print("  Initial angle:", initial_angle, "rad (~", initial_angle * 180.0 / Float32(pi), "deg)")
    ...
    # Create host buffers
    var state_host = create_gc_state_buffer[DTYPE, NQ, NV, NBODY, MAX_CONTACTS, BATCH](ctx)
    ...
    copy_data_to_buffer[DTYPE, NQ, NV, NBODY, NJOINT, MAX_CONTACTS](data, state_host, 0)
    ...
    for step in range(num_steps):
        # Run one step on GPU
        SemiImplicitEulerIntegrator.step_gpu[DTYPE, NQ, NV, NBODY, NJOINT, MAX_CONTACTS, BATCH](
            ctx,
            state_buf,
            model_buf,
            Scalar[DTYPE](dt),
    ...
    if zero_crossings >= 2:
        ...
        var measured_period = (last_crossing_time - first_crossing_time) / Float32(num_measured_periods)
        var period_error_pct = abs_f32(measured_period - expected_period) / expected_period * 100.0
        ...
        print("  PERIOD TEST: FAILED (not enough zero crossings:", zero_crossings, ")")
