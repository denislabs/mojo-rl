...
struct LinearReLU[in_dim: Int, out_dim: Int](Model):
    """Fused Linear + ReLU layer: y = max(0, x @ W + b).

    This fused layer eliminates:
    - 1 kernel launch (Linear + ReLU -> single kernel)
    - 1 global memory write/read (Linear output -> ReLU input)
    ...
    Cache layout (different from separate Linear + ReLU):
    - cache: [input (in_dim) | pre_activation (out_dim)] per sample
    - input is needed for dW computation
    - pre_activation is needed for ReLU backward (gradient mask)

    PARAM_SIZE = in_dim * out_dim + out_dim
    CACHE_SIZE = in_dim + out_dim (input + pre_activation per sample)
    """
    ...
    comptime CACHE_SIZE: Int = Self.IN_DIM + Self.OUT_DIM  # input + pre_activation
    ...
    fn forward[
        ...
        """Forward pass: output = max(0, input @ W + b).

        Caches input and pre-activation for backward pass.
    ...
    fn forward_kernel_impl[
        ...
        """Fused forward kernel: y = max(0, x @ W + b) with caching.

        Uses tiled matrix multiplication with shared memory.
        ReLU is applied inline after computing each output element.
        ...
        # Write result with fused ReLU
        if global_row < BATCH and global_col < Self.OUT_DIM:
            # Cache pre-activation for ReLU backward (after IN_DIM in cache)
            cache[global_row, Self.IN_DIM + global_col] = acc
            # Apply ReLU inline
    ...
    fn backward_fused_kernel_impl[
        ...
        """Fused backward kernel: computes dx, dW, and db with ReLU gradient.

        The ReLU gradient mask (pre_activation > 0) is applied to grad_output
        before computing Linear gradients.

        Cache layout: [input (IN_DIM) | pre_activation (OUT_DIM)] per sample
        ...
        if block_y < dx_grid_y:
            # ================================================================
            # dx computation: grad_input = (grad_output * relu_mask) @ W.T
            # ================================================================
            ...
            for tile_idx in range((Self.OUT_DIM + TILE - 1) // TILE):
                # Load grad_output * relu_mask into shared_A
            ...
            # ================================================================
            # dW computation: dW = input.T @ (grad_output * relu_mask)
            ...
            for tile_idx in range(num_tiles):
                ...
                # Load grad_output * relu_mask tile
                var dy_row = tile_idx * TILE + local_row
                if dy_row < BATCH and global_col < Self.OUT_DIM:
                    var grad_val = grad_output[dy_row, global_col]
                    var pre_act = cache[dy_row, Self.IN_DIM + global_col]
                    var masked_grad = grad_val if pre_act > 0 else 0
    ...
    fn backward[
        ...
        for batch in range(BATCH):
            ...
            # db += sum(dy * relu_mask, axis=0)
            for j in range(Self.out_dim):
                var pre_act = cache[batch, Self.in_dim + j]
                var masked_grad = grad_output[batch, j] if pre_act > 0 else 0
                grads[db_offset + j] = grads[db_offset + j] + masked_grad
