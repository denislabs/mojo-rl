...
struct PrioritizedReplayBuffer[
    ...
    fn __init__(
        ...
        self.next_obs = InlineArray[Scalar[Self.dtype], Self.capacity * Self.obs_dim](fill=0)
        self.dones = InlineArray[Scalar[Self.dtype], Self.capacity](fill=0)
        self.tree = InlineArray[Scalar[Self.dtype], 2 * Self.capacity - 1](fill=0)
        self.size = 0
        self.ptr = 0
        self.alpha = alpha
        self.beta = beta
    ...
    fn _compute_priority(self, td_error: Scalar[Self.dtype]) -> Scalar[Self.dtype]:
    ...
    fn update_priorities[batch_size: Int](
        mut self,
    ...
    fn sample[batch_size: Int](
        ...
        var max_weight = (Scalar[Self.dtype](self.size) * min_prob) ** (-self.beta)

        # Stratified sampling
        for b in range(batch_size):
            var low = segment_size * Scalar[Self.dtype](b)
            var high = segment_size * Scalar[Self.dtype](b + 1)
            var target = low + random_float64().cast[Self.dtype]() * (high - low)
            ...
            var weight = ((Scalar[Self.dtype](self.size) * prob) ** (-self.beta)) / max_weight
    ...
    fn anneal_beta(mut self, progress: Scalar[Self.dtype], beta_start: Scalar[Self.dtype] = 0.4):
        ...
        """
        self.beta = beta_start + progress * (Scalar[Self.dtype](1.0) - beta_start)
