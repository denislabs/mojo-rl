...
Vendored verbatim from the legacy `noeira.nn.training.scheduler` module during
the nn re-architecture so that `noeira.experimental.pcn` carries no dependency
