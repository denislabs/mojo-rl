"""Host-side scalar Gaussian RNG (Box-Muller).

Self-contained, CPU-only helpers used by the linear/tabular continuous
agents (`noeira/agents/{sac,ddpg,td3}`). Ported verbatim from the legacy
`nn/gpu/random.mojo` ahead of the legacy-`nn` sunset.

Lives inside the `nn/random/` package (alongside `box_muller`) rather than a
sibling `nn/random.mojo` module — a sibling file shadows the package, which
breaks `from noeira.nn.random.box_muller import ...` resolution.
"""
