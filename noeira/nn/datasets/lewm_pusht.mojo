...
(~13 GB) over ``noeira/io/http.mojo``, decompressing zstd ON THE WAY to
``~/.cache/noeira/lewm_pusht/pusht_expert_train.h5`` (~47 GB). All subsequent
loads read that file through the libhdf5 FFI in ``noeira/io/hdf5``.

⚠ NO PYTHON, and no ``huggingface_hub`` / ``zstandard``. Both the transfer and
the decompression happen inside ``native/nra_http.c`` (libcurl + libzstd),
...
— but the documented shapes were. Corrected after ``noeira.data``'s foreign
...
from noeira.io.fileio import file_size, remove_file, rename_over
from noeira.io.hf import HF_DATASET, hf_client, hf_file_size, noeira_cache
from noeira.nn.core.ptr import mptr, untracked


comptime _HF_REPO = "quentinll/lewm-pusht"
comptime _HF_FILE = "pusht_expert_train.h5.zst"
comptime _CACHE_SUBDIR = ".cache/noeira/lewm_pusht"
...
def _ensure_dataset_cached() raises -> String:
    ...
    """
    var cache = noeira_cache() + "/lewm_pusht"
        ...
        *,
        ...
        Args:
            ...
            path: Optional override of the cached H5 path. Empty string
                ...
                ``~/.cache/noeira/lewm_pusht/``. Used by tests to point
