...
**Backed by `noeira.data` since 2026-08-05 (4d batch 2).** The wrapper itself
...
method-body `comptime if` — the nn carry-both idiom, confined to one
...
struct AnyReplay[
    ...
    capacity option (see `noeira/data/quantize.mojo`). CPU backend ignores it
