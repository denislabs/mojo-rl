...
`C51Trainer`). The CNN config needed no obs-pipeline change either: nn
...
from noeira.nn.primitives.dueling_head import DuelingHead
...
nn `Conv2D`/`Flatten` expose FLAT `IN_DIMS`/`OUT_DIM` (Conv2D's
...
struct DQNCNNConfig[
    ...
    `DQNTrainer` / `DQNAgent` / discrete off-policy driver — nn `Conv2D`/
