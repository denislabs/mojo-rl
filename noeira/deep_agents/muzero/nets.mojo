"""MuZero networks (nn) — the learned model h / g / f.

Three torsos, one per MuZero function, ported from the legacy
``deep_agents/muzero/configs.mojo`` MLP config onto nn ``Sequential`` /
...
from noeira.nn.primitives.min_max_norm import MinMaxNorm
...
from noeira.nn.constants import LAYOUT_NCHW
...
# scaling gradient — see the MLP `MZRepNet` note above). nn `Conv2D`/`Flatten`
...
# BN-free preserves the safe params-only promotion. nn `Conv2D`/`Flatten` expose
