...
`native/nra_qhull.c` is a transcription of `MakeGraph`'s extraction loop —
...
`noeira/io/serial/native.mojo` records at length: **`mojo run`'s JIT does not
...
`libnra_qhull.dylib` beside it**, exactly as the imgui viewers need
...
def _lib_name() -> String:
    comptime if CompilationTarget.is_macos():
        return String("libnra_qhull.dylib")
    else:
        return String("libnra_qhull.so")
...
def _init_qhull_handle() -> OwnedDLHandle:
    ...
    abort(
        ...
        + " NOEIRA_QHULL_LIB=/path/to/"
        + _lib_name()
    )


comptime lib = _Global["NOEIRA_QHULL", _init_qhull_handle]()
...
def qhull_faces(
    ...
    if r < 0:
        ...
        raise Error("nra_qhull_faces failed: " + why)
    return r
...
def poly_order(
    ...
    stretch_3 meshes. See `native/nra_polyorder.cc`.
    ...
    if r < 0:
        raise Error(
            "nra_poly_order rejected its arguments (nkeys "
            + String(nkeys) + ")"
        )
    return r
