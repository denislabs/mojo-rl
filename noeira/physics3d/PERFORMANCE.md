...
**Today `noeira/physics3d/` contains no explicit SIMD at all** — no
...
**The Ant env** (`noeira/envs/ant/`) now resets from z 0.75 with every
...
Aug 27 tree's own copy (`3f3a3763`, the kernel before any of it):
    ...
    git worktree add /workspace/noeira-aug27 3f3a3763
    time timeout 600 pixi run -e nvidia mojo build -I /workspace/noeira-aug27 \
        -o /tmp/tbt_aug27 /workspace/noeira-aug27/tests/physics3d/test_newton_blocked_tendon_fields.mojo
...
Context: the Unitree G1 port (`noeira/envs/robots/unitree_g1*`, BFM-Zero's
...
--sample=cpu`, `--timing-only`):
    ...
    GPU kernels (ms per physics substep)
      Newton solver          23.6   60.3%   noeira_physics3d_solver_newt..._4e2c6c1c27bd450a
      collision (SAP)        11.2   28.6%   noeira_physics3d_collision_b..._4ba3bb0e904ca920
...
⚠ On this Mac the MAIN tree with its in-flight `noeira/tasks/*` edits
