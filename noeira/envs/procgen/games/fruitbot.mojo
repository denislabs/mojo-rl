...
struct FruitbotGame(Copyable, Movable, ProcgenGame):
    # ─── ProcgenGame conformance glue (see games/procgen_env.mojo) ──────
    comptime AssetsT = FruitbotAssets
    comptime DEFAULT_DIST = DIST_EASY
    comptime GYM_MAX_STEPS = 1000

    @staticmethod
    def load_assets(asset_root: String) raises -> FruitbotAssets:
        return FruitbotAssets(asset_root)

    @staticmethod
    def make(assets: ArcPointer[FruitbotAssets], dist_mode: Int) -> Self:
    ...
    def pg_render_obs(self, assets: FruitbotAssets) -> List[UInt8]:
        return self.render_obs(assets)

    def pg_render_obs_train(
        self, assets: FruitbotAssets, res: Int, ss: Int
    ) -> List[UInt8]:
        return self.render_obs(assets, res, ss)

    def pg_render(self, assets: FruitbotAssets, res: Int) -> List[UInt8]:
