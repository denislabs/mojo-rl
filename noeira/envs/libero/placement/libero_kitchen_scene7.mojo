"""`libero_kitchen_scene7`'s device placement table — GENERATED, DO NOT EDIT.

Regenerate with:  pixi run gen-placement-tables
CI checks it with: pixi run gen-placement-tables --check

From `noeira/envs/libero/families/libero_kitchen_scene7.family`,
`noeira/envs/libero/scenes/libero_kitchen_scene7.xml` and forward kinematics on it.
2 free slots, 6 regions (0 moving, 0 followed on one slide), 1 drawable joints.
...
struct LiberoKitchenScene7Placement(PlacementTable):
    ...
    def region_site_x[DTYPE: DType](r: Int) -> Scalar[DTYPE]:
        if r == 0:
            return Scalar[DTYPE](-1.2246467991473532e-18)
        if r == 1:
            return Scalar[DTYPE](-1.2246467991473532e-18)
        if r == 2:
            return Scalar[DTYPE](0.0)
    ...
    def region_has_rect(r: Int) -> Bool:
        ...
        if r == 2:
            return True  # kitchen_table_plate_right_region_zone
        if r == 3:
            return True  # kitchen_table_microwave_init_region
        if r == 4:
            return True  # kitchen_table_plate_init_region
        return True  # kitchen_table_plate_right_region
