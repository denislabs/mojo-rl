"""`libero_study_scene3`'s device placement table — GENERATED, DO NOT EDIT.

Regenerate with:  pixi run gen-placement-tables
CI checks it with: pixi run gen-placement-tables --check

From `noeira/envs/libero/families/libero_study_scene3.family`,
`noeira/envs/libero/scenes/libero_study_scene3.xml` and forward kinematics on it.
3 free slots, 12 regions (0 moving, 0 followed on one slide), 0 drawable joints.
...
struct LiberoStudyScene3Placement(PlacementTable):
    comptime N_SLOTS: Int = 5
    comptime N_FREE: Int = 3
    comptime N_REGIONS: Int = 12
    comptime NQ: Int = 30
    comptime NV: Int = 27
    ...
    def free_slot(j: Int) -> Int:
        ...
        if j == 1:
            return 3  # red_coffee_mug_1
        return 4  # porcelain_mug_1
    ...
    def region_has_rect(r: Int) -> Bool:
        ...
        if r == 5:
            return True  # study_table_red_coffee_mug_init_region
        if r == 6:
            return True  # study_table_red_coffee_mug_behind_region
        if r == 7:
            return True  # study_table_porcelain_mug_init_region
        if r == 8:
            return True  # study_table_black_book_init_region
        if r == 9:
            return True  # study_table_desk_caddy_init_region
        if r == 10:
            return True  # study_table_desk_caddy_front_left_contain_region
