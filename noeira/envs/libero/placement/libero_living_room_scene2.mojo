"""`libero_living_room_scene2`'s device placement table — GENERATED, DO NOT EDIT.

Regenerate with:  pixi run gen-placement-tables
CI checks it with: pixi run gen-placement-tables --check

From `noeira/envs/libero/families/libero_living_room_scene2.family`,
`noeira/envs/libero/scenes/libero_living_room_scene2.xml` and forward kinematics on it.
8 free slots, 9 regions (1 moving, 0 followed on one slide), 0 drawable joints.
...
struct LiberoLivingRoomScene2Placement(PlacementTable):
    comptime N_SLOTS: Int = 9
    comptime N_FREE: Int = 8
    comptime N_REGIONS: Int = 9
    comptime NQ: Int = 65
    comptime NV: Int = 57
    comptime N_JOINTS: Int = 0
    comptime NBODY: Int = 28
    ...
    def region_has_rect(r: Int) -> Bool:
        ...
        if r == 2:
            return True  # living_room_table_milk_init_region
        if r == 3:
            return True  # living_room_table_cream_cheese_init_region
        if r == 4:
            return True  # living_room_table_orange_juice_init_region
        if r == 5:
