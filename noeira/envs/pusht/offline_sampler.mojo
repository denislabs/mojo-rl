...
``noeira.core.offline_buffer.OfflineBuffer``) is generic on its buffer
type and requires:
...
from noeira.nn.datasets.lewm_pusht import LewmPushTExpert, LewmPushTWindow
        ...
        *,
        ...
        Args:
            ...
            path: Optional override of the cached `.h5` path. Empty string
                triggers HF auto-download to `~/.cache/noeira/lewm_pusht/`.
    ...
    def sample_batch_uint8(
        ...
        for b in range(B):
            ...
            self.dataset.sample_clip_pixels_uint8(
                clip_idx,
                pixels_out.unsafe_offset(b * pix_per_sample),
                actions_out.unsafe_offset(b * act_per_sample),
            ...
            if self.normalize_actions:
                ...
                var adim = self.dataset.action_dim
                var base = actions_out.unsafe_offset(b * act_per_sample)
