 ...
 * It arms only when /etc/nv_tegra_release exists or NOEIRA_NVML_PCI_FROM_CUDA=1
...
static int tegra_nvml_pci_shim(void) {
    static int on = -1;
    if (on < 0) {
        const char *e = getenv("NOEIRA_NVML_PCI_FROM_CUDA");
        if (e && *e) on = (e[0] != '0');
...
static int capture_mode(void) {
    const char *e = getenv("NOEIRA_CAPTURE_MODE");
    if (e && *e) {
        int m = atoi(e);
        if (m >= 0 && m <= 2) return m;
    }
...
CUresult intercept_stream_end_capture(void *stream, void **graph_out) {
    ...
    /* ⚠ OPT-IN, BECAUSE THE PROBE ITSELF CRASHED THE DRIVER. Even called
       ...
       trip. Off by default; NOEIRA_CAPTURE_PROBE=1 to re-enable. */
    const char *probe = getenv("NOEIRA_CAPTURE_PROBE");
...
/* Constructor: print banner on load.
   NOEIRA_INTERCEPT_LOG=1 turns on per-launch tracing from the very first
...
static void on_load(void) {
    const char *e = getenv("NOEIRA_INTERCEPT_LOG");
