"""Dear ImGui bindings — a composable, immediate-mode UI for the 3D viewer.
    ...
    from noeira.render.imgui import ig_begin_panel, ig_button, ig_end
...
⚠ THIS REPLACES `noeira/render/ui.mojo`, it does not extend it. The two are
...
from noeira.render.sdl import Ptr, untracked
...
def _init_imgui_handle() -> OwnedDLHandle:
    ...
    abort(
        ...
        + " NOEIRA_IMGUI_LIB=/path/to/"
        + _imgui_lib_name()
    )


comptime lib = _Global["NOEIRA_IMGUI", _init_imgui_handle]()
...
def ig_shutdown() raises:
    _get_dylib_function[lib, "nra_ig_shutdown", def() thin -> None]()()


def ig_new_frame() raises:
    """Open a frame. All widget calls must fall between this and `ig_prepare`.
    """
    _get_dylib_function[lib, "nra_ig_new_frame", def() thin -> None]()()
...
def ig_want_mouse() raises -> Bool:
    ...
    return _get_dylib_function[lib, "nra_ig_want_mouse", def() thin -> Bool]()()
...
def ig_want_keyboard() raises -> Bool:
    ...
    unnecessary."""
    return _get_dylib_function[
        lib, "nra_ig_want_keyboard", def() thin -> Bool
    ]()()


def ig_framerate() raises -> Float32:
    return _get_dylib_function[
        lib, "nra_ig_framerate", def() thin -> c_float
    ]()()
...
def ig_end() raises:
    _get_dylib_function[lib, "nra_ig_end", def() thin -> None]()()
...
def ig_end_child() raises:
    _get_dylib_function[lib, "nra_ig_end_child", def() thin -> None]()()


def ig_separator() raises:
    _get_dylib_function[lib, "nra_ig_separator", def() thin -> None]()()
...
def ig_spacing() raises:
    _get_dylib_function[lib, "nra_ig_spacing", def() thin -> None]()()


def ig_indent(w: Float32 = 0.0) raises:
    _get_dylib_function[lib, "nra_ig_indent", def(c_float) thin -> None]()(w)


def ig_unindent(w: Float32 = 0.0) raises:
    _get_dylib_function[lib, "nra_ig_unindent", def(c_float) thin -> None]()(w)
...
def ig_set_next_item_width(w: Float32) raises:
    ...
    _get_dylib_function[
        lib, "nra_ig_set_next_item_width", def(c_float) thin -> None
    ]()(w)
...
def ig_content_width() raises -> Float32:
    ...
    return _get_dylib_function[
        lib, "nra_ig_content_width", def() thin -> c_float
    ]()()
...
def ig_push_id_int(id: Int) raises:
    ...
    button."""
    _get_dylib_function[lib, "nra_ig_push_id_int", def(c_int) thin -> None]()(
        c_int(id)
    )
...
def ig_pop_id() raises:
    _get_dylib_function[lib, "nra_ig_pop_id", def() thin -> None]()()
...
def ig_tree_pop() raises:
    _get_dylib_function[lib, "nra_ig_tree_pop", def() thin -> None]()()
...
def ig_set_next_item_open(open: Bool) raises:
    ...
    _get_dylib_function[
        lib, "nra_ig_set_next_item_open", def(Bool) thin -> None
    ]()(open)
...
def ig_begin_main_menu_bar() raises -> Bool:
    ...
    """
    return _get_dylib_function[
        lib, "nra_ig_begin_main_menu_bar", def() thin -> Bool
    ]()()


def ig_end_main_menu_bar() raises:
    _get_dylib_function[lib, "nra_ig_end_main_menu_bar", def() thin -> None]()()
...
def ig_end_menu() raises:
    _get_dylib_function[lib, "nra_ig_end_menu", def() thin -> None]()()
...
def ig_frame_height_with_spacing() raises -> Float32:
    ...
    return _get_dylib_function[
        lib, "nra_ig_frame_height_with_spacing", def() thin -> c_float
    ]()()
...
def ig_begin_tab_bar(var id: String) raises -> Bool:
    ...
    return _get_dylib_function[
        lib,
        "nra_ig_begin_tab_bar",
        def(Ptr[c_char, MutUntrackedOrigin]) thin -> Bool,
    ]()(_c(id))


def ig_end_tab_bar() raises:
    _get_dylib_function[lib, "nra_ig_end_tab_bar", def() thin -> None]()()
...
def ig_begin_tab_item(var label: String) raises -> Bool:
    ...
    return _get_dylib_function[
        lib,
        "nra_ig_begin_tab_item",
        def(Ptr[c_char, MutUntrackedOrigin]) thin -> Bool,
    ]()(_c(label))


def ig_end_tab_item() raises:
    _get_dylib_function[lib, "nra_ig_end_tab_item", def() thin -> None]()()


def ig_columns(count: Int, border: Bool = False) raises:
    """Split the region into `count` columns; `ig_columns(1)` ends them."""
    _get_dylib_function[lib, "nra_ig_columns", def(c_int, Bool) thin -> None]()(
        c_int(count), border
    )


def ig_next_column() raises:
    _get_dylib_function[lib, "nra_ig_next_column", def() thin -> None]()()


def ig_set_column_width(index: Int, w: Float32) raises:
    _get_dylib_function[
        lib, "nra_ig_set_column_width", def(c_int, c_float) thin -> None
    ]()(c_int(index), w)
...
def ig_set_scroll_here_y(ratio: Float32 = 0.5) raises:
    ...
    _get_dylib_function[
        lib, "nra_ig_set_scroll_here_y", def(c_float) thin -> None
    ]()(ratio)


def ig_is_item_hovered() raises -> Bool:
    return _get_dylib_function[
        lib, "nra_ig_is_item_hovered", def() thin -> Bool
    ]()()
...
def ig_style_dark() raises:
    _get_dylib_function[lib, "nra_ig_style_dark", def() thin -> None]()()


def ig_style_light() raises:
    _get_dylib_function[lib, "nra_ig_style_light", def() thin -> None]()()


def ig_style_classic() raises:
    _get_dylib_function[lib, "nra_ig_style_classic", def() thin -> None]()()


def ig_set_font_scale(s: Float32) raises:
    """Scale the whole UI — HiDPI, or simply legibility."""
    _get_dylib_function[
        lib, "nra_ig_set_font_scale", def(c_float) thin -> None
    ]()(s)
...
# OpenGL convention. `noeira.render.gpu_types.mat4_to_gpu_f32` transposes
...
def gz_begin_frame() raises:
    ...
    _get_dylib_function[lib, "nra_gz_begin_frame", def() thin -> None]()()
...
def gz_set_orthographic(ortho: Bool) raises:
    _get_dylib_function[
        lib, "nra_gz_set_orthographic", def(Bool) thin -> None
    ]()(ortho)
...
def gz_set_size(v: Float32 = 0.1) raises:
    ...
    _get_dylib_function[lib, "nra_gz_set_size", def(c_float) thin -> None]()(v)
...
def gz_is_over() raises -> Bool:
    ...
    return _get_dylib_function[lib, "nra_gz_is_over", def() thin -> Bool]()()


def gz_is_using() raises -> Bool:
    """True while a handle is being dragged. See the header note."""
    return _get_dylib_function[lib, "nra_gz_is_using", def() thin -> Bool]()()
...
struct IgTexture(Movable):
    ...
    `noeira.vision.opencv` is BGR HWC with 3 — converting is the caller's job
    ...
    def image(self, w: Float32, h: Float32) raises:
        ...
        _get_dylib_function[
            lib, "nra_ig_image", def(Int, c_float, c_float) thin -> None
    ...
    def close(mut self):
        ...
        try:
            _get_dylib_function[
                lib, "nra_ig_texture_destroy", def(Int) thin -> None
            ]()(self._h)
        except:
            pass
        self._h = 0
