...
//
// ⚠ EVERY `nra_ig_*` NAME IS AN FFI CONTRACT. `imgui.mojo` looks these up by
...
bool nra_ig_init(void* window, void* device, unsigned int color_format) {
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
    ...
    // want persistence opt in with nra_ig_set_ini_filename().
...
void nra_ig_shutdown(void) {
    ImGui_ImplSDLGPU3_Shutdown();
    ImGui_ImplSDL3_Shutdown();
    ImGui::DestroyContext();
}

void nra_ig_set_ini_filename(const char* path) {
...
void nra_ig_prepare(void* cmd_buf) {
    ImGui::Render();
    ImGui_ImplSDLGPU3_PrepareDrawData(ImGui::GetDrawData(),
                                      (SDL_GPUCommandBuffer*)cmd_buf);
}

void nra_ig_render(void* cmd_buf, void* render_pass) {
    ImGui_ImplSDLGPU3_RenderDrawData(ImGui::GetDrawData(),
                                     (SDL_GPUCommandBuffer*)cmd_buf,
                                     (SDL_GPURenderPass*)render_pass, nullptr);
}

void nra_ig_process_event(void* event) {
...
bool nra_ig_want_mouse(void) { return ImGui::GetIO().WantCaptureMouse; }
bool nra_ig_want_keyboard(void) { return ImGui::GetIO().WantCaptureKeyboard; }
float nra_ig_framerate(void) { return ImGui::GetIO().Framerate; }
...
void nra_ig_begin_panel(const char* name, float x, float y, float w, float h) {
...
bool nra_ig_begin_window(const char* name, float first_x, float first_y,
...
void nra_ig_end(void) { ImGui::End(); }

bool nra_ig_begin_child(const char* id, float w, float h, bool border) {
    return ImGui::BeginChild(id, ImVec2(w, h),
                             border ? ImGuiChildFlags_Borders : 0);
}
void nra_ig_end_child(void) { ImGui::EndChild(); }

void nra_ig_separator(void) { ImGui::Separator(); }
void nra_ig_separator_text(const char* label) { ImGui::SeparatorText(label); }
void nra_ig_same_line(float offset_x, float spacing) {
    ImGui::SameLine(offset_x, spacing);
}
void nra_ig_spacing(void) { ImGui::Spacing(); }
void nra_ig_indent(float w) { ImGui::Indent(w); }
void nra_ig_unindent(float w) { ImGui::Unindent(w); }
void nra_ig_set_next_item_width(float w) { ImGui::SetNextItemWidth(w); }
float nra_ig_content_width(void) {
...
void nra_ig_push_id_int(int id) { ImGui::PushID(id); }
void nra_ig_push_id_str(const char* id) { ImGui::PushID(id); }
void nra_ig_pop_id(void) { ImGui::PopID(); }
...
void nra_ig_text(const char* s) { ImGui::TextUnformatted(s); }

void nra_ig_text_colored(const char* s, float r, float g, float b, float a) {
    ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(r, g, b, a));
    ImGui::TextUnformatted(s);
    ImGui::PopStyleColor();
}

void nra_ig_text_disabled(const char* s) {
...
void nra_ig_text_wrapped(const char* s) {
...
bool nra_ig_button(const char* label, float w, float h) {
    return ImGui::Button(label, ImVec2(w, h));
}

bool nra_ig_small_button(const char* label) {
...
bool nra_ig_toggle_button(const char* label, bool active, float w, float h) {
    if (active) {
...
bool nra_ig_selectable(const char* label, bool selected) {
    return ImGui::Selectable(label, selected);
}

bool nra_ig_checkbox(const char* label, bool* v) {
    return ImGui::Checkbox(label, v);
}

bool nra_ig_radio(const char* label, bool active) {
    return ImGui::RadioButton(label, active);
}

bool nra_ig_slider_float(const char* label, float* v, float lo, float hi,
                         const char* fmt) {
    return ImGui::SliderFloat(label, v, lo, hi, fmt ? fmt : "%.3f");
}

bool nra_ig_slider_int(const char* label, int* v, int lo, int hi) {
    return ImGui::SliderInt(label, v, lo, hi);
}

bool nra_ig_drag_float(const char* label, float* v, float speed, float lo,
...
bool nra_ig_combo(const char* label, int* current, const char* items) {
    return ImGui::Combo(label, current, items);
}

bool nra_ig_input_text(const char* label, char* buf, int buf_size) {
...
bool nra_ig_input_text_hint(const char* label, const char* hint, char* buf,
                            int buf_size) {
...
// a menu-bar offset.
bool nra_ig_begin_main_menu_bar(void) { return ImGui::BeginMainMenuBar(); }
void nra_ig_end_main_menu_bar(void) { ImGui::EndMainMenuBar(); }
bool nra_ig_begin_menu(const char* label) { return ImGui::BeginMenu(label); }
void nra_ig_end_menu(void) { ImGui::EndMenu(); }
bool nra_ig_menu_item(const char* label, const char* shortcut, bool selected,
                      bool enabled) {
    return ImGui::MenuItem(label, shortcut[0] ? shortcut : nullptr, selected,
                           enabled);
}
float nra_ig_frame_height_with_spacing(void) {
...
bool nra_ig_begin_tab_bar(const char* id) { return ImGui::BeginTabBar(id); }
void nra_ig_end_tab_bar(void) { ImGui::EndTabBar(); }
bool nra_ig_begin_tab_item(const char* label) {
    return ImGui::BeginTabItem(label);
}
void nra_ig_end_tab_item(void) { ImGui::EndTabItem(); }
...
void nra_ig_columns(int count, bool border) {
    ImGui::Columns(count, nullptr, border);
}
void nra_ig_next_column(void) { ImGui::NextColumn(); }
void nra_ig_set_column_width(int i, float w) { ImGui::SetColumnWidth(i, w); }

bool nra_ig_tree_node(const char* label) { return ImGui::TreeNode(label); }
void nra_ig_tree_pop(void) { ImGui::TreePop(); }
void nra_ig_set_next_item_open(bool open) { ImGui::SetNextItemOpen(open); }

bool nra_ig_collapsing_header(const char* label, bool default_open) {
    return ImGui::CollapsingHeader(
        label, default_open ? ImGuiTreeNodeFlags_DefaultOpen : 0);
}

void nra_ig_plot_lines(const char* label, const float* values, int count,
                       int offset, float lo, float hi, float w, float h) {
    ImGui::PlotLines(label, values, count, offset, nullptr, lo, hi,
                     ImVec2(w, h));
}

void nra_ig_progress_bar(float frac, float w, float h, const char* overlay) {
    ImGui::ProgressBar(frac, ImVec2(w, h), overlay);
}

void nra_ig_set_scroll_here_y(float ratio) { ImGui::SetScrollHereY(ratio); }

bool nra_ig_is_item_hovered(void) { return ImGui::IsItemHovered(); }

void nra_ig_set_tooltip(const char* s) {
    if (ImGui::IsItemHovered()) ImGui::SetTooltip("%s", s);
}

// ─── style ──────────────────────────────────────────────────────────────────

void nra_ig_style_dark(void) { ImGui::StyleColorsDark(); }
void nra_ig_style_light(void) { ImGui::StyleColorsLight(); }
void nra_ig_style_classic(void) { ImGui::StyleColorsClassic(); }

// Global scale for the whole UI, for HiDPI or simply for readability.
void nra_ig_set_font_scale(float s) {
...
void nra_gz_begin_frame(void) { ImGuizmo::BeginFrame(); }
...
void nra_gz_set_rect(float x, float y, float w, float h) {
    ImGuizmo::SetRect(x, y, w, h);
}

void nra_gz_set_orthographic(bool ortho) { ImGuizmo::SetOrthographic(ortho); }
...
void nra_gz_set_size(float v) { ImGuizmo::SetGizmoSizeClipSpace(v); }
...
bool nra_gz_manipulate(const float* view, const float* proj, int op, int mode,
...
// ⚠⚠ THESE ARE THE MOUSE ARBITRATION, and `nra_ig_want_mouse` does NOT cover
...
bool nra_gz_is_over(void) { return ImGuizmo::IsOver(); }
bool nra_gz_is_using(void) { return ImGuizmo::IsUsing(); }
...
void* nra_ig_texture_create(void* device, int w, int h) {
    ...
    NraIgTexture* t = new NraIgTexture();
    t->device = dev;
    t->tex    = tex;
    t->xfer   = xfer;
    t->w      = w;
...
bool nra_ig_texture_upload(void* handle, const unsigned char* rgba) {
    NraIgTexture* t = (NraIgTexture*)handle;
...
void nra_ig_image(void* handle, float w, float h) {
    NraIgTexture* t = (NraIgTexture*)handle;
    if (t == nullptr) return;
    ImGui::Image((ImTextureID)(intptr_t)t->tex, ImVec2(w, h));
}

// Where the last `nra_ig_image` landed on screen, in window coordinates, so a
// caller can draw an overlay over it without duplicating ImGui's layout rules.
void nra_ig_last_item_rect(float* x, float* y, float* w, float* h) {
...
void nra_ig_overlay_line(float x0, float y0, float x1, float y1,
                         unsigned int rgba, float thickness) {
    ImGui::GetWindowDrawList()->AddLine(ImVec2(x0, y0), ImVec2(x1, y1),
                                        rgba, thickness);
}

void nra_ig_texture_destroy(void* handle) {
    NraIgTexture* t = (NraIgTexture*)handle;
    if (t == nullptr) return;
