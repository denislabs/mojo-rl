...
Example usage:
    from noeira.core import PolynomialFeatures, ContinuousReplayBuffer
    from noeira.agents.ddpg import DDPGAgent
...
from std.math import exp, tanh, sqrt, cos, log
from std.random import random_float64
from noeira.core.continuous_replay_buffer import (
...
from noeira.nn.random import gaussian_noise
...
struct DDPGAgent[DTYPE: DType](Copyable, Movable):
    ...
    def __init__(out self, *, deinit move: Self):
        ...
        self.target_critic_weights = move.target_critic_weights^
    ...
    def select_action_with_noise(self, features: List[Float64]) -> Float64:
        ...
        if u1 < 1e-10:
            u1 = 1e-10
        var noise = sqrt(-2.0 * log(u1)) * cos(2.0 * 3.141592653589793 * u2)
    ...
    def _get_q_value_target(
        ...
        var n = min(
            len(critic_features),
            len(self.target_critic_weights),
        )
        for i in range(n):
