...
Example:
    ```mojo
    from noeira.math3d import Vec3, Quat, Mat3, Mat4

    # Create a position
    var position = Vec3[DType.float32](1.0, 2.0, 3.0)

    # Create a rotation (45 degrees around Y axis)
    var rotation = Quat.from_axis_angle(Vec3[DType.float32].unit_y(), 0.785)
    ...
    var transform = Mat4.compose(
        Vec3[DType.float32](0.0, 1.0, 0.0),  # translation
        rotation,             # rotation
        Vec3[DType.float32].one(),          # scale
    )
    ```
"""
