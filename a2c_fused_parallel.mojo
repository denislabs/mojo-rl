...
fn fused_rollout_kernel[
    ...
    rollout_hidden: LayoutTensor[
        dtype, Layout.row_major(NUM_ENVS, ROLLOUT_LEN, HIDDEN_DIM), MutAnyOrigin
    ],
    ...
    for step in range(ROLLOUT_LEN):
        ...
        if is_valid:
            var partial_v = rebind[Scalar[dtype]](
                shared_hidden[local_env_idx, hidden_idx]
            ) * rebind[Scalar[dtype]](W_critic[hidden_idx, 0])
        ...
        while stride_v > 0:
            if is_valid and hidden_idx < stride_v:
                shared_reduce[local_env_idx, hidden_idx] = rebind[
                    ...
                    shared_reduce[local_env_idx, hidden_idx + stride_v]
                )
            barrier()
            stride_v = stride_v // 2

        if is_valid and hidden_idx == 0:
            rollout_values[global_env_idx, step] = rebind[Scalar[dtype]](
                shared_reduce[local_env_idx, 0]
        ...
        if is_valid and hidden_idx == 0:
            var max_logit = rebind[Scalar[dtype]](
                shared_logits[local_env_idx, 0]
            )
            ...
            for a in range(NUM_ACTIONS):
                var e = exp(
                    rebind[Scalar[dtype]](shared_logits[local_env_idx, a])
                    - max_logit
                )
                shared_reduce[local_env_idx, a] = e
                sum_exp += e

            var prob0 = rebind[Scalar[dtype]](
                shared_reduce[local_env_idx, 0]
            ) / (sum_exp + Scalar[dtype](1e-10))
            var prob1 = rebind[Scalar[dtype]](
                shared_reduce[local_env_idx, 1]
            ) / (sum_exp + Scalar[dtype](1e-10))
            ...
            log_prob = log(
                (prob0 if selected_action == 0 else prob1)
                + Scalar[dtype](1e-10)
            )
        ...
        if is_valid and hidden_idx == 0:
            ...
            var force = Scalar[dtype](
                FORCE_MAG
            ) if selected_action == 1 else Scalar[dtype](-FORCE_MAG)
            var costheta = cos(theta)
            var sintheta = sin(theta)

            var temp = (
                force
                + Scalar[dtype](POLEMASS_LENGTH)
                * theta_dot
                * theta_dot
                * sintheta
            ) / Scalar[dtype](TOTAL_MASS)
            var theta_acc = (
                Scalar[dtype](GRAVITY) * sintheta - costheta * temp
            ) / (
                Scalar[dtype](LENGTH)
                * (
                    Scalar[dtype](4.0 / 3.0)
                    - Scalar[dtype](MASSPOLE)
                    * costheta
                    * costheta
                    / Scalar[dtype](TOTAL_MASS)
                )
            )
            var x_acc = temp - Scalar[dtype](
                POLEMASS_LENGTH
            ) * theta_acc * costheta / Scalar[dtype](TOTAL_MASS)
            ...
            shared_stats[local_env_idx, 0] = rebind[Scalar[dtype]](
                shared_stats[local_env_idx, 0]
            ) + Scalar[dtype](1.0)

            if done:
                shared_stats[local_env_idx, 1] = rebind[Scalar[dtype]](
                    shared_stats[local_env_idx, 1]
                ) + Scalar[dtype](1.0)
                var rng = rebind[UInt32](shared_rng[local_env_idx])
    ...
    if is_valid and hidden_idx == 0:
        for d in range(OBS_DIM):
            env_states[global_env_idx, d] = rebind[Scalar[dtype]](
                shared_obs[local_env_idx, d]
            )
        rng_states[global_env_idx] = rebind[Scalar[DType.uint32]](
            shared_rng[local_env_idx]
        )
        total_rewards[global_env_idx] = rebind[Scalar[dtype]](
            shared_stats[local_env_idx, 0]
        )
        episode_counts[global_env_idx] = Scalar[DType.int32](
            Int32(rebind[Scalar[dtype]](shared_stats[local_env_idx, 1]))
        )
...
fn gae_kernel[
    ...
    var last_value = rebind[Scalar[dtype]](
        rollout_values[env_idx, ROLLOUT_LEN - 1]
    )
    ...
    for t in range(ROLLOUT_LEN - 1, -1, -1):
        ...
        var next_non_terminal = Scalar[dtype](1.0) if done == 0 else Scalar[
            dtype
        ](0.0)
...
fn backward_kernel[
    ...
    for step in range(ROLLOUT_LEN):
        if is_valid:
            var h = rebind[Scalar[dtype]](
                rollout_hidden[global_env_idx, step, hidden_idx]
            )
            var action = Int(rollout_actions[global_env_idx, step])
            var advantage = rebind[Scalar[dtype]](
                advantages[global_env_idx, step]
            )
            var ret = rebind[Scalar[dtype]](returns[global_env_idx, step])
            var value = rebind[Scalar[dtype]](
                rollout_values[global_env_idx, step]
            )
            ...
            for a in range(NUM_ACTIONS):
                ...
                var grad = (
                    h * advantage * (indicator - Scalar[dtype](0.5))
                )  # Simplified
                # Atomic add would be needed for correctness across envs
                # For single env per thread, direct accumulation works
                shared_grad_W_actor[hidden_idx, a] = (
                    rebind[Scalar[dtype]](shared_grad_W_actor[hidden_idx, a])
                    + grad
                )
            ...
            shared_grad_W_critic[hidden_idx, 0] = (
                rebind[Scalar[dtype]](shared_grad_W_critic[hidden_idx, 0])
                + critic_grad
            )
    ...
    if is_valid:
        ...
        for a in range(NUM_ACTIONS):
            ...
            W_actor[hidden_idx, a] = (
                rebind[Scalar[dtype]](W_actor[hidden_idx, a]) - actor_lr * grad
            )

        var critic_grad = (
            rebind[Scalar[dtype]](shared_grad_W_critic[hidden_idx, 0]) * scale
        )
        W_critic[hidden_idx, 0] = (
            rebind[Scalar[dtype]](W_critic[hidden_idx, 0])
            - critic_lr * critic_grad
        )
...
fn main() raises:
    ...
    with DeviceContext() as ctx:
        ...
        var rollout_hidden_buf = ctx.enqueue_create_buffer[dtype](
            NUM_ENVS * ROLLOUT_LEN * HIDDEN_DIM
        )

        var advantages_buf = ctx.enqueue_create_buffer[dtype](
            NUM_ENVS * ROLLOUT_LEN
        )
        var returns_buf = ctx.enqueue_create_buffer[dtype](
            NUM_ENVS * ROLLOUT_LEN
        )
        ...
        with env_states_buf.map_to_host() as h:
            for i in range(NUM_ENVS):
                h[i * OBS_DIM + 0] = Scalar[dtype](
                    (random_float64() - 0.5) * 0.1
                )
                h[i * OBS_DIM + 1] = Scalar[dtype](
                    (random_float64() - 0.5) * 0.1
                )
                h[i * OBS_DIM + 2] = Scalar[dtype](
                    (random_float64() - 0.5) * 0.1
                )
                h[i * OBS_DIM + 3] = Scalar[dtype](
                    (random_float64() - 0.5) * 0.1
                )
        ...
        var rollout_hidden = LayoutTensor[
            dtype,
            Layout.row_major(NUM_ENVS, ROLLOUT_LEN, HIDDEN_DIM),
            MutAnyOrigin,
        ](rollout_hidden_buf)
        ...
        for update in range(NUM_UPDATES):
            ...
            ctx.enqueue_function[
                ...
                W_critic,
                b_critic,
                Scalar[dtype](ACTOR_LR),
                Scalar[dtype](CRITIC_LR),
            ...
            if (update + 1) % 10 == 0:
                with episode_counts_buf.map_to_host() as e, total_rewards_buf.map_to_host() as r:
                    ...
                    var avg_len = (
                        total_rew / Float64(ep_count) if ep_count > 0 else 0.0
                    )
                    print(
                        "Update",
                        update + 1,
