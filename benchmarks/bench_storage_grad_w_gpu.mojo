...
The backward of Linear/Embedding/LSTM/GRU computes

    dW[IN, OUT] = xᵀ @ go        (x = [B, IN], go = [B, OUT])

`max_matmul` has NO `transpose_a` (GPU asserts `not transpose_a`), so the
storage primitives materialise xᵀ with a separate transpose kernel, run the
GEMM, then accumulate into the param grad with a third kernel:

    naive_transpose(x → cacheT)  →  max_matmul(dW, cacheT, go)  →  gw += dW

A native `transpose_a` would collapse all three into one GEMM. This bench
quantifies how much that would actually save, by timing:

    floor   : max_matmul alone over a pre-transposed cacheT  (= transpose_a==free)
    Tnaive  : the naive one-thread/elem transpose kernel (linear.mojo:_transpose_kernel)
    Ttiled  : the 32x8 BLOCK_ROWS shared-mem transpose (B1', transpose_2d.mojo)
    accum   : the gw += dW kernel alone
...
Read-out:
  * (full_* − floor)  = the overhead a native transpose_a could remove.
  * (full_naive − full_tiled) = the free win from reusing the existing tiled transpose.
  * (full_tiled − fused) = the win from fusing accumulate into the GEMM epilogue.
  * fused vs floor = how close the in-house fix gets to the transpose_a ideal.
If full_* ≈ floor at the real RL shapes, transpose_a is an A3/B3-style no-op.

Shapes (B, IN, OUT): SAC/TD3 MLP hidden 256; Dreamer-ish 512; a couple of
skinny grad_w cases (large B·IN, small OUT) where the transpose is most likely
to surface above the GEMM.
