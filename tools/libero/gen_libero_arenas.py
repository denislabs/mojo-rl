...
"""Generate the styled, re-posed LIBERO arena per problem class.
    ...
    pixi run python tools/libero/gen_libero_arenas.py          # write
    pixi run python tools/libero/gen_libero_arenas.py --check  # CI: stale?
...
from the numbers in `noeira/envs/libero/tables/categories.kv`, and writes one
arena per problem into `noeira/envs/libero/arenas/`, which the family
...
TABLE = "noeira/envs/libero/tables/categories.kv"
PACK = "noeira/envs/libero/assets"
OUT_DIR = "noeira/envs/libero/arenas"
...
def generate(prob):
    ...
    return (
        "<!-- GENERATED by tools/libero/gen_libero_arenas.py from\n"
...
def main():
    ...
    if a.check and stale:
        sys.exit(f"{stale} arena(s) stale; run `pixi run python tools/libero/gen_libero_arenas.py`")
