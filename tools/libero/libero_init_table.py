...
`examples/libero/libero_init_freeze.mojo` reads these dumps and freezes them.
...
def write_remap_kv(suite, remap, their_joints, nq, nv, n_tasks):
    ...
    (G13, `noeira/envs/libero/demos.mojo`, which reads their HDF5 natively and
    ...
    lines = [
        ...
        "# tools/libero/libero_init_table.py — do not edit; re-run the tool.",
        "#",
...
def main():
    ...
    print(f"wrote {index}")
    print()
    print("now:  pixi run mojo run -I . examples/libero/libero_init_freeze.mojo "
          + index)
