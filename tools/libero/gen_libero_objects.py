...
"""Generate the attachable copy of every LIBERO object XML.
    ...
    pixi run python tools/libero/gen_libero_objects.py          # write
    pixi run python tools/libero/gen_libero_objects.py --check  # CI: stale?
...
category of `noeira/envs/libero/tables/categories.kv` into
`noeira/envs/libero/objects/`. Each output is loaded by MuJoCo on its own
...
OUT_DIR = "noeira/envs/libero/objects"
...
def generate(rel, free):
    ...
    text = (
        "<!-- GENERATED by tools/libero/gen_libero_objects.py from\n"
...
def main():
    ...
    if a.check and stale:
        sys.exit(f"{stale} object XML(s) stale; run `pixi run python tools/libero/gen_libero_objects.py`")
