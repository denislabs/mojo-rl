...
Writes `noeira/envs/libero/tables/init_z_<family>.kv`: one line per prop, the z its
free joint holds in `libero/libero/init_files/<suite>/*.pruned_init`. The
grouping mirrors `tools/libero/gen_libero_family.mojo` — one file per FAMILY,
...
OUT_DIR = "noeira/envs/libero/tables"
N_ROBOT_QPOS = 9
...
def build_group(name, files, check):
    ...
    for bddl, idir, stem in files:
        ...
        if slack < 0 or slack % 2 != 0:
            sys.exit(
                ...
                " `tools/libero/libero_init_table.py` answers the general case"
    ...
    lines = [
        f"# GENERATED by tools/libero/libero_init_z.py — the z each prop's free",
