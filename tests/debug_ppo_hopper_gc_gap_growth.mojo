"""Debug script to track train-eval gap growth over time for HopperGC.
...
Run with:
    pixi run -e apple mojo run tests/debug_ppo_hopper_gc_gap_growth.mojo
    pixi run -e nvidia mojo run tests/debug_ppo_hopper_gc_gap_growth.mojo
"""
...
fn main() raises:
    seed(42)
    print("=" * 70)
    print("DEBUG: Tracking Train-Eval Gap Growth Over Time (HopperGC)")
    print("=" * 70)
    print()
    print("Configuration:")
    ...
    with DeviceContext() as ctx:
        ...
        while episodes_trained < TOTAL_EPISODES:
            ...
            var metrics = agent.train_gpu[HopperGC[gpu_dtype]](
                ctx,
            ...
            var eval_gpu_avg = agent.evaluate_gpu[HopperGC[gpu_dtype]](
                ctx,
        ...
        print()
        print("=" * 110)
        print("GAP GROWTH ANALYSIS (HopperGC)")
        print("=" * 110)
        print()
    ...
    print(">>> Gap growth analysis completed (HopperGC) <<<")
