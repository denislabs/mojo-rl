"""C51 storage smoke — compile the full stack (trainer→blocks) + short CPU
CartPole train/eval. Fast compile-check before the convergence gate; asserts
learning beats random. v_min/v_max=0/100 covers CartPole's discounted returns.

Run:
  pixi run mojo run -I . tests/deep_agents/test_storage_c51_smoke.mojo
"""
...
def main() raises:
    print("=== C51 storage smoke (CPU CartPole) ===")
    seed(42)
    var trainer = C51Trainer[
        "cpu", UniformSampleCpuStep[OBS, 1, BATCH, CAP], C51QNet,
    ...
    _ = run_offpolicy_discrete_train(
        trainer, env, 14_000, print_every=5000, verbose=True,
    )
    ...
    assert_true(ret > Scalar[DT](60.0), "C51 did not learn (eval <= 60)")
    print("=== PASSED ===")
