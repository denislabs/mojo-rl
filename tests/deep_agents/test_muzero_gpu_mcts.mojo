...
fn main() raises:
    ...
    ctx.enqueue_function[run_init, run_init](
        ...
        Scalar[dtype](0.25), Scalar[DType.uint32](42),
    ...
    for sim in range(NUM_SIMS):
        # Selection
        comptime run_select = gpu_mcts_select_kernel[N_ENVS, MAX_NODES, ACT, dtype]
        ...
        comptime run_build = gpu_mcts_build_dyn_input_kernel[N_ENVS, MAX_NODES, ACT, LATENT, DYN_IN, dtype]
        ...
        comptime run_expand = gpu_mcts_expand_kernel[N_ENVS, MAX_NODES, ACT, LATENT, PRED_OUT, DYN_OUT, dtype]
        ...
        var pred_in_net = LayoutTensor[dtype, Layout.row_major(N_ENVS, PRED_IN_DIM), MutAnyOrigin](mcts.pred_input.unsafe_ptr())
        ...
        comptime run_extract = extract_hidden_kernel[N_ENVS, LATENT, DYN_OUT, dtype]
        ctx.enqueue_function[run_extract, run_extract](
            pred_in_flat, dyn_out_flat,
            grid_dim=(EXTRACT_BLOCKS,), block_dim=(TPB,),
        )

        # Prediction forward
        var pred_out_net = LayoutTensor[dtype, Layout.row_major(N_ENVS, PRED_OUT_DIM), MutAnyOrigin](mcts.pred_output.unsafe_ptr())
        PredNet.forward_gpu[N_ENVS](
            ctx, pred_in_net, pred_out_net, gpu_pred.params_view(), workspace
        )

        # Expand nodes (creates child, sets prior, decodes value)
        var pred_out_expand = LayoutTensor[dtype, Layout.row_major(N_ENVS * PRED_OUT), MutAnyOrigin](mcts.pred_output.unsafe_ptr())
        ctx.enqueue_function[run_expand, run_expand](
            vc_t, tv_t, pr_t, rw_t, ci_t, tvis_t, nc_t,
            hs_t, pp_t, pa_t,
            do_t, pred_out_expand,
            Scalar[dtype](-10.0), Scalar[dtype](10.0),
            lv_t,
            grid_dim=(ENV_BLOCKS,), block_dim=(TPB,),
        )

        # Backup
        comptime run_backup = gpu_mcts_backup_kernel[N_ENVS, MAX_NODES, ACT, dtype]
    ...
    var act_out_t = LayoutTensor[dtype, Layout.row_major(N_ENVS), MutAnyOrigin](mcts.actions_out.unsafe_ptr())
    var pol_out_t = LayoutTensor[dtype, Layout.row_major(N_ENVS * ACT), MutAnyOrigin](mcts.policies_out.unsafe_ptr())
    comptime run_extract_act = gpu_mcts_extract_actions_kernel[N_ENVS, MAX_NODES, ACT, dtype]
