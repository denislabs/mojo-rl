...
Run with:
    pixi run -e apple mojo run -I . tests/test_ppo_bipedal_continuous_eval_gpu.mojo
    pixi run -e nvidia mojo run -I . tests/test_ppo_bipedal_continuous_eval_gpu.mojo
"""
...
def main() raises:
    ...
    with DeviceContext() as ctx:
        ...
        var stochastic_reward = agent.evaluate_gpu[BipedalWalker[gpu_dtype]](
            ctx,
        ...
        var deterministic_reward = agent.evaluate_gpu[BipedalWalker[gpu_dtype]](
            ctx,
            num_episodes=EVAL_EPISODES,
            max_steps=MAX_STEPS,
            verbose=False,
