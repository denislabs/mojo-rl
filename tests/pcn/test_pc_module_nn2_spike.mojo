"""Phase A spike — PCN on nn2 storage + nn2 Adam (CPU).

Gate for the PCN → nn2 re-architecture's Phase A: prove that PCN's weight
slab can live in an nn2 `Param` and be trained by nn2 `Adam` driven from
...
Validates:
  - `PCModule[*BLOCKS]` conforms to nn2 `Module` (Adam.make accepts it; the
...
Run:
    pixi run mojo run -I . tests/pcn/test_pc_module_nn2_spike.mojo
"""
...
def main() raises:
    ...
    print("PASS: PCN trains on nn2 storage via nn2 Adam (Phase A spike)")
