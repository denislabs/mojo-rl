"""Does P16's "energy law" survive adequate capacity?

P16 found, on the 1.3M-param P10 net, that test accuracy fell MONOTONICALLY as
the relaxation budget rose (T=6 -> 0.4445, T=20 -> 0.4360, T=100 -> 0.4085)
while sup_loss fell 273x. That drove the reading "minimizing the PC energy
harder makes conv-PCN generalize worse", and via it the verdicts in P11-P13
(Qi et al.'s fixes) and P17 (ePC).

The capacity run then showed that net was badly undersized: at the SAME budget,
the VGG5-matched net scores 0.4905/0.4815 vs 0.4445/0.4460, and sits at
sup_loss 21.7-43.7 -- clearly UNDERFIT, where the P10 net's ePC arm had driven
sup_loss to 0.0666, i.e. memorization. "More optimization hurts" is what an
overfitting regime looks like. P16 may simply have measured that.

This repeats P16's T sweep on the capacity-matched net, one axis, same recipe:

    sPC, lr_x = 0.025, T in {6, 20}

  * accuracy FALLS with T again  => the law is real and survives capacity;
                                    P11-P17's interpretation stands.
  * accuracy RISES with T        => the law was an undersized-net artifact,
                                    and every verdict screened on the P10 net
                                    (Qi's spiking + forward updates, ePC) was
                                    reached in the wrong regime and must be
                                    re-run here.

T=100 is deliberately omitted: at this size it is ~75 min, worth spending only
once the first two arms show a direction.

Run:
    pixi run mojo run -I . tests/pcn/test_conv_pc_cifar10_capacity_tsweep_cpu.mojo
"""
...
def main() raises:
    print("=" * 72)
    print("Conv-PCN CIFAR-10 — CAPACITY-MATCHED T-SWEEP (CPU)")
    print("=" * 72)
    ...
    # ── Summary ──────────────────────────────────────────────────────────────
    print("\n" + "=" * 72)
    print("CAPACITY-MATCHED T-SWEEP SUMMARY —", N_TRAIN_SAMPLES, "train /", EPOCHS, "ep")
    print("=" * 72)
    ...
    print("\n  On the 16x-smaller P10 net these same T values gave:")
    print("      T= 6 -> 0.4445     T=20 -> 0.4360    (accuracy FELL with T)")
    print("  If the sign flips here, P16's law was an undersized-net artifact.")
