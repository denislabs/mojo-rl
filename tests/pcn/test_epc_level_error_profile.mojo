...
def _show_epc(lbl: String, err_raw: Pointer[Scalar[dtype], MutAnyOrigin]):
    var rms = List[Float64]()
    comptime for l in range(NET.N_LATENTS):
        comptime LO = BATCH * NET._latent_offset[l]()
        comptime CNT = BATCH * NET.block_types[l].OUT_DIM
        var acc: Float64 = 0.0
        for k in range(CNT):
            var v = Float64(err_raw[unsafe_offset=LO + k])
            acc += v * v
        rms.append(sqrt(acc / Float64(CNT)))
    var line = String("    ") + fit(lbl, 12) + " rms:"
    for i in range(len(rms)):
        line += " " + fit(String(rms[i]), 13)
    print(line)
    var rl = String("    ") + fit(String(""), 12) + " r_l:"
    for i in range(len(rms) - 1):
        rl += " " + fit(String(rms[i + 1] / (rms[i] + 1e-30)), 13)
    print(rl)
...
def _probe_epc(
    tag: String,
    err_raw: Pointer[Scalar[dtype], MutAnyOrigin],
    t_infer: Int,
    lr_e: Float64,
    x_batch: LayoutTensor[dtype, Layout.row_major(BATCH, NET.IN_DIM), MutAnyOrigin],
    y_batch: LayoutTensor[dtype, Layout.row_major(BATCH, NET.OUT_DIM), MutAnyOrigin],
    params: LayoutTensor[dtype, Layout.row_major(NET.PARAM_SIZE), MutAnyOrigin],
    mut errors: LayoutTensor[dtype, Layout.row_major(BATCH, NET.LATENT_DIM), MutAnyOrigin],
    mut latents: LayoutTensor[dtype, Layout.row_major(BATCH, NET.LATENT_DIM), MutAnyOrigin],
    mut mu_eps_buf: LayoutTensor[dtype, Layout.row_major(BATCH, NET.SCRATCH_OUT_DIM), MutAnyOrigin],
    mut a_below_buf: LayoutTensor[dtype, Layout.row_major(BATCH, NET.SCRATCH_IN_DIM), MutAnyOrigin],
    mut z_below_buf: LayoutTensor[dtype, Layout.row_major(BATCH, NET.SCRATCH_IN_DIM), MutAnyOrigin],
    ...
    """ePC profile. In ePC the ERRORS are the free variables, so we read the
    `errors` buffer directly — it holds exactly the ε the sPC probe measures.

    NOTE the latent-slab layout: level l's ε lives at `_latent_offset[l]`, not
    `_out_offset[l]`, so this walks the buffer itself rather than reusing
    `_print_profile`."""
    for i in range(BATCH * NET.LATENT_DIM):
        err_raw[unsafe_offset=i] = 0

    _show_epc(tag + " t=0", err_raw)
    for t in range(t_infer):
        TRAINER._epc_inference_step[BATCH](
            x_batch, y_batch, params, errors, latents, mu_eps_buf,
            a_below_buf, z_below_buf, dx_buf, Scalar[dtype](lr_e),
        )
        if t == 0 or t == t_infer // 2 - 1 or t == t_infer - 1:
            _show_epc(tag + " t=" + String(t + 1), err_raw)
...
def main() raises:
    ...
    var err_raw2 = alloc[Scalar[dtype]](BATCH * NET.LATENT_DIM).as_unsafe_any_origin()
    ...
    var errors = LayoutTensor[dtype, Layout.row_major(BATCH, NET.LATENT_DIM), MutAnyOrigin](err_raw2)
    ...
    print("\n  ── ePC ON THE SAME TRAINED WEIGHTS (T=5, lr_e=0.001 = the reference) ──")
    print("           level:        0         1         2         3         4")
    _probe_epc(String("ePC"), err_raw2, 5, 0.001, x_batch, y_batch, params,
               errors, latents, mu_eps_buf, a_below_buf, z_below_buf, dx_buf)

    print("\n  ── ePC at lr_e=0.025 (matched to sPC's lr_x) ──")
    _probe_epc(String("ePC-fast"), err_raw2, 5, 0.025, x_batch, y_batch, params,
               errors, latents, mu_eps_buf, a_below_buf, z_below_buf, dx_buf)

    print("\n" + "=" * 78)
    print("THE QUESTION: does ePC flatten r_l relative to sPC at a matched budget?")
