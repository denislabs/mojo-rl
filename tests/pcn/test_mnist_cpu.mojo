...
Run:
    pixi run mojo run -I . tests/pcn/test_mnist_cpu.mojo
"""
...
def main() raises:
    ...
    var x_batch_buf = alloc[Scalar[dtype]](BATCH * NET.IN_DIM).as_unsafe_any_origin()
    var y_batch_buf = alloc[Scalar[dtype]](BATCH * NET.OUT_DIM).as_unsafe_any_origin()
