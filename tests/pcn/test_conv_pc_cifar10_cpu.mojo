"""Conv-PCN CIFAR-10 scale check (P4, CPU Accelerate) — see docs/PCN_CONV_DESIGN.md.

Does the Bogacz local energy rule scale to natural images? CIFAR-10 (3×32×32)
is the literature's "competitive but not matching deep backprop CNNs" regime, so
the bar is "learns natural images well above chance / a linear baseline", not
SOTA accuracy.

Architecture (all-conv → flat readout):
  ConvPCBlock[3,  32, 3, 2, 1, 32, 32, PCIdentity]  # 3×32×32  → 32×16×16
  ConvPCBlock[32, 64, 3, 2, 1, 16, 16, PCReLU]      # 32×16×16 → 64×8×8
  ConvPCBlock[64, 64, 3, 2, 1, 8,  8,  PCReLU]      # 64×8×8   → 64×4×4 (1024)
  PCBlock[1024, 256, PCReLU]                         # 1024 → 256
  PCBlock[256, 10, PCIdentity]                       # 256 → 10 logits

Runs the CPU PCTrainer path (compute_grads_only → Adam.step) using the im2col +
Apple Accelerate ConvPCBlock kernels.

Run:
    pixi run mojo run -I . tests/pcn/test_conv_pc_cifar10_cpu.mojo
"""
...
comptime EPOCHS = 6
comptime T_INFER = 12
comptime LR_X: Float32 = 0.04
comptime ADAM_LR: Float64 = 0.0004  # lower LR: deep unnormalized conv-PC diverges at 1e-3 over many steps
comptime N_TRAIN_SAMPLES = 20000  # more data → less overfit → better test acc
comptime N_TEST_SAMPLES = 1000
comptime N_TRAIN_BATCHES = N_TRAIN_SAMPLES // BATCH
comptime N_TEST_BATCHES = N_TEST_SAMPLES // BATCH
comptime IN = 3 * 32 * 32  # 3072
comptime PASS_ACC = 0.40
...
def main() raises:
    print("Conv-PCN CIFAR-10 scale check (P4, CPU Accelerate)\n")
    ...
    for epoch in range(EPOCHS):
        ...
        # train-subset accuracy probe (cheap signal each epoch)
    ...
    print("")
    if acc >= PASS_ACC:
        print("✅ PASS — conv-PCN learns CIFAR-10 natural images (acc", acc, ")")
    else:
        print("❌ FAIL — acc", acc, "<", PASS_ACC)
        raise Error("conv-PCN CIFAR-10 below threshold")
