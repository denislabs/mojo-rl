"""Phase B gate — conv-PC PCModule trains on nn2 storage + nn2 Adam (CPU).
...
block's `pc_init_params`), and trained by nn2 `Adam` driven from PCN's
settling loop.
...
Validates:
  - `PCModule[ConvPCBlock, PCBlock]` (mixed block types) conforms to nn2
...
Run:
    pixi run mojo run -I . tests/pcn/test_pc_module_conv_nn2.mojo
"""
...
def main() raises:
    ...
    print("PASS: conv-PC PCModule trains on nn2 storage via nn2 Adam (Phase B)")
