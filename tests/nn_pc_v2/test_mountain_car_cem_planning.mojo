"""MountainCar Continuous CEM planning demo using the PCN amortized world model.

Trains the same world model as `test_mountain_car_amortized_pc.mojo`, then runs
a Cross-Entropy Method (CEM) planner that uses the world model in imagination
to pick actions, executing them in real env physics. Tests whether the world
model is accurate enough at the horizons CEM needs (~20 steps) to actually
solve the env.
...
Run:
    pixi run mojo run -I . tests/nn_pc_v2/test_mountain_car_cem_planning.mojo
"""
...
def main() raises:
    print("=" * 60)
    print("MountainCar Continuous — CEM planning demo (PCN world model)")
    print("=" * 60)
    ...
    print("  σ₀=", INITIAL_SIGMA, "  σ_min=", MIN_SIGMA, "  action_penalty=", ACTION_PENALTY)
