"""Pendulum CEM planning demo (PCN world model, swish+identity variant).

Identical to `test_pendulum_cem_planning.mojo` except the world-model
PCBlock activations:
...
Motivation: the PCTanh choice was inherited from earlier MNIST/world-model
work (PCIdentity recurrence was unstable, PCTanh worked, PCSwish never
tested). PCN MBPO switched to PCSwish hidden + PCIdentity readout (matching
vanilla MBPO's 4-LinearSwish + linear-readout dynamics) and saw better
behavior. Question: does the same swap rescue Pendulum CEM (baseline 0/5)?

Run:
    pixi run mojo run -I . tests/nn_pc_v2/test_pendulum_cem_planning_swish.mojo
"""
...
def main() raises:
    print("=" * 60)
    print("Pendulum — CEM planning demo (PCN world model, swish+identity)")
    print("=" * 60)
    print("  PC arch    : PCBlock[", AUG_DIM, ",", HIDDEN, ",PCSwish] → PCBlock[", HIDDEN, ",", OBS_DIM, ",PCIdentity]")
    ...
    print("  Eval       : ", N_EVAL_EPISODES, " episodes,", MAX_EPISODE_STEPS, " steps each")
        ...
        for step in range(MAX_EPISODE_STEPS):
            ...
            for cem_iter in range(N_CEM_ITERS):
                ...
                for h in range(PLAN_HORIZON):
                    ...
                    for s in range(N_SAMPLES):
                        var cos_pred = Float64(cem_obs_pred_buf[s * OBS_DIM + 0])
