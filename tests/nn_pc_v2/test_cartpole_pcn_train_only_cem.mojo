"""CartPole-Continuous CEM planning — Phase-1 ablation (Experiment 1).

PCN-train + encoder-only-eval. Trains the world model exactly as
`test_cartpole_cem_planning.mojo` (full PCN: amortized PC + SGLD
...
See `docs/PCN_MBRL_PLAN.md`, Phase-1 Experiment 1 for hypothesis details.
Comparison only — pass criterion (≥ 3/5) reported but not enforced.

Run:
    pixi run mojo run -I . tests/nn_pc_v2/test_cartpole_pcn_train_only_cem.mojo
"""
...
# World-model architecture (PC) — identical to PCN baseline.
comptime BATCH = 32
comptime HIDDEN = 64
...
def main() raises:
    print("=" * 60)
    print("CartPole-Continuous — Exp 1: PCN-train + encoder-only-eval")
    print("=" * 60)
    ...
    # BATCH=1 views for per-step encoder pass at eval time.
