"""Pendulum SAC — Phase-2 PCN-encoded variant.

End-to-end pipeline in one Mojo process:
1. Train a PCN-style world model on Pendulum random rollouts using the
   Exp-3 procedure (per-step local energy weight rule, no SGLD inference).
2. Freeze the encoder. Wrap PendulumEnv with EncoderWrappedEnv so that
   reset/step return the encoded latent z (HIDDEN-dim) instead of raw obs.
3. Train SAC on the wrapped env using the same hyperparameters as the
   baseline (`test_pendulum_sac_baseline.mojo`) but with `obs_dim=HIDDEN`.

Encoder is trained on `[cos θ, sin θ, θ_dot/8]` and `action_norm ∈ [-1, 1]`
(matching the existing PCN Pendulum tests). Wrapper applies the same
normalization (obs_divisor=[1,1,8], action_divisor=[2]) before encoding so
that training-time and inference-time scales match.

See `docs/PCN_MBRL_PLAN.md` Phase 2 for context. Comparison-only — output
is the per-episode return curve (CSV).

Run:
    pixi run mojo run -I . tests/nn_pc_v2/test_pendulum_sac_pcn_encoded.mojo
"""
...
# Pendulum physics (Gymnasium defaults — same as test_pendulum_cem_planning).
...
# World-model / encoder architecture — same family as Phase-1 tests.
comptime BATCH = 32
comptime HIDDEN = 64
...
def _gen_rollout_into[
    ...
    """Encoder training rollout — matches normalization scheme used in
    test_pendulum_cem_planning.mojo: action_norm ∈ [-1, 1], obs[2] = θ_dot/8.
    """
...
def main() raises:
    print("=" * 60)
    print("Pendulum SAC — Phase-2 PCN-encoded")
    print("=" * 60)
    ...
    # encoder-output as the latent, energy ∂E/∂z fed back to the encoder).
    # ────────────────────────────────────────────────────────────────────────
    ...
    # Per-step scratch for Exp-3-style training.
    ...
    # ────────────────────────────────────────────────────────────────────────
    # PHASE B — Wrap PendulumEnv with the trained encoder + run SAC.
    # ────────────────────────────────────────────────────────────────────────

    print("\n  --- Phase B: SAC on encoded latent ---")

    # Per-env scratch for the wrapper (BATCH=1).
    ...
    # Normalization to match encoder training: divide θ_dot by 8, torque by 2.
    var w_obs_div = alloc[Scalar[dtype]](OBS_DIM)
    w_obs_div[0] = Scalar[dtype](1.0)
    w_obs_div[1] = Scalar[dtype](1.0)
    w_obs_div[2] = Scalar[dtype](PEND_MAX_SPEED)  # = 8.0
    var w_act_div = alloc[Scalar[dtype]](ACTION_DIM)
    w_act_div[0] = Scalar[dtype](PEND_MAX_TORQUE)  # = 2.0

    var base_env_buf = alloc[PendulumEnv[dtype]](1)
    base_env_buf[0] = PendulumEnv[dtype]()
    var wrapped = EncoderWrappedEnv[
        PendulumEnv[dtype], HIDDEN, ENC_HIDDEN_DIM, ACTION_DIM, OBS_DIM
    ](
        base_env=base_env_buf,
    ...
    var agent = DeepSACAgent[
        ...
        gamma=0.99,
        tau=0.005,
        action_scale=2.0,  # Pendulum torque ∈ [-2, 2]
        alpha=0.1,
        auto_alpha=True,
        alpha_lr=0.0001,
    )
    ...
    var metrics = agent.train(
        ...
        environment_name="Pendulum (PCN-encoded)",
    )
    var sac_train_t = Float64(perf_counter_ns() - t_sac0) / 1e9

    # NOTE: agent.evaluate requires RenderableEnv; the wrapper doesn't
    # implement that trait. Use the train metrics' last-N average instead
    # as the eval-style summary.
