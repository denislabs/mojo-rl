"""CartPole-Continuous CEM planning demo using the PCN amortized world model.

Same training pipeline as the Pendulum amortized PC test (encoder + amortized
posterior + cosine LR + bootstrap refinement), then a CEM planner that uses
the world model in imagination to pick continuous forces. This is the third
real env in the world-model line, after MountainCar (PCN won) and Pendulum
(all approaches failed).

CartPole-Continuous (Gymnasium analytical dynamics, inlined):
- State / Obs (4D): [x, x_dot, θ, θ_dot] (cart pos, cart vel, pole angle, pole
  angular vel). Normalized to [-1, 1] for the world model:
  x/2.4, x_dot/3.0, θ/0.2, θ_dot/2.0.
- Action (1D continuous): force_norm ∈ [-1, 1]; applied force = action × 10 N.
- Reset: each state dim ∼ U(-0.05, 0.05).
- Termination: |x| > 2.4 or |θ| > 0.2 rad.
- Reward (env-native): +1 per surviving step; we use the planner score below.

Planner score: −Σ_h (θ_h² + 0.05·x_h² + 0.001·a_h²). Penalizes pole tilt and
cart drift; tiny action cost prevents wasted force.

Pass criterion: episode survives ≥ 195 of 200 steps without termination. Pass
if ≥ 3 of 5 eval episodes meet that bar.

Run:
    pixi run mojo run -I . tests/nn_pc_v2/test_cartpole_cem_planning.mojo
"""
...
def main() raises:
    print("=" * 60)
    print("CartPole-Continuous — CEM planning demo (PCN world model)")
    print("=" * 60)
    ...
    for ep in range(N_EVAL_EPISODES):
        ...
        _ = TRAINER.compute_grads_from_latents[1](
            pc_params, pc_grads, latents_1,
            mu_eps_buf_1, a_below_buf_1, z_below_buf_1, dx_buf_1,
            x_in_1, y_target_1,
