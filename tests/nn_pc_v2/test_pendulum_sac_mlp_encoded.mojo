"""Pendulum SAC — Phase-2 MLP-encoded variant (control for PCN-encoded).

Same end-to-end pipeline as `test_pendulum_sac_pcn_encoded.mojo` but the
encoder is trained jointly with an MLP transition + decoder via K-step BPTT
(matches the existing `test_pendulum_mlp_bptt_cem.mojo` recipe). Only the
encoder is used at SAC time; transition + decoder are discarded after
training.

Same architecture (HIDDEN=64, single Linear+tanh per module), same SAC
hyperparameters as baseline + PCN-encoded. The only thing that varies
across the three Phase-2 variants is what the encoder was trained on
(nothing / PCN / MLP+BPTT).

See `docs/PCN_MBRL_PLAN.md` Phase 2 for context.

Run:
    pixi run mojo run -I . tests/nn_pc_v2/test_pendulum_sac_mlp_encoded.mojo
"""
