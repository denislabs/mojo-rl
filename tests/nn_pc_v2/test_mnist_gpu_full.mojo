"""Full Bogacz notebook 1 reproduction — MNIST PCN (GPU).

Mirrors `references/pcn/PredictiveCoding-main/1_supervised_learning_pc.ipynb`
exactly: 784→256→256→10 with ReLU, 60k train, 10 epochs, batch 500,
T=20 inference iterations, latent SGD lr=0.01, Adam lr=0.001 on weights.

Bogacz reports ≥95% test accuracy with this recipe.

Pass criterion: ≥95% test accuracy at the end of training.

Run:
    pixi run -e apple  mojo run -I . tests/nn_pc_v2/test_mnist_gpu_full.mojo
    pixi run -e nvidia mojo run -I . tests/nn_pc_v2/test_mnist_gpu_full.mojo
"""
...
comptime EPOCHS = 10
comptime T_INFER = 20
comptime LR_X: Float64 = 0.01           # Bogacz default
comptime ADAM_LR: Float64 = 0.001       # Bogacz default

comptime N_TRAIN_SAMPLES = 60000        # full set
comptime N_TEST_SAMPLES = 10000         # full set
comptime N_TRAIN_BATCHES = N_TRAIN_SAMPLES // BATCH   # 120
comptime N_TEST_BATCHES = N_TEST_SAMPLES // BATCH      # 20

# Bogacz notebook 1: 784 → 256 → 256 → 10 (3-block PCN)
comptime NET = PCSequential[
    PCBlock[784, 256, PCIdentity],
...
def main() raises:
    print("=" * 65)
    print("MNIST PCN (Bogacz canonical, GPU) — full notebook 1 reproduction")
    print("=" * 65)
    print("  arch       : 784 → 256 → 256 → 10")
    ...
    if final_acc >= 0.95:
        print("\n  [PASS] final test acc ≥ 95% — matches Bogacz notebook 1 target")
    elif final_acc >= 0.90:
        print("\n  [PARTIAL] final test acc ≥ 90% but < 95% — close to target, may need tuning")
    elif final_acc >= 0.80:
        print("\n  [PARTIAL] final test acc ≥ 80% but < 90% — algorithm works, needs more epochs/tuning")
    else:
        print("\n  [FAIL] final test acc < 80%")
