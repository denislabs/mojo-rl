"""Caveflyer P2 smoke: render + env wrapper + gym adapter.

Asset-backed (loads the caveflyer sprites). Checks the 64×64×3 observation is
well-formed + non-degenerate, the env steps a thrust+fire rollout, and
`CaveflyerGymEnv` conforms (obs shape/range, action count, terminating episodes).
See `docs/PROCGEN_CAVEFLYER_SCOPE.md`.

Run from repo root:
    pixi run mojo run -I . tests/envs/procgen/test_caveflyer_env.mojo
"""
...
from noeira.envs.procgen.games.caveflyer import DIST_HARD
...
def _tape(step: Int) -> Int:
    var t: List[Int] = [5, 5, 8, 5, 9, 5, 2, 5, 9, 8]
    return t[step % len(t)]


def test_caveflyer_render_obs_wellformed() raises:
    var assets = CaveflyerAssets(ASSET_ROOT)
    var game = CaveflyerGame(DIST_HARD)
    game.reset(0)
    for s in range(30):  # thrust around the cave + fire
        _ = game.step(_tape(s))
...
def test_caveflyer_env_obs() raises:
    var env = CaveflyerEnv(ASSET_ROOT, rand_seed=0, num_levels=10, dist_mode=DIST_HARD)
    var obs = env.reset()
    assert_equal(len(obs), CaveflyerEnv.OBS_DIM)
    assert_true(
    ...
    for s in range(100):
        ...
        assert_equal(len(res.obs), CaveflyerEnv.OBS_DIM)


def test_caveflyer_gym_env() raises:
    comptime E = CaveflyerGymEnv[DT]
