"""Coinrun P2 smoke: render + env wrapper + gym adapter.

Asset-backed (loads the kenney coinrun sprites). Checks the 64×64×3 observation is
well-formed + non-degenerate, the env steps a jump-heavy rollout, and
`CoinrunGymEnv` conforms (obs shape/range, action count, terminating episodes).
See `docs/PROCGEN_COINRUN_SCOPE.md`.

Run from repo root:
    pixi run mojo run -I . tests/envs/procgen/test_coinrun_env.mojo
"""
...
from noeira.envs.procgen.games.coinrun import DIST_HARD
...
def _tape(step: Int) -> Int:
    var t: List[Int] = [7, 7, 7, 8, 7, 7, 5, 7, 8, 7]
    return t[step % len(t)]


def test_coinrun_render_obs_wellformed() raises:
    var assets = CoinrunAssets(ASSET_ROOT)
    var game = CoinrunGame(DIST_HARD)
    game.reset(0)
    for s in range(30):  # let the agent run + jump
        _ = game.step(_tape(s))
...
def test_coinrun_env_obs() raises:
    var env = CoinrunEnv(ASSET_ROOT, rand_seed=0, num_levels=10, dist_mode=DIST_HARD)
    var obs = env.reset()
    assert_equal(len(obs), CoinrunEnv.OBS_DIM)
    assert_true(
    ...
    for s in range(100):
        ...
        assert_equal(len(res.obs), CoinrunEnv.OBS_DIM)


def test_coinrun_gym_env() raises:
    comptime E = CoinrunGymEnv[DT]
