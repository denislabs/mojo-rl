"""Starpilot P2 smoke: render + env wrapper + gym adapter.

Asset-backed (loads the ~50 starpilot sprites). Checks the 64×64×3 observation is
well-formed + non-degenerate, the env accrues kill reward over a fire-heavy rollout,
and `StarpilotGymEnv` conforms (obs shape/range, action count, terminating episodes).
See `docs/PROCGEN_STARPILOT_SCOPE.md`.

Run from repo root:
    pixi run mojo run -I . tests/envs/procgen/test_starpilot_env.mojo
"""
...
from noeira.envs.procgen.games.starpilot import DIST_HARD
...
def _tape(step: Int) -> Int:
    var t: List[Int] = [9, 4, 7, 9, 5, 9, 3, 9, 10, 4]
    return t[step % len(t)]


def test_starpilot_render_obs_wellformed() raises:
    var assets = StarpilotAssets(ASSET_ROOT)
    var game = StarpilotGame(DIST_HARD)
    game.reset(0)
    for s in range(60):  # let enemies fly in
        _ = game.step(_tape(s))
...
def test_starpilot_env_obs() raises:
    var env = StarpilotEnv(ASSET_ROOT, rand_seed=0, num_levels=10, dist_mode=DIST_HARD)
    var obs = env.reset()
    assert_equal(len(obs), StarpilotEnv.OBS_DIM)
    assert_true(
    ...
    for s in range(100):
        ...
        assert_equal(len(res.obs), StarpilotEnv.OBS_DIM)


def test_starpilot_gym_env() raises:
    comptime E = StarpilotGymEnv[DT]
