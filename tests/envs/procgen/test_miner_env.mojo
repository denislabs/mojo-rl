"""Miner P2 smoke: render + env wrapper + gym adapter.

Asset-backed. Checks the 64×64×3 observation is well-formed + non-degenerate, the
env produces correctly-sized obs, and `MinerGymEnv` conforms (obs shape/range,
action count, terminating episodes). See `docs/PROCGEN_MINER_SCOPE.md`.

Run from repo root:
    pixi run mojo run -I . tests/envs/procgen/test_miner_env.mojo
"""
...
def _tape(step: Int) -> Int:
    var t: List[Int] = [7, 7, 3, 3, 1, 1, 5, 7, 3, 4]
    return t[step % len(t)]


def test_miner_render_obs_wellformed() raises:
    var assets = MinerAssets(ASSET_ROOT)
    var game = MinerGame(DIST_HARD)  # 20×20, dense dirt/boulders/diamonds
    game.reset(0)
...
def test_miner_env_obs() raises:
    var env = MinerEnv(ASSET_ROOT, rand_seed=0, num_levels=10, dist_mode=DIST_HARD)
    var obs = env.reset()
    assert_equal(len(obs), MinerEnv.OBS_DIM)
    assert_true(
    ...
    for s in range(50):
        ...
        assert_equal(len(res.obs), MinerEnv.OBS_DIM)


def test_miner_gym_env() raises:
    comptime E = MinerGymEnv[DT]
