"""Climber P2 smoke: render + env wrapper + gym adapter.

Asset-backed. Checks the 64×64×3 obs is well-formed + non-degenerate, the env
steps a jump rollout, and `ClimberGymEnv` conforms. See `docs/PROCGEN_CLIMBER_SCOPE.md`.
"""

from std.testing import assert_equal, assert_true, TestSuite

from noeira.envs.procgen.games import (
    ClimberGame, ClimberAssets, ClimberEnv, ClimberGymEnv
)
from noeira.envs.procgen.games.climber import DIST_HARD
...
def test_climber_render_obs_wellformed() raises:
...
def test_climber_env_obs() raises:
    var env = ClimberEnv(ASSET_ROOT, rand_seed=0, num_levels=10, dist_mode=DIST_HARD)
    var obs = env.reset()
    assert_equal(len(obs), ClimberEnv.OBS_DIM)
    assert_true(env.current_level_seed >= 0 and env.current_level_seed < 10, "level seed range")
    for s in range(100):
        var res = env.step(_tape(s))
        assert_equal(len(res.obs), ClimberEnv.OBS_DIM)


def test_climber_gym_env() raises:
    comptime E = ClimberGymEnv[DT]
