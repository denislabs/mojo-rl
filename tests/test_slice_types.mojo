    ...
    comptime if N == 0:
        ...
        comptime if (ops[0].OP_ID == OpID.MATMUL._value
            ...
            comptime if ops[2].OP_ID == OpID.RELU._value:
                print("    -> FusedMBR[", ops[0].IN_DIM, ",", ops[0].OUT_DIM, "]")
                comptime if N > 3:
                    comptime rest = Variadic.slice_types[
                        element_types=ops, start=3, end=Variadic.size(ops)]
                    greedy_fuse[*rest]()
            elif ops[2].OP_ID == OpID.TANH._value:
                print("    -> FusedMBT[", ops[0].IN_DIM, ",", ops[0].OUT_DIM, "]")
                comptime if N > 3:
                    comptime rest = Variadic.slice_types[
                        element_types=ops, start=3, end=Variadic.size(ops)]
                    greedy_fuse[*rest]()
...
fn main():
    ...
    greedy_fuse[
        ...
        MatMul[32, 16], BiasAdd[16], ReLUOp[16],
        MatMul[16, 2], BiasAdd[2],
    ]()
    print()
