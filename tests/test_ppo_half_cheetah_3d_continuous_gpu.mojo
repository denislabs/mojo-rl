"""Test PPO Continuous Agent GPU Training on HalfCheetah3D.

This tests the GPU implementation of PPO with continuous actions using the
HalfCheetah3D environment with:
- Parallel environments on GPU
- Full 3D rigid body physics with physics3d modular components
...
Run with:
    pixi run -e apple mojo run tests/test_ppo_half_cheetah_3d_continuous_gpu.mojo    # Apple Silicon
    pixi run -e nvidia mojo run tests/test_ppo_half_cheetah_3d_continuous_gpu.mojo   # NVIDIA GPU
"""
...
from envs.half_cheetah_3d.constants3d import HC3DConstantsGPU
...
# HalfCheetah3D: 17D observation, 6D continuous action
comptime OBS_DIM = HC3DConstantsGPU.OBS_DIM  # 17
comptime ACTION_DIM = HC3DConstantsGPU.ACTION_DIM  # 6
...
fn main() raises:
    seed(42)
    print("=" * 70)
    print("PPO Continuous Agent GPU Test on HalfCheetah3D")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        var agent = DeepPPOContinuousAgent[
            ...
            checkpoint_path="ppo_half_cheetah_3d.ckpt",
            normalize_rewards=True,
            obs_noise_std=0.0,
        )

        print("Environment: HalfCheetah3D Continuous (GPU)")
        ...
        print()
        print("HalfCheetah3D specifics:")
        print("  - Full 3D physics with quaternion orientations")
        print("  - physics3d modular components")
        print("  - GPU-compatible atan2 for joint angle computation")
        print("  - 17D observations: [torso_z, torso_pitch,")
        ...
        print("  - 6D continuous actions (joint torques)")
        print("  - Reward: forward_velocity - ctrl_cost")
        print("  - No termination on falling (MuJoCo-style)")
        print()
        ...
        try:
            var metrics = agent.train_gpu[HalfCheetah3D[dtype]](
                ctx,
