"""Stage-4 end-to-end: DQN trained on the DATA-LAYER replay buffer.

A copy of `tests/nn/test_dqn_trainer_smoke.mojo` with ONE token changed —
`UniformSampleCpuStep` -> `StoreUniformSampleCpuStep`. That is the entire
migration for an algorithm, because DQNAgent/DQNTrainer take
...
from noeira.data.blocks import StoreUniformSampleCpuStep
...
def test_dqn_cpu() raises:
    ...
    var trainer = DQNTrainer[
        "cpu",
        StoreUniformSampleCpuStep[OBS_DIM, 1, BATCH, CAP],
        QNet,
    ].make(
        lr=Scalar[DT](1e-3),
