"""GPU MNIST MLP — Phase 2 exit criterion.

Uses `trainer.train_gpu(...)` whole-dataset API: upload the full train +
test sets to the GPU once outside the loop, then let the trainer slice
batches internally by pointer offset. No host→host or host→device copies
in the training inner loop.

Target: >=97% test accuracy after 5 epochs.
"""
...
def test_mnist_mlp_gpu() raises:
    ...
    # ── Upload the entire train + test sets to the GPU once. ──────────
    ...
    print("  test_mnist_mlp_gpu PASSED")


def main() raises:
    print("=" * 60)
    print("nn2 GPU MNIST MLP (Phase 2 exit criterion)")
    print("=" * 60)
    test_mnist_mlp_gpu()
    print("=" * 60)
    print("ALL PASSED")
