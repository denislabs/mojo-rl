"""DreamerV3Agent smoke — on-policy running-carry action selection.

Real PendulumV2 loop: warmup with random actions, then collect with the
agent's `select_action` (running belief carry), train every N steps, and
run a short greedy eval. Gate: actions in range, WM/AC losses finite, WM
decreases, greedy eval finite.

Run: `pixi run mojo run -I . tests/nn2/test_dreamerv3_agent_smoke.mojo`
"""
...
def main() raises:
    print("=" * 70)
    print("DreamerV3Agent smoke — on-policy select_action (Pendulum)")
    print("=" * 70)
    seed(7)
    ...
    print("=" * 70)
    print("SMOKE PASSED — DreamerV3Agent on-policy select_action works")
    print("=" * 70)
