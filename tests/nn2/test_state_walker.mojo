...
Phase A.1 smoke. Mirrors `tests/nn2/spikes/spike_reflect_checkpoint.mojo`
but exercises the *production* `Saveable` trait + `SaveScalar[DT]` +
`SaveI` + `dump_state` / `load_state` from `mojo_rl.nn2.core`.
...
from mojo_rl.nn2.core.save_scalar import SaveScalar, SaveI
