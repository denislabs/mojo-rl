...
from mojo_rl.nn2.primitives.binary_add import BinaryAdd
...
from mojo_rl.nn2.primitives.ops.binary_add_op import BinaryAddOp
...
def test_binary_add_parity() raises:
    print("test_binary_add_parity ...")
    comptime BATCH = 4
    comptime DIM = 8
    comptime N = BATCH * DIM
    var old_op = BinaryAdd[DIM].make[target="cpu", INIT=Zero]()
    var new_op = BinaryElementwise[DIM, BinaryAddOp].make[
    ...
    assert_true(df == Scalar[DT](0), "BinaryAdd forward mismatch")
    assert_true(dgx == Scalar[DT](0), "BinaryAdd gx mismatch")
    assert_true(dgy == Scalar[DT](0), "BinaryAdd gy mismatch")
    print("  ok")
