...
def main() raises:
    ...
    print(
        ...
        bn.running_mean.value[0], bn.running_mean.value[1],
    )
    # Sanity: the EMA actually moved the stats off their 0/1 init.
    assert_true(
        bn.running_mean.value[0] != Scalar[DT](0.0)
        or bn.running_var.value[0] != Scalar[DT](1.0),
    ...
    for f in range(DIM):
        var dm = fresh.running_mean.value[f] - bn.running_mean.value[f]
        var dv = fresh.running_var.value[f] - bn.running_var.value[f]
