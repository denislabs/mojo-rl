...
def test_basic_record_and_sample() raises:
    ...
    var s_buf: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](OBS)
    var a_buf: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](ACT)
    for k in range(12):
        s_buf[0] = Scalar[DT](k)
    ...
    var obs_out: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](B * (T + 1) * OBS)
    var act_out: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](B * T * ACT)
    var rew_out: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](B * T)
    var dne_out: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](B * T)
...
def main() raises:
    print("=" * 60)
    print("nn2 SequenceReplay tests (Block D-5)")
    print("=" * 60)
