...
Compile-viability + correctness probe for the real fix to nn2's per-update
kernel count. nn2 stores each parameter tensor as its OWN `DeviceBuffer`
...
This file wires NOTHING into Adam — it's a throwaway probe. Run:
    pixi run -e apple  mojo run -I . tests/nn2/test_grouped_adam_prototype.mojo
    pixi run -e nvidia mojo run -I . tests/nn2/test_grouped_adam_prototype.mojo
"""
...
def main() raises:
    ...
    # DeviceBuffers (exactly how nn2's Param.make_gpu allocates).
