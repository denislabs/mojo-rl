"""ExternalNode + set_external smoke (Phase 3).

Builds a tiny ComputeGraph that contains an ExternalNode: the
...
def test_external_unary_node_basic() raises:
    ...
    for b in range(BATCH):
        ...
        assert_true(
            ...
            "ExternalNode forward must equal 3·input",
        )
    ...
    for b in range(BATCH):
        ...
        assert_true(
            ...
            "ExternalNode backward must equal 5·grad_output",
        )
...
def test_external_binary_node() raises:
    ...
    Verifies ExternalNode dispatch through a trainer-owned BinarySub."""
    ...
    comptime G = ComputeGraph[
        ...
        InputSlot["a", 1],
        InputSlot["b", 1],
        ExternalNode["sub_ext", BinarySub[1], "a", "b"],
    ]
    var g = G.make[target="cpu", INIT=Kaiming]()

    var sub = BinarySub[1].make[target="cpu", INIT=Kaiming]()
    g.set_external["sub_ext", BinarySub[1]](sub)
    ...
    for i in range(BATCH):
        ...
        assert_true(
            ...
            "ExternalNode forward must equal a − b",
        )
    ...
    # pointer stored in the graph by `set_external` isn't dangling.
    # Without this, the compiler can end `sub`'s lifetime at the
    # `set_external` call site and zero-fill its storage.
    _ = sub^
    print("  ok")
...
def main() raises:
    print("=" * 70)
    print("ExternalNode + ExternalNode smoke (Phase 3)")
    print("=" * 70)
