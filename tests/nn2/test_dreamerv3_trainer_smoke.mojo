...
def main() raises:
    ...
    for it in range(ITERS):
        ...
        assert_true(tr.last_wm_loss == tr.last_wm_loss, "WM loss finite")
        assert_true(tr.last_ac_loss == tr.last_ac_loss, "AC loss finite")
        if it == 0:
            first_wm = tr.last_wm_loss
            first_ac = tr.last_ac_loss
            print("  iter 0   WM =", first_wm, " AC =", first_ac)
        if it == ITERS - 1:
            last_wm = tr.last_wm_loss
            last_ac = tr.last_ac_loss
    ...
    assert_true(last_ac < first_ac, "AC loss must decrease")
    print("=" * 70)
    print("SMOKE PASSED — DreamerV3Trainer trains (WM+AC decrease), no NaN")
    print("=" * 70)
