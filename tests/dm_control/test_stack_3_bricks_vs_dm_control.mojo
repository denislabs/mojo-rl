"""`manipulation/stack_3_bricks_features` against dm_control.
...
`_add_or_remove_freejoints` always strips the freejoint from brick 0. None of
...
⚠⚠ THE REWARD AVERAGES TWO PAIRS, and that is what leg 4 is really testing.
With `k` of the three bricks stacked the reward should step roughly 0 -> 0.5 ->
1.0, because `np.mean` over `[clicked_pair_0, clicked_pair_1]` is a MEAN and not
a min or a sum. A reward that took the worst pair would read 0 until the whole
tower is built; one that summed would reach 2.
...
FIVE LEGS:
  ...
  4. the reward, including that a HALF-BUILT tower scores about 0.5;
...
comptime ENV = DMStack3Bricks[DTYPE]
comptime TASK: StaticString = "stack_3_bricks_features"
...
# ── leg 1 ──────────────────────────────────────────────────────────────────
def test_stack_3_bricks_element_indices_match_mujoco() raises:
...
# ── leg 2 ──────────────────────────────────────────────────────────────────
def test_stack_3_bricks_position_stage_observation_matches_dm_control() raises:
...
# ── leg 3 ──────────────────────────────────────────────────────────────────
def test_stack_3_bricks_joints_torque_matches_dm_control() raises:
...
# ── leg 4 ──────────────────────────────────────────────────────────────────
def test_stack_3_bricks_reward_matches_dm_control() raises:
    print("=== 4. the reward ===")
    var refmod = _refmod()
    var env = ENV()
    var arm = _arm_of(0)
    ...
    # ⚠ NON-VACUITY FOR THE MEAN. Two of three bricks stacked clicks exactly
    # one of the two pairs, so the mean must land near 0.5 — a `min` would
    # read ~0 here and a sum ~1.
    assert_true(
        rewards[2] > 0.45 and rewards[2] < 0.56,
        "a half-built tower does not score about 0.5, so the reward is not"
        " the MEAN over pairs — `min` would read near 0 and a sum near 1",
    )
    assert_true(
...
# ── leg 5 ──────────────────────────────────────────────────────────────────
def test_stack_3_bricks_reset_matches_dm_control() raises:
    print("=== 5. reset() ===")
    var refmod = _refmod()
    var env = ENV()
