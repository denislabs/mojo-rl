"""`manipulation/place_cradle_features` against dm_control.

Phase 7's fifth task and the third and last task CLASS in the family, after
`Reach` and `Lift`. Three things here have no precedent in the four already
gated:

  * A PROP THAT IS NOT A FREE BODY. `Place` attaches its pedestal with
    `arena.attach`, so it is moved at reset by writing its attachment frame's
    `body_pos` — a MODEL constant, not `qpos`. `nbody` is 20 while `njnt` stays
    10, and a gate that injects only `qpos` would compare two engines with the
    pedestal in different places and nothing in the state to show for it.
  * A COMPOSITE REWARD WITH A SWITCH. `grasp` is weighted by `1 - in_place`
    and `hand_away` by `in_place`, so the task pays for holding the brick until
    it is on the target and for letting go after. Summing the terms instead is
    a plausible reward that rewards hovering, and at any single probe point the
    two are hard to tell apart — leg 4 sweeps the brick from the gripper to the
    target so `in_place` crosses from 0 to 1.
  * `sigmoid='long_tail'` ON ALL THREE TERMS, not the gaussian default.
...
from mojo_rl.envs.dm_control.manipulation_place_cradle import DMPlaceCradle
...
comptime ENV = DMPlaceCradle[DTYPE]
comptime TASK: StaticString = "place_cradle_features"
...
# ── leg 1 ──────────────────────────────────────────────────────────────────
def test_place_cradle_element_indices_match_mujoco() raises:
...
# ── leg 2 ──────────────────────────────────────────────────────────────────
def test_place_cradle_position_stage_observation_matches_dm_control() raises:
...
# ── leg 3 ──────────────────────────────────────────────────────────────────
def test_place_cradle_joints_torque_matches_dm_control() raises:
...
# ── leg 4 ──────────────────────────────────────────────────────────────────
def test_place_cradle_reward_matches_dm_control() raises:
...
# ── leg 5 ──────────────────────────────────────────────────────────────────
def test_place_cradle_reset_matches_dm_control() raises:
