"""Quick GPU-only evaluation to verify continuous PPO checkpoint works on HalfCheetah3D.

This tests that the trained continuous PPO model performs well on the GPU environment
it was trained on.

Run with:
    pixi run -e apple mojo run tests/test_ppo_half_cheetah_3d_continuous_eval_gpu.mojo
    pixi run -e nvidia mojo run tests/test_ppo_half_cheetah_3d_continuous_eval_gpu.mojo
"""
...
from physics3d import dtype as gpu_dtype
...
fn main() raises:
    seed(42)
    print("=" * 70)
    print("PPO Continuous Agent GPU Evaluation on HalfCheetah3D")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        try:
            agent.load_checkpoint("ppo_half_cheetah_3d.ckpt")
            ...
            print(
                ...
                " tests/test_ppo_half_cheetah_3d_continuous_gpu.mojo"
            )
            return
        ...
        var stochastic_reward = agent.evaluate_gpu[HalfCheetah3D[gpu_dtype]](
            ctx,
        ...
        var deterministic_reward = agent.evaluate_gpu[HalfCheetah3D[gpu_dtype]](
            ctx,
            num_episodes=EVAL_EPISODES,
            max_steps=MAX_STEPS,
            verbose=False,
        ...
        print()
        print("=" * 70)
        print("GPU EVALUATION SUMMARY - HalfCheetah3D")
        print("=" * 70)
        print()
        ...
        print("HalfCheetah3D expected rewards:")
