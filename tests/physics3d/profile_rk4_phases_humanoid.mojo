...
def main() raises:
    ...
    with DeviceContext() as ctx:
        ...
        # NOTE: the full RK4 step_gpu is intentionally NOT instantiated here.
        # With FK/vel/cdof-mt default-ON, the fused rk4_stage_kernel inlines all
        # three cooperative walks + the blocked solver — by far the largest
        # compile unit in the codebase — and compiling it alongside these phase
        # kernels OOMs the host (Mojo→PTX) compiler against the container cgroup
        # ("Killed" before runtime). The per-phase split doesn't need it; the
        # PYRAMIDAL full step was already confirmed to run (0 MB reserved) earlier.
