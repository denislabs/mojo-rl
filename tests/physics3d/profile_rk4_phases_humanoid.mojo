...
def main() raises:
    ...
    with DeviceContext() as ctx:
        ...
        print("Profiling individual phases (" + String(N_STEPS) + " launches)...")
        print("-" * 60)
        ...
        t0 = perf_counter_ns()
        for _ in range(N_STEPS):
            ctx.enqueue_function[detect_kernel, detect_kernel](
                state, model, grid_dim=(ENV_BLOCKS,), block_dim=(TPB,)
            )
        ctx.synchronize()
        t1 = perf_counter_ns()
        var detect_us = Float64(t1 - t0) / 1000.0 / Float64(N_STEPS)
        print("3.  Contact detection:       " + String(detect_us)[byte=:8] + " μs")

        # Phase 3a: Subtree COM
        ctx.synchronize()
        t0 = perf_counter_ns()
        for _ in range(N_STEPS):
            ctx.enqueue_function[com_kernel, com_kernel](
                state, model, grid_dim=(ENV_BLOCKS,), block_dim=(TPB,)
            )
        ctx.synchronize()
        t1 = perf_counter_ns()
        ...
        print("4.  CDOF (motion axes):      " + String(cdof_us)[byte=:8] + " μs")
        ...
        print("8.  M_inv from LDL:          " + String(minv_us)[byte=:8] + " μs")
        ...
        print("10. LDL solve:               " + String(solve_us)[byte=:8] + " μs")
        ...
        print("Full RK4 step (×20 passes):  " + String(full_us)[byte=:8] + " μs")
        ...
        print(
            "  Contacts:    " + String(detect_us / phases_total * 100)[byte=:5] + "%"
        )
        print("  SubtreeCOM:  " + String(com_us / phases_total * 100)[byte=:5] + "%")
        print(
        ...
        print("=" * 60)
        print()
        print("FK + Velocities (Lever 1 target) = "
            + String((fk_us + vel_us) / phases_total * 100)[byte=:5] + "% of phases")
