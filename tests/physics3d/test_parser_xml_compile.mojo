"""Experiment: can we parse a MuJoCo XML string at comptime and get
the model dimensions (NBODY, NJOINT, NQ, NV, NGEOM, NACT)?

Expected output for the inline HalfCheetah XML:
  NBODY  = 8  (worldbody + 7 bodies)
  NJOINT = 9  (rootx, rootz, rooty + 6 limb joints)
  NQ     = 9  (all hinge/slide → 1 DOF each)
  NV     = 9
  NGEOM  = 9  (1 floor + 8 body geoms)
...
def test_parser_xml_compile() raises:
    ...
    print("NBODY  =", model.NBODY, " (expected 8)")
    print("NJOINT =", model.NJOINT, " (expected 9)")
    print("NQ     =", model.NQ, " (expected 9)")
    print("NV     =", model.NV, " (expected 9)")
    print("NGEOM  =", model.NGEOM, " (expected 9)")
