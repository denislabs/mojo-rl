...
Run:  pixi run -e apple mojo run -I . tests/experimental/lewm2/test_paper_protocol.mojo
"""
...
# run_lewm2_paper_protocol (the NVIDIA example uses exactly this wiring).
...
def main() raises:
    ...
    var r = run_lewm2_paper_protocol[
        ...
        viz_path=String("/tmp/lewm2_protocol_toy.ppm"),
        ctx=ctx,
        verbose=True,
    )
    ...
    var rc = run_lewm2_paper_protocol[
        IN_CH, IMG, PATCH, HIDDEN, ENC_HEADS, ENC_LAYERS, EMB, ENC_PROJ_H,
        ENC_FF_MULT, T, ACT, SMOOTHED, AE_MLP, H, N_PREDS, PRED_HEADS,
        PRED_FF, DEPTH, PRED_PROJ_H, SIG_PROJ, SIG_KNOTS, B, MPC_HORIZON,
