...
def test_select_action_only() raises:
    ...
    try:
        ...
        var obs_host: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](OBS_DIM)
        var act_host: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](1)
        for d in range(OBS_DIM):
            obs_host[d] = Scalar[DT](0.1 * Float64(d))
        ctx.enqueue_copy(obs_dev, obs_host)
        var obs_p = rebind[UnsafePointer[Scalar[DT], MutAnyOrigin]](obs_dev.unsafe_ptr())
        var act_p = rebind[UnsafePointer[Scalar[DT], MutAnyOrigin]](act_dev.unsafe_ptr())
