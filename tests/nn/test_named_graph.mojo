...
fn test_simple_chain() raises:
    ...
    var pt = LayoutTensor[dtype, Layout.row_major(M.PARAM_SIZE), MutAnyOrigin](p.unsafe_ptr())
    ...
    var out_t = LayoutTensor[dtype, Layout.row_major(BATCH, M.OUT_DIM), MutAnyOrigin](out.unsafe_ptr())
    var cache = InlineArray[Scalar[dtype], BATCH * M.CACHE_SIZE](uninitialized=True)
    var cache_t = LayoutTensor[dtype, Layout.row_major(BATCH, M.CACHE_SIZE), MutAnyOrigin](cache.unsafe_ptr())
    ...
    var go_t = LayoutTensor[dtype, Layout.row_major(BATCH, M.OUT_DIM), MutAnyOrigin](go.unsafe_ptr())

    var gi = InlineArray[Scalar[dtype], BATCH * M.IN_DIM](uninitialized=True)
    var gi_t = LayoutTensor[dtype, Layout.row_major(BATCH, M.IN_DIM), MutAnyOrigin](gi.unsafe_ptr())
    var g = InlineArray[Scalar[dtype], M.PARAM_SIZE](uninitialized=True)
    for i in range(M.PARAM_SIZE):
        g[i] = Scalar[dtype](0.0)
    var gt = LayoutTensor[dtype, Layout.row_major(M.PARAM_SIZE), MutAnyOrigin](g.unsafe_ptr())
