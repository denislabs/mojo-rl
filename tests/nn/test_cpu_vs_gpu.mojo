"""Level 2: CPU vs GPU consistency test for all nn layer types.

Same model, same params, same input -- compares forward outputs and backward
gradients between CPU and GPU implementations. Catches GPU kernel bugs,
accumulation order differences, and matmul padding issues.

On Apple Silicon, expect near-exact match (< 1e-5).
On NVIDIA, differences may be larger due to cuBLAS matmul implementation.

Usage:
    pixi run -e apple mojo run -I . tests/nn/test_cpu_vs_gpu.mojo
    pixi run -e nvidia mojo run -I . tests/nn/test_cpu_vs_gpu.mojo
"""
...
def cpu_vs_gpu_check[M: Model, BS: Int = 4](
    ...
    Reports per-component max_abs and max_rel errors, plus PASS/FAIL.
    """
    ...
    # ── Create grad_output (same for CPU and GPU) ────────────
...
def main() raises:
    print("=== NN CPU vs GPU Consistency ===")
    print()
    ...
    # Conv trunk + Parallel FC dual-head (AlphaZero-like, small)
