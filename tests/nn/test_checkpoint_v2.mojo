...
Phase A.2 validation. Mirrors `tests/nn/test_checkpoint.mojo` (v1)
...
def test_linear_roundtrip_v2() raises:
    ...
    var path = String("/tmp/test_nn_ckpt_v2_linear.txt")
    comptime BATCH = 2
    comptime IN = 3
    comptime OUT = 2
...
def test_sequential_roundtrip_v2_after_training() raises:
    ...
    var path = String("/tmp/test_nn_ckpt_v2_sequential.txt")
    comptime BATCH = 4
    comptime IN = 4
    comptime HID = 8
    comptime OUT = 2
