"""Checksum parity for FusedConv2DActivation (Conv2DReLU/Tanh/Mish wrappers).

Captures output / grad_input / grad_W sums for each shape, used to validate
the BLAS-matmul refactor: numbers should match baseline to ~1e-5 relative.

Run:
    pixi run mojo run -I . tests/nn/test_fused_conv_act_parity.mojo
"""
...
from mojo_rl.nn.model import Conv2DReLU, Conv2DTanh, Conv2DSigmoid, Conv2DLayer
...
def check[
    ...
    """Skeleton — uses LAYER for the actual struct. Mojo trait constraint
    'AnyType' here is just a placeholder; we use specific Conv2D... names
    in main() to keep things simple. The body below is type-erased via
    the inline check_conv helper below."""
    pass


# Mojo nightly doesn't easily pass a Model trait type around; just write
# one big test function per variant inline.
...
def main() raises:
    print("=" * 70)
    print(" FusedConv2DActivation parity checksum")
    print("=" * 70)
    print()

    check_conv[Conv2DReLU[2, 4, 3, 1, 1, 5, 5], 4](
        "Conv2DReLU[2,4,3x3,5x5]"
    )
    check_conv[Conv2DReLU[3, 8, 3, 1, 0, 3, 3], 4](
        "Conv2DReLU[3,8,3x3,3x3 valid]"
    )
    check_conv[Conv2DReLU[128, 128, 3, 1, 1, 3, 3], 8](
        "Conv2DReLU[128,128,3x3,3x3 same] (AZ TTT-CNN sized)"
    )
    check_conv[Conv2DReLU[128, 128, 3, 1, 0, 3, 3], 8](
        "Conv2DReLU[128,128,3x3,1x1 valid] (AZ TTT-CNN sized L4)"
    )
    check_conv[Conv2DTanh[2, 4, 3, 1, 1, 5, 5], 4](
        "Conv2DTanh[2,4,3x3,5x5]"
    )
    check_conv[Conv2DSigmoid[2, 4, 3, 1, 1, 5, 5], 4](
        "Conv2DSigmoid[2,4,3x3,5x5]"
    )
    check_conv[Conv2DLayer[2, 4, 3, 1, 1, 5, 5], 4](
        "Conv2DLayer[2,4,3x3,5x5] (no activation — sanity)"
    )
