"""Phase 8.3 exit criterion: MNIST MLP CPU with `POLICY=Bf16Compute`.

Mirror of `test_mnist_mlp.mojo` (Phase 1 baseline) but the Trainer
carries `POLICY=Bf16Compute` so Linear runs its CPU matmuls via the
cast-around-matmul path (fp32 weights → bf16 → linalg.matmul[target="cpu"]
in bf16 → bf16 output → fp32). ReLU, CrossEntropyLoss, and Adam stay fp32.

Exit criterion (Phase 8.3): test top-1 within 0.3% of the fp32 baseline
(97.6% from Phase 8.0). bf16 should be within noise for a small MLP.
"""
...
from mojo_rl.nn.core import Bf16Compute


def test_mnist_mlp_cpu_amp() raises:
    comptime IN_DIM = 784
    comptime H1 = 256
    comptime H2 = 128
    ...
    # Phase 8.0 fp32 CPU baseline = 97.6%. bf16 should land within 0.3%.
    comptime TARGET_ACC: Float64 = 0.973
    ...
    print("initializing network (CPU, POLICY=Bf16Compute)...")
    var net = Sequential(
    ...
    tgt_buf.free()
    out_buf.free()
    print("  test_mnist_mlp_cpu_amp PASSED")


def main() raises:
    print("=" * 60)
    print("nn MNIST MLP CPU AMP (Phase 8.3 — Bf16Compute exit criterion)")
    print("=" * 60)
    test_mnist_mlp_cpu_amp()
    print("=" * 60)
    print("ALL PASSED")
    print("=" * 60)
