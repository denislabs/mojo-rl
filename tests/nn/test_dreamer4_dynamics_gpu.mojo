"""Dreamer4Dynamics — CPU↔GPU parity (Phase 2.4).
    ...
    pixi run -e apple  mojo run -I . tests/nn/test_dreamer4_dynamics_gpu.mojo
    pixi run -e nvidia mojo run -I . tests/nn/test_dreamer4_dynamics_gpu.mojo
...
def main() raises:
    ...
    ctx.enqueue_copy(hsg, dgpu.signal_table.grd.dev.value())
    ctx.enqueue_copy(hst, dgpu.step_table.grd.dev.value())
    ctx.enqueue_copy(hrg, dgpu.register.grd.dev.value())
    ctx.synchronize()
    var cab = rebind[UnsafePointer[Scalar[DT], MutAnyOrigin]](dcpu.action_base.grd.cpu.unsafe_ptr())
    var csg = rebind[UnsafePointer[Scalar[DT], MutAnyOrigin]](dcpu.signal_table.grd.cpu.unsafe_ptr())
    var cst = rebind[UnsafePointer[Scalar[DT], MutAnyOrigin]](dcpu.step_table.grd.cpu.unsafe_ptr())
    var crg = rebind[UnsafePointer[Scalar[DT], MutAnyOrigin]](dcpu.register.grd.cpu.unsafe_ptr())
