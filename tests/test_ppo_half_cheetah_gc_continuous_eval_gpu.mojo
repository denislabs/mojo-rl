"""Quick GPU-only evaluation to verify continuous PPO checkpoint works on HalfCheetahGC.
...
Run with:
    pixi run -e apple mojo run tests/test_ppo_half_cheetah_gc_continuous_eval_gpu.mojo
    pixi run -e nvidia mojo run tests/test_ppo_half_cheetah_gc_continuous_eval_gpu.mojo
"""
...
comptime MAX_STEPS = 1000  # HalfCheetahGC MAX_STEPS
...
fn main() raises:
    seed(42)
    print("=" * 70)
    print("PPO Continuous Agent GPU Evaluation on HalfCheetahGC")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var stochastic_reward = agent.evaluate_gpu[HalfCheetahGC[gpu_dtype]](
            ctx,
        ...
        var deterministic_reward = agent.evaluate_gpu[HalfCheetahGC[gpu_dtype]](
            ctx,
            num_episodes=EVAL_EPISODES,
            max_steps=MAX_STEPS,
            verbose=False,
        ...
        print()
        print("=" * 70)
        print("GPU EVALUATION SUMMARY - HalfCheetahGC")
        print("=" * 70)
        print()
        ...
        print("HalfCheetahGC expected rewards:")
