"""Compile smoke test for the SampledGumbelGPUMCTS orchestrator.

Drives one ``search_gpu`` pass through the orchestrator with stub
``Sequential[Linear]`` rep / dyn / pred networks, asserting only that
the dispatch compiles end-to-end and that chosen_actions / root_visits
land in plausible ranges. Bit-parity vs the legacy
``run_sampled_gumbel_search_gpu`` is the next slice — this is the
build-time signature check.

Usage:
    pixi run -e apple mojo run -I . \\
        tests/planners/tree_search/test_mcts_gpu_gumbel_sampled_compile.mojo
"""
...
def main() raises:
    print("=== SampledGumbelGPUMCTS compile smoke ===")
    ...
    print("SampledGumbelGPUMCTS constructed OK")
    ...
    ](ctx, rep, dyn, pred, obs_t, deterministic=False, rng_seed=UInt32(42))
    ctx.synchronize()
    print("search_gpu returned OK")

    var chosen = mcts.chosen_actions_view()
    var visits = mcts.root_visits_view()
    var root_values = mcts.root_value_view()

    with chosen.map_to_host() as ca_h:
        for e in range(N_ENVS):
            for d in range(ACT_DIM):
                var a = Float64(ca_h[e * ACT_DIM + d])
                print("env", e, "act_dim", d, "=", a)
                if a != a:  # NaN check
                    raise Error("chosen_action NaN")

    with visits.map_to_host() as v_h:
        for e in range(N_ENVS):
            var total = Float64(0.0)
            for i in range(K_ROOT):
                total += Float64(v_h[e * K_ROOT + i])
            print("env", e, "root_visit_sum=", total)
    ...
    with root_values.map_to_host() as rv_h:
        for e in range(N_ENVS):
            ...
            if v != v:
                raise Error("root_value NaN")

    print("PASS: SampledGumbelGPUMCTS compile smoke")
