"""Test PPO Continuous Agent GPU Training on Hopper3D.
...
- action[0]: hip torque (-1.0 to 1.0)
- action[1]: knee torque (-1.0 to 1.0)
- action[2]: ankle torque (-1.0 to 1.0)

Run with:
    pixi run -e apple mojo run tests/test_ppo_hopper_3d_continuous_gpu.mojo    # Apple Silicon
    pixi run -e nvidia mojo run tests/test_ppo_hopper_3d_continuous_gpu.mojo   # NVIDIA GPU
"""
...
# Hopper3D: 11D observation, 3D continuous action
...
fn main() raises:
    seed(42)
    print("=" * 70)
    print("PPO Continuous Agent GPU Test on Hopper3D")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        print("Environment: Hopper3D Continuous (GPU)")
        ...
        print("                       hip_omega, knee_omega, ankle_omega]")
        print("  - 3D continuous actions (hip, knee, ankle torques)")
        print("  - Reward: forward_velocity + alive_bonus - ctrl_cost")
        print("  - Terminates on: torso_z < 0.7 or |pitch| > 1.0 rad")
        print()
        ...
        try:
            var metrics = agent.train_gpu[Hopper3D[dtype]](
                ctx,
