...
struct DeepPPOContinuousAgent[
    ...
    # =========================================================================
    # GPU Training for CarRacing with Track Support
    # =========================================================================

    fn train_gpu_car_racing(
        mut self,
        ctx: DeviceContext,
        num_episodes: Int,
        verbose: Bool = False,
        print_every: Int = 10,
        track_seed: UInt64 = 42,
    ) raises -> TrainingMetrics:
        """Train PPO on CarRacing with full track and tile visit reward support.

        This specialized method creates proper track and visited buffers for
        accurate tile-based rewards matching the CPU evaluation environment.
        ...
        Args:
            ...
            verbose: Whether to print progress.
            print_every: Print progress every N rollouts.
            track_seed: Random seed for track generation.
        ...
        from physics_gpu.car.constants import MAX_TRACK_TILES, TILE_DATA_SIZE

        var metrics = TrainingMetrics(
            algorithm_name="Deep PPO Continuous (GPU+Track)",
            environment_name="CarRacing GPU",
        )
        ...
        comptime ENV_ACTOR_OUT_SIZE = Self.n_envs * Self.ACTOR_OUT
        # Full environment state size
        comptime ENV_STATE_SIZE = Self.n_envs * CRConstants.STATE_SIZE
        comptime ROLLOUT_TOTAL = Self.TOTAL_ROLLOUT_SIZE
        comptime ROLLOUT_OBS_SIZE = ROLLOUT_TOTAL * Self.OBS
        comptime ROLLOUT_ACTION_SIZE = ROLLOUT_TOTAL * Self.ACTIONS
        ...
        # Track and visited buffer sizes
        comptime TRACK_BUFFER_SIZE = MAX_TRACK_TILES * TILE_DATA_SIZE
        comptime VISITED_BUFFER_SIZE = Self.n_envs * MAX_TRACK_TILES
        ...
        # Track buffer (shared across all environments)
        var tiles_buf = ctx.enqueue_create_buffer[dtype](TRACK_BUFFER_SIZE)
        # Per-env visited flags [n_envs, MAX_TRACK_TILES]
        var visited_buf = ctx.enqueue_create_buffer[dtype](VISITED_BUFFER_SIZE)
        ...
        # =====================================================================
        # Generate track and copy to GPU
        # =====================================================================
        if verbose:
            print("Generating track with seed", track_seed, "...")

        var num_tiles = CarRacingV2[dtype].generate_track_to_buffer(
            ctx, tiles_buf, track_seed
        )
        ctx.synchronize()

        if verbose:
            print("Track generated with", num_tiles, "tiles")

        # Initialize visited buffer to all zeros
        ctx.enqueue_memset(visited_buf, 0)
        ctx.synchronize()
        ...
        # =====================================================================
        var states_tensor = LayoutTensor[
            dtype, Layout.row_major(Self.n_envs, CRConstants.STATE_SIZE), MutAnyOrigin
        ](states_buf.unsafe_ptr())
        ...
        # =====================================================================
        comptime extract_obs_wrapper = _extract_obs_from_state_continuous_kernel[
            dtype, Self.n_envs, CRConstants.STATE_SIZE, Self.OBS
        ]
        ...
        # Reset all environments with track and visited buffers
        CarRacingV2[dtype].reset_kernel_gpu_with_visited[Self.n_envs](
            ctx, states_buf, tiles_buf, num_tiles, visited_buf
        )
        ctx.synchronize()
        ...
        while completed_episodes < num_episodes:
            ...
            for t in range(Self.rollout_len):
                ...
                # Step all environments with track and visited tracking
                CarRacingV2[dtype].step_kernel_gpu_with_visited[Self.n_envs](
                    ctx,
                    states_buf,
                    actions_buf,
                    tiles_buf,
                ...
                # Auto-reset done environments with visited buffer clearing
                CarRacingV2[dtype].selective_reset_kernel_gpu_with_visited[
                    Self.n_envs
                ](
                    ctx,
                    states_buf,
            ...
            # =================================================================
            # Phase 3: PPO updates (multiple epochs over minibatches)
            # =================================================================

            # Update learning rate if annealing
            if self.anneal_lr:
                var frac = Scalar[dtype](
                    1.0 - Float64(total_steps) / Float64(annealing_target_steps)
                )
                if frac < Scalar[dtype](0.0):
                    frac = Scalar[dtype](0.0)
                self.actor.optimizer.lr = initial_actor_lr * Float64(frac)
                self.critic.optimizer.lr = initial_critic_lr * Float64(frac)

            # Update entropy coefficient if annealing
            if self.anneal_entropy:
                var frac = Scalar[dtype](
                    1.0 - Float64(total_steps) / Float64(annealing_target_steps)
                )
                if frac < Scalar[dtype](0.0):
                    frac = Scalar[dtype](0.0)
                self.entropy_coef = initial_entropy_coef * Float64(frac)
            ...
            for epoch in range(self.num_epochs):
                ...
                for mb in range(num_minibatches):
                    ...
                    var mb_start = mb * MINIBATCH

                    # Copy indices to host buffer
                    for i in range(MINIBATCH):
                        mb_indices_host[i] = Int32(indices_list[mb_start + i])
                    ...
                    # Actor forward pass with caching
                    self.actor.model.forward_gpu_ws[MINIBATCH](
                        ctx,
                        actor_output_mb_buf,
                        mb_obs_buf,
                    ...
                    # Actor gradient computation (computes KL divergence too)
                    ctx.enqueue_function[
                        actor_grad_wrapper, actor_grad_wrapper
                    ](
                        actor_grad_output_tensor,
                    ...
                    if self.target_kl > 0.0:
                        ...
                        var mean_kl = Scalar[dtype](0.0)
                        for i in range(MINIBATCH):
                            mean_kl += kl_divergences_host[i]
                        mean_kl /= Scalar[dtype](MINIBATCH)

                        if Float64(mean_kl) > 1.5 * self.target_kl:
                            kl_early_stop = True
                            if verbose:
                                print(
                                    "  KL early stop at epoch",
                    ...
                    # Critic forward pass with caching
                    self.critic.model.forward_gpu_ws[MINIBATCH](
                        ctx,
                        critic_values_buf,
                        mb_obs_buf,
            ...
            if verbose and rollout_count % print_every == 0:
                ...
                print(
                    ...
                    total_steps,
                    "| This rollout",
                    episodes_this_rollout,
                )

        # Copy final params back to host
        self.actor.copy_params_from_device(ctx, actor_params_buf)
        self.actor.copy_state_from_device(ctx, actor_state_buf)
        self.critic.copy_params_from_device(ctx, critic_params_buf)
        self.critic.copy_state_from_device(ctx, critic_state_buf)
