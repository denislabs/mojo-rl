...
struct DeepSACAgent[
    ...
    fn select_action(
        ...
        var clipped_action = InlineArray[Scalar[Self.dtype], Self.action_dim](fill=0)
        for i in range(Self.action_dim):
            var a = action[i]
    ...
    fn train_step(mut self) -> Scalar[Self.dtype]:
        ...
        var next_log_prob_sum = InlineArray[Scalar[Self.dtype], Self.batch_size](fill=0)
        ...
        var actor_actions_logprobs = self.actor.sample_action_with_cache[Self.batch_size](
            batch_obs, actor_noise, h1_actor, h2_actor,
            mean_actor, log_std_actor, tanh_actor
        )
        ...
        var actor_log_prob_sum = InlineArray[Scalar[Self.dtype], Self.batch_size](fill=0)
        for i in range(Self.batch_size):
            var sum_log_prob: Scalar[Self.dtype] = 0.0
            for j in range(Self.action_dim):
        ...
        self.actor.backward[Self.batch_size](
            d_actor_loss, batch_obs, actor_noise,
            h1_actor, h2_actor, mean_actor, log_std_actor, tanh_actor
        )
    ...
    fn train[
        ...
        while warmup_done < warmup_steps:
            ...
            while not done and warmup_done < warmup_steps:
                ...
                var obs = _list_to_inline_sac[Self.obs_dim, Self.dtype](obs_list)
