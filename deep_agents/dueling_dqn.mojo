...
Example usage:
    from deep_agents import DeepDuelingDQNAgent
...
struct DuelingQNetwork[
    ...
    var value_hidden: LinearAdam[Self.hidden2_dim, Self.value_hidden_dim, Self.dtype]
    var value_out: LinearAdam[Self.value_hidden_dim, 1, Self.dtype]

    # Advantage stream: estimates A(s,a) for each action
    var advantage_hidden: LinearAdam[Self.hidden2_dim, Self.advantage_hidden_dim, Self.dtype]
    var advantage_out: LinearAdam[Self.advantage_hidden_dim, Self.num_actions, Self.dtype]
    ...
    fn __init__(out self):
        ...
        # Value stream
        self.value_hidden = LinearAdam[Self.hidden2_dim, Self.value_hidden_dim, Self.dtype]()
        self.value_out = LinearAdam[Self.value_hidden_dim, 1, Self.dtype]()

        # Advantage stream
        self.advantage_hidden = LinearAdam[Self.hidden2_dim, Self.advantage_hidden_dim, Self.dtype]()
        self.advantage_out = LinearAdam[Self.advantage_hidden_dim, Self.num_actions, Self.dtype]()
    ...
    fn forward[batch_size: Int](
        ...
        var v_hidden = relu[batch_size * Self.value_hidden_dim, Self.dtype](v_hidden_pre)
        var value = self.value_out.forward[batch_size](v_hidden)  # [batch_size, 1]
        ...
        var a_hidden = relu[batch_size * Self.advantage_hidden_dim, Self.dtype](a_hidden_pre)
        var advantage = self.advantage_out.forward[batch_size](a_hidden)  # [batch_size, num_actions]

        # ============================
        # Combine: Q(s,a) = V(s) + (A(s,a) - mean(A))
        # ============================

        var q_values = InlineArray[Scalar[Self.dtype], batch_size * Self.num_actions](fill=0)
    ...
    fn forward_with_cache[batch_size: Int](
        ...
        mut v_hidden_out: InlineArray[Scalar[Self.dtype], batch_size * Self.value_hidden_dim],
        mut a_hidden_out: InlineArray[Scalar[Self.dtype], batch_size * Self.advantage_hidden_dim],
        mut value_out: InlineArray[Scalar[Self.dtype], batch_size],
        mut advantage_out: InlineArray[Scalar[Self.dtype], batch_size * Self.num_actions],
    ) -> InlineArray[Scalar[Self.dtype], batch_size * Self.num_actions]:
    ...
    fn backward[batch_size: Int](
        ...
        v_hidden: InlineArray[Scalar[Self.dtype], batch_size * Self.value_hidden_dim],
        a_hidden: InlineArray[Scalar[Self.dtype], batch_size * Self.advantage_hidden_dim],
    ):
        ...
        var da = InlineArray[Scalar[Self.dtype], batch_size * Self.num_actions](fill=0)

        comptime one_over_n = Scalar[Self.dtype](1.0) / Scalar[Self.dtype](Self.num_actions)
        ...
        var relu_g_v = relu_grad[batch_size * Self.value_hidden_dim, Self.dtype](v_hidden)
        var dv_hidden_pre = elementwise_mul[batch_size * Self.value_hidden_dim, Self.dtype](
            dv_hidden, relu_g_v
        )

        var dh2_from_v = self.value_hidden.backward[batch_size](dv_hidden_pre, h2)
        ...
        # Backprop through relu
        var relu_g_a = relu_grad[batch_size * Self.advantage_hidden_dim, Self.dtype](a_hidden)
        var da_hidden_pre = elementwise_mul[batch_size * Self.advantage_hidden_dim, Self.dtype](
            da_hidden, relu_g_a
        )

        var dh2_from_a = self.advantage_hidden.backward[batch_size](da_hidden_pre, h2)
...
struct DeepDuelingDQNAgent[
    ...
    fn __init__(
        ...
        self.buffer = ReplayBuffer[Self.buffer_capacity, Self.obs_dim, 1, Self.dtype]()
    ...
    fn train_step(mut self) -> Scalar[Self.dtype]:
        ...
        var batch_actions = InlineArray[Scalar[Self.dtype], Self.batch_size](fill=0)
        ...
        var value_cache = InlineArray[Scalar[Self.dtype], Self.batch_size](fill=0)
