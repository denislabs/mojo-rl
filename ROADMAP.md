...
### Deep RL Infrastructure (`nn/`) - PyTorch-like Trait-Based Architecture

- [x] **Model Trait** - Stateless neural network layers with compile-time dimensions
  - `nn/model/model.mojo` - Base `Model` trait with `forward()`, `backward()`, GPU variants
...
- [x] **Layer Implementations** - Pluggable neural network layers
  - `nn/model/linear.mojo` - `Linear[in_dim, out_dim]` with tiled matmul GPU kernels
  - `nn/model/relu.mojo` - `ReLU[dim]` activation layer
  - `nn/model/tanh.mojo` - `Tanh[dim]` activation layer
  - `nn/model/sigmoid.mojo` - `Sigmoid[dim]` activation layer
  - `nn/model/softmax.mojo` - `Softmax[dim]` activation layer (for policy networks)
  - `nn/model/layer_norm.mojo` - `LayerNorm[dim]` layer normalization
  - `nn/model/dropout.mojo` - `Dropout[dim, p, training]` regularization (compile-time training flag)
  - `nn/model/sequential.mojo` - `Seq2[L0, L1]` composition + `seq()` helpers (up to 8 layers)
- [x] **Optimizer Trait** - Pluggable parameter update rules
  - `nn/optimizer/optimizer.mojo` - Base `Optimizer` trait with `step()`, `step_gpu()`
  - `nn/optimizer/sgd.mojo` - `SGD` optimizer
  - `nn/optimizer/adam.mojo` - `Adam` optimizer with bias correction
  - `nn/optimizer/rmsprop.mojo` - `RMSprop` optimizer with adaptive learning rates
  - `nn/optimizer/adamw.mojo` - `AdamW` optimizer with decoupled weight decay
- [x] **Loss Function Trait** - Pluggable loss computation
  - `nn/loss/loss.mojo` - Base `LossFunction` trait
  - `nn/loss/mse.mojo` - `MSELoss` with block reduction for GPU
  - `nn/loss/huber.mojo` - `HuberLoss` (Smooth L1) for DQN variants
  - `nn/loss/cross_entropy.mojo` - `CrossEntropyLoss` for classification/policy gradients
- [x] **Initializer Trait** - Pluggable weight initialization
  - `nn/initializer/initializers.mojo` - Xavier, Kaiming, LeCun, Zeros, Ones, Uniform, Normal
- [x] **Stochastic Actor** - Gaussian policy network for SAC/PPO
  - `nn/model/stochastic_actor.mojo` - `StochasticActor[in_dim, action_dim]` with learned mean and log_std
...
- [x] **Training Utilities** - High-level training management
  - `nn/training/trainer.mojo` - `Trainer[MODEL, OPTIMIZER, LOSS, INITIALIZER]`
...
- [x] **Constants** - Global configuration
  - `nn/constants.mojo` - `dtype`, `TILE` (matmul tile size), `TPB` (threads per block)
...
New neural network components for nn.
...
- [x] **Checkpointing** - Save/load agent state for resuming training
  - `nn/checkpoint/` - Checkpoint utilities module (text-based format)
...
### New PyTorch-like Deep RL Architecture (`nn/`)
...
```mojo
from nn import seq, Linear, ReLU, Adam, Kaiming, Trainer
