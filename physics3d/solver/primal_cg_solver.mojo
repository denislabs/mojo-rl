"""Primal CG constraint solver (MuJoCo-matching).

Operates in qacc (acceleration) space, minimizing the same cost as PrimalNewton:
...
Both share: constraint_update, PrimalSearch (linesearch), convergence, cost.

MuJoCo's CG is a primal solver (shares mj_solPrimal with Newton).
...
# Primal CG solver parameters
comptime PRIMAL_CG_ITERATIONS: Int = 200
comptime PRIMAL_CG_TOLERANCE: Float64 = 1e-12
# Debug flag
comptime PRIMAL_CG_DEBUG: Bool = False
comptime MINVAL: Float64 = 1e-10


struct PrimalCGSolver(ConstraintSolver):
    """MuJoCo-style primal CG constraint solver.
    ...
    fn solver_workspace_size[NV: Int, MAX_CONTACTS: Int]() -> Int:
        """Primal CG workspace: same as PGS for GPU compatibility."""
    ...
    fn solve[
        ...
        """Solve constraints using primal CG on CPU.
        ...
        if scale * sqrt(grad_norm) < Scalar[DTYPE](PRIMAL_CG_TOLERANCE):
        ...
        for iter in range(PRIMAL_CG_ITERATIONS):
            total_iter += 1
            ...
            var alpha = primal_linesearch_with_D[DTYPE, MAX_ROWS, NV, V_SIZE, MR](
                ...
                jar,
                force,
                Scalar[DTYPE](PRIMAL_CG_TOLERANCE),
            )
            ...
            if improvement < Scalar[DTYPE](PRIMAL_CG_TOLERANCE) and iter > 0:
    ...
    fn solve_gpu[
        ...
        """GPU solve — delegates to dual PGS GPU for now.

        The GPU primal CG is Phase 2 work. For now, use the same
        GPU path as the PGS solver (which already works well).
        ...
        # PGS normal iterations (dual approach for GPU)
