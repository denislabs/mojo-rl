...
struct GcCGSolver(GcConstraintSolver):
    ...
    fn solve[
        ...
        var p = InlineArray[Scalar[DTYPE], MC](uninitialized=True)    # search direction
    ...
    for c in range(nc):
        ...
        if lambda_t1[c] != Scalar[DTYPE](0) or lambda_t2[c] != Scalar[DTYPE](0):
            for i in range(NV):
                ...
                var mi_j_sum2: Scalar[DTYPE] = 0
                for j_idx in range(NV):
                    mi_j_sum1 += M_inv[i * NV + j_idx] * J_t1_all[c * NV + j_idx]
                    mi_j_sum2 += M_inv[i * NV + j_idx] * J_t2_all[c * NV + j_idx]
        ...
        for c in range(nc):
            ...
            for i in range(NV):
                ...
                for j_idx in range(NV):
                    mi_j_sum1 += M_inv[i * NV + j_idx] * J_t1_all[c * NV + j_idx]
                    mi_j_sum2 += M_inv[i * NV + j_idx] * J_t2_all[c * NV + j_idx]
                qvel[i] += mi_j_sum1 * actual_delta_t1 + mi_j_sum2 * actual_delta_t2
...
fn _solve_friction_pgs_gpu[
    ...
    for c in range(nc):
        ...
        # Warm start tangent impulses
        var c_off = contacts_off + c * GC_CONTACT_SIZE
        lambda_t1[c] = rebind[Scalar[DTYPE]](state[env, c_off + GC_CONTACT_IDX_IMPULSE_T1])
        lambda_t2[c] = rebind[Scalar[DTYPE]](state[env, c_off + GC_CONTACT_IDX_IMPULSE_T2])
    ...
    for c in range(nc):
        var c_off = contacts_off + c * GC_CONTACT_SIZE
        state[env, c_off + GC_CONTACT_IDX_IMPULSE_N] = lambda_n[c]
        state[env, c_off + GC_CONTACT_IDX_IMPULSE_T1] = lambda_t1[c]
        state[env, c_off + GC_CONTACT_IDX_IMPULSE_T2] = lambda_t2[c]
