...
Run with:
    cd mojo-rl
    pixi run mojo run physics3d/examples/hopper_render_demo.mojo
...
fn main() raises:
    ...
    data.set_body_position(
        1, Scalar[DTYPE](0.0), Scalar[DTYPE](0.0), Scalar[DTYPE](thigh_z)
    )
    data.set_body_position(
        2, Scalar[DTYPE](0.0), Scalar[DTYPE](0.0), Scalar[DTYPE](leg_z)
    )
    data.set_body_position(
        3, Scalar[DTYPE](0.0), Scalar[DTYPE](0.0), Scalar[DTYPE](foot_z)
    )
    ...
    # Simulation loop
    var steps_per_frame = (
        25  # Run more physics steps per frame due to smaller timestep
    )
    var frame_count = 0
    ...
    while not renderer.check_quit():
        ...
        for _ in range(steps_per_frame):
            ...
            for i in range(NUM_BODIES):
                ...
                data.angular_velocities[i * 3 + 0] *= Scalar[DTYPE](
                    angular_damping
                )
                data.angular_velocities[i * 3 + 1] *= Scalar[DTYPE](
                    angular_damping
                )
                data.angular_velocities[i * 3 + 2] *= Scalar[DTYPE](
                    angular_damping
                )
