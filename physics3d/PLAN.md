...
- Reference acceleration: aref = -k _ pos - b _ vel
...
```mojo
from physics3d import ImpulseIntegrator
from physics3d.gpu.constants import compute_state_size, body_offset, joint_offset
...
pixi run mojo run physics3d/tests/test_multi_body_impulse.mojo
pixi run mojo run physics3d/tests/test_multi_body_pgs.mojo
...
pixi run -e apple mojo run physics3d/tests/test_double_pendulum_gpu.mojo

# GPU tests (requires GPU environment)
pixi run -e apple mojo run physics3d/tests/test_gpu.mojo

# Rendering tests (requires SDL2)
pixi run mojo run physics3d/tests/test_render_multi_body_impulse.mojo
pixi run mojo run physics3d/tests/test_render_multi_body_pgs.mojo

# Visual double pendulum demo (requires SDL2)
pixi run mojo run physics3d/examples/double_pendulum_render_demo.mojo

# Phase 6: Friction tests
pixi run mojo run physics3d/tests/test_friction.mojo

# Phase 6: Friction GPU tests
pixi run -e apple mojo run physics3d/tests/test_friction_gpu.mojo

# Phase 6: Visual friction demo (requires SDL2)
pixi run mojo run physics3d/tests/test_render_friction.mojo

# Phase 7: Joint torque actuation tests
pixi run mojo run physics3d/tests/test_joint_torque.mojo

# Phase 7: Joint sensing tests
pixi run mojo run physics3d/tests/test_joint_sensing.mojo

# Phase 7: Joint torque GPU tests
pixi run -e apple mojo run physics3d/tests/test_joint_torque_gpu.mojo

# Phase 7: Joint sensing GPU tests
pixi run -e apple mojo run physics3d/tests/test_joint_sensing_gpu.mojo

# Phase 7: HopperEnv tests
pixi run mojo run physics3d/tests/test_hopper.mojo

# Phase 7: HopperEnv GPU tests
pixi run -e apple mojo run physics3d/tests/test_hopper_gpu.mojo

# Phase 8: Capsule collision tests
pixi run mojo run physics3d/tests/test_capsule_plane.mojo
pixi run mojo run physics3d/tests/test_capsule_sphere.mojo
pixi run mojo run physics3d/tests/test_capsule_capsule.mojo
pixi run -e apple mojo run physics3d/tests/test_capsule_gpu.mojo

# Phase 9: Box collision tests
pixi run mojo run physics3d/tests/test_box_plane.mojo
pixi run mojo run physics3d/tests/test_box_sphere.mojo
pixi run mojo run physics3d/tests/test_box_capsule.mojo
pixi run mojo run physics3d/tests/test_box_box.mojo
pixi run -e apple mojo run physics3d/tests/test_box_gpu.mojo

# Phase 10a: Walker environment tests
pixi run mojo run physics3d/tests/test_walker.mojo
pixi run -e apple mojo run physics3d/tests/test_walker_gpu.mojo

# Visual walker demo (requires SDL2)
pixi run mojo run physics3d/examples/walker_render_demo.mojo
...
- `physics3d/types.mojo` - `ModelGC`, `DataGC` structs with compile-time NQ, NV, NBODY, NJOINT, MAX_CONTACTS parameters
- `physics3d/kinematics/quat_math.mojo` - Quaternion operations (CPU + GPU colocated)
- `physics3d/kinematics/forward_kinematics.mojo` - FK traversal (CPU + GPU colocated)
- `physics3d/dynamics/mass_matrix.mojo` - Diagonal mass matrix computation (CPU + GPU colocated)
- `physics3d/dynamics/bias_forces.mojo` - Gravity torques in joint space (CPU + GPU colocated)
- `physics3d/integrator.mojo` - `SemiImplicitEulerIntegrator` with `step()` (CPU) and `step_gpu()` (GPU)
- `physics3d/gpu/constants.mojo` - GC buffer layout constants (`gc_state_size`, `gc_model_size`, offsets)
- `physics3d/gpu/buffer_utils.mojo` - GC buffer utilities (`create_gc_state_buffer`, `copy_model_to_buffer`, etc.)
- `physics3d/gpu/gc_kernels.mojo` - Integration-specific GPU kernels (orchestration, contacts, normalization)
