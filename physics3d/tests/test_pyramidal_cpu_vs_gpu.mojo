"""Test Pyramidal Cone: CPU vs GPU.

Compares qpos/qvel after running physics steps with PYRAMIDAL cone on CPU vs GPU
for the HalfCheetah model. This validates the full pyramidal pipeline on GPU including:
FK → contacts → M → pyramidal edge constraints → solver → integration.

The CPU uses EulerIntegrator[NewtonSolver].step() with ConeType.PYRAMIDAL (float32).
The GPU uses EulerIntegrator[NewtonSolver].step_gpu() with CONE_TYPE=ConeType.PYRAMIDAL (float32).

Note: CPU uses primal Newton solver, GPU uses coupled PGS for friction/pyramidal.
Results may differ slightly due to different solver algorithms.

Run with:
    cd mojo-rl && pixi run -e apple mojo run physics3d/tests/test_pyramidal_cpu_vs_gpu.mojo
"""
...
fn main() raises:
    print("=" * 60)
    print("Pyramidal Cone: CPU vs GPU")
    print("=" * 60)
    print("Model: HalfCheetah (NQ=9, NV=9, NGEOM=", NGEOM, ")")
    print("CPU: EulerIntegrator[Newton] (PYRAMIDAL, primal Newton)")
    print("GPU: EulerIntegrator[Newton] (PYRAMIDAL, coupled PGS)")
    ...
    if compare_step(
        "Ground contact (1 step)", qpos1, qvel1, act1, 1,
    ...
    act2[5] = 0.1   # ffoot
    if compare_step(
        "Ground contact + actions (1 step)", qpos1, qvel1, act2, 1,
        ctx, model_buf, state_host, state_buf, workspace_buf, ws_host,
    ):
        num_pass += 1
    else:
    ...
    if compare_step(
        "Deep penetration (1 step)", qpos3, qvel1, act1, 1,
        ctx, model_buf, state_host, state_buf, workspace_buf, ws_host,
    ):
        num_pass += 1
    else:
    ...
    if compare_step(
        "Moving + contacts (1 step)", qpos1, qvel4, act2, 1,
        ctx, model_buf, state_host, state_buf, workspace_buf, ws_host,
    ):
        num_pass += 1
    else:
    ...
    if compare_step(
        "Ground contact (5 steps)", qpos1, qvel1, act1, 5,
        ctx, model_buf, state_host, state_buf, workspace_buf, ws_host,
    ):
        num_pass += 1
    else:
    ...
    if compare_step(
        "Ground contact + actions (5 steps)", qpos1, qvel1, act2, 5,
        ctx, model_buf, state_host, state_buf, workspace_buf, ws_host,
    ):
        num_pass += 1
    else:
    ...
    print("=" * 60)
    print(
        "Results:", num_pass, "passed,", num_fail,
        "failed out of", num_pass + num_fail,
    )
    if num_fail == 0:
