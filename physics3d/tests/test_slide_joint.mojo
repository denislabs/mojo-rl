...
Run with:
    cd mojo-rl
    pixi run mojo run physics3d/tests/test_slide_joint.mojo
"""

from math import sqrt, sin, cos
from physics3d.types import Model, Data
from physics3d.integrator import PGSIntegrator
from physics3d.joints import get_slide_joint_position, get_slide_joint_velocity
...
fn test_slide_rotation_locked() -> Bool:
    ...
    print("\nResults:")
    print(
        "  Initial angular velocity magnitude:",
        sqrt(1.0 + 0.25 + 0.09),
        "rad/s",
    )
...
fn test_slide_velocity_sensing() -> Bool:
    ...
    for i in range(len(test_velocities)):
        ...
        data.set_body_position(
            0, Scalar[DTYPE](0.0), Scalar[DTYPE](0.0), Scalar[DTYPE](anchor_z)
        )
        data.set_body_velocity(
            0, Scalar[DTYPE](test_vx), Scalar[DTYPE](0.0), Scalar[DTYPE](0.0)
        )
...
fn test_slide_with_force() -> Bool:
    ...
    var passed = (
        x_error < 0.2
        and vx_error < 0.2
        and max_y_drift < 0.01
        and max_z_drift < 0.01
    )
