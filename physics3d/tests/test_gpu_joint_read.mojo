...
fn run_debug_kernel(
    ...
    var debug = LayoutTensor[DTYPE, Layout.row_major(20), MutAnyOrigin](
    ...
    fn kernel_wrapper(
        ...
        var num_joints = rebind[Scalar[DTYPE]](
            state[env, m_off + META_IDX_NUM_JOINTS]
        )
...
fn main() raises:
    ...
    print(
        "  state_host[",
        j_off + JOINT_IDX_PARENT,
        "] =",
        state_host[j_off + JOINT_IDX_PARENT],
        "(parent)",
    )
    print(
        "  state_host[",
        j_off + JOINT_IDX_CHILD,
        "] =",
        state_host[j_off + JOINT_IDX_CHILD],
        "(child)",
    )
    print(
        "  state_host[",
        j_off + JOINT_IDX_ANCHOR_PZ,
        "] =",
        state_host[j_off + JOINT_IDX_ANCHOR_PZ],
        "(anchor_pz)",
    )
    print(
        "  state_host[",
        j_off + JOINT_IDX_AXIS_Y,
        "] =",
        state_host[j_off + JOINT_IDX_AXIS_Y],
        "(axis_y)",
    )
    print(
        "  state_host[",
        m_off + META_IDX_NUM_JOINTS,
        "] =",
        state_host[m_off + META_IDX_NUM_JOINTS],
        "(num_joints)",
    )
