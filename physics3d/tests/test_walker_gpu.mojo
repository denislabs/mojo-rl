...
fn setup_walker_state[
    ...
    )  # 0.14 - top of capsule above center
...
fn setup_walker_model(mut model_host: List[Float32]):
    ...
    var leg_inertia_xy = (
        leg_mass / 12.0 * (3.0 * leg_radius * leg_radius + h * h)
    )
    ...
    model_host[2 * MODEL_BODY_SIZE + MODEL_IDX_GEOM_TYPE] = Float32(
        GEOM_CAPSULE
    )
...
fn test_gpu_cpu_parity() raises:
    ...
    model_cpu.set_body_capsule(
        1, mass=0.3, radius=leg_radius, half_length=leg_half_length
    )  # Left Leg
    model_cpu.set_body_capsule(
        2, mass=0.3, radius=leg_radius, half_length=leg_half_length
    )  # Right Leg
