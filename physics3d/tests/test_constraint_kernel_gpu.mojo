...
from physics3d.gpu.gc_kernels import (
...
from physics3d.solver.gc_pgs_solver import GcPGSSolver
...
fn setup_minimal_model(model_host: HostBuffer[DTYPE]):
    ...
    model_host[meta_off + GC_MODEL_META_IDX_NBODY] = Float32(NBODY)
    model_host[meta_off + GC_MODEL_META_IDX_NJOINT] = Float32(NJOINT)
    model_host[meta_off + GC_MODEL_META_IDX_GRAVITY_Z] = Float32(-9.81)
    model_host[meta_off + GC_MODEL_META_IDX_TIMESTEP] = Float32(0.002)
    model_host[meta_off + GC_MODEL_META_IDX_GROUND_Z] = Float32(0.0)
    model_host[meta_off + GC_MODEL_META_IDX_FRICTION] = Float32(0.9)

    for b in range(NBODY):
        var off = gc_model_body_offset(b)
        model_host[off + GC_BODY_IDX_MASS] = Float32(1.0)
        model_host[off + GC_BODY_IDX_INV_MASS] = Float32(1.0)
        model_host[off + GC_BODY_IDX_IXX] = Float32(0.01)
        model_host[off + GC_BODY_IDX_IYY] = Float32(0.01)
        model_host[off + GC_BODY_IDX_IZZ] = Float32(0.01)
        model_host[off + GC_BODY_IDX_RADIUS] = Float32(0.046)
        model_host[off + GC_BODY_IDX_HALF_LENGTH] = Float32(0.1)
        model_host[off + GC_BODY_IDX_PARENT] = Float32(-1) if b == 0 else Float32(0)
        model_host[off + GC_BODY_IDX_QUAT_W] = Float32(1.0)

    for j in range(NJOINT):
        var off = gc_model_joint_offset[NBODY](j)
        model_host[off + GC_JOINT_IDX_BODY_ID] = Float32(0) if j < 3 else Float32(j - 2)
        model_host[off + GC_JOINT_IDX_QPOS_ADR] = Float32(j)
        model_host[off + GC_JOINT_IDX_DOF_ADR] = Float32(j)
        if j < 2:  # slides
            model_host[off + GC_JOINT_IDX_TYPE] = Float32(GC_JNT_SLIDE)
            if j == 0:
                model_host[off + GC_JOINT_IDX_AXIS_X] = Float32(1.0)
            else:
                model_host[off + GC_JOINT_IDX_AXIS_Z] = Float32(1.0)
            model_host[off + GC_JOINT_IDX_RANGE_MIN] = Float32(-100.0)
            model_host[off + GC_JOINT_IDX_RANGE_MAX] = Float32(100.0)
        else:  # hinges
            model_host[off + GC_JOINT_IDX_TYPE] = Float32(GC_JNT_HINGE)
            model_host[off + GC_JOINT_IDX_AXIS_Y] = Float32(1.0)
            model_host[off + GC_JOINT_IDX_RANGE_MIN] = Float32(-1.0)
            model_host[off + GC_JOINT_IDX_RANGE_MAX] = Float32(1.0)
            model_host[off + GC_JOINT_IDX_ARMATURE] = Float32(0.1)
            model_host[off + GC_JOINT_IDX_DAMPING] = Float32(3.0)
            model_host[off + GC_JOINT_IDX_STIFFNESS] = Float32(100.0)
...
fn main() raises:
    ...
    # (The full pipeline in step_gc_constraint_kernel_with_solver)
    # =====================================================
    ...
    fn combined_kernel(
        ...
        # Replicate the full pipeline from step_gc_constraint_kernel_with_solver
        ...
        # f_net + stiffness
        var qpos_off2 = gc_qpos_offset[NQ, NV]()
