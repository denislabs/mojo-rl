...
fn copy_gpu_state_to_cpu[
    ...
    for i in range(NUM_BODIES):
        ...
        # Angular velocities
        data.angular_velocities[i * 3 + 0] = Float64(
            state_host[b_off + BODY_IDX_WX]
        )
        data.angular_velocities[i * 3 + 1] = Float64(
            state_host[b_off + BODY_IDX_WY]
        )
        data.angular_velocities[i * 3 + 2] = Float64(
            state_host[b_off + BODY_IDX_WZ]
        )
...
fn test_angle_sensing_after_gpu_simulation() raises:
    ...
    # Check initial angle
    copy_gpu_state_to_cpu[NUM_BODIES, MAX_CONTACTS, MAX_JOINTS](
        state_host, data
    )
