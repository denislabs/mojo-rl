...
fn compare_J_row(
    ...
    if fail_count == 0:
        print(
            "    ", label, " OK  max_abs=", max_abs_err,
            " max_rel=", max_rel_err,
        )
    else:
        print(
            "    ", label, " FAILED", fail_count, "/", NV,
            " max_abs=", max_abs_err,
        )
...
fn compare_jacobians(
    ...
    ](model, data, cdof, M_inv, qvel_arr, dt, constraints)
    ...
    for r in range(mj_nefc):
        ...
        if t == 7:  # mjCNSTR_CONTACT_ELLIPTIC
            ...
            mj_limit_rows += 1
    print(
        "  MJ:  contact_rows=", mj_contact_rows,
        " limit_rows=", mj_limit_rows,
    )
    ...
    if mj_contact_rows != expected_contact_rows:
        print(
            "  WARN: MJ contact rows=", mj_contact_rows,
            " expected=", expected_contact_rows,
        )

    if our_nnorm + our_nfric != mj_contact_rows:
        print(
            "  FAIL: total contact rows mismatch! ours=", our_nnorm + our_nfric,
            " mj=", mj_contact_rows,
        )
        all_pass = False
    ...
    for c in range(our_ncon):
        ...
        if not swap:
            ...
            if not compare_J_row(
                "Fric_t1(↔mj_t2)", our_t1_idx, mj_J[mj_t2_idx], constraints, sign
            ):
                all_pass = False
            if not compare_J_row(
                "Fric_t2(↔mj_t1)", our_t2_idx, mj_J[mj_t1_idx], constraints, sign
            ):
                all_pass = False
    ...
    if our_nlim > 0 or mj_limit_rows > 0:
        ...
        if our_nlim != mj_limit_rows:
            print(
                "  FAIL: limit count mismatch! ours=", our_nlim,
                " mj=", mj_limit_rows,
            )
            all_pass = False
        else:
