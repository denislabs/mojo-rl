...
from math import exp, log, tanh, sqrt, max, min, pi, cos
...
fn td_target_min_twin_kernel[
    ...
    if use_entropy:
        ...
        td_targets[i] = rewards[i] + gamma * (q_min - alpha * log_probs[i]) * (one - dones[i])
    else:
        # TD3: no entropy
...
fn actor_grad_from_critic_kernel[
    ...
    d_actor_out: LayoutTensor[dtype, Layout.row_major(BATCH, ACTION_DIM), MutAnyOrigin],
...
fn add_gaussian_noise_kernel[
    ...
    noisy_actions: LayoutTensor[dtype, Layout.row_major(BATCH, ACTION_DIM), MutAnyOrigin],
    actions: LayoutTensor[dtype, Layout.row_major(BATCH, ACTION_DIM), MutAnyOrigin],
    rng_states: LayoutTensor[DType.uint32, Layout.row_major(BATCH, ACTION_DIM), MutAnyOrigin],
...
fn sac_reparameterize_kernel[
    ...
    for a in range(ACTION_DIM):
        ...
        lp += -Scalar[dtype](0.5) * eps * eps - half_log_2pi - ls - log(one_minus_tanh2)
...
fn a2c_gae_kernel[
    ...
    advantages: LayoutTensor[dtype, Layout.row_major(ROLLOUT, N_ENVS), MutAnyOrigin],
    returns: LayoutTensor[dtype, Layout.row_major(ROLLOUT, N_ENVS), MutAnyOrigin],
    rewards: LayoutTensor[dtype, Layout.row_major(ROLLOUT, N_ENVS), MutAnyOrigin],
    ...
    for t_rev in range(ROLLOUT):
        ...
        # TD residual
        var delta = rewards[t, env] + gamma * values[t + 1, env] * mask - values[t, env]
...
fn a2c_softmax_sample_kernel[
    ...
    logits: LayoutTensor[dtype, Layout.row_major(N_ENVS, N_ACTIONS), MutAnyOrigin],
    rng_states: LayoutTensor[DType.uint32, Layout.row_major(N_ENVS), MutAnyOrigin],
):
...
fn td_mse_grad_kernel[
    ...
    if i >= BATCH:
        return
    q_grad[i, 0] = Scalar[dtype](2.0) * (q[i, 0] - targets[i]) / Scalar[dtype](BATCH)
