...
fn main() raises:
    ...
    var renderer = Renderer2D(width=560, height=530, fps=30, title="ConnectFour")
    ...
    while renderer.begin_frame_with_color(bg_color):
        ...
        if game_result == 0:
            var player = env.current_player()
            if player == 0:
                renderer.draw_text("Red's turn", 230, 50 + board_height + 20, red_color)
            else:
                renderer.draw_text("Yellow's turn", 218, 50 + board_height + 20, yellow_color)
        elif game_result == 1:
            renderer.draw_text("Red Wins!  (R to reset)", 180, 50 + board_height + 20, win_text_color)
        elif game_result == 2:
            renderer.draw_text("Yellow Wins!  (R to reset)", 168, 50 + board_height + 20, win_text_color)
        else:
