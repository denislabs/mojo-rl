"""Play TicTacToe against a trained MuZero/AlphaZero agent.

Load a checkpoint from self-play training and play interactively.
You play as X (first), the AI plays as O using MCTS.
...
Usage:
    ...
    pixi run -e apple mojo run -I . examples/board_games/tictactoe_play_vs_muzero.mojo
"""
...
fn main() raises:
    print("=== Play vs MuZero on TicTacToe ===")
    print("You are X (first player). AI is O.")
    print()

    # ── Load trained agent ───────────────────────────────────────
    comptime TTT = TicTacToeEnv[DType.float64]
    ...
    var renderer = Renderer2D(width=400, height=450, fps=30, title="TicTacToe vs MuZero")
    ...
    while renderer.begin_frame_with_color(bg_color):
        ...
        var mouse_on_board = (
            mouse_col >= 0 and mouse_col < 3
            and mouse_row >= 0 and mouse_row < 3
        ...
        if not game_over and env.current_player() == 0:
            ...
            # Map: kp7=cell0, kp8=cell1, kp9=cell2, kp4=cell3, ...
            # But numpad layout is bottom-up, env is top-down
            var kp_to_cell = InlineArray[Int, 9](uninitialized=True)
            kp_to_cell[0] = 0  # kp7 → top-left
            kp_to_cell[1] = 1  # kp8 → top-center
            kp_to_cell[2] = 2  # kp9 → top-right
            kp_to_cell[3] = 3  # kp4 → mid-left
            kp_to_cell[4] = 4  # kp5 → center
            kp_to_cell[5] = 5  # kp6 → mid-right
            kp_to_cell[6] = 6  # kp1 → bot-left
            kp_to_cell[7] = 7  # kp2 → bot-center
            kp_to_cell[8] = 8  # kp3 → bot-right
            for k in range(9):
        ...
        # AI turn (player 1 = O) — use MCTS
        if not game_over and env.current_player() == 1:
            # Get canonical observation (from AI's perspective)
            ...
            # Use raw policy network (no broken CPU MCTS)
            var ai_action = agent.select_action_policy_only(obs, legal)

            # Verify action is legal, fallback to random legal if not
        ...
        for row in range(3):
            for col in range(3):
                ...
                if not game_over and row == cursor_row and col == cursor_col and env.current_player() == 0:
                    renderer.draw_rect(
                        col * cell_size + 4, row * cell_size + 4,
                        cell_size - 8, cell_size - 8,
                        cursor_color, border_width=3,
                    )
                ...
                if cell_val == 1:
                    ...
                    renderer.draw_line(
                        (col + 1) * cell_size - margin, row * cell_size + margin,
                        col * cell_size + margin, (row + 1) * cell_size - margin,
                        x_color, 4,
                    )
                elif cell_val == 2:
        ...
        if game_result == 0:
            if env.current_player() == 0:
                renderer.draw_text("Your turn (X)", 150, board_size + 20, text_color)
            else:
                renderer.draw_text("AI thinking...", 150, board_size + 20, ai_color)
        elif game_result == 1:
            renderer.draw_text("You win!  (R to reset)", 115, board_size + 20, win_color)
        elif game_result == 2:
            renderer.draw_text("AI wins!  (R to reset)", 115, board_size + 20, win_color)
        else:
