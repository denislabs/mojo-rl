"""Rainbow DQN CNN GPU training on Procgen Jumper — pixel obs, CPU-batched envs.

Scales the CPU proof to a real run: `N_ENVS` jumper envs step in parallel across
...
pixel trainers — used because jumper generation + rendering are CPU-side (no
GPU-env kernels). A full GPU-resident jumper env is a separate, larger effort.

Jumper reward is SPARSE (single +10 for the coin, hard death on any hazard) →
...
level-count increase needs proportionally more steps (jumper is sparse).

Run:
    pixi run mojo run -I . examples/procgen/jumper_rainbow_training_gpu.mojo   # (apple) compile/smoke
    pixi run -e nvidia mojo run -I . examples/procgen/jumper_rainbow_training_gpu.mojo   # training
...
from noeira.envs.procgen.games import JumperGymEnv, JumperAssets
from noeira.envs.procgen.games.jumper import DIST_EASY
from noeira.core.fmt import fit

comptime ASSET_ROOT = String("assets/procgen/")

comptime JumperCNNEnv = JumperGymEnv[DT]
comptime OBS_DIM = JumperCNNEnv.OBS_DIM  # 3 × 84 × 84 = 21168
comptime NUM_ACTIONS = JumperCNNEnv.NUM_ACTIONS  # 15

comptime FRAMES = 3  # RGB channels (single frame — jumper is fully observable)
comptime NUM_ATOMS = 51
comptime HIDDEN = 512
comptime N_STEP = 3

# Jumper envs stepped in parallel across CPU cores; GPU trains the CNN.
comptime N_ENVS = 16
...
comptime BatchedJumper = BatchedCpuDiscreteEnv[JumperCNNEnv, N_ENVS, OBS_DIM]


def _make_envs(assets: ArcPointer[JumperAssets]) -> List[JumperCNNEnv]:
    """N_ENVS independent jumper envs sharing one read-only asset bundle, each
    with a distinct rand_seed so they sample different levels from the train set.
    """
    var envs = List[JumperCNNEnv]()
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN GPU Training on Procgen Jumper — Pixel (CPU-batched)")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var assets = ArcPointer(JumperAssets(ASSET_ROOT))
        var env = BatchedJumper(_make_envs(assets))
        var eval_env = BatchedJumper(_make_envs(assets))

        print(
            "Environment: Procgen Jumper (CPU-batched,",
            N_ENVS,
            "parallel envs, Easy,",
            NUM_LEVELS,
            "levels)",
        ...
        var run = RunContext(
            ...
            driver=String("examples/procgen/jumper_rainbow_training_gpu.mojo"),
            slug=String("rainbow-jumper"),
            env=String("builtin:procgen/jumper"),
        )
        ...
        logger.set_config("env", "procgen-jumper-easy")
        ...
        try:
            var _ret = agent.train_cpu_batched[
                BatchedJumper, N_ENVS, N_STEP, RunLogger
            ](
                env,
                NUM_STEPS,
                rng_seed=UInt64(42),
