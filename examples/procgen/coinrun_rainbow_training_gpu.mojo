"""Rainbow DQN CNN GPU training on Procgen Coinrun — pixel obs, CPU-batched envs.

Scales the CPU proof to a real run: `N_ENVS` coinrun envs step in parallel across
...
pixel trainers — used because coinrun generation + rendering are CPU-side (no
GPU-env kernels). A full GPU-resident coinrun env is a separate, larger effort.

Coinrun reward is SPARSE (single +10 for the coin, hard death on any hazard) →
...
level-count increase needs proportionally more steps (coinrun is sparse).

Run:
    pixi run mojo run -I . examples/procgen/coinrun_rainbow_training_gpu.mojo   # (apple) compile/smoke
    pixi run -e nvidia mojo run -I . examples/procgen/coinrun_rainbow_training_gpu.mojo   # training
...
from noeira.envs.procgen.games import CoinrunGymEnv, CoinrunAssets
from noeira.envs.procgen.games.coinrun import DIST_EASY
from noeira.core.fmt import fit

comptime ASSET_ROOT = String("assets/procgen/")

comptime CoinrunCNNEnv = CoinrunGymEnv[DT]
comptime OBS_DIM = CoinrunCNNEnv.OBS_DIM  # 3 × 84 × 84 = 21168
comptime NUM_ACTIONS = CoinrunCNNEnv.NUM_ACTIONS  # 15

comptime FRAMES = 3  # RGB channels (single frame — coinrun is fully observable)
comptime NUM_ATOMS = 51
comptime HIDDEN = 512
comptime N_STEP = 3

# Coinrun envs stepped in parallel across CPU cores; GPU trains the CNN.
comptime N_ENVS = 16
...
# Value support: -1 .. 11 (single +10 coin; sparse terminal).
...
comptime BatchedCoinrun = BatchedCpuDiscreteEnv[CoinrunCNNEnv, N_ENVS, OBS_DIM]


def _make_envs(assets: ArcPointer[CoinrunAssets]) -> List[CoinrunCNNEnv]:
    """N_ENVS independent coinrun envs sharing one read-only asset bundle, each
    with a distinct rand_seed so they sample different levels from the train set.
    """
    var envs = List[CoinrunCNNEnv]()
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN GPU Training on Procgen Coinrun — Pixel (CPU-batched)")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var assets = ArcPointer(CoinrunAssets(ASSET_ROOT))
        var env = BatchedCoinrun(_make_envs(assets))
        var eval_env = BatchedCoinrun(_make_envs(assets))

        print(
            "Environment: Procgen Coinrun (CPU-batched,",
            N_ENVS,
            "parallel envs, Easy,",
            NUM_LEVELS,
            "levels)",
        ...
        var run = RunContext(
            ...
            driver=String("examples/procgen/coinrun_rainbow_training_gpu.mojo"),
            slug=String("rainbow-coinrun"),
            env=String("builtin:procgen/coinrun"),
        )
        ...
        logger.set_config("env", "procgen-coinrun-easy")
        ...
        try:
            var _ret = agent.train_cpu_batched[
                BatchedCoinrun, N_ENVS, N_STEP, RunLogger
            ](
                env,
                NUM_STEPS,
                rng_seed=UInt64(42),
