"""Rainbow DQN CNN GPU training on Procgen Bigfish — pixel obs, CPU-batched envs.

Scales the CPU proof: `N_ENVS` bigfish envs step in parallel across CPU cores while
...
maze/chaser/heist. Bigfish reward is DENSE (+1 per fish, +10 quota) → expect
chaser-like learnability; start with `num_levels=1` and step up.

Run:
    pixi run mojo run -I . examples/procgen/bigfish_rainbow_training_gpu.mojo   # (apple) compile/smoke
    pixi run -e nvidia mojo run -I . examples/procgen/bigfish_rainbow_training_gpu.mojo   # training
...
from noeira.envs.procgen.games import BigfishGymEnv, BigfishAssets
from noeira.envs.procgen.games.bigfish import DIST_EASY
...
# Dense reward: up to ~40 (30 fish + 10 completion).
...
comptime BatchedBigfish = BatchedCpuDiscreteEnv[BigfishCNNEnv, N_ENVS, OBS_DIM]


def _make_envs(assets: ArcPointer[BigfishAssets]) -> List[BigfishCNNEnv]:
    var envs = List[BigfishCNNEnv]()
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN GPU Training on Procgen Bigfish — Pixel (CPU-batched)")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var assets = ArcPointer(BigfishAssets(ASSET_ROOT))
        var env = BatchedBigfish(_make_envs(assets))
        var eval_env = BatchedBigfish(_make_envs(assets))

        print(
            "Environment: Procgen Bigfish (CPU-batched,",
            N_ENVS,
            "parallel envs, Easy,",
            NUM_LEVELS,
            "levels)",
        ...
        var run = RunContext(
            ...
            driver=String("examples/procgen/bigfish_rainbow_training_gpu.mojo"),
            slug=String("rainbow-bigfish"),
            env=String("builtin:procgen/bigfish"),
        )
        ...
        logger.set_config("env", "procgen-bigfish-easy")
        ...
        try:
            var _ret = agent.train_cpu_batched[
                BatchedBigfish, N_ENVS, N_STEP, RunLogger
            ](
                env,
                NUM_STEPS,
                rng_seed=UInt64(42),
