"""Rainbow DQN CNN GPU training on Procgen Plunder — pixel obs, CPU-batched envs.

Scales the CPU proof to a real run: `N_ENVS` plunder envs step in parallel across
...
trainers — used because plunder generation + rendering are CPU-side (no GPU-env
kernels). A full GPU-resident plunder env is a separate, much larger effort.

Plunder reward is DENSE (+1 per target ship sunk, +10 for the target quota) → expect chaser-like learnability; start with `num_levels=1`.
...
climb as it sinks target ships), then step up 1 → 10 → 50 → 200. Each level-count
increase needs proportionally more steps.

Run:
    pixi run mojo run -I . examples/procgen/plunder_rainbow_training_gpu.mojo   # (apple) compile/smoke
    pixi run -e nvidia mojo run -I . examples/procgen/plunder_rainbow_training_gpu.mojo   # training
...
from noeira.envs.procgen.games import PlunderGymEnv, PlunderAssets
from noeira.envs.procgen.games.plunder import DIST_EASY
...
comptime FRAMES = 3  # RGB channels (single frame — plunder is fully observable)
comptime NUM_ATOMS = 51
comptime HIDDEN = 512
comptime N_STEP = 3

# Plunder envs stepped in parallel across CPU cores; GPU trains the CNN.
comptime N_ENVS = 16
...
comptime BatchedPlunder = BatchedCpuDiscreteEnv[PlunderCNNEnv, N_ENVS, OBS_DIM]


def _make_envs(assets: ArcPointer[PlunderAssets]) -> List[PlunderCNNEnv]:
    """N_ENVS independent plunder envs sharing one read-only asset bundle, each
    with a distinct rand_seed so they sample different levels from the train set.
    """
    var envs = List[PlunderCNNEnv]()
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN GPU Training on Procgen Plunder — Pixel (CPU-batched)")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var assets = ArcPointer(PlunderAssets(ASSET_ROOT))
        var env = BatchedPlunder(_make_envs(assets))
        var eval_env = BatchedPlunder(_make_envs(assets))

        print(
            "Environment: Procgen Plunder (CPU-batched,",
            N_ENVS,
            "parallel envs, Easy,",
            NUM_LEVELS,
            "levels)",
        ...
        var run = RunContext(
            ...
            driver=String("examples/procgen/plunder_rainbow_training_gpu.mojo"),
            slug=String("rainbow-plunder"),
            env=String("builtin:procgen/plunder"),
        )
        ...
        logger.set_config("env", "procgen-plunder-easy")
        ...
        try:
            var _ret = agent.train_cpu_batched[
                BatchedPlunder, N_ENVS, N_STEP, RunLogger
            ](
                env,
                NUM_STEPS,
                rng_seed=UInt64(42),
