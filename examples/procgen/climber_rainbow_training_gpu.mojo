"""Rainbow DQN CNN GPU training on Procgen Climber — pixel obs, CPU-batched envs.

Scales the CPU proof to a real run: `N_ENVS` climber envs step in parallel across
...
pixel trainers — used because climber generation + rendering are CPU-side (no
GPU-env kernels). A full GPU-resident climber env is a separate, larger effort.

Climber reward is SPARSE (single +10 for the coin, hard death on any hazard) →
...
level-count increase needs proportionally more steps (climber is sparse).

Run:
    pixi run mojo run -I . examples/procgen/climber_rainbow_training_gpu.mojo   # (apple) compile/smoke
    pixi run -e nvidia mojo run -I . examples/procgen/climber_rainbow_training_gpu.mojo   # training
...
from noeira.envs.procgen.games import ClimberGymEnv, ClimberAssets
from noeira.envs.procgen.games.climber import DIST_EASY
from noeira.core.fmt import fit

comptime ASSET_ROOT = String("assets/procgen/")

comptime ClimberCNNEnv = ClimberGymEnv[DT]
comptime OBS_DIM = ClimberCNNEnv.OBS_DIM  # 3 × 84 × 84 = 21168
comptime NUM_ACTIONS = ClimberCNNEnv.NUM_ACTIONS  # 15

comptime FRAMES = 3  # RGB channels (single frame — climber is fully observable)
comptime NUM_ATOMS = 51
comptime HIDDEN = 512
comptime N_STEP = 3

# Climber envs stepped in parallel across CPU cores; GPU trains the CNN.
comptime N_ENVS = 16
...
comptime BatchedClimber = BatchedCpuDiscreteEnv[ClimberCNNEnv, N_ENVS, OBS_DIM]


def _make_envs(assets: ArcPointer[ClimberAssets]) -> List[ClimberCNNEnv]:
    """N_ENVS independent climber envs sharing one read-only asset bundle, each
    with a distinct rand_seed so they sample different levels from the train set.
    """
    var envs = List[ClimberCNNEnv]()
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN GPU Training on Procgen Climber — Pixel (CPU-batched)")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var assets = ArcPointer(ClimberAssets(ASSET_ROOT))
        var env = BatchedClimber(_make_envs(assets))
        var eval_env = BatchedClimber(_make_envs(assets))

        print(
            "Environment: Procgen Climber (CPU-batched,",
            N_ENVS,
            "parallel envs, Easy,",
            NUM_LEVELS,
            "levels)",
        ...
        var run = RunContext(
            ...
            driver=String("examples/procgen/climber_rainbow_training_gpu.mojo"),
            slug=String("rainbow-climber"),
            env=String("builtin:procgen/climber"),
        )
        ...
        logger.set_config("env", "procgen-climber-easy")
        ...
        try:
            var _ret = agent.train_cpu_batched[
                BatchedClimber, N_ENVS, N_STEP, RunLogger
            ](
                env,
                NUM_STEPS,
                rng_seed=UInt64(42),
