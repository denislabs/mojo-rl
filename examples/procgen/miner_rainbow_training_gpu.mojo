"""Rainbow DQN CNN GPU training on Procgen Miner — pixel obs, CPU-batched envs.

Scales the CPU proof to a real run: `N_ENVS` miner envs step in parallel across
...
trainers — used because miner generation + rendering are CPU-side (no GPU-env
kernels). A full GPU-resident miner env is a separate, much larger effort.

Miner reward is dense-ish (+1 per diamond, +10 for exiting once all are
collected) → expect chaser-like learnability; start with `num_levels=1`.
...
climb as it collects diamonds + exits), then step up 1 → 10 → 50 → 200. Each level-count
increase needs proportionally more steps.

Run:
    pixi run mojo run -I . examples/procgen/miner_rainbow_training_gpu.mojo   # (apple) compile/smoke
    pixi run -e nvidia mojo run -I . examples/procgen/miner_rainbow_training_gpu.mojo   # training
...
from noeira.envs.procgen.games import MinerGymEnv, MinerAssets
from noeira.envs.procgen.games.miner import DIST_EASY
from noeira.core.fmt import fit

comptime ASSET_ROOT = String("assets/procgen/")

comptime MinerCNNEnv = MinerGymEnv[DT]
comptime OBS_DIM = MinerCNNEnv.OBS_DIM  # 3 × 84 × 84 = 21168
comptime NUM_ACTIONS = MinerCNNEnv.NUM_ACTIONS  # 15

comptime FRAMES = 3  # RGB channels (single frame — miner is fully observable)
comptime NUM_ATOMS = 51
comptime HIDDEN = 512
comptime N_STEP = 3

# Miner envs stepped in parallel across CPU cores; GPU trains the CNN.
comptime N_ENVS = 16
...
# Value support: 0 .. ~23 (up to ~12 diamonds at +1 plus the +10 exit bonus).
comptime V_MIN = Scalar[DT](-1.0)
comptime V_MAX = Scalar[DT](23.0)
...
comptime BatchedMiner = BatchedCpuDiscreteEnv[MinerCNNEnv, N_ENVS, OBS_DIM]


def _make_envs(assets: ArcPointer[MinerAssets]) -> List[MinerCNNEnv]:
    """N_ENVS independent miner envs sharing one read-only asset bundle, each
    with a distinct rand_seed so they sample different levels from the train set.
    """
    var envs = List[MinerCNNEnv]()
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN GPU Training on Procgen Miner — Pixel (CPU-batched)")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var assets = ArcPointer(MinerAssets(ASSET_ROOT))
        var env = BatchedMiner(_make_envs(assets))
        var eval_env = BatchedMiner(_make_envs(assets))

        print(
            "Environment: Procgen Miner (CPU-batched,",
            N_ENVS,
            "parallel envs, Easy,",
            NUM_LEVELS,
            "levels)",
        ...
        var run = RunContext(
            ...
            driver=String("examples/procgen/miner_rainbow_training_gpu.mojo"),
            slug=String("rainbow-miner"),
            env=String("builtin:procgen/miner"),
        )
        ...
        logger.set_config("env", "procgen-miner-easy")
        ...
        try:
            var _ret = agent.train_cpu_batched[
                BatchedMiner, N_ENVS, N_STEP, RunLogger
            ](
                env,
                NUM_STEPS,
                rng_seed=UInt64(42),
