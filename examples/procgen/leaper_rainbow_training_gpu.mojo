"""Rainbow DQN CNN GPU training on Procgen Leaper — pixel obs, CPU-batched envs.

Scales the CPU proof to a real run: `N_ENVS` leaper envs step in parallel across
...
trainers — used because leaper generation + rendering are CPU-side (no GPU-env
kernels). A full GPU-resident leaper env is a separate, much larger effort.

Leaper reward is SPARSE (+10 at the finish only) — expect maze-like difficulty;
start with `num_levels=1` and step up.
...
climb as it reaches the finish), then step up 1 → 10 → 50 → 200. Each level-count
increase needs proportionally more steps.

Run:
    pixi run mojo run -I . examples/procgen/leaper_rainbow_training_gpu.mojo   # (apple) compile/smoke
...
from mojo_rl.envs.procgen.games import LeaperGymEnv, LeaperAssets
from mojo_rl.envs.procgen.games.leaper import DIST_EASY

comptime ASSET_ROOT = String("assets/procgen/")

comptime LeaperCNNEnv = LeaperGymEnv[DT]
comptime OBS_DIM = LeaperCNNEnv.OBS_DIM  # 3 × 84 × 84 = 21168
comptime NUM_ACTIONS = LeaperCNNEnv.NUM_ACTIONS  # 15

comptime FRAMES = 3  # RGB channels (single frame — leaper is fully observable)
comptime NUM_ATOMS = 51
comptime HIDDEN = 512
comptime N_STEP = 3

# Leaper envs stepped in parallel across CPU cores; GPU trains the CNN.
comptime N_ENVS = 16
...
# Sparse reward: 0 (timeout/death) .. ~10 (finish). Same support as maze.
...
def _make_envs(assets: ArcPointer[LeaperAssets]) -> List[LeaperCNNEnv]:
    """N_ENVS independent leaper envs sharing one read-only asset bundle, each
    with a distinct rand_seed so they sample different levels from the train set.
    """
    var envs = List[LeaperCNNEnv]()
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN GPU Training on Procgen Leaper — Pixel (CPU-batched)")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var assets = ArcPointer(LeaperAssets(ASSET_ROOT))
        var env = BatchedLeaper(_make_envs(assets))
        var eval_env = BatchedLeaper(_make_envs(assets))

        print(
            "Environment: Procgen Leaper (CPU-batched,",
            N_ENVS,
            "parallel envs, Easy,",
            NUM_LEVELS,
            "levels)",
