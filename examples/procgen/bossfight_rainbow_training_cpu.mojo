"""Rainbow DQN (CNN) CPU training on Procgen Bossfight — pixel obs.

Single-env CPU proof that `BossfightGymEnv` learns with the deep_agents Rainbow
stack (C51 + Double + PER + Dueling + Noisy + N-step) on a Nature-CNN backbone
over 84×84×3 observations. Mirrors `plunder_rainbow_training_cpu.mojo` but on the
bossfight substrate. Reward is sparse-ish (+1 per boss health chunk cleared, +10
for the kill) with a hard death (touch the boss / a meteor / an enemy bullet →
episode over) → harder than plunder; expect slow early learning.
...
`docs/PROCGEN_BOSSFIGHT_SCOPE.md`.

Run:
    pixi run mojo run -I . examples/procgen/bossfight_rainbow_training_cpu.mojo
"""
...
from noeira.envs.procgen.games import BossfightGymEnv
from noeira.envs.procgen.games.bossfight import DIST_EASY
...
comptime FRAMES = 3  # RGB channels (no frame stack — bossfight is fully observable)
...
comptime BossfightTrainer = C51Trainer[
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN CPU Training on Procgen Bossfight (pixel, single-env)")
    print("=" * 70)

    var trainer = BossfightTrainer.make(
        lr=LR,
    ...
    print("Environment: Procgen Bossfight (CPU, single env, Easy, 1 level)")
    ...
    var logger = RemoteLogger(
        server_url=url,
        run_name="Rainbow Bossfight CPU (procgen)",
        buffer_size=64,
        api_key=api_key,
    )
    logger.set_config("agent", "Rainbow DQN CNN (deep_agents, CPU)")
    logger.set_config("env", "procgen-bossfight-easy-1level")
    ...
    var _ret = run_offpolicy_discrete_train[
        BossfightTrainer, BossfightCNNEnv, RemoteLogger
    ](
        trainer,
        env,
        NUM_STEPS,
