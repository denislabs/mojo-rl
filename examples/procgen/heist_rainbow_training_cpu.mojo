"""Rainbow DQN (CNN) CPU training on Procgen Heist — pixel obs.

Single-env CPU proof that `HeistGymEnv` learns with the deep_agents Rainbow stack
on a Nature-CNN backbone over 84×84×3 observations. Mirrors the maze/chaser CPU
trainers. Heist reward is SPARSE (+10 at the gem only, after collecting keys and
opening doors) — expect maze-like difficulty, so it defaults to Easy mode, a
single level (`num_levels=1`). Move to NVIDIA GPU-batched for a real run. See
`docs/PROCGEN_HEIST_SCOPE.md`.

Run:
    pixi run mojo run -I . examples/procgen/heist_rainbow_training_cpu.mojo
"""
...
from noeira.envs.procgen.games import HeistGymEnv
...
# Sparse reward: 0 (timeout) .. ~10 (gem). Same support as maze.
...
comptime HeistTrainer = C51Trainer[
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN CPU Training on Procgen Heist (pixel, single-env)")
    print("=" * 70)

    var trainer = HeistTrainer.make(
        lr=LR,
    ...
    var eval_env = HeistCNNEnv(
        ASSET_ROOT, rand_seed=0, num_levels=1, start_level=0, dist_mode=DIST_EASY
    )

    print("Environment: Procgen Heist (CPU, single env, Easy, 1 level)")
    ...
    var logger = RemoteLogger(
        server_url=url,
        run_name="Rainbow Heist CPU (procgen)",
        buffer_size=64,
        api_key=api_key,
    )
    logger.set_config("agent", "Rainbow DQN CNN (deep_agents, CPU)")
    logger.set_config("env", "procgen-heist-easy-1level")
    ...
    var _ret = run_offpolicy_discrete_train[
        HeistTrainer, HeistCNNEnv, RemoteLogger
    ](
        trainer,
        env,
        NUM_STEPS,
