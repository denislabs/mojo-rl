"""Rainbow DQN CNN GPU training on Procgen Starpilot — pixel obs, CPU-batched envs.

Scales the CPU proof to a real run: `N_ENVS` starpilot envs step in parallel across
...
trainers — used because starpilot generation + rendering are CPU-side (no GPU-env
kernels). A full GPU-resident starpilot env is a separate, much larger effort.

Starpilot reward is DENSE (+1 per enemy shot down, +10 for surviving to the
finish) → expect chaser-like learnability; start with `num_levels=1`.
...
climb as it shoots down enemies), then step up 1 → 10 → 50 → 200. Each level-count
increase needs proportionally more steps.

Run:
    pixi run mojo run -I . examples/procgen/starpilot_rainbow_training_gpu.mojo   # (apple) compile/smoke
    pixi run -e nvidia mojo run -I . examples/procgen/starpilot_rainbow_training_gpu.mojo   # training
...
from noeira.envs.procgen.games import StarpilotGymEnv, StarpilotAssets
from noeira.envs.procgen.games.starpilot import DIST_EASY
...
comptime FRAMES = 3  # RGB channels (single frame — starpilot is fully observable)
comptime NUM_ATOMS = 51
comptime HIDDEN = 512
comptime N_STEP = 3

# Starpilot envs stepped in parallel across CPU cores; GPU trains the CNN.
comptime N_ENVS = 16
...
comptime BatchedStarpilot = BatchedCpuDiscreteEnv[StarpilotCNNEnv, N_ENVS, OBS_DIM]


def _make_envs(assets: ArcPointer[StarpilotAssets]) -> List[StarpilotCNNEnv]:
    """N_ENVS independent starpilot envs sharing one read-only asset bundle, each
    with a distinct rand_seed so they sample different levels from the train set.
    """
    var envs = List[StarpilotCNNEnv]()
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN GPU Training on Procgen Starpilot — Pixel (CPU-batched)")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var assets = ArcPointer(StarpilotAssets(ASSET_ROOT))
        var env = BatchedStarpilot(_make_envs(assets))
        var eval_env = BatchedStarpilot(_make_envs(assets))

        print(
            "Environment: Procgen Starpilot (CPU-batched,",
            N_ENVS,
            "parallel envs, Easy,",
            NUM_LEVELS,
            "levels)",
        ...
        var run = RunContext(
            ...
            driver=String("examples/procgen/starpilot_rainbow_training_gpu.mojo"),
            slug=String("rainbow-starpilot"),
            env=String("builtin:procgen/starpilot"),
        )
        ...
        logger.set_config("env", "procgen-starpilot-easy")
        ...
        try:
            var _ret = agent.train_cpu_batched[
                BatchedStarpilot, N_ENVS, N_STEP, RunLogger
            ](
                env,
                NUM_STEPS,
                rng_seed=UInt64(42),
