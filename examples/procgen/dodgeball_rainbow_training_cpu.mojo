"""Rainbow DQN (CNN) CPU training on Procgen Dodgeball — pixel obs.

Single-env CPU proof that `DodgeballGymEnv` learns with the deep_agents Rainbow stack
(C51 + Double + PER + Dueling + Noisy + N-step) on a Nature-CNN backbone over
84×84×3 observations. Mirrors `plunder_rainbow_training_cpu.mojo` but on the
platformer substrate. Reward is SPARSE (single +10 for the coin, hard death on any
hazard) → expect maze-like difficulty (the terminal signal is far from the start).
...
`docs/PROCGEN_COINRUN_SCOPE.md`.

Run:
    pixi run mojo run -I . examples/procgen/dodgeball_rainbow_training_cpu.mojo
"""
...
from noeira.envs.procgen.games import DodgeballGymEnv
...
comptime FRAMES = 3  # RGB channels (no frame stack — dodgeball is fully observable)
...
comptime DodgeballTrainer = C51Trainer[
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN CPU Training on Procgen Dodgeball (pixel, single-env)")
    print("=" * 70)

    var trainer = DodgeballTrainer.make(
        lr=LR,
    ...
    var env = DodgeballCNNEnv(
        ASSET_ROOT, rand_seed=0, num_levels=1, start_level=0, dist_mode=DIST_EASY
    )
    var eval_env = DodgeballCNNEnv(
        ASSET_ROOT, rand_seed=0, num_levels=1, start_level=0, dist_mode=DIST_EASY
    )

    print("Environment: Procgen Dodgeball (CPU, single env, Easy, 1 level)")
    ...
    var logger = RemoteLogger(
        server_url=url,
        run_name="Rainbow Dodgeball CPU (procgen)",
        buffer_size=64,
        api_key=api_key,
    )
    logger.set_config("agent", "Rainbow DQN CNN (deep_agents, CPU)")
    logger.set_config("env", "procgen-dodgeball-easy-1level")
    ...
    var _ret = run_offpolicy_discrete_train[
        DodgeballTrainer, DodgeballCNNEnv, RemoteLogger
    ](
        trainer,
        env,
        NUM_STEPS,
