"""Rainbow DQN (CNN) CPU training on Procgen Miner — pixel obs.

Single-env CPU proof that `MinerGymEnv` learns with the deep_agents Rainbow stack
(C51 + Double + PER + Dueling + Noisy + N-step) on a Nature-CNN backbone over
84×84×3 observations. Mirrors `maze_rainbow_training_cpu.mojo` but on the miner
grid cellular-automaton game. Reward is dense-ish (+1 per diamond, +10 for
exiting once all are collected) → expect chaser-like learnability.
...
`docs/PROCGEN_MINER_SCOPE.md`.

Run:
    pixi run mojo run -I . examples/procgen/miner_rainbow_training_cpu.mojo
"""
...
from noeira.envs.procgen.games import MinerGymEnv
...
comptime FRAMES = 3  # RGB channels (no frame stack — miner is fully observable)
...
comptime MinerTrainer = C51Trainer[
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN CPU Training on Procgen Miner (pixel, single-env)")
    print("=" * 70)

    var trainer = MinerTrainer.make(
        lr=LR,
    ...
    print("Environment: Procgen Miner (CPU, single env, Easy, 1 level)")
    ...
    var logger = RemoteLogger(
        server_url=url,
        run_name="Rainbow Miner CPU (procgen)",
        buffer_size=64,
        api_key=api_key,
    )
    logger.set_config("agent", "Rainbow DQN CNN (deep_agents, CPU)")
    logger.set_config("env", "procgen-miner-easy-1level")
    ...
    var _ret = run_offpolicy_discrete_train[
        MinerTrainer, MinerCNNEnv, RemoteLogger
    ](
        trainer,
        env,
        NUM_STEPS,
