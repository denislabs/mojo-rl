"""Rainbow DQN (CNN) CPU training on Procgen Climber — pixel obs.

Single-env CPU proof that `ClimberGymEnv` learns with the deep_agents Rainbow stack
...
Run:
    pixi run mojo run -I . examples/procgen/climber_rainbow_training_cpu.mojo
"""
...
from noeira.envs.procgen.games import ClimberGymEnv
...
comptime FRAMES = 3  # RGB channels (no frame stack — climber is fully observable)
...
comptime ClimberTrainer = C51Trainer[
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN CPU Training on Procgen Climber (pixel, single-env)")
    print("=" * 70)

    var trainer = ClimberTrainer.make(
        lr=LR,
    ...
    var env = ClimberCNNEnv(
        ASSET_ROOT, rand_seed=0, num_levels=1, start_level=0, dist_mode=DIST_EASY
    )
    var eval_env = ClimberCNNEnv(
        ASSET_ROOT, rand_seed=0, num_levels=1, start_level=0, dist_mode=DIST_EASY
    )

    print("Environment: Procgen Climber (CPU, single env, Easy, 1 level)")
    ...
    var logger = RemoteLogger(
        server_url=url,
        run_name="Rainbow Climber CPU (procgen)",
        buffer_size=64,
        api_key=api_key,
    )
    logger.set_config("agent", "Rainbow DQN CNN (deep_agents, CPU)")
    logger.set_config("env", "procgen-climber-easy-1level")
    ...
    var _ret = run_offpolicy_discrete_train[
        ClimberTrainer, ClimberCNNEnv, RemoteLogger
    ](
        trainer,
        env,
        NUM_STEPS,
