"""Rainbow DQN (CNN) CPU training on Procgen Caveflyer — pixel obs.

Single-env CPU proof that `CaveflyerGymEnv` learns with the deep_agents Rainbow
stack (C51 + Double + PER + Dueling + Noisy + N-step) on a Nature-CNN backbone over
84×84×3 observations. Mirrors `coinrun_rainbow_training_cpu.mojo` but on the
roomgen cave-flyer. Reward = +3 per target ship shot + 10 for reaching the goal
(hard death on any collision) → navigation + aiming; sparse-ish terminal.
...
`docs/PROCGEN_CAVEFLYER_SCOPE.md`.

Run:
    pixi run mojo run -I . examples/procgen/caveflyer_rainbow_training_cpu.mojo
"""
...
from noeira.deep_agents.c51.config import RainbowCNNNet
...
from noeira.envs.procgen.games import CaveflyerGymEnv
...
comptime FRAMES = 3  # RGB channels (no frame stack — caveflyer is fully observable)
...
comptime CaveflyerTrainer = C51Trainer[
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN CPU Training on Procgen Caveflyer (pixel, single-env)")
    print("=" * 70)

    var trainer = CaveflyerTrainer.make(
        lr=LR,
    ...
    var env = CaveflyerCNNEnv(
        ASSET_ROOT, rand_seed=0, num_levels=1, start_level=0, dist_mode=DIST_EASY
    )
    var eval_env = CaveflyerCNNEnv(
        ASSET_ROOT, rand_seed=0, num_levels=1, start_level=0, dist_mode=DIST_EASY
    )

    print("Environment: Procgen Caveflyer (CPU, single env, Easy, 1 level)")
    ...
    var logger = RemoteLogger(
        server_url=url,
        run_name="Rainbow Caveflyer CPU (procgen)",
        buffer_size=64,
        api_key=api_key,
    )
    logger.set_config("agent", "Rainbow DQN CNN (deep_agents, CPU)")
    logger.set_config("env", "procgen-caveflyer-easy-1level")
    ...
    var _ret = run_offpolicy_discrete_train[
        CaveflyerTrainer, CaveflyerCNNEnv, RemoteLogger
    ](
        trainer,
        env,
        NUM_STEPS,
