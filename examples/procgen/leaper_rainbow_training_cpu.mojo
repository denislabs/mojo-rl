"""Rainbow DQN (CNN) CPU training on Procgen Leaper — pixel obs.

Single-env CPU proof that `LeaperGymEnv` learns with the deep_agents Rainbow stack
(C51 + Double + PER + Dueling + Noisy + N-step) on a Nature-CNN backbone over
84×84×3 observations. Mirrors `maze_rainbow_training_cpu.mojo` but on the leaper
substrate. Leaper's reward is SPARSE (+10 at the finish only, after crossing the
road + river) so expect maze-like difficulty; defaults to Easy mode, a single
level (`num_levels=1`). Move to NVIDIA GPU-batched for a real run. See
`docs/PROCGEN_LEAPER_SCOPE.md`.

Run:
    pixi run mojo run -I . examples/procgen/leaper_rainbow_training_cpu.mojo
"""
...
from noeira.envs.procgen.games import LeaperGymEnv
...
comptime FRAMES = 3  # RGB channels (no frame stack — leaper is fully observable)
...
comptime LeaperTrainer = C51Trainer[
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN CPU Training on Procgen Leaper (pixel, single-env)")
    print("=" * 70)

    var trainer = LeaperTrainer.make(
        lr=LR,
    ...
    print("Environment: Procgen Leaper (CPU, single env, Easy, 1 level)")
    ...
    var logger = RemoteLogger(
        server_url=url,
        run_name="Rainbow Leaper CPU (procgen)",
        buffer_size=64,
        api_key=api_key,
    )
    logger.set_config("agent", "Rainbow DQN CNN (deep_agents, CPU)")
    logger.set_config("env", "procgen-leaper-easy-1level")
    ...
    var _ret = run_offpolicy_discrete_train[
        LeaperTrainer, LeaperCNNEnv, RemoteLogger
    ](
        trainer,
        env,
        NUM_STEPS,
