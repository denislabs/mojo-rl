"""Rainbow DQN (CNN) CPU training on Procgen Chaser — pixel obs.

Single-env CPU proof that `ChaserGymEnv` learns with the deep_agents Rainbow stack
(C51 + Double + PER + Dueling + Noisy + N-step) on a Nature-CNN backbone over
84×84×3 observations. Mirrors `maze_rainbow_training_cpu.mojo` but on the chaser
entity substrate — chaser's reward is DENSE (+0.04 per orb, +10 for clearing the
level) so the learning signal appears much sooner than sparse maze, making it a
good end-to-end check that the substrate physics/render/env are correct.
...
Run:
    pixi run mojo run -I . examples/procgen/chaser_rainbow_training_cpu.mojo
"""
...
from noeira.envs.procgen.games import ChaserGymEnv
...
comptime FRAMES = 3  # RGB channels (no frame stack — chaser is fully observable)
...
comptime ChaserTrainer = C51Trainer[
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN CPU Training on Procgen Chaser (pixel, single-env)")
    print("=" * 70)

    var trainer = ChaserTrainer.make(
        lr=LR,
    ...
    print("Environment: Procgen Chaser (CPU, single env, Easy, 1 level)")
    ...
    var logger = RemoteLogger(
        server_url=url,
        run_name="Rainbow Chaser CPU (procgen)",
        buffer_size=64,
        api_key=api_key,
    )
    logger.set_config("agent", "Rainbow DQN CNN (deep_agents, CPU)")
    logger.set_config("env", "procgen-chaser-easy-1level")
    ...
    var _ret = run_offpolicy_discrete_train[
        ChaserTrainer, ChaserCNNEnv, RemoteLogger
    ](
        trainer,
        env,
        NUM_STEPS,
