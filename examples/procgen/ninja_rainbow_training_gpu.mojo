"""Rainbow DQN CNN GPU training on Procgen Ninja — pixel obs, CPU-batched envs.

Scales the CPU proof to a real run: `N_ENVS` ninja envs step in parallel across
...
pixel trainers — used because ninja generation + rendering are CPU-side (no
GPU-env kernels). A full GPU-resident ninja env is a separate, larger effort.

Ninja reward is SPARSE (single +10 for the coin, hard death on any hazard) →
...
level-count increase needs proportionally more steps (ninja is sparse).

Run:
    pixi run mojo run -I . examples/procgen/ninja_rainbow_training_gpu.mojo   # (apple) compile/smoke
    pixi run -e nvidia mojo run -I . examples/procgen/ninja_rainbow_training_gpu.mojo   # training
...
from noeira.envs.procgen.games import NinjaGymEnv, NinjaAssets
from noeira.envs.procgen.games.ninja import DIST_EASY
from noeira.core.fmt import fit

comptime ASSET_ROOT = String("assets/procgen/")

comptime NinjaCNNEnv = NinjaGymEnv[DT]
comptime OBS_DIM = NinjaCNNEnv.OBS_DIM  # 3 × 84 × 84 = 21168
comptime NUM_ACTIONS = NinjaCNNEnv.NUM_ACTIONS  # 15

comptime FRAMES = 3  # RGB channels (single frame — ninja is fully observable)
comptime NUM_ATOMS = 51
comptime HIDDEN = 512
comptime N_STEP = 3

# Ninja envs stepped in parallel across CPU cores; GPU trains the CNN.
comptime N_ENVS = 16
...
comptime BatchedNinja = BatchedCpuDiscreteEnv[NinjaCNNEnv, N_ENVS, OBS_DIM]


def _make_envs(assets: ArcPointer[NinjaAssets]) -> List[NinjaCNNEnv]:
    """N_ENVS independent ninja envs sharing one read-only asset bundle, each
    with a distinct rand_seed so they sample different levels from the train set.
    """
    var envs = List[NinjaCNNEnv]()
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN GPU Training on Procgen Ninja — Pixel (CPU-batched)")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var assets = ArcPointer(NinjaAssets(ASSET_ROOT))
        var env = BatchedNinja(_make_envs(assets))
        var eval_env = BatchedNinja(_make_envs(assets))

        print(
            "Environment: Procgen Ninja (CPU-batched,",
            N_ENVS,
            "parallel envs, Easy,",
            NUM_LEVELS,
            "levels)",
        ...
        var run = RunContext(
            ...
            driver=String("examples/procgen/ninja_rainbow_training_gpu.mojo"),
            slug=String("rainbow-ninja"),
            env=String("builtin:procgen/ninja"),
        )
        ...
        logger.set_config("env", "procgen-ninja-easy")
        ...
        try:
            var _ret = agent.train_cpu_batched[
                BatchedNinja, N_ENVS, N_STEP, RunLogger
            ](
                env,
                NUM_STEPS,
                rng_seed=UInt64(42),
