"""Rainbow DQN (CNN) CPU training on Procgen Ninja — pixel obs.

Single-env CPU proof that `NinjaGymEnv` learns with the deep_agents Rainbow stack
...
Run:
    pixi run mojo run -I . examples/procgen/ninja_rainbow_training_cpu.mojo
"""
...
from noeira.envs.procgen.games import NinjaGymEnv
...
comptime FRAMES = 3  # RGB channels (no frame stack — ninja is fully observable)
...
comptime NinjaTrainer = C51Trainer[
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN CPU Training on Procgen Ninja (pixel, single-env)")
    print("=" * 70)

    var trainer = NinjaTrainer.make(
        lr=LR,
    ...
    var eval_env = NinjaCNNEnv(
        ASSET_ROOT, rand_seed=0, num_levels=1, start_level=0, dist_mode=DIST_EASY
    )

    print("Environment: Procgen Ninja (CPU, single env, Easy, 1 level)")
    ...
    var logger = RemoteLogger(
        server_url=url,
        run_name="Rainbow Ninja CPU (procgen)",
        buffer_size=64,
        api_key=api_key,
    )
    logger.set_config("agent", "Rainbow DQN CNN (deep_agents, CPU)")
    logger.set_config("env", "procgen-ninja-easy-1level")
    ...
    var _ret = run_offpolicy_discrete_train[
        NinjaTrainer, NinjaCNNEnv, RemoteLogger
    ](
        trainer,
        env,
        NUM_STEPS,
