"""Rainbow DQN CNN GPU training on Procgen Caveflyer — pixel obs, CPU-batched envs.

Scales the CPU proof to a real run: `N_ENVS` caveflyer envs step in parallel across
...
with a GPU-resident uint8 obs replay. Same path as the maze/coinrun/Atari-Pong
pixel trainers — used because caveflyer generation + rendering are CPU-side (no
GPU-env kernels). A full GPU-resident caveflyer env is a separate, larger effort.

Caveflyer reward = +3 per target ship shot + 10 for the goal, hard death on any
collision → navigation + aiming; start with `num_levels=1` and a longer horizon.
...
climb as it reaches the goal), then step up 1 → 10 → 50 → 200. Each level-count
increase needs proportionally more steps.

Run:
    pixi run mojo run -I . examples/procgen/caveflyer_rainbow_training_gpu.mojo   # (apple) compile/smoke
    pixi run -e nvidia mojo run -I . examples/procgen/caveflyer_rainbow_training_gpu.mojo   # training
...
from noeira.envs.procgen.games import CaveflyerGymEnv, CaveflyerAssets
from noeira.envs.procgen.games.caveflyer import DIST_EASY
from noeira.core.fmt import fit

comptime ASSET_ROOT = String("assets/procgen/")

comptime CaveflyerCNNEnv = CaveflyerGymEnv[DT]
comptime OBS_DIM = CaveflyerCNNEnv.OBS_DIM  # 3 × 84 × 84 = 21168
comptime NUM_ACTIONS = CaveflyerCNNEnv.NUM_ACTIONS  # 15

comptime FRAMES = 3  # RGB channels (single frame — caveflyer is fully observable)
comptime NUM_ATOMS = 51
comptime HIDDEN = 512
comptime N_STEP = 3

# Caveflyer envs stepped in parallel across CPU cores; GPU trains the CNN.
comptime N_ENVS = 16
...
# Value support: -1 .. 19 (targets at +3 each plus the +10 goal).
comptime V_MIN = Scalar[DT](-1.0)
comptime V_MAX = Scalar[DT](19.0)
...
comptime BatchedCaveflyer = BatchedCpuDiscreteEnv[CaveflyerCNNEnv, N_ENVS, OBS_DIM]


def _make_envs(assets: ArcPointer[CaveflyerAssets]) -> List[CaveflyerCNNEnv]:
    """N_ENVS independent caveflyer envs sharing one read-only asset bundle, each
    with a distinct rand_seed so they sample different levels from the train set.
    """
    var envs = List[CaveflyerCNNEnv]()
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN GPU Training on Procgen Caveflyer — Pixel (CPU-batched)")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var assets = ArcPointer(CaveflyerAssets(ASSET_ROOT))
        var env = BatchedCaveflyer(_make_envs(assets))
        var eval_env = BatchedCaveflyer(_make_envs(assets))

        print(
            "Environment: Procgen Caveflyer (CPU-batched,",
            N_ENVS,
            "parallel envs, Easy,",
            NUM_LEVELS,
            "levels)",
        ...
        var run = RunContext(
            ...
            driver=String("examples/procgen/caveflyer_rainbow_training_gpu.mojo"),
            slug=String("rainbow-caveflyer"),
            env=String("builtin:procgen/caveflyer"),
        )
        ...
        logger.set_config("env", "procgen-caveflyer-easy")
        ...
        try:
            var _ret = agent.train_cpu_batched[
                BatchedCaveflyer, N_ENVS, N_STEP, RunLogger
            ](
                env,
                NUM_STEPS,
                rng_seed=UInt64(42),
