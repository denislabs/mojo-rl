"""Rainbow DQN (CNN) CPU training on Procgen Coinrun — pixel obs.

Single-env CPU proof that `CoinrunGymEnv` learns with the deep_agents Rainbow stack
...
Run:
    pixi run mojo run -I . examples/procgen/coinrun_rainbow_training_cpu.mojo
"""
...
from noeira.envs.procgen.games import CoinrunGymEnv
...
comptime FRAMES = 3  # RGB channels (no frame stack — coinrun is fully observable)
...
comptime CoinrunTrainer = C51Trainer[
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN CPU Training on Procgen Coinrun (pixel, single-env)")
    print("=" * 70)

    var trainer = CoinrunTrainer.make(
        lr=LR,
    ...
    var env = CoinrunCNNEnv(
        ASSET_ROOT, rand_seed=0, num_levels=1, start_level=0, dist_mode=DIST_EASY
    )
    var eval_env = CoinrunCNNEnv(
        ASSET_ROOT, rand_seed=0, num_levels=1, start_level=0, dist_mode=DIST_EASY
    )

    print("Environment: Procgen Coinrun (CPU, single env, Easy, 1 level)")
    ...
    var logger = RemoteLogger(
        server_url=url,
        run_name="Rainbow Coinrun CPU (procgen)",
        buffer_size=64,
        api_key=api_key,
    )
    logger.set_config("agent", "Rainbow DQN CNN (deep_agents, CPU)")
    logger.set_config("env", "procgen-coinrun-easy-1level")
    ...
    var _ret = run_offpolicy_discrete_train[
        CoinrunTrainer, CoinrunCNNEnv, RemoteLogger
    ](
        trainer,
        env,
        NUM_STEPS,
