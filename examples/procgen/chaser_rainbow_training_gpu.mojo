"""Rainbow DQN CNN GPU training on Procgen Chaser — pixel obs, CPU-batched envs.

Scales the CPU proof to a real run: `N_ENVS` chaser envs step in parallel across
...
trainers — used because chaser generation + rendering are CPU-side (no GPU-env
kernels). A full GPU-resident chaser env is a separate, much larger effort.

Chaser is the entity-substrate game; its reward is DENSE (+0.04 per orb, +10 for
clearing), so learning should appear much sooner than sparse maze — a good
end-to-end confirmation the substrate is correct.
...
climb as the agent clears orbs), then step up 1 → 10 → 50 → 200. Each level-count
increase needs proportionally more steps.

Run:
    pixi run mojo run -I . examples/procgen/chaser_rainbow_training_gpu.mojo   # (apple) compile/smoke
    pixi run -e nvidia mojo run -I . examples/procgen/chaser_rainbow_training_gpu.mojo   # training
...
from noeira.envs.procgen.games import ChaserGymEnv, ChaserAssets
from noeira.envs.procgen.games.chaser import DIST_EASY
from noeira.core.fmt import fit

comptime ASSET_ROOT = String("assets/procgen/")

comptime ChaserCNNEnv = ChaserGymEnv[DT]
comptime OBS_DIM = ChaserCNNEnv.OBS_DIM  # 3 × 84 × 84 = 21168
comptime NUM_ACTIONS = ChaserCNNEnv.NUM_ACTIONS  # 15

comptime FRAMES = 3  # RGB channels (single frame — chaser is fully observable)
comptime NUM_ATOMS = 51
comptime HIDDEN = 512
comptime N_STEP = 3

# Chaser envs stepped in parallel across CPU cores; GPU trains the CNN.
comptime N_ENVS = 16
...
# Value support must bracket the discounted return: 0 .. ~13 (all ~75 orbs at
# +0.04 ≈ 3 plus the +10 completion bonus).
comptime V_MIN = Scalar[DT](-1.0)
comptime V_MAX = Scalar[DT](13.0)
...
comptime BatchedChaser = BatchedCpuDiscreteEnv[ChaserCNNEnv, N_ENVS, OBS_DIM]


def _make_envs(assets: ArcPointer[ChaserAssets]) -> List[ChaserCNNEnv]:
    """N_ENVS independent chaser envs sharing one read-only asset bundle, each
    with a distinct rand_seed so they sample different levels from the train set.
    """
    var envs = List[ChaserCNNEnv]()
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN GPU Training on Procgen Chaser — Pixel (CPU-batched)")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var assets = ArcPointer(ChaserAssets(ASSET_ROOT))
        var env = BatchedChaser(_make_envs(assets))
        var eval_env = BatchedChaser(_make_envs(assets))

        print(
            "Environment: Procgen Chaser (CPU-batched,",
            N_ENVS,
            "parallel envs, Easy,",
            NUM_LEVELS,
            "levels)",
        ...
        var run = RunContext(
            ...
            driver=String("examples/procgen/chaser_rainbow_training_gpu.mojo"),
            slug=String("rainbow-chaser"),
            env=String("builtin:procgen/chaser"),
        )
        ...
        logger.set_config("env", "procgen-chaser-easy")
        ...
        try:
            var _ret = agent.train_cpu_batched[
                BatchedChaser, N_ENVS, N_STEP, RunLogger
            ](
                env,
                NUM_STEPS,
                rng_seed=UInt64(42),
