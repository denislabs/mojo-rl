"""Rainbow DQN CNN GPU training on Procgen Dodgeball — pixel obs, CPU-batched envs.

Scales the CPU proof to a real run: `N_ENVS` dodgeball envs step in parallel across
...
pixel trainers — used because dodgeball generation + rendering are CPU-side (no
GPU-env kernels). A full GPU-resident dodgeball env is a separate, larger effort.

Dodgeball reward is SPARSE (single +10 for the coin, hard death on any hazard) →
maze-like difficulty; start with `num_levels=1` and a longer horizon.
...
climb toward 10 as it reaches the coin), then step up 1 → 10 → 50 → 200. Each
level-count increase needs proportionally more steps (dodgeball is sparse).

Run:
    pixi run mojo run -I . examples/procgen/dodgeball_rainbow_training_gpu.mojo   # (apple) compile/smoke
    pixi run -e nvidia mojo run -I . examples/procgen/dodgeball_rainbow_training_gpu.mojo   # training
...
from noeira.envs.procgen.games import DodgeballGymEnv, DodgeballAssets
from noeira.envs.procgen.games.dodgeball import DIST_EASY
from noeira.core.fmt import fit

comptime ASSET_ROOT = String("assets/procgen/")

comptime DodgeballCNNEnv = DodgeballGymEnv[DT]
comptime OBS_DIM = DodgeballCNNEnv.OBS_DIM  # 3 × 84 × 84 = 21168
comptime NUM_ACTIONS = DodgeballCNNEnv.NUM_ACTIONS  # 15

comptime FRAMES = 3  # RGB channels (single frame — dodgeball is fully observable)
comptime NUM_ATOMS = 51
comptime HIDDEN = 512
comptime N_STEP = 3

# Dodgeball envs stepped in parallel across CPU cores; GPU trains the CNN.
comptime N_ENVS = 16
...
# Value support: -1 .. 11 (coins (+1 each) + 10 completion; sparse-ish).
...
comptime BatchedDodgeball = BatchedCpuDiscreteEnv[DodgeballCNNEnv, N_ENVS, OBS_DIM]


def _make_envs(assets: ArcPointer[DodgeballAssets]) -> List[DodgeballCNNEnv]:
    """N_ENVS independent dodgeball envs sharing one read-only asset bundle, each
    with a distinct rand_seed so they sample different levels from the train set.
    """
    var envs = List[DodgeballCNNEnv]()
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("Rainbow DQN CNN GPU Training on Procgen Dodgeball — Pixel (CPU-batched)")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        var assets = ArcPointer(DodgeballAssets(ASSET_ROOT))
        var env = BatchedDodgeball(_make_envs(assets))
        var eval_env = BatchedDodgeball(_make_envs(assets))

        print(
            "Environment: Procgen Dodgeball (CPU-batched,",
            N_ENVS,
            "parallel envs, Easy,",
            NUM_LEVELS,
            "levels)",
        ...
        var run = RunContext(
            ...
            driver=String("examples/procgen/dodgeball_rainbow_training_gpu.mojo"),
            slug=String("rainbow-dodgeball"),
            env=String("builtin:procgen/dodgeball"),
        )
        ...
        logger.set_config("env", "procgen-dodgeball-easy")
        ...
        try:
            var _ret = agent.train_cpu_batched[
                BatchedDodgeball, N_ENVS, N_STEP, RunLogger
            ](
                env,
                NUM_STEPS,
                rng_seed=UInt64(42),
