"""REDQ Agent GPU Training on Ant.
...
Ant terminates early when z-position leaves [0.2, 1.0] (handled by the env
when TERMINATE_ON_UNHEALTHY=True).

Run with:
    pixi run -e apple mojo run -I . examples/ant/redq_ant_training_gpu.mojo
    pixi run -e nvidia mojo run -I . examples/ant/redq_ant_training_gpu.mojo
"""
...
comptime REDQAntConfig = DefaultREDQConfig[
    OBS_DIM,
    ACTION_DIM,
    HIDDEN_DIM,
    BUFFER_CAPACITY,
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("REDQ Agent GPU Training on Ant")
    print("=" * 70)
    print()

    with DeviceContext() as ctx:
        var agent = REDQAgent[REDQAntConfig, max_n_envs=N_ENVS](
            gamma=0.99,
            tau=0.005,
            action_scale=1.0,
            auto_alpha=True,
