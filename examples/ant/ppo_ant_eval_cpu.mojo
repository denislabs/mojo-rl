"""CPU evaluation with 3D rendering for continuous PPO on Ant.
...
The Ant environment uses the Generalized Coordinates (GC) physics
engine which provides MuJoCo-style joint-space dynamics.

Run with:
    pixi run mojo run -I . tests/test_ppo_ant_continuous_eval_cpu.mojo
"""
...
comptime MAX_STEPS = 1000  # Ant episodes run for 1000 steps
...
fn main() raises:
    ...
    print()
    print("-" * 70)
    print("CPU EVALUATION SUMMARY - Ant")
    print("-" * 70)
    print("Episodes:", NUM_EPISODES)
