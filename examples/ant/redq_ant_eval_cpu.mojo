"""CPU evaluation for REDQ on Ant.

This evaluates a trained REDQ agent using CPU inference on the Ant
...
Run with:
    pixi run mojo run -I . examples/ant/redq_ant_eval_cpu.mojo
"""
...
def main() raises:
    ...
    var agent = REDQAgent[REDQAntConfig, max_n_envs=1](
        gamma=0.99,
        tau=0.005,
        action_scale=1.0,
        auto_alpha=True,
    ...
    try:
        ...
        print(
            ...
            " examples/ant/redq_ant_training_gpu.mojo"
        )
        return
    ...
    print("  - 14 bodies: torso + 4 legs (hip + ankle)")
    print("  - 8 actuated joints (hip + ankle x 4 legs)")
    print()
    print("-" * 70)
    ...
    print()
    print("-" * 70)
    print("CPU EVALUATION SUMMARY - Ant (REDQ)")
    print("-" * 70)
    print("Episodes:", NUM_EPISODES)
