"""CPU evaluation for MBPO on Ant.

This evaluates a trained MBPO agent using CPU inference on the Ant
...
Run with:
    pixi run mojo run -I . examples/ant/mbpo_ant_eval_cpu.mojo
"""
...
def main() raises:
    ...
    var agent = MBPOAgent[MBPOAntConfig](
        gamma=0.99,
        tau=0.005,
        action_scale=1.0,
        alpha=0.2,
    ...
    try:
        agent.load_checkpoint("mbpo_ant.ckpt")
        ...
        print(
            ...
            " examples/ant/mbpo_ant_training_gpu.mojo"
        )
        return
    ...
    print()
    print("-" * 70)
    print("CPU EVALUATION SUMMARY - Ant (MBPO)")
    print("-" * 70)
    print("Episodes:", NUM_EPISODES)
