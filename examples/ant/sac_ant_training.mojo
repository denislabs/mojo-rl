...
storage `deep_agents/` surface:
  ...
  * Single-file checkpointing — `run.checkpoint_path("last")`, overwritten each
    chunk and uploaded through the run's artifact sink.
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("SAC (deep_agents) — Ant CPU + checkpoints + logger")
    print("=" * 70)
    ...
    var run = RunContext(
        ...
        driver=String("examples/ant/sac_ant_training.mojo"),
        slug=String("sac-ant"),
        env=String("builtin:mujoco/ant"),
    )
