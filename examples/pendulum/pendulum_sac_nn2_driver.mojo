"""SAC training on Pendulum V1 via the nn2 CPU off-policy driver.

Phase B.1 — same algorithm + hyperparameters + RNG-consumption order as
`pendulum_sac_nn2_trainer.mojo`, but routed through
`mojo_rl.nn2.training.run_offpolicy_train_cpu`. The bit-identity gate is
that this script produces `mean_ret(10) = -167.572` at 30k steps with
seed=42.

Run:
    pixi run mojo run -I . examples/pendulum/pendulum_sac_nn2_driver.mojo
"""
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("nn2 SAC (Phase B.1 driver) — Pendulum V1 (CPU)")
    print("=" * 70)
