"""PPO training on Pendulum V1 via the nn on-policy driver.

Phase I.2.f. Uses `PPOTrainer` + `run_onpolicy_train` — the
trainer conforms to `OnPolicyAgent`, so the driver loop is the
same shape as `pendulum_sac_nn_driver.mojo` / `pendulum_mbpo_nn.mojo`
(swap trainer type, drop in driver).

Reference: `pendulum_ppo_nn.mojo` is the hand-rolled bespoke loop
...
Run:
    pixi run mojo run -I . examples/pendulum/pendulum_ppo_nn_driver.mojo
"""
...
from noeira.deep_agents.ppo.trainer import PPOTrainer
from noeira.deep_agents.training.driver_onpolicy import run_onpolicy_train
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("nn PPO Continuous (Phase I.2.f, driver) — Pendulum V1 (CPU)")
    print("=" * 70)
