"""SAC Agent CPU Training on Pendulum V1.

This trains the SAC (Soft Actor-Critic) agent on the original Pendulum (V1)
environment using single-env CPU training. The V1 implementation is CPU-only.

Hyperparameters are aligned with sac_pendulum_v2_training_cpu.mojo and
sac_pendulum_v2_training_gpu.mojo so the three runs can be compared
apples-to-apples (V1-vs-V2 physics on CPU; V2 CPU-vs-GPU).
...
Run with:
    pixi run mojo run -I . examples/pendulum/sac_pendulum_v1_training_cpu.mojo
"""
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("SAC Agent CPU Training on Pendulum V1")
    print("=" * 70)
    print()
    ...
    print("Environment: Pendulum V1 (CPU)")
    ...
    # Pendulum reward ranges: ~-1600 (random) to ~-150 (near-optimal swing-up).
