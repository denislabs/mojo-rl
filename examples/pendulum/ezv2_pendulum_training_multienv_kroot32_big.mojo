"""EZ-V2 Pendulum multi-env — experiment: K_ROOT=32 + bigger network.

Combines the two most promising levers from the prior sweep:
    1. K_ROOT=16 was the biggest single mover (multi-env -1140 → -1024)
    2. The V-net signal looked like the real bottleneck (L_V plateau ≈ 1.1)
       so a 2× network capacity bump should help differentiate states.

Doubles vs `ezv2_pendulum_training_multienv.mojo`:
    K_ROOT       8   → 32     (4× — push past the K_ROOT=16 result)
    K_NON_ROOT   4   → 16     (4× — keep the per-non-root expansion ratio)
    LATENT      64   → 128
    HIDDEN      64   → 128
    PROJ       128   → 256
    PRED_BOTTLENECK 64 → 128

NODES kept at 128 (only ~32 ever used per search since SIMS=32). BS still
128. Expect ~60-90 min wall-clock (vs ~27 min for K_ROOT=16) — bigger net
+ wider root sampling both contribute.
"""
...
def main() raises:
    print("=" * 72)
    print("    EZ-V2 Pendulum — multi-env, K_ROOT=32 + 2× network experiment")
    print("=" * 72)
    ...
    print()
    print("=" * 72)
    print("    Training complete (K_ROOT=32 + 2× network)")
    print("=" * 72)
