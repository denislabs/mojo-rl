"""SAC training on Pendulum V1 via `SACTrainer` — GPU.

GPU mirror of `pendulum_sac_nn2_trainer.mojo`. Validates Block A + D GPU
unblockers end-to-end against the CPU baseline (mean10 ≈ -170 at 30k
steps). Pendulum env stays on CPU; only the SAC update path runs on GPU
(actor/critic forward+backward, target-y, twin critic update, Polyak).

Run:
    pixi run mojo run -I . examples/pendulum/pendulum_sac_nn2_trainer_gpu.mojo
"""
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("nn2 SAC Continuous (Block D GPU) — Pendulum V1 (GPU)")
    print("=" * 70)
