"""EZ-V2 Pendulum multi-env — experiment: with reanalyze.

Adds the freshly-ported `GenericEZV2ContinuousAgent.reanalyze` method
(2026-05-10) to the K_ROOT=16 multi-env baseline. Reanalyze refreshes
stale stored MCTS targets with the *current* target net, which the
EZ-V2 paper claims is critical when the target net evolves quickly
relative to data collection.

Builds on `ezv2_pendulum_training_multienv_kroot16.mojo` (best non-
reanalyze run: -1024 mean10) — same hyperparameters except for
reanalyze cadence (every 200 train steps, 32 samples per call, after
1000-train-step warmup so the target net is past the random-init
regime).

If this clears the -1024 ceiling we know reanalyze was the missing
piece. If not, we know the bottleneck is elsewhere (network capacity
+ training budget).
"""
...
def main() raises:
    print("=" * 72)
    print("    EZ-V2 Pendulum — multi-env, K_ROOT=16 + reanalyze experiment")
    print("=" * 72)
    ...
    # Reanalyze knobs (matching paper App. G defaults — interval = target
    # sync interval, samples ≈ 1/4 of BS).
    ...
    for batch in range(num_batches):
        ...
        if (
            ...
            # Reanalyze: refresh stale stored targets via target net
            # search. Coupled with target sync to align target-net
            # evolution with refresh cadence.
            if (
    ...
    print()
    print("=" * 72)
    print("    Training complete (K_ROOT=16 + reanalyze)")
    print("=" * 72)
