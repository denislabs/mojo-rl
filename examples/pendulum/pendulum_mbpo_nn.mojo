"""MBPO training on Pendulum V1 via the nn Tier-3 off-policy driver.

Uses the unified `MBPOTrainer` (ref-based blocks). Same generic
driver as `pendulum_sac_nn_driver.mojo` — `run_offpolicy_train_batched`
...
Hyperparameters mirror the deep_agents reference where applicable:
    ...
    - SAC half: same lr / γ / τ / α defaults as the SAC bit-identity
      anchor (`pendulum_sac_nn_driver.mojo` ⇒ −167.572 @ 30k steps).
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("nn MBPO (Phase I.1.d) — Pendulum V1 (CPU)")
    print("=" * 70)
