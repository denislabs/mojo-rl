"""SAC Agent GPU Training on Pendulum V2.

This trains the SAC (Soft Actor-Critic) agent on Pendulum V2 using
GPU-accelerated off-policy training (multi-env). Hyperparameters match
sac_pendulum_v1_training_cpu.mojo and sac_pendulum_v2_training_cpu.mojo so the
three runs can be compared apples-to-apples.

Pendulum:
- 3D observation: [cos(theta), sin(theta), theta_dot]
- 1D continuous action: torque in [-2, 2]
- Reward: -(theta^2 + 0.1*theta_dot^2 + 0.001*torque^2)
- Episode length: 200 (no early termination)

Run with:
    pixi run -e apple mojo run -I . examples/pendulum/sac_pendulum_v2_training_gpu.mojo    # Apple Silicon
    pixi run -e nvidia mojo run -I . examples/pendulum/sac_pendulum_v2_training_gpu.mojo   # NVIDIA GPU
"""

from std.random import seed
from std.time import perf_counter_ns
from std.memory import UnsafePointer

from std.gpu.host import DeviceContext

from mojo_rl.core.dotenv import load_dotenv
from mojo_rl.core.logger import RemoteLogger
from mojo_rl.deep_agents.core.agents import DeepSACAgent
from mojo_rl.envs.pendulum import PendulumV2


# =============================================================================
# Constants (shared with sac_pendulum_v{1,2}_training_cpu.mojo)
# =============================================================================

comptime OBS_DIM = 3
comptime ACTION_DIM = 1

comptime HIDDEN_DIM = 64
comptime BUFFER_CAPACITY = 50_000
comptime BATCH_SIZE = 64

# Total env transitions: 500 episodes * 200 steps = 100_000, matching CPU.
comptime NUM_STEPS = 100_000
comptime WARMUP_STEPS = 2_000

comptime MAX_N_ENVS = 8

comptime dtype = DType.float32


def main() raises:
    seed(42)
    print("=" * 70)
    print("SAC Agent GPU Training on Pendulum V2")
    print("=" * 70)
    print()

    with DeviceContext() as ctx:
        var agent = DeepSACAgent[
            obs_dim=OBS_DIM,
            action_dim=ACTION_DIM,
            hidden_dim=HIDDEN_DIM,
            buffer_capacity=BUFFER_CAPACITY,
            batch_size=BATCH_SIZE,
            actor_lr=0.0003,
            critic_lr=0.0003,
            L=RemoteLogger,
            max_n_envs=MAX_N_ENVS,
        ](
            gamma=0.99,
            tau=0.005,
            action_scale=2.0,
            alpha=0.1,
            auto_alpha=True,
            alpha_lr=0.0003,
            target_entropy=-1.0,
            checkpoint_every=50_000,
            checkpoint_path="sac_pendulum_v2.ckpt",
        )

        print("Environment: Pendulum V2 (GPU batched)")
        print("Agent: SAC (Soft Actor-Critic)")
        print("  Observation dim: " + String(OBS_DIM))
        print("  Action dim: " + String(ACTION_DIM))
        print("  Hidden dim: " + String(HIDDEN_DIM))
        print("  Buffer capacity: " + String(BUFFER_CAPACITY))
        print("  Batch size: " + String(BATCH_SIZE))
        print("  Max parallel envs: " + String(MAX_N_ENVS))
        print("  Total env steps: " + String(NUM_STEPS))
        print("  Warmup steps: " + String(WARMUP_STEPS))
        print()

        var env_vars = load_dotenv()
        var api_key = env_vars.get("RL_MONITOR_API_KEY", "")
        var url = env_vars.get("RL_MONITOR_URL", "")

        var logger = RemoteLogger(
            server_url=url,
            run_name="SAC Pendulum V2 GPU",
            buffer_size=64,
            api_key=api_key,
        )
        logger.set_config("agent", "SAC")
        logger.set_config("env", "PendulumV2")
        logger.set_config("hidden_dim", String(HIDDEN_DIM))
        logger.set_config("actor_lr", "3e-4")
        logger.set_config("critic_lr", "3e-4")
        logger.set_config("alpha_lr", "3e-4")
        logger.set_config("batch_size", String(BATCH_SIZE))
        logger.set_config("buffer_capacity", String(BUFFER_CAPACITY))
        logger.set_config("max_n_envs", String(MAX_N_ENVS))

        print("Starting GPU training...")
        print("-" * 70)

        var start_time = perf_counter_ns()

        try:
            var metrics = agent.train_gpu[PendulumV2[dtype]](
                ctx,
                num_steps=NUM_STEPS,
                warmup_steps=WARMUP_STEPS,
                verbose=True,
                print_every=10_000,
                logger=UnsafePointer(to=logger),
                diag_every=1_000,
            )

            var end_time = perf_counter_ns()
            var elapsed_s = Float64(end_time - start_time) / 1e9

            logger.close()

            print("-" * 70)
            print()
            print("=" * 70)
            print("Pendulum V2 GPU Training Complete")
            print("=" * 70)
            print()
            print("Total env steps: " + String(NUM_STEPS))
            print("Training time: " + String(elapsed_s)[byte=:6] + " seconds")
            print()

            print(
                "Final average reward (last 100 episodes): "
                + String(metrics.mean_reward_last_n(100))[byte=:8]
            )
            print(
                "Best episode reward: " + String(metrics.max_reward())[byte=:8]
            )
            print()

            var final_avg = metrics.mean_reward_last_n(100)
            if final_avg > -200.0:
                print("EXCELLENT: Agent solved swing-up! (avg reward > -200)")
            elif final_avg > -500.0:
                print("SUCCESS: Agent largely swings up (avg reward > -500)")
            elif final_avg > -1000.0:
                print("GOOD PROGRESS: Agent is learning (avg reward > -1000)")
            else:
                print("EARLY STAGE: Agent still exploring (avg reward < -1000)")

            print()
            print("=" * 70)

        except e:
            print("!!! EXCEPTION CAUGHT !!!")
            print("Error:", e)
            print("!!! END EXCEPTION !!!")

    print(">>> main() completed normally <<<")
