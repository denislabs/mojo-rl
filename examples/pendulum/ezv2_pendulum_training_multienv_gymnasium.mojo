"""EZ-V2 Pendulum single-env — driven by the **Gymnasium Pendulum-v1 wrapper**.

Single-env (N_ENVS=1) because `GymPendulumEnv` isn't `Movable`-declared
so we can't use the list-of-pointers pattern from the native multi-env
script. Direct comparison is therefore vs the single-env post-fix Apple
run (best mean10 -1122).

Same other hyperparameters as `ezv2_pendulum_training.mojo`. Swap is
native `PendulumEnv` → `GymPendulumEnv`. Tests whether the small physics
divergence in our native V1/V2 (reward computed post-step instead of
pre-step; theta_dot clip applied after theta update) is affecting the
agent's convergence path.

  - native V1/V2: reward = -(new_θ² + 0.1·new_θ_dot² + 0.001·u²)
                  theta uses unclipped theta_dot for update
  - Gymnasium:    reward = -(θ² + 0.1·θ_dot² + 0.001·u²)  (pre-step)
                  theta uses CLIPPED theta_dot for update

If this run breaks the -1122 ceiling, our native physics is part of
the problem. If not, the ceiling is the agent/budget.

Requires gymnasium + numpy in the apple pixi env. Python-FFI per step —
expect ~2× wall time vs native V1.
"""
...
from mojo_rl.envs.gymnasium import GymPendulumEnv
...
def main() raises:
    print("=" * 72)
    print("    EZ-V2 Pendulum — multi-env, Gymnasium Pendulum-v1 wrapper")
    print("=" * 72)

    comptime NUM_ENV_STEPS = 30_000
    comptime N_ENVS = 1                            # single-env (FFI bottleneck)
    comptime MAX_STEPS_PER_EPISODE = 200
    comptime BATCH_TRAIN_INTERVAL = 4              # train every 4 env steps
    comptime START_TRANSITIONS = 2_000
    comptime SYNC_INTERVAL = 50
    comptime TARGET_SYNC_INTERVAL = 200
    comptime LOG_EVERY_BATCHES = NUM_ENV_STEPS // 50
    comptime EPISODE_REWARD_TARGET = Float64(-200.0)

    # Use same config as K_ROOT=16 baseline (best non-reanalyze Apple run).
    ...
    # Single GymPendulumEnv (Python FFI per step). Cast obs from
    # List[Float64] (env native) → List[Scalar[dtype]] (agent native).
    ...
    var obs64 = env.reset_obs_list()
    var obs = List[Scalar[dtype]](capacity=len(obs64))
    for i in range(len(obs64)):
        obs.append(Scalar[dtype](obs64[i]))
    obs_list.append(obs^)
    ...
    print("    env                   = Gymnasium Pendulum-v1 (Python FFI)")
    print(
    ...
    for batch in range(num_batches):
        ...
        var step_result = env.step_continuous_vec(action_vec)
        ...
        if done_or_trunc:
            ...
            var obs64_r = env.reset_obs_list()
            var new_obs = List[Scalar[dtype]](capacity=len(obs64_r))
            for i in range(len(obs64_r)):
                new_obs.append(Scalar[dtype](obs64_r[i]))
            obs_list[env_id] = new_obs^
        else:
    ...
    print()
    print("=" * 72)
    print("    Training complete (Gymnasium Pendulum-v1)")
    print("=" * 72)
