...
Run (NVIDIA):
  pixi run -e nvidia mojo run -I . examples/pendulum/pendulum_dreamerv3_nn_gpu.mojo
Run (Apple Metal — parity/smoke only, slow at this scale):
  pixi run -e apple   mojo run -I . examples/pendulum/pendulum_dreamerv3_nn_gpu.mojo
...
Both previously-missing GPU pieces are done (2026-05-29):
  1. `DreamerV3Agent.select_action` GPU inference path (B=1 device-forward
     hybrid) — `tests/nn/test_dreamerv3_agent_gpu_smoke.mojo`.
  2. `_ac_gpu` brought to `_ac_cpu` parity (NS=T·B starts, mean cotangents,
     repval) — `tests/nn/test_dreamerv3_ac_parity.mojo` matches the CPU
...
def main() raises:
    print("=" * 70)
    print("DreamerV3 (nn) Pendulum lighthouse [GPU/size1m] —", TOTAL_STEPS, "steps")
    print("=" * 70)
    seed(42)
