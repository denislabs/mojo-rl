"""Train GPU-accelerated Deep DQN on LunarLander.

Run with: pixi run mojo run examples/lunar_lander_gpu_dqn.mojo

This trains a GPU-accelerated Deep Q-Network on the native Mojo LunarLander environment.
GPU DQN uses:
- GPU-accelerated forward pass (tiled matmul with fused bias + ReLU)
- Neural network Q-function (2-layer MLP with ReLU)
- Target network with soft updates
- Experience replay (CPU-side for random sampling)
...
fn main() raises:
    print("=" * 60)
    print("GPU Deep DQN on LunarLander")
    print("=" * 60)
    print()
    ...
    with DeviceContext() as ctx:
        ...
        # Create GPU DQN agent with tuned hyperparameters
        ...
        var metrics = agent.train(
            ...
            verbose=True,
            print_every=25,
            environment_name="LunarLander (GPU)",
        )
