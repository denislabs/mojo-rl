"""LeWM (nn2) — evaluate a trained PushT world model (GPU).

Loads a checkpoint from `lewm2_pusht_train_gpu.mojo` and runs the
...
Run (after training):
  pixi run -e nvidia mojo run -I . examples/lewm/lewm2_pusht_eval_gpu.mojo
"""
...
from mojo_rl.experimental.lewm2.eval import lewm2_shuffled_eval
...
def main() raises:
    print("=" * 70)
    print("LeWM nn2 — PushT world-model eval (GPU, H6 action-awareness)")
    print("=" * 70)
