"""LeWM (nn2) — PushT JEPA training, PAPER-WIDTH with the CLS-TOKEN encoder.

Identical to `lewm2_pusht_train_gpu_paper.mojo` EXCEPT the encoder is the
...
Validation gates after this run (cheap → expensive):
  1. decoder probe on this WM — does the agent dot now reconstruct?
     (re-point lewm2_pusht_decode_gpu.mojo at this checkpoint + CLS encoder)
...
Run (NVIDIA; long, like the paper-width run):
  pixi run -e nvidia mojo run -I . examples/lewm/lewm2_pusht_train_gpu_paper_cls.mojo
"""
...
# ── paper-width recipe (matches lewm2_pusht_train_gpu_paper.mojo) ──────
comptime IN_CH = 3
comptime IMG = 224
...
def main() raises:
    print("=" * 70)
    print("LeWM nn2 — PushT JEPA training (GPU, PAPER-WIDTH, CLS-token)")
    print("=" * 70)
