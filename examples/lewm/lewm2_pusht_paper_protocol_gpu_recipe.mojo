"""LeWM (nn2) — PAPER-PROTOCOL planning eval for the RECIPE WM (GPU).

Identical protocol to `lewm2_pusht_paper_protocol_gpu_cls.mojo` (dataset
...
(no-stop-grad, AdamW 5e-5, z-scored actions):
  ...
  * loads /tmp/lewm2_pusht_recipe_world_model.txt,
...
Run (NVIDIA, after lewm2_pusht_train_gpu_recipe.mojo):
  pixi run -e nvidia mojo run -I . examples/lewm/lewm2_pusht_paper_protocol_gpu_recipe.mojo
"""
...
# ── must match lewm2_pusht_train_gpu_recipe.mojo ───────────────────────
comptime IN_CH = 3
comptime IMG = 224
comptime PATCH = 14
...
comptime ENC_FF_MULT = 4    # must match lewm2_pusht_train_gpu_recipe.mojo
comptime T = 6
comptime ACT = 10
...
comptime CKPT_PATH: String = "/tmp/lewm2_pusht_recipe_world_model.txt"
...
def main() raises:
    print("=" * 70)
    print("LeWM nn2 — PushT PAPER-PROTOCOL eval, RECIPE WM (GPU)")
    print("=" * 70)
    ...
    for round in range(ROUNDS):
        ...
        var r = run_lewm2_paper_protocol[
            ...
            init_std=INIT_STD,
            seed0=1 + round * B,
            viz_path=String("/tmp/lewm2_pusht_protocol_recipe_r")
