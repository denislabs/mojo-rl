"""LeWM (nn2) — PushT action denormalization CALIBRATION.
...
Dataset-only (no WM/GPU). Run on the NVIDIA box (has the dataset):
  pixi run -e nvidia mojo run -I . examples/lewm/lewm2_pusht_action_calib.mojo
"""
...
def main() raises:
    print("=" * 70)
    print("LeWM nn2 — PushT action denormalization calibration")
    print("=" * 70)
    rng_seed(2)
