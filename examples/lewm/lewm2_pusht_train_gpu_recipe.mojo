"""LeWM (nn2) — PushT JEPA training, REFERENCE RECIPE (CLS encoder, GPU).
...
Run (NVIDIA; long):
  pixi run -e nvidia mojo run -I . examples/lewm/lewm2_pusht_train_gpu_recipe.mojo
"""
...
# ── paper-width dims (matches lewm2_pusht_train_gpu_paper_cls.mojo) ────
comptime IN_CH = 3
comptime IMG = 224
comptime PATCH = 14
...
def main() raises:
    print("=" * 70)
    print("LeWM nn2 — PushT JEPA training, REFERENCE RECIPE (CLS, GPU)")
    print("=" * 70)
    ...
    _ = tr^
    print("=" * 70)
    print("DONE — eval: lewm2_pusht_paper_protocol_gpu_recipe.mojo")
    print("=" * 70)
