"""LeWM (nn2) — decoder probe on the CLS-TOKEN PushT world model (GPU).
...
Identical to `lewm2_pusht_decode_gpu.mojo` except the WM uses the CLS encoder
...
Run (NVIDIA, after lewm2_pusht_train_gpu_paper_cls.mojo):
  pixi run -e nvidia mojo run -I . examples/lewm/lewm2_pusht_decode_gpu_cls.mojo
"""
...
# ── CLS WM config (matches lewm2_pusht_train_gpu_paper_cls.mojo) ───────
comptime IN_CH = 3
comptime IMG = 224
comptime PATCH = 14
...
comptime WM_CKPT: String = "/tmp/lewm2_pusht_paper_cls_world_model.txt"
...
comptime N_VIZ = 8
comptime DEC_CKPT: String = "/tmp/lewm2_pusht_decoder_cls.txt"
...
def main() raises:
    print("=" * 70)
    print("LeWM nn2 — decoder probe on the CLS-token PushT WM (GPU)")
    print("=" * 70)
    ...
    for step in range(1, STEPS + 1):
        ...
        if step % VIZ_EVERY == 0 or step == STEPS:
            ...
            ctx.synchronize()
            var path = "/tmp/lewm2_pusht_cls_recon_" + String(step) + ".ppm"
    ...
    _ = src^; _ = wm^; _ = dec^
    print("=" * 70)
    print("DONE — compare /tmp/lewm2_pusht_cls_recon_*.ppm vs the mean-pool grid")
