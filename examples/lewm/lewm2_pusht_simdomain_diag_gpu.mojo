"""LeWM (nn2) — PushT sim-domain transfer diagnostic (GPU).
...
Loads `/tmp/lewm2_pusht_paper_world_model.txt` + `/tmp/lewm2_pusht_decoder.txt`.
Run (NVIDIA; 224² WM):
  pixi run -e nvidia mojo run -I . examples/lewm/lewm2_pusht_simdomain_diag_gpu.mojo
"""
...
# ── decoder config — must match lewm2_pusht_decode_gpu.mojo ─────────────
comptime PATCH_D = 16
...
def main() raises:
    print("=" * 70)
    print("LeWM nn2 — PushT sim-domain transfer diagnostic (GPU)")
    print("=" * 70)
    rng_seed(0)
    ...
    save_reconstruction_grid(
        String("/tmp/lewm2_pusht_simdomain_recon.ppm"),
        pix_host.unsafe_ptr(),
    ...
    _ = wm^; _ = dec^
    print("=" * 70)
    print("DONE — grid at /tmp/lewm2_pusht_simdomain_recon.ppm")
