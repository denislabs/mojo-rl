"""LeWM (nn2) — train the JEPA world model on real PushT pixels (GPU).

Reproduces the legacy `lewm_pusht_pixel_train_gpu.mojo` on nn2. PushT is the
...
sig_num_proj=1024). This is EXACTLY reproducible on nn2: its predictor
head_dim = EMB/HEADS = 96/4 = 24 matches the legacy pred_dim_head=24.
  ...
  from emb/heads. nn2's ARPredictor (locked decision #5) ties head_dim=EMB/HEADS
...
Run (NVIDIA; first run downloads the dataset):
  pixi run -e nvidia mojo run -I . examples/lewm/lewm2_pusht_train_gpu.mojo
"""
...
comptime CKPT_PATH: String = "/tmp/lewm2_pusht_world_model.txt"
...
def main() raises:
    print("=" * 70)
    print("LeWM nn2 — PushT JEPA world model training (GPU, real pixels)")
    print("=" * 70)
