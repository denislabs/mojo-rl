"""REDQ-OFE Agent GPU Training on Walker2D.

REDQ + OFENet (Ota et al., ICML 2020): DenseNet-style feature extractor
(total_units=240, num_layers=6 per paper's Walker2d.gin config) trained
...
Walker2D terminates early when |angle| > 1.0 (handled by the env when
TERMINATE_ON_UNHEALTHY=True).

Run with:
    pixi run -e apple mojo run -I . examples/walker2d/redq_ofe_walker2d_training_gpu.mojo
    pixi run -e nvidia mojo run -I . examples/walker2d/redq_ofe_walker2d_training_gpu.mojo
"""
...
comptime REDQOFEWalker2dConfig = DefaultREDQOFEConfig6[
    OBS_DIM,
    ACTION_DIM,
    HIDDEN_DIM,
    BUFFER_CAPACITY,
    ...
    240,     # OFE_TOTAL_UNITS (paper's Walker2d.gin)
    1.0,     # action_scale
]
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("REDQ-OFE Agent GPU Training on Walker2D")
    print("=" * 70)
    print()

    with DeviceContext() as ctx:
        var agent = REDQOFEAgent[REDQOFEWalker2dConfig, max_n_envs=N_ENVS](
            gamma=0.99,
            tau=0.005,
            action_scale=1.0,
            auto_alpha=True,
            alpha=0.2,
            alpha_lr=0.0003,
            target_entropy=-6,  # -ACTION_DIM (matches vanilla REDQ Walker2D)
            max_grad_norm=0.0,  # paper does not clip
            checkpoint_every=50_000,
            checkpoint_path="redq_ofe_walker2d.ckpt",
            diag_every=1_000,
            disable_aux=False,
        ...
        # agent.load_checkpoint("redq_ofe_walker2d.ckpt")
        ...
        print("  phi_s dim: " + String(REDQOFEWalker2dConfig.PHI_S_DIM))
        print("  phi_sa dim: " + String(REDQOFEWalker2dConfig.PHI_SA_DIM))
        ...
        var logger = RemoteLogger(
            server_url=url,
            run_name="REDQ-OFE Walker2D GPU",
            buffer_size=64,
            api_key=api_key,
        )
