"""SAC Walker2d (deep_agents2 / nn2 agent) — short run for nsys profiling.

nn2 counterpart of `sac_walker2d_profile_graph.mojo` (which profiles the legacy
...
the only difference is the agent stack (deep_agents2 `SACAgent` facade +
...
Run with:
    pixi run -e nvidia nsys profile --trace=cuda --cuda-graph-trace=node \
        ...
        examples/walker2d/sac_walker2d_profile_graph_nn2.mojo
...
def main() raises:
    seed(42)
    print("=== SAC Walker2d nsys profile (deep_agents2 / nn2) ===")
