...
HalfCheetah examples). One task-conditioned agent over Pendulum (obs3/act1) +
InvertedPendulum (obs4/act1), padded to MAX_OBS=4 / MAX_ACT=1, learned per-task
embedding. Acting is MPC-off.

Run (Apple): `pixi run -e apple mojo run -I . examples/multitask/tdmpc2_pendulum_invpendulum_gpu.mojo`
...
def main() raises:
    print("=" * 70)
    print("Multi-task TD-MPC2 (GPU) — Pendulum + InvertedPendulum")
    ...
    var ag = TDMPC2MultiTask[
        ...
        ENC, LATENT, MLP, BINS,
    ](ctx=ctx, lr=Scalar[DT](LR), learning_starts=LEARN_START)
