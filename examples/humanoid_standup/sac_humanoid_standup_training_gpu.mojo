"""SAC Agent GPU Training on HumanoidStandup.

This trains the SAC (Soft Actor-Critic) agent on the HumanoidStandup environment
...
HumanoidStandup starts lying on the ground and must learn to stand up.
The reward is based on the height of the torso.

Run with:
    pixi run -e apple mojo run -I . examples/humanoid_standup/sac_humanoid_standup_training_gpu.mojo    # Apple Silicon
    pixi run -e nvidia mojo run -I . examples/humanoid_standup/sac_humanoid_standup_training_gpu.mojo   # NVIDIA GPU
"""
...
# HumanoidStandup: 45D observation, 17D continuous action
...
# Training duration (needs many steps for standing behavior)
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("SAC Agent GPU Training on HumanoidStandup")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        var agent = DeepSACAgent[
            ...
            checkpoint_path="sac_humanoid_standup.ckpt",
        )

        print("Environment: HumanoidStandup Continuous (GPU)")
        ...
        var logger = RemoteLogger(
            server_url=url,
            run_name="SAC HumanoidStandup GPU",
            buffer_size=64,
            api_key=api_key,
        )
        logger.set_config("agent", "SAC")
        logger.set_config("env", "HumanoidStandup")
        ...
        try:
            var metrics = agent.train_gpu[
                HumanoidStandup[dtype, TERMINATE_ON_UNHEALTHY=False],
            ](
                ctx,
                num_steps=NUM_STEPS,
            ...
            if final_avg > 100000.0:
                print(
                    "EXCELLENT: Humanoid standing tall!"
                    " (avg reward > 100000)"
                )
            elif final_avg > 50000.0:
                print(
                    "SUCCESS: Humanoid learned to stand!"
                    " (avg reward > 50000)"
                )
            elif final_avg > 10000.0:
                print(
                    "GOOD PROGRESS: Humanoid is getting up"
                    " (avg reward > 10000)"
                )
            elif final_avg > 0.0:
                print(
