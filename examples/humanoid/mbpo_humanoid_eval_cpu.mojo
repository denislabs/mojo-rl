"""CPU evaluation for MBPO on Humanoid.

This evaluates a trained MBPO agent using CPU inference on the Humanoid
...
Run with:
    pixi run mojo run -I . examples/humanoid/mbpo_humanoid_eval_cpu.mojo
"""
...
def main() raises:
    ...
    var agent = MBPOAgent[MBPOHumanoidConfig](
        gamma=0.99,
        tau=0.005,
        action_scale=0.4,
        alpha=0.2,
    ...
    try:
        agent.load_checkpoint("mbpo_humanoid.ckpt")
        ...
        print(
            ...
            " examples/humanoid/mbpo_humanoid_training_gpu.mojo"
        )
        return
    ...
    print()
    print("-" * 70)
    print("CPU EVALUATION SUMMARY - Humanoid (MBPO)")
    print("-" * 70)
    print("Episodes:", NUM_EPISODES)
