"""CPU evaluation for SAC on Humanoid.

This evaluates a trained SAC agent using CPU inference on the Humanoid
...
Run with:
    pixi run mojo run -I . examples/humanoid/sac_humanoid_eval_cpu.mojo
"""
...
def main() raises:
    ...
    try:
        agent.load_checkpoint("sac_humanoid.ckpt")
    ...
    print()
    print("-" * 70)
    print("CPU EVALUATION SUMMARY - Humanoid (SAC)")
    print("-" * 70)
    print("Episodes:", NUM_EPISODES)
