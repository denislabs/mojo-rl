"""Evaluate + render a trained Humanoid SAC (deep_agents2) **LayerNorm-critic**
checkpoint.

Companion to `sac_humanoid_nn2_eval_cpu.mojo`, but matched to the LayerNorm
critic variant of the trainer (`sac_humanoid_nn2_agent_gpu.mojo`). That run
...
critic's `PARAM_SIZE` — so its checkpoint (`sac_humanoid_nn2_ln.ckpt`) is NOT
...
Run:
    pixi run mojo run -I . examples/humanoid/sac_humanoid_nn2_ln_eval_cpu.mojo
"""
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("SAC (deep_agents2) — Humanoid CPU eval + 3D rendering [LayerNorm]")
    print("=" * 70)
    ...
    print("-" * 70)
    print("EVAL SUMMARY — Humanoid (SAC deep_agents2, LayerNorm critic)")
    print("-" * 70)
    print("  Episodes        =", NUM_EPISODES)
