"""nn2 char-LSTM on TinyShakespeare (CPU).
...
Pipeline per training step:
  ...
  5. global-norm grad clip, then Adam on cell + proj.
...
def _global_clip(mut cell: Cell, mut proj: Proj, max_norm: Scalar[DT]):
    var sq: Float64 = 0.0
    for i in range(Cell.W_IH_SIZE):
        sq += Float64(cell.W_ih.grad[i]) ** 2
    for i in range(Cell.W_HH_SIZE):
        sq += Float64(cell.W_hh.grad[i]) ** 2
    for i in range(Cell.B_SIZE):
        sq += Float64(cell.b.grad[i]) ** 2
    for i in range(Proj.W_SIZE):
        sq += Float64(proj.weight.grad[i]) ** 2
    for i in range(Proj.B_SIZE):
        sq += Float64(proj.bias.grad[i]) ** 2
    var nrm = sqrt(sq)
    if nrm > Float64(max_norm):
        var scale = Scalar[DT](Float64(max_norm) / nrm)
        for i in range(Cell.W_IH_SIZE):
            cell.W_ih.grad[i] = cell.W_ih.grad[i] * scale
        for i in range(Cell.W_HH_SIZE):
            cell.W_hh.grad[i] = cell.W_hh.grad[i] * scale
        for i in range(Cell.B_SIZE):
            cell.b.grad[i] = cell.b.grad[i] * scale
        for i in range(Proj.W_SIZE):
            proj.weight.grad[i] = proj.weight.grad[i] * scale
        for i in range(Proj.B_SIZE):
            proj.bias.grad[i] = proj.bias.grad[i] * scale
...
def train_step(
    ...
    var h_buf: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]]((SEQ + 1) * BATCH * HIDDEN)
    var c_buf: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]]((SEQ + 1) * BATCH * HIDDEN)
    var cache_buf: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](SEQ * BATCH * Cell.CACHE_SIZE)
    var x_step: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * VOCAB)
    var H_all: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](NROWS * HIDDEN)
    var logits: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](NROWS * VOCAB)
    var targets: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](NROWS * VOCAB)
    var dlogits: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](NROWS * VOCAB)
    var dH_all: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](NROWS * HIDDEN)
    ...
    for t in range(SEQ):
        ...
        var hp = TileTensor(h_buf + t * BATCH * HIDDEN, row_major[BATCH, HIDDEN]())
        var cp = TileTensor(c_buf + t * BATCH * HIDDEN, row_major[BATCH, HIDDEN]())
    ...
    var dh_t: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * HIDDEN)
    var dc_t: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * HIDDEN)
    var dh_recur: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * HIDDEN)
    var dc_recur: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * HIDDEN)
    var dx_unused: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * VOCAB)
    var dh_prev: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * HIDDEN)
    var dc_prev: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * HIDDEN)
    ...
    for tt in range(SEQ):
        ...
        for b in range(BATCH):
            var r = _row(t, b)
            for j in range(HIDDEN):
                dh_t[b * HIDDEN + j] = dH_all[r * HIDDEN + j] + dh_recur[b * HIDDEN + j]
    ...
    h_buf.free(); c_buf.free(); cache_buf.free(); x_step.free()
    H_all.free(); logits.free(); targets.free(); dlogits.free(); dH_all.free()
    dh_t.free(); dc_t.free(); dh_recur.free(); dc_recur.free()
    dx_unused.free(); dh_prev.free(); dc_prev.free()
    return loss
...
def eval_loss(
    ...
    for _ in range(n_batches):
        ...
        var h_buf: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * HIDDEN)
        var c_buf: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * HIDDEN)
        var h_new: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * HIDDEN)
        var c_new: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * HIDDEN)
        var x_step: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](BATCH * VOCAB)
        var H_all: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](NROWS * HIDDEN)
        var logits: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](NROWS * VOCAB)
        var targets: UnsafePointer[Scalar[DT], MutAnyOrigin] = alloc[Scalar[DT]](NROWS * VOCAB)
        ...
        h_buf.free(); c_buf.free(); h_new.free(); c_new.free(); x_step.free()
        H_all.free(); logits.free(); targets.free()
...
def main() raises:
    ...
    print("=" * 70)
    print("  vocab=" + String(VOCAB) + " hidden=" + String(HIDDEN)
    ...
    print("  tokens=" + String(len(ids)) + " train=" + String(len(split.train))
    ...
    var adam_cell = Adam.make["cpu", M=Cell](cell); adam_cell.lr = LR
    var adam_proj = Adam.make["cpu", M=Proj](proj); adam_proj.lr = LR

    var val0 = eval_loss(cell, proj, ce, split.val, 4)
    print("\n[step 0] val_loss=" + String(val0)
    ...
    for step in range(N_STEPS):
        ...
        _global_clip(cell, proj, CLIP_NORM)
        adam_cell.step["cpu"](cell)
        adam_proj.step["cpu"](proj)
        run += loss
        cnt += 1
        if (step + 1) % EVAL_EVERY == 0:
            var v = eval_loss(cell, proj, ce, split.val, 4)
            print("[step " + String(step + 1) + "] train_loss="
                  + String(run / Scalar[DT](cnt)) + " val_loss=" + String(v))
            run = 0.0
            cnt = 0
