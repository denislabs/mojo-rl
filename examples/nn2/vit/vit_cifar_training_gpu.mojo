"""ViT CIFAR-10 training — nn2 GPU (real-dataset parity run).

nn2 port of `examples/nn/vit/vit_cifar_training_gpu.mojo`. Trains the nn2
`ViT` composite (PatchEmbed → pos BiasAdd → non-causal TransformerBlock×N →
LayerNorm → TokenMean → head) on CIFAR-10 via the stateful nn2
...
Run:
    pixi run -e apple  mojo run -I . examples/nn2/vit/vit_cifar_training_gpu.mojo
    pixi run -e nvidia mojo run -I . examples/nn2/vit/vit_cifar_training_gpu.mojo
"""
...
from mojo_rl.nn2.training.augmenter import CIFAR10CropFlipAugmenter
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("ViT CIFAR-10 training — nn2 GPU")
    print("=" * 70)
    print(
        "  patch=" + String(PATCH) + " n_patches=" + String(N_PATCHES)
        + " embed=" + String(EMBED) + " heads=" + String(HEADS)
        + " layers=" + String(LAYERS) + " ff_mult=" + String(FF_MULT)
    )
    print(
        "  batch=" + String(BATCH) + " epochs=" + String(EPOCHS)
        + " base_lr=" + String(BASE_LR) + " wd=" + String(WD)
        + " | random baseline ≈ " + String(1.0 / Float64(N_CLASSES))
    )
    ...
    var tr_img_h = ctx.enqueue_create_host_buffer[DT](N_TRAIN * CIFAR10.IMG_SIZE)
    var tr_tgt_h = ctx.enqueue_create_host_buffer[DT](N_TRAIN * N_CLASSES)
    for i in range(N_TRAIN * CIFAR10.IMG_SIZE):
        tr_img_h.unsafe_ptr()[i] = ds.train_images[i]
    for i in range(N_TRAIN * N_CLASSES):
        tr_tgt_h.unsafe_ptr()[i] = 0.0
    for i in range(N_TRAIN):
        tr_tgt_h.unsafe_ptr()[i * N_CLASSES + Int(ds.train_labels[i])] = 1.0
    ...
    var tr_img_p: UnsafePointer[Scalar[DT], MutAnyOrigin] = tr_img_d.unsafe_ptr()
    var tr_tgt_p: UnsafePointer[Scalar[DT], MutAnyOrigin] = tr_tgt_d.unsafe_ptr()
    var train_x = TileTensor(tr_img_p, row_major[N_TRAIN, CIFAR10.IMG_SIZE]())
    var train_y = TileTensor(tr_tgt_p, row_major[N_TRAIN, N_CLASSES]())
    ...
    for i in range(N_TEST * CIFAR10.IMG_SIZE):
        te_img_h.unsafe_ptr()[i] = ds.test_images[i]
    var te_img_d = ctx.enqueue_create_buffer[DT](N_TEST * CIFAR10.IMG_SIZE)
    ctx.enqueue_copy(te_img_d, te_img_h)
    var te_img_p: UnsafePointer[Scalar[DT], MutAnyOrigin] = te_img_d.unsafe_ptr()
    var test_x = TileTensor(te_img_p, row_major[N_TEST, CIFAR10.IMG_SIZE]())
    var test_lbl: UnsafePointer[Int32, MutAnyOrigin] = alloc[Int32](N_TEST)
    for i in range(N_TEST):
        test_lbl[i] = Int32(ds.test_labels[i])
    ctx.synchronize()

    print("\n── Training ──")
    var t0 = perf_counter_ns()
    var result = trainer.train_gpu[
        N_TRAIN, N_TEST, CIFAR10CropFlipAugmenter, WarmupCosineSchedule[
            WARMUP_EPOCHS, 0.1
        ],
    ](
        train_x, train_y, test_x, test_lbl,
        epochs=EPOCHS,
        print_progress=True,
        shuffle=True,
        rng_seed=UInt64(42),
