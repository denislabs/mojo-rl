...
def main() raises:
    seed(42)
    print("=" * 70)
    print("REDQ-OFE Agent GPU Training on HalfCheetah")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        var agent = REDQOFEAgent[REDQOFEHalfCheetahConfig, max_n_envs=N_ENVS](
            ...
            disable_aux=False,
            # OFENet pretraining (paper-faithful): run this many aux_train_step
            # calls on the random-policy buffer right after env-collection
            # warmup ends, before the first RL update. Mirrors OFENet's
            # `for i in range(random_collect): extractor.train(...)`. The
            # paper uses random_collect = 10000.
        ...
        print("  phi_s dim: " + String(REDQOFEHalfCheetahConfig.PHI_S_DIM))
        print("  phi_sa dim: " + String(REDQOFEHalfCheetahConfig.PHI_SA_DIM))
        ...
        var logger = RemoteLogger(
            server_url=url,
            run_name="REDQ-OFE HalfCheetah GPU",
            buffer_size=64,
            api_key=api_key,
        )
