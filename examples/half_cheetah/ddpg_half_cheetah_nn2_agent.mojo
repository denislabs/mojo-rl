...
Sibling of `sac_half_cheetah_nn2_agent.mojo` for the deterministic
DDPG actor + single-critic flavour. Uses the new `deep_agents2/`
surface:
  ...
  * Single-file checkpointing — `agent.save(CHECKPOINT_PATH)` writes
    ...
    `nn2-ckpt v2` envelope containing actor + critic + their Adam
    states.
...
Run:
    pixi run mojo run -I . examples/half_cheetah/ddpg_half_cheetah_nn2_agent.mojo
"""
...
comptime CHECKPOINT_PATH = "ddpg_half_cheetah_nn2.ckpt"
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("DDPG (deep_agents2) — HalfCheetah CPU + checkpoints + logger")
    print("=" * 70)
    ...
    var logger = RemoteLogger(
        server_url=url,
        run_name="DDPG HalfCheetah NN2 (CPU)",
        buffer_size=200,
        api_key=api_key,
    )
