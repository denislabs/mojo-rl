"""Autodiff SAC Agent GPU Training on HalfCheetah.

Same as sac_half_cheetah_training_gpu.mojo but using the autodiff-composed
actor loss (AutodiffSACConfig) instead of manual backward code (SACConfig).

The actor loss graph is expressed as a composed Model type:
    obs → SkipConcat[Actor → RSample] → [obs, action, log_prob]
        → SplitApply[DualPath[Critic1, Critic2] → Min, Identity]
        → [min_Q, log_prob]

Forward and backward are fully automatic — no manual gradient stitching.

Run with:
    pixi run -e nvidia mojo run -I . examples/half_cheetah/autodiff_sac_half_cheetah_training_gpu.mojo
"""
...
def main() raises:
    ...
    with DeviceContext() as ctx:
        # Use AutodiffSACConfig instead of SACConfig
        comptime Config = AutodiffSACConfig[
            OBS_DIM,
            ACTION_DIM,
            HIDDEN_DIM,
            BUFFER_CAPACITY,
        ...
        var agent = GenericOffPolicyAgent[
            ...
            checkpoint_path="autodiff_sac_half_cheetah.ckpt",
        )

        print("Environment: HalfCheetah Continuous (GPU)")
        print("Agent: Autodiff SAC (composed autodiff graph)")
        print("  Actor loss: AutodiffMaxEntLoss")
        print("    Graph: obs → SkipConcat[Actor→RSample]")
        print("         → SplitApply[DualPath[C1,C2]→Min, Identity]")
        print("         → [min_Q, log_prob]")
        ...
        var logger = RemoteLogger(
            server_url=url,
            run_name="Autodiff SAC HalfCheetah GPU",
            buffer_size=64,
            api_key=api_key,
        )
        logger.set_config("agent", "Autodiff SAC")
        logger.set_config("env", "HalfCheetah")
        logger.set_config("actor_loss", "AutodiffMaxEntLoss")
        ...
        try:
            ...
            print("=" * 70)
            print("Autodiff SAC GPU Training Complete")
            print("=" * 70)
            print()
