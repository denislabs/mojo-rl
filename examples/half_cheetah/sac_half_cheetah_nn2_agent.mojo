...
`deep_agents2/` surface:
  ...
  * Single-file checkpointing — `agent.save(CHECKPOINT_PATH)` writes
    ...
    `nn2-ckpt v2` envelope containing actor + twin critics + their
...
Run:
    pixi run mojo run -I . examples/half_cheetah/sac_half_cheetah_nn2_agent.mojo
"""
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("SAC (deep_agents2) — HalfCheetah CPU + checkpoints + logger")
    print("=" * 70)
