...
Sibling of `sac_half_cheetah_nn2_agent.mojo`. MBPO is model-based:
...
Uses the new `deep_agents2/` surface:
  ...
  * `MBPOAgent[...]` — facade over `MBPOTrainer` + the single-env
    ...
    Settings here mirror the `pendulum_mbpo_nn2_agent.mojo` tuning
...
Run:
    pixi run mojo run -I . examples/half_cheetah/mbpo_half_cheetah_nn2_agent.mojo
"""
...
comptime CHECKPOINT_PATH = "mbpo_half_cheetah_nn2.ckpt"
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("MBPO (deep_agents2) — HalfCheetah CPU + checkpoints + logger")
    print("=" * 70)
    ...
    var logger = RemoteLogger(
        server_url=url,
        run_name="MBPO HalfCheetah NN2 (CPU)",
        buffer_size=200,
        api_key=api_key,
    )
