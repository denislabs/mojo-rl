"""MBPO CPU-vs-GPU target-parity diagnostic (HalfCheetah, deep_agents2).

Purpose: localize the GPU-MBPO HalfCheetah under-performance (≈20x slower
than legacy at 30k env-steps). Runs the SAME nn2 MBPO code twice —
...
into:
  ...
  * CPU and GPU TRACK each other  → the GPU kernels are fine; the
    legacy-regime slowness is an nn2-MBPO-vs-legacy ALGORITHMIC gap
    (dynamics target handling, elite ranking, rollout reward scale, …).
    Next step: diff nn2-MBPO math against deep_agents MBPO.
...
Run:
    pixi run -e apple  mojo run -I . examples/half_cheetah/mbpo_hc_nn2_parity.mojo
    pixi run -e nvidia mojo run -I . examples/half_cheetah/mbpo_hc_nn2_parity.mojo
...
# =============================================================================
# Architecture — identical to mbpo_half_cheetah_nn2_gpu.mojo
# =============================================================================
...
def _print_header():
    print("=" * 72)
    print("MBPO CPU-vs-GPU target parity — HalfCheetah (deep_agents2)")
    print("=" * 72)
...
def main() raises:
    ...
    with DeviceContext() as ctx:
        ...
        print("TRACK   → GPU kernels OK; gap is nn2-MBPO-vs-legacy (algorithm).")
