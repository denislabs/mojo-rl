...
Sibling of `sac_half_cheetah_nn_agent.mojo` for the deterministic
...
surface:
  ...
  * Single-file checkpointing — `agent.save(CHECKPOINT_PATH)` writes
    ...
    `nn-ckpt v2` envelope containing actor + critic + their Adam
    states.
...
Run:
    pixi run mojo run -I . examples/half_cheetah/ddpg_half_cheetah_nn_agent.mojo
"""
...
comptime CHECKPOINT_PATH = "ddpg_half_cheetah_nn.ckpt"
