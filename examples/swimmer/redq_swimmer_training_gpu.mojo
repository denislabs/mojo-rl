"""REDQ Agent GPU Training on Swimmer.
...
Swimmer has no termination condition, so the env runs for the full
1000-step horizon each episode.

Run with:
    pixi run -e apple mojo run -I . examples/swimmer/redq_swimmer_training_gpu.mojo
    pixi run -e nvidia mojo run -I . examples/swimmer/redq_swimmer_training_gpu.mojo
"""
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("REDQ Agent GPU Training on Swimmer")
    print("=" * 70)
    print()

    with DeviceContext() as ctx:
        var agent = REDQAgent[REDQSwimmerConfig, max_n_envs=N_ENVS](
            gamma=0.999,  # Matches SAC Swimmer's long-horizon reward
            tau=0.005,
            action_scale=1.0,
            auto_alpha=True,
            alpha=0.2,
            ...
            checkpoint_path="redq_swimmer.ckpt",
            diag_every=1_000,
        )
