...
fn cpu_forward_batch[OBS_DIM: Int, HIDDEN_DIM: Int, OUT_DIM: Int, BATCH: Int](
    obs: List[Float32],
    W1: List[Float32],
    W2: List[Float32],
    mut h: List[Float32],
...
fn benchmark_cpu[OBS_DIM: Int, HIDDEN_DIM: Int, OUT_DIM: Int, BATCH: Int](
    num_iters: Int
) -> Float64:
    ...
    # Warm up
    for _ in range(100):
        cpu_forward_batch[OBS_DIM, HIDDEN_DIM, OUT_DIM, BATCH](obs, W1, W2, h, output)
...
fn benchmark_gpu[OBS_DIM: Int, HIDDEN_DIM: Int, OUT_DIM: Int, BATCH: Int](
    ...
    comptime kernel = gpu_forward_kernel[dtype, BATCH, OBS_DIM, HIDDEN_DIM, OUT_DIM, TPB]

    for _ in range(100):
        ctx.enqueue_function_checked[kernel, kernel](
            out_t, obs_t, W1_t, W2_t,
            grid_dim=(num_blocks,),
            block_dim=(TPB,),
        )
    ctx.synchronize()
...
def main():
    ...
    print("=" * 70)
    print("Fixed: obs_dim=" + String(OBS_DIM) + ", out_dim=" + String(OUT_DIM) + ", batch=64")
    print()
    ...
    with DeviceContext() as ctx:
        ...
        print("  64   | " + String(cpu_h64/1000)[:8] + " | " + String(gpu_h64/1000)[:8] + " | " + String(gpu_h64/cpu_h64)[:5])
        ...
        print(" 128   | " + String(cpu_h128/1000)[:8] + " | " + String(gpu_h128/1000)[:8] + " | " + String(gpu_h128/cpu_h128)[:5])
        ...
        print(" 256   | " + String(cpu_h256/1000)[:8] + " | " + String(gpu_h256/1000)[:8] + " | " + String(gpu_h256/cpu_h256)[:5])
        ...
        print(" 512   | " + String(cpu_h512/1000)[:8] + " | " + String(gpu_h512/1000)[:8] + " | " + String(gpu_h512/cpu_h512)[:5])

        print()
        print("=" * 70)
        print("ANALYSIS 2: Impact of Batch Size")
        print("=" * 70)
        print("Fixed: obs_dim=" + String(OBS_DIM) + ", hidden=128, out_dim=" + String(OUT_DIM))
        print()
        ...
        print("   32  | " + String(cpu_b32/1000)[:8] + " | " + String(gpu_b32/1000)[:8] + " | " + String(gpu_b32/cpu_b32)[:5] + " | " + String(Int(32 * 1e9 / gpu_b32)) + " obs/s")

        var cpu_b64 = benchmark_cpu[OBS_DIM, 128, OUT_DIM, 64](num_iters)
        var gpu_b64 = benchmark_gpu[OBS_DIM, 128, OUT_DIM, 64](ctx, num_iters)
        print("   64  | " + String(cpu_b64/1000)[:8] + " | " + String(gpu_b64/1000)[:8] + " | " + String(gpu_b64/cpu_b64)[:5] + " | " + String(Int(64 * 1e9 / gpu_b64)) + " obs/s")
        ...
        print("  128  | " + String(cpu_b128/1000)[:8] + " | " + String(gpu_b128/1000)[:8] + " | " + String(gpu_b128/cpu_b128)[:5] + " | " + String(Int(128 * 1e9 / gpu_b128)) + " obs/s")
        ...
        print("  256  | " + String(cpu_b256/1000)[:8] + " | " + String(gpu_b256/1000)[:8] + " | " + String(gpu_b256/cpu_b256)[:5] + " | " + String(Int(256 * 1e9 / gpu_b256)) + " obs/s")
        ...
        print("  512  | " + String(cpu_b512/1000)[:8] + " | " + String(gpu_b512/1000)[:8] + " | " + String(gpu_b512/cpu_b512)[:5] + " | " + String(Int(512 * 1e9 / gpu_b512)) + " obs/s")

        var cpu_b1024 = benchmark_cpu[OBS_DIM, 128, OUT_DIM, 1024](num_iters)
        var gpu_b1024 = benchmark_gpu[OBS_DIM, 128, OUT_DIM, 1024](ctx, num_iters)
        print(" 1024  | " + String(cpu_b1024/1000)[:8] + " | " + String(gpu_b1024/1000)[:8] + " | " + String(gpu_b1024/cpu_b1024)[:5] + " | " + String(Int(1024 * 1e9 / gpu_b1024)) + " obs/s")
        ...
        print("   - This is 10-100x the actual computation time for small networks")
        print()
