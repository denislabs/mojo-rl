"""EZ-V2 CartPole — Step 3: multi-env rollout (N_ENVS=4) on GPU.

Builds on Step 2. The GPU env step + GPU MCTS now run with N_ENVS=4 in
parallel — same kernels, ~4× more useful work per launch. Replay
buffer and `store_transition` stay on CPU; the agent already supports
`env_id` parameter (Phase A multi-env work).

Plan:    docs/EZV2_FULL_GPU_PLAN.md
Gate:    rerun. Compare wall-time vs Step 2 (should be much closer to
         Step 1 wall on Apple — kernel-launch overhead amortized
         across 4 envs). Loss curves should track Step 2 in shape.
         Sample efficiency vs env-steps should match Phase A's
         hybrid 4-env result.

Run:
    pixi run mojo run -I . examples/cartpole/cartpole_ezv2_full_gpu_step3.mojo
"""
...
def main() raises:
    print("=== EZ-V2 CartPole demo — Step 3 (multi-env GPU, N_ENVS=4) ===")

    # Match Step 0/1/2 baseline config exactly so log lines are directly
    # comparable. NUM_ENV_STEPS counts *total* env-step transitions
    # (sum across envs) — same total budget as the single-env runs.
    ...
    print()
    print("--- Run config ---")
    print("    NUM_ENV_STEPS         =", NUM_ENV_STEPS, "(total across envs)")
    print("    N_ENVS                =", N_ENVS)
    ...
    # ── GPU MCTS state + workspace (sized for N_ENVS) ────────────────────
    ...
    # Initial reset on GPU. PhiloxRandom inside reset uses thread-local
    # seeding so each of the N_ENVS=4 envs gets a different initial state.
    ...
    # Per-env CPU obs view, fed into store_transition.
    ...
    var total_env_steps = 0  # sum across envs

    var t0 = perf_counter_ns()

    while total_env_steps < NUM_ENV_STEPS:
        # ── GPU MCTS — batched across N_ENVS ─────────────────────────────
        ...
        # Per-env action sampling on host. Build per-env policy arrays
        # for store_transition.
        var actions_per_env = List[Int]()
        var policies_per_env = List[InlineArray[Float64, ACT]]()
        var root_value_per_env = List[Float64]()

        for e in range(N_ENVS):
            # Root visits/total_value: index e * NODES * ACT + node=0 + a.
            ...
            # Temperature sampling — mirrors CPU `select_action` logic.
            var action: Int
        ...
        # ── GPU env step (batched across N_ENVS) ─────────────────────────
        ...
        # Per-env: store transition + accumulate ep_return + flag dones.
        var any_done = False
        for e in range(N_ENVS):
        ...
        # Selective reset on GPU (resets only envs where dones[e] > 0.5,
        # also clears those done flags). Then refresh obs_buf for reset
        # envs from new state.
        if any_done:
            step_seed += 1
        ...
        # Refresh CPU obs view for the next iter's store_transition.
        ctx.enqueue_copy(host_obs.unsafe_ptr(), obs_buf)
        ctx.synchronize()
        for e in range(N_ENVS):
            obs_per_env[e].clear()
        ...
        # ── GPU train (unchanged) ────────────────────────────────────────
        # Train cadence: train_step every TRAIN_INTERVAL env-batches (= N_ENVS
        # env-step transitions). Matches Step 0/1/2's "every TRAIN_INTERVAL
        # env steps" in spirit.
        if (
        ...
        # Log threshold-based on total_env_steps.
