"""CPU evaluation for MBPO on Hopper.

This evaluates a trained MBPO agent using CPU inference on the Hopper
environment. Load a checkpoint from GPU training and run deterministic
evaluation episodes (using mean action, no sampling).

Run with:
    pixi run mojo run -I . examples/hopper/mbpo_hopper_eval_cpu.mojo
"""
...
def main() raises:
    ...
    var agent = MBPOAgent[MBPOHopperConfig](
        gamma=0.99,
        tau=0.005,
        action_scale=1.0,
        alpha=0.2,
    ...
    try:
        agent.load_checkpoint("mbpo_hopper.ckpt")
        ...
        print(
            ...
            " examples/hopper/mbpo_hopper_training_gpu.mojo"
        )
        return
    ...
    print()
    print("-" * 70)
    print("CPU EVALUATION SUMMARY - Hopper (MBPO)")
    print("-" * 70)
    print("Episodes:", NUM_EPISODES)
