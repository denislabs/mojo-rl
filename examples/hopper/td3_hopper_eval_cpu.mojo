"""CPU evaluation for TD3 on Hopper.

This evaluates a trained TD3 agent using CPU inference on the Hopper
environment with 3D rendering. Load a checkpoint from GPU training and
run deterministic evaluation episodes (using mean action, no noise).

Run with:
    pixi run mojo run -I . examples/hopper/td3_hopper_eval_cpu.mojo
"""
...
def main() raises:
    ...
    try:
        agent.load_checkpoint("td3_hopper.ckpt")
        ...
        print(
            ...
            " examples/hopper/td3_hopper_training_gpu.mojo"
        )
        return
    ...
    print()
    print("-" * 70)
    print("CPU EVALUATION SUMMARY - Hopper (TD3)")
    print("-" * 70)
    print("Episodes:", NUM_EPISODES)
