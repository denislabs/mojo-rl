"""SAC Agent GPU Training on InvertedDoublePendulum.

This trains the SAC (Soft Actor-Critic) agent on the InvertedDoublePendulum
environment using GPU-accelerated off-policy training with:
- Parallel environments on GPU
- Generalized Coordinates (GC) physics engine (MuJoCo-style)
- 1D continuous action space (cart force)
- 9D observation (custom: cart_x, sin/cos of joint angles, clipped velocities)

InvertedDoublePendulum is harder than single pendulum due to the coupled
dynamics of two poles.
...
# InvertedDoublePendulum: 9D observation, 1D continuous action
comptime OBS_DIM = 9  # [cart_x, sin(q1), sin(q2), cos(q1), cos(q2), qvel[0:3] clipped, 0]
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("SAC Agent GPU Training on InvertedDoublePendulum")
    print("=" * 70)
    print()
    ...
    with DeviceContext() as ctx:
        var agent = DeepSACAgent[
            ...
            checkpoint_path="sac_inverted_double_pendulum.ckpt",
        )

        print("Environment: InvertedDoublePendulum Continuous (GPU)")
        ...
        var logger = RemoteLogger(
            server_url=url,
            run_name="SAC InvertedDoublePendulum GPU",
            buffer_size=64,
            api_key=api_key,
        )
        ...
        try:
            var metrics = agent.train_gpu[
                InvertedDoublePendulum[dtype, TERMINATE_ON_UNHEALTHY=True],
            ](
                ctx,
                num_steps=NUM_STEPS,
