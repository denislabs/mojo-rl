"""CPU evaluation for SAC on InvertedDoublePendulum.

This evaluates a trained SAC agent using CPU inference on the InvertedDoublePendulum
...
Run with:
    pixi run mojo run -I . examples/inverted_double_pendulum/sac_inverted_double_pendulum_eval_cpu.mojo
"""
...
comptime OBS_DIM = 9  # custom sin/cos encoding
...
def main() raises:
    ...
    print("InvertedDoublePendulum (Generalized Coordinates Physics)")
    print("=" * 70)
    print()
    ...
    try:
        ...
        print(
            "  pixi run -e apple mojo run examples/inverted_double_pendulum/"
            "sac_inverted_double_pendulum_training_gpu.mojo"
        )
        return
    ...
    print()
    print("-" * 70)
    print("CPU EVALUATION SUMMARY - InvertedDoublePendulum (SAC)")
    print("-" * 70)
    print("Episodes:", NUM_EPISODES)
    ...
    if avg_reward > 9000:
        print("Result: EXCELLENT - Perfect double balance!")
