"""IDP Diagnostic: CPU f32 vs GPU f32 — SAME actions fed to both.

Both CPU and GPU get the exact same action each step (computed from CPU obs).
This isolates physics divergence from observation/action-selection divergence.

Run with:
    pixi run -e apple mojo run -I . examples/inverted_double_pendulum/idp_f32_cpu_vs_gpu_same_actions.mojo
    pixi run -e nvidia mojo run -I . examples/inverted_double_pendulum/idp_f32_cpu_vs_gpu_same_actions.mojo
"""
...
def main() raises:
    seed(42)
    print("=" * 110)
    print("IDP Diagnostic: CPU f32 vs GPU f32 — SAME actions")
    print("CPU action drives both — isolates physics divergence")
    print("=" * 110)
    ...
    print(
        "Step | action   | cpu_rew  | gpu_rew  | qpos_err         "
        "| qvel_err         | cpu_z_tip | gpu_z_tip | cpu_done | gpu_done"
    )
    print("-" * 130)

    for step in range(MAX_ENV_STEPS):
        # === Get obs from CPU state and compute action ===
        var cpu_obs = get_cpu_obs(cpu_data)
        var action = get_greedy_action(agent, cpu_obs)

        # === CPU step: apply action ===
        for i in range(NV):
        ...
        # === GPU step: apply SAME action ===
        ctx.enqueue_copy(gpu_state_host.unsafe_ptr(), gpu_state_buf)
        ctx.synchronize()
        # Zero qfrc then set the motor force
        for i in range(NV):
            gpu_state_host[qfrc_offset[NQ, NV]() + i] = Scalar[DTYPE](0)
        gpu_state_host[
            qfrc_offset[NQ, NV]() + InvertedDoublePendulumModel._acd.motor_dof_adr[0]
        ] = Scalar[DTYPE](InvertedDoublePendulumModel._acd.motor_gears[0] * ctrl)
        ctx.enqueue_copy(gpu_state_buf, gpu_state_host.unsafe_ptr())
        ctx.synchronize()
        ...
        for _ in range(FRAME_SKIP):
            RK4Integrator[SOLVER=NewtonSolver].step_gpu[
                ...
                NGEOM=NGEOM,
                CONE_TYPE=InvertedDoublePendulumModel.CONE_TYPE,
                STEP_THREADS=NV,
        ...
        var z_tip_cpu = POLE_LEN * cos(q1) + POLE_LEN * cos(q1 + q2)
        var cpu_terminated = z_tip_cpu <= 1.0
        var x_tip_cpu = q0 + POLE_LEN * sin(q1) + POLE_LEN * sin(q1 + q2)
        var dist_p = 0.01 * x_tip_cpu * x_tip_cpu + (z_tip_cpu - 2.0) * (z_tip_cpu - 2.0)
        ...
        # === GPU reward/done (manual computation from GPU state) ===
        var gq0 = Float64(gpu_state_host[qpos_offset[NQ, NV]() + 0])
        var gq1 = Float64(gpu_state_host[qpos_offset[NQ, NV]() + 1])
        var gq2 = Float64(gpu_state_host[qpos_offset[NQ, NV]() + 2])
        var z_tip_gpu = POLE_LEN * cos(gq1) + POLE_LEN * cos(gq1 + gq2)
        var gpu_terminated = z_tip_gpu <= 1.0
        var x_tip_gpu = gq0 + POLE_LEN * sin(gq1) + POLE_LEN * sin(gq1 + gq2)
        var dist_p_gpu = 0.01 * x_tip_gpu * x_tip_gpu + (z_tip_gpu - 2.0) * (z_tip_gpu - 2.0)
        var gv1 = Float64(gpu_state_host[qvel_offset[NQ, NV]() + 1])
        var gv2 = Float64(gpu_state_host[qvel_offset[NQ, NV]() + 2])
        var vel_p_gpu = 1e-3 * gv1 ** 2 + 5e-3 * gv2 ** 2
        var gpu_alive = 0.0 if gpu_terminated else 10.0
        var gpu_reward = gpu_alive - dist_p_gpu - vel_p_gpu
        ...
        # Print every step or on divergence
        var should_print = step % 5 == 0 or qpos_err > 1e-4 or qvel_err > 1e-3
        if should_print:
            print(
                String(step)[byte=:4]
                + " | "
                ...
                + " | "
                + String(qvel_err)[byte=:16]
                + " | "
                + String(z_tip_cpu)[byte=:9]
                + " | "
                + String(z_tip_gpu)[byte=:9]
                + " | "
                + String(cpu_terminated)
                + "    | "
                + String(gpu_terminated)
            )

        if qpos_err > 0.01 or qvel_err > 0.1:
            print("  >>> LARGE DIVERGENCE at step " + String(step) + " <<<")
            print("    CPU qpos:", end="")
            for i in range(NQ):
                print(" " + String(Float64(cpu_data.qpos[i]))[byte=:14], end="")
            print()
            print("    GPU qpos:", end="")
            for i in range(NQ):
                print(
            ...
            print("    CPU qvel:", end="")
            for i in range(NV):
                print(" " + String(Float64(cpu_data.qvel[i]))[byte=:14], end="")
            print()
            print("    GPU qvel:", end="")
            for i in range(NV):
                print(
                    " " + String(Float64(gpu_state_host[qvel_offset[NQ, NV]() + i]))[byte=:14],
                    end="",
                )
            print()

        if cpu_terminated or gpu_terminated:
            if cpu_terminated:
    ...
    print()
    if abs(cpu_total - gpu_total) < 1.0:
        print("MATCH: Physics are identical — divergence is elsewhere (obs/action/eval loop)")
    else:
        print("DIVERGED: Physics produce different results — investigate RK4/solver")
