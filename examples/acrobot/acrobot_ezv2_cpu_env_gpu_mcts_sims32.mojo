"""EfficientZero V2 Acrobot — bisect with SIMS=32.

Same setup as `acrobot_ezv2_cpu_env_gpu_mcts.mojo` (CPU env + GPU MCTS
+ GPU train) but with SIMS doubled from 16 to 32. Tests whether GPU
MCTS's noisier Gumbel-Top-k targets (Float32 + Philox vs CPU's Float64
+ Mojo `random`) can be averaged out by more simulations per env-step.

Reference convergence at 100k env-steps (NVIDIA):
  Phase A — CPU env + CPU MCTS + GPU train, SIMS=16:  mean=-252 best=-112
  Bisect  — CPU env + GPU MCTS + GPU train, SIMS=16:  mean=-322 best=-98
  Phase B — GPU env + GPU MCTS + GPU train, SIMS=16:  mean=-363 best=-127

If SIMS=32 here recovers Phase A's mean (~-252), the gap is variance-
driven and Float32 Gumbel noise is the proximate cause. If it stays
near the Bisect SIMS=16 result (~-322), the gap is structural —
something other than sim count differs between CPU and GPU MCTS.

Wall-time roughly doubles vs SIMS=16 (each tree gets 32 simulations
through dyn+pred forwards instead of 16).

Identical to the SIMS=16 bisect (`acrobot_ezv2_cpu_env_gpu_mcts.mojo`)
except SIMS=32. Phase A's per-env `agent.select_action` is replaced by
a single batched GPU MCTS call across all 4 envs (CPU obs upload → GPU
search → host download → per-env temperature multinomial sample).

This run also picks up the strengthened Philox seed mixing in
`gz_init_root_kernel` (golden-ratio multiplication for inter-call
decorrelation), applied alongside this experiment.

Run:
    pixi run -e nvidia mojo run -I . examples/acrobot/acrobot_ezv2_cpu_env_gpu_mcts_sims32.mojo
    pixi run -e apple  mojo run -I . examples/acrobot/acrobot_ezv2_cpu_env_gpu_mcts_sims32.mojo
"""
