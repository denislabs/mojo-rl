...
def main() raises:
    ...
    # Random-action warmup phase (paper `start_transitions: 2000`). For
    # the first START_TRANSITIONS env steps we bypass MCTS entirely and
    # pick uniformly random actions, with a uniform `1/ACT` policy
    # stored as the search target. Two reasons:
    #
    #   • Untrained MCTS produces noise-shaped action distributions
    #     anyway, but ~10× slower than random selection — wasted compute.
    #   • Random actions seed the buffer with broader trajectory shapes,
    #     which on sparse-reward envs like Acrobot is the difference
    #     between "stumbles onto the goal once or twice" and "sees the
    #     goal in its random rollouts often enough to learn from it".
    ...
    for env_step in range(NUM_ENV_STEPS):
        ...
        if (
            ...
            and env_step + 1 >= START_TRANSITIONS
