"""Char-LSTM on TinyShakespeare — nn GPU, via `AutoregressiveTrainer`.

The LSTM analog of the GPT example (Karpathy's char-rnn). The recurrence
is wrapped behind the standard `Module` interface by `LSTMSeq` (nn's
...
launches), so keep SEQ modest. Run:
    pixi run -e nvidia mojo run -I . examples/nn/lstm/lstm_tinyshakespeare_training.mojo
"""
...
def main() raises:
    seed(42)
    print("=" * 70)
    print("nn TinyShakespeare char-LSTM — GPU (AutoregressiveTrainer)")
    print("=" * 70)
    print(
