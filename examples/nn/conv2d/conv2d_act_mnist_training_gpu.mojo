...
Same setup as conv2d_mnist_training_gpu.mojo but uses Conv2DReLU (fused
Conv+ReLU via FusedConv2DActivation) instead of Conv2DLayer + separate ReLU.
Since both paths have identical PARAM_SIZE and Kaiming init is deterministic
for a given seed, the loss trajectory should be bitwise-identical to the
unfused variant — that's our regression check against FusedConv2DActivation's
TileTensor port.
...
Run:
    pixi run -e apple  mojo run -I . examples/nn/conv2d/conv2d_act_mnist_training_gpu.mojo
    pixi run -e nvidia mojo run -I . examples/nn/conv2d/conv2d_act_mnist_training_gpu.mojo
"""
...
def main() raises:
    ...
    var result = TRAINER.train_gpu_minibatch_full[
        ...
        show_progress=True,
        eval_every_epochs=1,
        progress_label="MNIST-Conv2DReLU",
    )
    ctx.synchronize()
