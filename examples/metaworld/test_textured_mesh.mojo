...
def main() raises:
    ...
    var debug_mesh = load_stl("mojo_rl/envs/metaworld/assets/meshes/table/tablebody.stl")
    print("Mesh vertices:", len(debug_mesh.vertices))
    for i in range(min(6, len(debug_mesh.vertices))):
        var v = debug_mesh.vertices[i]
        print("  v[", i, "] pos=(", v.px, v.py, v.pz,
              ") uv=(", v.u, v.v, ")")
    ...
    while not renderer.check_quit() and step < 500:
        ...
        renderer.draw_mesh(
            ...
            color=Color(255, 255, 255, 255),  # White base — texture provides color
            texture_name="wood2",
        ...
        renderer.draw_box(
            ...
            color=Color(150, 100, 50, 255),  # Fallback color (no texture on box)
        )
