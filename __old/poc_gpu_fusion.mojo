...
fn forward_layer1_relu_kernel[
    ...
    output_pre: LayoutTensor[dtype, Layout.row_major(BATCH, OUT_DIM), MutAnyOrigin],
...
fn backward_mse_dW_db_kernel[
    ...
    d_output: LayoutTensor[dtype, Layout.row_major(BATCH, OUT_DIM), MutAnyOrigin],
    x: LayoutTensor[dtype, Layout.row_major(BATCH, IN_DIM), ImmutAnyOrigin],
    pred: LayoutTensor[dtype, Layout.row_major(BATCH, OUT_DIM), ImmutAnyOrigin],
    target: LayoutTensor[dtype, Layout.row_major(BATCH, OUT_DIM), ImmutAnyOrigin],
):
    ...
    if compute_db:
        ...
        while stride > 0:
            if local_row < stride:
                dy_shared[local_row, local_col] += dy_shared[local_row + stride, local_col]
            barrier()
            stride //= 2
...
fn backward_dx_relu_dW_db_kernel[
    ...
    x: LayoutTensor[dtype, Layout.row_major(BATCH, IN_DIM), ImmutAnyOrigin],
    h_pre: LayoutTensor[dtype, Layout.row_major(BATCH, HIDDEN_DIM), ImmutAnyOrigin],
    ...
    for tile_idx in range(num_tiles):
        ...
        if dh_batch < BATCH and hidden_idx < HIDDEN_DIM:
            ...
            dh_val = rebind[dh_shared.element_type](relu_backward_scalar(dh_scalar, h_pre_val))
    ...
    if compute_db:
        ...
        while stride > 0:
            if local_row < stride:
                dh_shared[local_row, local_col] += dh_shared[local_row + stride, local_col]
            barrier()
            stride //= 2
...
fn adam_fused_kernel[
    ...
    lr: Scalar[dtype], beta1: Scalar[dtype], beta2: Scalar[dtype],
    eps: Scalar[dtype], bc1: Scalar[dtype], bc2: Scalar[dtype],
):
    ...
    if idx < W1_SIZE:
        ...
        W1[idx] = rebind[Scalar[dtype]](W1[idx]) - lr * (m_new / bc1) / (sqrt(v_new / bc2) + eps)
        m_W1[idx] = m_new
        v_W1[idx] = v_new
        ...
        var m_new = beta1 * m + (1 - beta1) * g
        var v_new = beta2 * v + (1 - beta2) * g * g
        b1[i] = rebind[Scalar[dtype]](b1[i]) - lr * (m_new / bc1) / (sqrt(v_new / bc2) + eps)
        m_b1[i] = m_new
        v_b1[i] = v_new
        ...
        var m_new = beta1 * m + (1 - beta1) * g
        var v_new = beta2 * v + (1 - beta2) * g * g
        W2[i] = rebind[Scalar[dtype]](W2[i]) - lr * (m_new / bc1) / (sqrt(v_new / bc2) + eps)
        m_W2[i] = m_new
        v_W2[i] = v_new
    else:
        ...
        var m_new = beta1 * m + (1 - beta1) * g
        var v_new = beta2 * v + (1 - beta2) * g * g
        b2[i] = rebind[Scalar[dtype]](b2[i]) - lr * (m_new / bc1) / (sqrt(v_new / bc2) + eps)
        m_b2[i] = m_new
        v_b2[i] = v_new
...
fn generate_data_kernel[BATCH: Int](
    ...
    state ^= state >> 17
    state ^= state << 5
    var x1 = Scalar[dtype](state) / Scalar[dtype](Scalar[DType.uint32].MAX) * 2 - 1
    state ^= state << 13
    state ^= state >> 17
    state ^= state << 5
    var x2 = Scalar[dtype](state) / Scalar[dtype](Scalar[DType.uint32].MAX) * 2 - 1
    x[idx, 0] = x1
    x[idx, 1] = x2
    y[idx, 0] = x1 * x2
    rng[idx] = state
...
fn xavier_init_kernel[SIZE: Int, FAN_IN: Int, FAN_OUT: Int](
    ...
    if idx >= SIZE:
        return
    var state = rebind[Scalar[DType.uint32]](rng_seed[0]) + UInt32(idx * 1099087573)
    state ^= state << 13
    state ^= state >> 17
    state ^= state << 5
    var u = Scalar[dtype](state) / Scalar[dtype](Scalar[DType.uint32].MAX) * 2 - 1
    var scale = sqrt(Scalar[dtype](6.0) / Scalar[dtype](FAN_IN + FAN_OUT))
    weights[idx] = u * scale


fn mse_loss_kernel[BATCH: Int, OUT_DIM: Int](
...
def main():
    ...
    with DeviceContext() as ctx:
        ...
        var rng_t = LayoutTensor[DType.uint32, Layout.row_major(1), MutAnyOrigin](rng_seed_buf)
        var W1_init = LayoutTensor[dtype, Layout.row_major(W1_SIZE), MutAnyOrigin](W1_buf)
        var W2_init = LayoutTensor[dtype, Layout.row_major(W2_SIZE), MutAnyOrigin](W2_buf)

        ctx.enqueue_function_checked[
            xavier_init_kernel[W1_SIZE, INPUT_DIM, HIDDEN_DIM],
            xavier_init_kernel[W1_SIZE, INPUT_DIM, HIDDEN_DIM],
        ](W1_init, rng_t, grid_dim=((W1_SIZE + TPB - 1) // TPB,), block_dim=(TPB,))
        ctx.enqueue_function_checked[
            xavier_init_kernel[W2_SIZE, HIDDEN_DIM, OUTPUT_DIM],
            xavier_init_kernel[W2_SIZE, HIDDEN_DIM, OUTPUT_DIM],
        ](W2_init, rng_t, grid_dim=((W2_SIZE + TPB - 1) // TPB,), block_dim=(TPB,))
        ctx.synchronize()
        print("Initialized weights")

        # Tensors
        var W1_t = LayoutTensor[dtype, Layout.row_major(INPUT_DIM, HIDDEN_DIM), MutAnyOrigin](W1_buf)
        var b1_t = LayoutTensor[dtype, Layout.row_major(HIDDEN_DIM), MutAnyOrigin](b1_buf)
        var W2_t = LayoutTensor[dtype, Layout.row_major(HIDDEN_DIM, OUTPUT_DIM), MutAnyOrigin](W2_buf)
        var b2_t = LayoutTensor[dtype, Layout.row_major(OUTPUT_DIM), MutAnyOrigin](b2_buf)
        var dW1_t = LayoutTensor[dtype, Layout.row_major(INPUT_DIM, HIDDEN_DIM), MutAnyOrigin](dW1_buf)
        var db1_t = LayoutTensor[dtype, Layout.row_major(HIDDEN_DIM), MutAnyOrigin](db1_buf)
        var dW2_t = LayoutTensor[dtype, Layout.row_major(HIDDEN_DIM, OUTPUT_DIM), MutAnyOrigin](dW2_buf)
        var db2_t = LayoutTensor[dtype, Layout.row_major(OUTPUT_DIM), MutAnyOrigin](db2_buf)
        var h1_pre_t = LayoutTensor[dtype, Layout.row_major(BATCH_SIZE, HIDDEN_DIM), MutAnyOrigin](h1_pre_buf)
        var h1_t = LayoutTensor[dtype, Layout.row_major(BATCH_SIZE, HIDDEN_DIM), MutAnyOrigin](h1_buf)
        var y_pred_t = LayoutTensor[dtype, Layout.row_major(BATCH_SIZE, OUTPUT_DIM), MutAnyOrigin](y_pred_buf)
        var dy_t = LayoutTensor[dtype, Layout.row_major(BATCH_SIZE, OUTPUT_DIM), MutAnyOrigin](dy_buf)
        var x_t = LayoutTensor[dtype, Layout.row_major(BATCH_SIZE, INPUT_DIM), MutAnyOrigin](x_buf)
        var y_t = LayoutTensor[dtype, Layout.row_major(BATCH_SIZE, OUTPUT_DIM), MutAnyOrigin](y_buf)
        var loss_t = LayoutTensor[dtype, Layout.row_major(1), MutAnyOrigin](loss_buf)
        var rng_t2 = LayoutTensor[DType.uint32, Layout.row_major(BATCH_SIZE), MutAnyOrigin](rng_buf)

        # Flat tensors for Adam
        var W1_flat = LayoutTensor[dtype, Layout.row_major(W1_SIZE), MutAnyOrigin](W1_buf)
        var dW1_flat = LayoutTensor[dtype, Layout.row_major(W1_SIZE), ImmutAnyOrigin](dW1_buf)
        var m_W1_t = LayoutTensor[dtype, Layout.row_major(W1_SIZE), MutAnyOrigin](m_W1_buf)
        var v_W1_t = LayoutTensor[dtype, Layout.row_major(W1_SIZE), MutAnyOrigin](v_W1_buf)
        var b1_flat = LayoutTensor[dtype, Layout.row_major(B1_SIZE), MutAnyOrigin](b1_buf)
        var db1_flat = LayoutTensor[dtype, Layout.row_major(B1_SIZE), ImmutAnyOrigin](db1_buf)
        var m_b1_t = LayoutTensor[dtype, Layout.row_major(B1_SIZE), MutAnyOrigin](m_b1_buf)
        var v_b1_t = LayoutTensor[dtype, Layout.row_major(B1_SIZE), MutAnyOrigin](v_b1_buf)
        var W2_flat = LayoutTensor[dtype, Layout.row_major(W2_SIZE), MutAnyOrigin](W2_buf)
        var dW2_flat = LayoutTensor[dtype, Layout.row_major(W2_SIZE), ImmutAnyOrigin](dW2_buf)
        var m_W2_t = LayoutTensor[dtype, Layout.row_major(W2_SIZE), MutAnyOrigin](m_W2_buf)
        var v_W2_t = LayoutTensor[dtype, Layout.row_major(W2_SIZE), MutAnyOrigin](v_W2_buf)
        var b2_flat = LayoutTensor[dtype, Layout.row_major(B2_SIZE), MutAnyOrigin](b2_buf)
        var db2_flat = LayoutTensor[dtype, Layout.row_major(B2_SIZE), ImmutAnyOrigin](db2_buf)
        var m_b2_t = LayoutTensor[dtype, Layout.row_major(B2_SIZE), MutAnyOrigin](m_b2_buf)
        var v_b2_t = LayoutTensor[dtype, Layout.row_major(B2_SIZE), MutAnyOrigin](v_b2_buf)

        # Grids
        comptime grid_h1 = ((HIDDEN_DIM + TILE - 1) // TILE, (BATCH_SIZE + TILE - 1) // TILE)
        comptime grid_out = ((OUTPUT_DIM + TILE - 1) // TILE, (BATCH_SIZE + TILE - 1) // TILE)
        comptime grid_dW2 = ((OUTPUT_DIM + TILE - 1) // TILE, (HIDDEN_DIM + TILE - 1) // TILE)
        comptime grid_dW1 = ((HIDDEN_DIM + TILE - 1) // TILE, (INPUT_DIM + TILE - 1) // TILE)
        ...
        var bwd1_fn = ctx.compile_function_checked[
            backward_dx_relu_dW_db_kernel[BATCH_SIZE, INPUT_DIM, HIDDEN_DIM, OUTPUT_DIM],
            backward_dx_relu_dW_db_kernel[BATCH_SIZE, INPUT_DIM, HIDDEN_DIM, OUTPUT_DIM],
        ]()
        ...
        ctx.enqueue_function_checked(data_fn, x_t, y_t, rng_t2,
            grid_dim=((BATCH_SIZE + TPB - 1) // TPB,), block_dim=(TPB,))
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            # Forward (2 kernels)
            ctx.enqueue_function_checked(fwd1_fn, h1_pre_t, h1_t, x_t, W1_t, b1_t,
                grid_dim=grid_h1, block_dim=block_2d)
            ctx.enqueue_function_checked(fwd2_fn, y_pred_t, h1_t, W2_t, b2_t,
                grid_dim=grid_out, block_dim=block_2d)

            # Backward (2 kernels)
            ctx.enqueue_function_checked(bwd2_fn, dW2_t, db2_t, dy_t, h1_t, y_pred_t, y_t,
                grid_dim=grid_dW2, block_dim=block_2d)
            ctx.enqueue_function_checked(bwd1_fn, dW1_t, db1_t, x_t, h1_pre_t, dy_t, W2_t,
                grid_dim=grid_dW1, block_dim=block_2d)
            ...
            var bc2 = 1 - beta2 ** t
            comptime TOTAL = W1_SIZE + B1_SIZE + W2_SIZE + B2_SIZE
            ctx.enqueue_function_checked(adam_fn,
                W1_flat, dW1_flat, m_W1_t, v_W1_t,
                b1_flat, db1_flat, m_b1_t, v_b1_t,
                W2_flat, dW2_flat, m_W2_t, v_W2_t,
                b2_flat, db2_flat, m_b2_t, v_b2_t,
                lr, beta1, beta2, eps, bc1, bc2,
                grid_dim=((TOTAL + TPB - 1) // TPB,), block_dim=(TPB,))

            if log:
                ctx.enqueue_function_checked(loss_fn, loss_t, y_pred_t, y_t,
                    grid_dim=(1,), block_dim=(TPB,))
                ctx.synchronize()
                with loss_buf.map_to_host() as host:
                    print("Epoch " + String(epoch + 1) + "/" + String(NUM_EPOCHS) +
                          " - Loss: " + String(Float32(host[0])))
        ...
        print("Per epoch: " + String(elapsed_ms / Float64(NUM_EPOCHS))[:6] + " ms")
        ...
        print("  4. backward_dx_relu_dW_db: dx + ReLU_backward + dW1 + db1 (FUSED)")
        print("  5. adam_fused: All 4 param groups (FUSED)")
        print()
        print("Key: relu_scalar() and relu_backward_scalar() are @always_inline")
        print("     -> Compiled directly into kernel code, no function call overhead")
        print()
        print("This demonstrates the functional layer design IS GPU-compatible!")
        print("=" * 70)
