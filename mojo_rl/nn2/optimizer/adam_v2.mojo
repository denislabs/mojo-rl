"""AdamV2 — retrofit (Phase D). Same algorithm + storage as v1 Adam;
...
struct AdamV2(OptimizerV2):
    ...
    def make[target: StaticString, M: ModuleV2](mut model: M) raises -> Self:
        ...
        comptime assert target == "cpu", (
            "AdamV2.make[target='gpu', M] requires a DeviceContext"
        )
        var opt = Self()
    ...
    def make[target: StaticString, M: ModuleV2](
        ...
        comptime assert target == "gpu", (
            "AdamV2.make[target='cpu', M](model, ctx) — drop ctx for CPU"
        )
        var opt = Self()
    ...
    def zero_grad[
        ...
        assert_tag_for["AdamV2", target](self.ts.target_tag)
