...
# ──────────────────────────────────────────────────────────────────────
# GPU kernels — copied from v1 verbatim. Kept here so LayerNormV2 is
...
struct LayerNormV2[DIM: Int](ModuleV2):
    ...
    def make[target: StaticString, INIT: Initializer]() raises -> Self:
        comptime assert target == "cpu", (
            "LayerNormV2.make[target='gpu', INIT] requires a DeviceContext"
        )
        var ln = Self()
    ...
    def make[
        ...
        comptime assert target == "gpu", (
            "LayerNormV2.make[target='cpu', INIT](ctx) — drop ctx for CPU"
        )
        var ln = Self()
    ...
    def forward[
        ...
        assert_tag_for["LayerNormV2", target](self.ts.target_tag)
