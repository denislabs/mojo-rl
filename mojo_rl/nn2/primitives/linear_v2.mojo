...
struct LinearV2[IN: Int, OUT: Int](ModuleV2):
    ...
    def make[target: StaticString, INIT: Initializer]() raises -> Self:
        comptime assert target == "cpu", (
            "LinearV2.make[target='gpu', INIT] requires a DeviceContext"
        )
        var lin = Self()
    ...
    def make[
        ...
        comptime assert target == "gpu", (
            "LinearV2.make[target='cpu', INIT](ctx) — drop ctx for CPU"
        )
        var lin = Self()
    ...
    def forward[
        ...
        assert_tag_for["LinearV2", target](self.ts.target_tag)
    ...
    def mark_weight_dirty(mut self):
        """Hook for optimizers: call after `weight` (or any other Param)
        is updated, so the next bf16 forward/backward re-casts."""
        self.amp.w_dirty = True
