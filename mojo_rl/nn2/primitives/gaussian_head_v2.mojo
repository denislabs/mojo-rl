...
**BACKWARD-ORDER INVARIANT**: same as LinearV2. Because `_cached_input_ptr`
...
struct GaussianHeadV2[IN: Int, ACT: Int](ModuleV2):
    ...
    def make[target: StaticString, INIT: Initializer]() raises -> Self:
        comptime assert target == "cpu", (
            "GaussianHeadV2.make[target='gpu', INIT] requires a DeviceContext"
        )
        var h = Self()
    ...
    def make[
        ...
        comptime assert target == "gpu", (
            "GaussianHeadV2.make[target='cpu', INIT](ctx) — drop ctx for CPU"
        )
        var h = Self()
    ...
    def set_log_std_init(mut self, value: Scalar[DT]) raises:
        ...
        if self.ts.target_tag == Int8(1):  # TARGET_CPU
    ...
    def forward[
        ...
        assert_tag_for["GaussianHeadV2", target](self.ts.target_tag)
