...
that every other leaf will follow. Diffs vs `relu.mojo`:
  ...
  5. **Slim `ModuleV2` conformance.** No Phase 10A `out_ptr/grad_in_ptr/
...
# ──────────────────────────────────────────────────────────────────────
# ReLUV2 — retrofitted ReLU. Two fields: `ts` and `_cached_input_ptr`.
# ──────────────────────────────────────────────────────────────────────


struct ReLUV2[DIM: Int](ModuleV2):
    ...
    def make[target: StaticString, INIT: Initializer]() raises -> Self:
        ...
        comptime assert (
            target == "cpu"
        ), "ReLUV2.make[target='gpu', INIT] requires a DeviceContext"
        var r = Self()
    ...
    def make[
        ...
        comptime assert (
            target == "gpu"
        ), "ReLUV2.make[target='cpu', INIT](ctx) — drop ctx for CPU"
        var r = Self()
    ...
    def forward[
        ...
        assert_tag_for["ReLUV2", target](self.ts.target_tag)
