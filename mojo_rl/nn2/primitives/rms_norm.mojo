...
struct RMSNorm[DIM: Int](Module):
    ...
    def make[
        ...
        comptime if target == "cpu":
            ...
            if not ctx:
                raise Error("RMSNorm.make[target='gpu']: ctx required")
            var ctx_v = ctx.value()
    ...
    def forward[
        ...
        comptime if target == "cpu":
            ...
            for b in range(BATCH):
                ...
                inv_v[b] = inv_rms
                for d in range(Self.DIM):
                    var n = input[b, d] * inv_rms
                    norm_v[b, d] = n
                    output_v[b, d] = n * gamma_v[d]
        else:
    ...
    def vjp[
        ...
        comptime if target == "cpu":
            ...
            for b in range(BATCH):
                ...
                var R: Scalar[DT] = 0.0
                for d in range(Self.DIM):
                    R += grad_output_v[b, d] * gamma_v[d] * norm_v[b, d]
                for e in range(Self.DIM):
                    var go = grad_output_v[b, e]
                    grad_input_v[b, e] = inv_rms * (
                        go * gamma_v[e] - norm_v[b, e] * R * inv_dim
                    )
                comptime if mode == "all":
                    for d in range(Self.DIM):
                        grad_gamma_v[d] += grad_output_v[b, d] * norm_v[b, d]
        else:
            var ctx = self.ts.ctx.value()
            comptime layout_2d = Layout.row_major(BATCH, Self.DIM)
            comptime layout_b = Layout.row_major(BATCH)
