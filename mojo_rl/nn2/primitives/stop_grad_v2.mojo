...
Conforms to `ModuleV2`. `mode` is accepted but has no behavioral
...
struct StopGradV2[DIM: Int](ModuleV2):
    ...
    def make[target: StaticString, INIT: Initializer]() raises -> Self:
        comptime assert (
            target == "cpu"
        ), "StopGradV2.make[target='gpu', INIT] requires a DeviceContext"
        var s = Self()
    ...
    def make[
        ...
        comptime assert (
            target == "gpu"
        ), "StopGradV2.make[target='cpu', INIT](ctx) — drop ctx for CPU"
        var s = Self()
    ...
    def forward[
        ...
        assert_tag_for["StopGradV2", target](self.ts.target_tag)
