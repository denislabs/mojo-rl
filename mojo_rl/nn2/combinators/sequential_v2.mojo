...
Variadic chain of N `ModuleV2` children. Mid-slabs persist on the
...
Differences vs v1 `sequential.mojo`:
  ...
  * `ModuleV2` children → `for_each_param` / `zero_grad` are provided
...
struct SequentialV2[*MODULES: ModuleV2](ModuleV2):
    ...
    def __init__(out self):
        comptime assert Self.N >= 1, "SequentialV2 requires at least one child"
        comptime if Self.N >= 2:
            comptime for i in range(Self.N - 1):
                comptime assert (
                    Self.MODULES[i].OUT_DIM == Self.MODULES[i + 1].IN_DIM
                ), "SequentialV2: adjacent child dims must match"
    ...
    def make[target: StaticString, INIT: Initializer]() raises -> Self:
        ...
        comptime assert target == "cpu", (
            "SequentialV2.make[target='gpu', INIT] requires a DeviceContext"
        )
        var s = Self()
    ...
    def make[
        ...
        comptime assert target == "gpu", (
            "SequentialV2.make[target='cpu', INIT](ctx) — drop ctx for CPU"
        )
        var s = Self()
        comptime for i in range(Self.N):
    ...
    def forward[
        ...
        assert_tag_for["SequentialV2", target](self.ts.target_tag)
