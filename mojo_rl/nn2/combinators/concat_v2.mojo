...
def _total_out_dim[*BRANCHES: ModuleV2]() -> Int:
    var s: Int = 0
...
def _cumulative_offset[index: Int, *BRANCHES: ModuleV2]() -> Int:
    var s: Int = 0
...
struct ConcatV2[*BRANCHES: ModuleV2](ModuleV2):
    ...
    def __init__(out self):
        comptime assert Self.N >= 1, "ConcatV2 requires at least one branch"
        comptime for i in range(Self.N):
            comptime assert (
                Self.BRANCHES[i].IN_DIM == Self.BRANCHES[0].IN_DIM
            ), "ConcatV2: all BRANCHES must share IN_DIM"
    ...
    def make[target: StaticString, INIT: Initializer]() raises -> Self:
        comptime assert target == "cpu", (
            "ConcatV2.make[target='gpu', INIT] requires a DeviceContext"
        )
        var c = Self()
        comptime for i in range(Self.N):
    ...
    def make[
        ...
        comptime assert target == "gpu", (
            "ConcatV2.make[target='cpu', INIT](ctx) — drop ctx for CPU"
        )
        var c = Self()
        comptime for i in range(Self.N):
    ...
    def forward[
        ...
        assert_tag_for["ConcatV2", target](self.ts.target_tag)

        comptime if target == "cpu":
            _concat_forward_cpu[target, BATCH, POLICY=POLICY](self, input, output)
        else:
            raise Error("ConcatV2: GPU forward not yet implemented")
    ...
    def backward[
        ...
        comptime if target == "cpu":
            ...
            raise Error("ConcatV2: GPU backward not yet implemented")
