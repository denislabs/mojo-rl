"""DDPGTargetYStepBlock — wraps DDPGTargetYBlock.step.

Reads state.mb_sp, state.mb_r → writes state.mb_y. Holds pointers to
the target actor + target critic + the trainer's DDPGTargetYBlock."""
...
struct DDPGTargetYStepBlock[
    ...
    def __init__(out self):
        ...
        self.critic_t_ptr = UnsafePointer[Self.CRITIC, MutAnyOrigin](
            unsafe_from_address=0
        )
    ...
    def bind(
        ...
        critic_t_ptr: UnsafePointer[Self.CRITIC, MutAnyOrigin],
        inner_ptr: UnsafePointer[Self.Inner, MutAnyOrigin],
    ):
        self.actor_t_ptr = actor_t_ptr
        self.critic_t_ptr = critic_t_ptr
