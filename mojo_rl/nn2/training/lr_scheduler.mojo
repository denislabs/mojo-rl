"""Epoch-based learning-rate schedules for the nn2 Trainer.

A `Scheduler` is a compile-time hook: `Trainer.train_gpu` takes a
`SCHEDULER` parameter (default `ConstantSchedule`) and, when it isn't
constant, multiplies the optimizer's base LR by `SCHEDULER.lr_scale_at(
epoch, total_epochs)` at the start of each epoch (via `optim.set_lr`).
The base LR is whatever the caller set on `trainer.optim` before calling
`train_gpu`.

`IS_CONSTANT` lets the trainer skip all LR-poking for the default case
(preserving the exact user-set LR and full backward-compatibility).
