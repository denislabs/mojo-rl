"""REDQ-OFE GPU kernels.

Includes the REDQ ensemble-target kernel (copied from `deep_agents/redq/
kernels.mojo`) plus `aux_mse_grad_kernel` for the OFE auxiliary
next-state prediction training step.
"""
...
# =============================================================================
# Aux MSE gradient — for next-state prediction loss
# =============================================================================
...
def aux_mse_grad_kernel[
    ...
    One thread per element; grid = ceil(BATCH*DIM / TPB).
    """
...
def extract_phi_s_grad_kernel[
    ...
    phi_s_grad: LayoutTensor[
        dtype, Layout.row_major(BATCH, PHI_S_DIM), MutAnyOrigin
    ],
    ...
    phi_s-gradient slice (columns 0..PHI_S_DIM-1) for backprop into the
    state branch. One thread per element.
    """
    var idx = Int(block_dim.x * block_idx.x + thread_idx.x)
    if idx >= BATCH * PHI_S_DIM:
        return
    var b = idx // PHI_S_DIM
    var d = idx % PHI_S_DIM
    phi_s_grad[b, d] = phi_sa_in_grad[b, d]
