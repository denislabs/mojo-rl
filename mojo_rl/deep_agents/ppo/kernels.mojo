...
# =============================================================================
# Autodiff Continuous PPO Actor Gradient Kernel
# =============================================================================


@always_inline
fn autodiff_ppo_continuous_actor_grad_kernel[
    dtype: DType,
    BATCH_SIZE: Int,
    ACTION_DIM: Int,
    ...
    """Autodiff-style continuous PPO actor gradient using chained DiffOp vjps.

    Chains GaussianLogProbOp + RatioOp + ClipSurrogateOp backward math:
      1. GaussianLogProbOp forward -> new_log_prob
      2. ClipSurrogateOp.vjp      -> grad_ratio
      3. RatioOp.vjp               -> grad_log_prob = grad_ratio * ratio
      4. GaussianLogProbOp.vjp     -> grad_mean, grad_log_std

    Produces the SAME outputs as ppo_continuous_actor_grad_kernel:
      grad_output, kl_divergences, entropies, clip_flags
    """
    var b = Int(block_dim.x * block_idx.x + thread_idx.x)
    if b >= batch_size:
        return
    ...
    comptime LOG_STD_MIN: Scalar[dtype] = -5.0
    comptime LOG_STD_MAX: Scalar[dtype] = 2.0
    comptime GRAD_CLIP: Scalar[dtype] = 10.0
    comptime LOG_PROB_DIFF_MAX: Scalar[dtype] = 20.0

    var advantage = advantages[b]
    var old_log_prob = old_log_probs[b]

    # =========================================================================
    # GaussianLogProbOp forward: compute new_log_prob + cache for backward
    # =========================================================================
    var new_log_prob: Scalar[dtype] = 0.0
    var entropy_sum: Scalar[dtype] = 0.0

    # Cache arrays for GaussianLogProbOp backward
    var std_vals = InlineArray[Scalar[dtype], ACTION_DIM](
        fill=Scalar[dtype](0.0)
    )
    var normalized_vals = InlineArray[Scalar[dtype], ACTION_DIM](
        fill=Scalar[dtype](0.0)
    )

    for j in range(ACTION_DIM):
        var mean_val = Scalar[dtype](actor_output[b, j][0])
        var log_std_val = Scalar[dtype](actor_output[b, ACTION_DIM + j][0])
        var action_val = Scalar[dtype](actions[b, j][0])
        ...
        var std_val = exp(log_std_val)
        var normalized = (action_val - mean_val) / (
            std_val + Scalar[dtype](1e-6)
        )

        # Cache for backward
        std_vals[j] = std_val
        normalized_vals[j] = normalized

        # Gaussian log probability (GaussianLogProbOp eval)
        var log_gaussian = neg_half * (
            log_2pi + two * log_std_val + normalized * normalized
        )
        new_log_prob = new_log_prob + log_gaussian
    ...
    # =========================================================================
    # RatioOp forward: ratio = exp(new_log_prob - old_log_prob)
    # =========================================================================
    var log_prob_diff = new_log_prob - old_log_prob
    if log_prob_diff > LOG_PROB_DIFF_MAX:
        log_prob_diff = LOG_PROB_DIFF_MAX
    ...
    # =========================================================================
    # ClipSurrogateOp forward + vjp: grad_ratio
    # =========================================================================
    ...
    var grad_mult: grad_output.element_type
    if surr1 <= surr2:
        grad_mult = -advantage
        clip_flags[b] = 0.0
    else:
    ...
    for j in range(ACTION_DIM):
        var std = std_vals[j]
        var normalized = normalized_vals[j]

        # GaussianLogProbOp.vjp:
        #   grad_mean = grad_log_prob * normalized / std
        var d_log_prob_d_mean = normalized / (std + Scalar[dtype](1e-6))

        #   grad_log_std = grad_log_prob * (normalized^2 - 1)
        var d_log_prob_d_log_std = normalized * normalized - one

        # Entropy gradient: d_entropy/d_log_std = 1
        var d_entropy_d_log_std: Scalar[dtype] = 1.0

        # Chain: grad from policy loss + entropy bonus
        var grad_mean = (grad_log_prob * d_log_prob_d_mean) / batch_size_scalar

        var grad_log_std = (
            grad_log_prob * d_log_prob_d_log_std
            - entropy_coef * d_entropy_d_log_std
        ) / batch_size_scalar
