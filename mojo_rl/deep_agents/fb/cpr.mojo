...
struct FBCPRTrainer[
    ...
    def make[
        ...
        *,
        ...
        `bc 1.0`, clip 1.0 — §18.7.6) plus BFM-Zero's released CPR values
        (`fb_cpr/configs.py`: `lr_discriminator 1e-5`, `lr_critic 1e-4`,
        `critic_target_tau 0.005`, `reg_coeff 0.01`,
        ...
        s.policy_noise = s.t.policy_noise
        s.noise_clip = s.t.noise_clip
    ...
    def _size_once(mut self) raises:
        ...
        self.t.has_pi_extra = self.reg_coeff > 0.0
        comptime if T == "gpu":
            var d = c.value()
    ...
    def ensure_sized(mut self) raises:
        """Size the owned batch (inner + expert) so a GPU caller can gather
        straight into `t.bs/ba/bsn/bsp` and `es/esn`."""
        self._size_once()
    ...
    def encode_expert(mut self) raises:
        ...
        the `z` mixture that draws from it, and before `train_step`."""
        comptime T = Self.TARGET
        var c = self.ctx
        self._size_once()
        call_forward[T, Self.BATCH](
            self.t.bnet.online, TensorRefs[1, MutAnyOrigin](self.esn),
            self.b_esn, c,
        )
    ...
    def train_step(mut self, want_loss: Bool = True) raises -> FBCPRLosses:
        """One CPR step over the owned batches. Precondition: the caller has
        written `t.bs/ba/bsn/bsp/bz` and `es/esn` and called
        `encode_expert` (so `ez` matches `esn`)."""
        comptime T = Self.TARGET
        var c = self.ctx
        self._size_once()
        self.steps += 1
        ...
        pack2_t[T, Self.OBS, Self.D, Self.BATCH](self.x_neg, self.t.bs, self.t.bz, c)
        ...
        # ── 2. style reward from the UPDATED D, then the twin critic ─────
        call_forward[T, Self.BATCH](
            self.disc, TensorRefs[1, MutAnyOrigin](self.x_neg), self.l_neg, c
        )
        ...
        pack2_t[T, Self.OBS, Self.D, Self.BATCH](self.ain_t, self.t.bsn, self.t.bz, c)
        call_forward[T, Self.BATCH](
            self.t.actor.target_net, TensorRefs[1, MutAnyOrigin](self.ain_t),
            self.pi_t, c,
        )
        comptime if T == "gpu":
        ...
        pack3_t[T, Self.OBS, Self.ACT, Self.D, Self.BATCH](
            self.qin_t, self.t.bsn, self.a_next, self.t.bz, c
        )
        call_forward[T, Self.BATCH](
        ...
        pack3_t[T, Self.OBS, Self.ACT, Self.D, Self.BATCH](
            self.qin, self.t.bs, self.t.ba, self.t.bz, c
        )
        ...
        if self.reg_coeff > 0.0:
            pack2_t[T, Self.OBS, Self.D, Self.BATCH](self.ain, self.t.bs, self.t.bz, c)
            call_forward[T, Self.BATCH](
                self.t.actor.online, TensorRefs[1, MutAnyOrigin](self.ain), self.pi, c
            )
            pack3_t[T, Self.OBS, Self.ACT, Self.D, Self.BATCH](
                self.qin_pi, self.t.bs, self.pi, self.t.bz, c
            )
            call_forward[T, Self.BATCH](
            ...
            slice_cols_t[T, Self.Q_IN, Self.OBS, Self.ACT, Self.BATCH](
                self.t.g_pi_extra, self.g_qin, c
            )

        # ── 4. the FB step, unchanged, reading `g_pi_extra` ──────────────
        var fb = self.t.train_step(want_loss)
    ...
    def load_state(mut self, path: String) raises:
        """Inner nets from `path`, CPR nets from the sidecar. ⚠ A missing
        sidecar RAISES: a resume that silently restarts `D` and `Q_D` from
        random init would train against a reward that has forgotten
        everything, and nothing in the log would say so."""
        self.t.load_state(path)
        var sp = path + ".cpr"
        if not _path_exists(sp):
            raise Error("FBCPRTrainer.load_state: sidecar missing: " + sp)
        var content: String
        with open(sp, "r") as f:
