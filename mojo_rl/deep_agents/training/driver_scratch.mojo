...
collapses both behind one typed wrapper, exactly mirroring nn's existing
trainer-side `Scratch[NAME, SIZE, STAGING]` (see `nn/core/scratch.mojo`)
...
struct DriverScratch[NAME: StaticString, N_ENVS: Int, DIM: Int](
    ...
    # the existing nn pattern (e.g. `Scratch.dev.value()` is used the
