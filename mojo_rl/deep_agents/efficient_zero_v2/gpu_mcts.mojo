"""GPU port of EfficientZero V2 Gumbel-search MCTS.

Mirrors the layout strategy of MuZero's `gpu_mcts.mojo`: every per-env tree
is laid out flat across the same DeviceBuffer with a per-env stride, and
every kernel runs one thread per env. Parallelism is across N_ENVS — within a
single env's tree, traversal is sequential.

Key differences vs MuZero PUCT GPU MCTS:
...
here — it's a perf optimisation deferred to Phase 2 alongside the EZ-V2
training loop.
