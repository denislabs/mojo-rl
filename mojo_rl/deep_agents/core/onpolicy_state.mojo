"""PPO continuous state containers.

Moved from mojo_rl/deep_agents/ppo/state.mojo to allow reuse by generic
on-policy continuous agents without pulling in the full PPO module.

Two state structs:
    PPOContinuousState    — CPU-resident state for continuous-action PPO
    PPOContinuousGPUState — GPU-resident state for continuous-action PPO

Both implement the corresponding OnPolicyContinuousState / GPUOnPolicyState
traits defined in deep_agents/core/.
"""
...
struct PPOContinuousGPUState[
    ...
    fn gpu_store_post_step[
        ...
        """
        from mojo_rl.deep_agents.core.kernels import _store_post_step_kernel
