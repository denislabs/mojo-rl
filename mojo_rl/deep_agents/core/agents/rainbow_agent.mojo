...
fn _dueling_dist_combine[
    BATCH: Int, ACTIONS: Int, ATOMS: Int, RAW_OUT: Int,
](
...
fn _rainbow_expected_q[
    BATCH: Int, ACTIONS: Int, ATOMS: Int, COMBINED_SIZE: Int,
](
...
struct RainbowCPUState[
    ...
    fn __init__(
        out self, alpha: Float64 = 0.6, beta: Float64 = 0.4, gamma: Float64 = 0.99
    ):
        ...
        self.buffer = PrioritizedReplayBuffer[
            ...
            alpha=Scalar[dtype](alpha), beta=Scalar[dtype](beta),
        )
...
struct GenericRainbowAgent[
    ...
    fn do_cpu_train_step(mut self, mut cpu_state: Self.CPUStateType) -> Float64:
        ...
        var grad_combined_arr = InlineArray[
            Scalar[dtype], Self.BATCH * COMB
        ](fill=Scalar[dtype](0))
        ...
        for b in range(Self.BATCH):
            ...
            for i in range(ATOMS):
                target_probs[i] = exp(
                    target_combined[t_base + i] - t_max
                ) / t_sum_exp
    ...
    fn upload_to_gpu(
        self, mut gpu_state: Self.GPUStateType, ctx: DeviceContext,
    ) raises -> None:
    ...
    fn download_from_gpu(
        mut self, mut gpu_state: Self.GPUStateType, ctx: DeviceContext,
    ) raises -> None:
        ...
        fn rainbow_select_kernel(
            ...
            for a in range(Self.ACTIONS):
                ...
                for i in range(Self.NUM_ATOMS):
                    ...
                    var q_ai = v_i + rebind[Scalar[dtype]](
    ...
    fn do_gpu_train_step(
        ...
        var q_comb_t = LayoutTensor[dtype, Layout.row_major(BATCH, COMB), MutAnyOrigin](gpu_state.q_combined.unsafe_ptr())
        var next_comb_t = LayoutTensor[dtype, Layout.row_major(BATCH, COMB), MutAnyOrigin](gpu_state.next_q_combined.unsafe_ptr())
        var online_next_comb_t = LayoutTensor[dtype, Layout.row_major(BATCH, COMB), MutAnyOrigin](gpu_state.online_next_q_combined.unsafe_ptr())
        var expected_q_t = LayoutTensor[dtype, Layout.row_major(BATCH, Self.ACTIONS), MutAnyOrigin](gpu_state.expected_q.unsafe_ptr())
        var grad_comb_t = LayoutTensor[dtype, Layout.row_major(BATCH, COMB), MutAnyOrigin](gpu_state.grad_combined.unsafe_ptr())
        ...
        var bins_t = LayoutTensor[dtype, Layout.row_major(ATOMS), MutAnyOrigin](gpu_state.bins_buf.unsafe_ptr())
        ...
        fn dueling_combine_kernel(
            raw: LayoutTensor[dtype, Layout.row_major(BATCH, Self.RAW_OUT), MutAnyOrigin],
            comb: LayoutTensor[dtype, Layout.row_major(BATCH, COMB), MutAnyOrigin],
        ):
            ...
            for i in range(ATOMS):
                ...
                var mean_a = Scalar[dtype](0)
                for a in range(Self.ACTIONS):
                    mean_a += rebind[Scalar[dtype]](raw[b, ATOMS + a * ATOMS + i])
                mean_a /= Scalar[dtype](Self.ACTIONS)
                for a in range(Self.ACTIONS):
                    comb[b, a * ATOMS + i] = v_i + rebind[Scalar[dtype]](raw[b, ATOMS + a * ATOMS + i]) - mean_a

        ctx.enqueue_function[dueling_combine_kernel, dueling_combine_kernel](
            q_raw_t, q_comb_t, grid_dim=(BATCH_BLOCKS,), block_dim=(TPB,),
        )
        ctx.enqueue_function[dueling_combine_kernel, dueling_combine_kernel](
            next_q_raw_t, next_comb_t, grid_dim=(BATCH_BLOCKS,), block_dim=(TPB,),
        )
        ctx.enqueue_function[dueling_combine_kernel, dueling_combine_kernel](
            online_next_q_raw_t, online_next_comb_t, grid_dim=(BATCH_BLOCKS,), block_dim=(TPB,),
        )
        ...
        fn expected_q_kernel(
            ...
            eq: LayoutTensor[dtype, Layout.row_major(BATCH, Self.ACTIONS), MutAnyOrigin],
        ):
            ...
            for a in range(Self.ACTIONS):
                ...
                var sum_exp = Scalar[dtype](0)
                for i in range(ATOMS):
                    sum_exp += exp(rebind[Scalar[dtype]](comb[b, base + i]) - max_val)
                var expected = Scalar[dtype](0)
                for i in range(ATOMS):
                    var prob = exp(rebind[Scalar[dtype]](comb[b, base + i]) - max_val) / sum_exp
                    expected += prob * rebind[Scalar[dtype]](bins[i])
                eq[b, a] = expected

        ctx.enqueue_function[expected_q_kernel, expected_q_kernel](
            online_next_comb_t, bins_t, expected_q_t,
        ...
        fn rainbow_project_grad_kernel(
            online_comb: LayoutTensor[dtype, Layout.row_major(BATCH, COMB), MutAnyOrigin],
            target_comb: LayoutTensor[dtype, Layout.row_major(BATCH, COMB), MutAnyOrigin],
            next_eq: LayoutTensor[dtype, Layout.row_major(BATCH, Self.ACTIONS), MutAnyOrigin],
            bins: LayoutTensor[dtype, Layout.row_major(ATOMS), MutAnyOrigin],
            ...
            grad_c: LayoutTensor[dtype, Layout.row_major(BATCH, COMB), MutAnyOrigin],
            grad_r: LayoutTensor[dtype, Layout.row_major(BATCH, Self.RAW_OUT), MutAnyOrigin],
            ...
            var t_sum_exp = Scalar[dtype](0)
            for i in range(ATOMS):
                t_sum_exp += exp(rebind[Scalar[dtype]](target_comb[b, t_base + i]) - t_max)

            # 3. Bellman projection with γ^n
            var projected = InlineArray[Scalar[dtype], ATOMS](fill=Scalar[dtype](0))
            var r = rebind[Scalar[dtype]](rewards[b])
            var dm = Scalar[dtype](1.0) - rebind[Scalar[dtype]](dones[b])
            for j in range(ATOMS):
                var t_prob = exp(rebind[Scalar[dtype]](target_comb[b, t_base + j]) - t_max) / t_sum_exp
                ...
                if l_idx == u_idx:
                    ...
                    projected[l_idx] = projected[l_idx] + t_prob * (Scalar[dtype](1.0) - u_w)
            ...
            for i in range(ATOMS):
                p_sum_exp += exp(rebind[Scalar[dtype]](online_comb[b, p_base + i]) - p_max)
            var log_sum_exp = p_max + log(p_sum_exp)

            # CE loss for priority
            var sample_loss = Scalar[dtype](0)
            for i in range(ATOMS):
                var log_sm = rebind[Scalar[dtype]](online_comb[b, p_base + i]) - log_sum_exp
                sample_loss = sample_loss - projected[i] * log_sm
            td_err[b] = sample_loss

            var weight = rebind[Scalar[dtype]](weights[b])
            for i in range(ATOMS):
                var sm = exp(rebind[Scalar[dtype]](online_comb[b, p_base + i]) - p_max) / p_sum_exp
                grad_c[b, p_base + i] = weight * (sm - projected[i]) / Scalar[dtype](BATCH)
            ...
            for i in range(ATOMS):
                ...
                for a in range(Self.ACTIONS):
                    grad_r[b, ATOMS + a * ATOMS + i] = (
                        rebind[Scalar[dtype]](grad_c[b, a * ATOMS + i]) - one_over_n * sum_dq
                    )

        ctx.enqueue_function[rainbow_project_grad_kernel, rainbow_project_grad_kernel](
            q_comb_t, next_comb_t, expected_q_t, bins_t,
            actions_t, rewards_t, dones_t, weights_t,
            grad_comb_t, grad_raw_t, td_errors_t,
            gamma_n_s, v_min_s, v_max_s, dz_s,
            grid_dim=(BATCH_BLOCKS,), block_dim=(TPB,),
        )
        ...
        Self.QNet.backward_gpu[BATCH](
            ctx, grad_raw_t, grad_in_t, p_online, cache_t, g, gpu_state.train_ws,
        )
