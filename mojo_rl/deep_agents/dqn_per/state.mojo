"""DQN+PER GPU state container.

The GPU training path uses a uniform replay buffer (no prioritization)
because the PER sum-tree is inherently serial and cannot run on GPU.
The GPU path trades PER for speed. The CPU path retains full PER.
"""
...
# =============================================================================
# DQNPERGPUState — GPU buffer container for DQN+PER
# =============================================================================
...
struct DQNPERGPUState[
    ...
    """GPU-resident state for DQN+PER training.

    Uses a UNIFORM GPU replay buffer (no prioritization) because the PER
    sum-tree is inherently serial and cannot run efficiently on GPU.
    The GPU path trades PER for speed; the CPU path retains full PER.

    Holds all device buffers needed for one DQN GPU training loop:
      - Online and target GPU network states
      - GPU replay buffer (uniform, discrete action stored as float scalar index)
      - Inference scratch buffers (sized by max_n_envs)
      - Training scratch buffers (sample output, Q caches, grad buffers)

    Created once at the start of GPU training via DQNPERAgent.make_gpu_state.
    CPU weights are uploaded separately via DQNPERAgent.upload_to_gpu.
    ...
    # GPU replay buffer (uniform — ACTION_DIM=1 default: scalar discrete action)
