"""MuZero adapters for the planners.tree_search GPU trait surface.
...
``GenericGPUMCTS.search_gpu`` and ``.extract_actions_temp``. Each
adapter is one ``Network[…].forward_gpu[B]`` call deep — the orchestrator
owns kernel dispatch and the agent owns network ownership.
...
Used by:
  * The ``test_generic_gpu_mcts_with_muzero_net.mojo`` validation test
    that exercises the orchestrator on the production MuZero MLP stack.
  * Future ``train_gpu`` rewiring behind ``Config.USE_NEW_MCTS=True``.
"""
