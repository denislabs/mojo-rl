"""EfficientZeroV2 **Atari** networks (nn2) — the spatial-latent model.
...
so the bias is redundant — BN re-centres). nn2 ``Conv2D`` carries a bias; for the
...
# head MLP is ELU in the reference; nn2 has no ELU, so ReLU is substituted
