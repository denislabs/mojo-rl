...
("cpu" / "gpu") like the SAC dual-path; today only the **CPU** self-play driver
exists (the GPU unroll is the deferred #24), so `train` / `eval_greedy` hard-stop
at compile time for `TARGET="gpu"`.
...
struct MuZeroAgent[
    ...
    def train(
        ...
        comptime if Self.TARGET == "cpu":
            ...
            comptime assert False, (
                "GPU MuZero self-play driver is deferred (#24) — use TARGET='cpu'"
            )
            return 0.0
    ...
    def eval_greedy(
        ...
        comptime if Self.TARGET == "cpu":
            ...
            comptime assert False, (
                "GPU MuZero eval is deferred (#24) — use TARGET='cpu'"
            )
            return 0.0
