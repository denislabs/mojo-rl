...
struct SplitApply[Left: Model, Right: Model, split: Int](Model):
    ...
    def forward_gpu_inference_with_cache[
        ...
        # Forward Left and Right (inference)
        var left_out_t = LayoutTensor[
            dtype, Layout.row_major(BATCH, Self.LEFT_OUT), MutAnyOrigin
        ](left_out_ptr)
        var right_out_t = LayoutTensor[
        ...
        Self.Left.forward_gpu_inference_with_cache[BATCH, dtype](ctx, left_out_t, left_t, pl, sl, cl, child_ws)
        ...
        Self.Right.forward_gpu_inference_with_cache[BATCH, dtype](
            ctx, right_out_t, right_t, pr, sr, cr, child_ws
        )
    ...
    def backward_gpu_inference[
        ...
        # Backward Left (inference) — params/cache/grads at offset 0 (always aligned)
        var gi_l_t = LayoutTensor[
            dtype, Layout.row_major(BATCH, Self.LEFT_IN), MutAnyOrigin
        ](gi_l_ptr)
        var pl = LayoutTensor[
        ...
        Self.Left.backward_gpu_inference[BATCH, dtype](
            ctx, gi_l_t, gl_t, pl, sl, cl, grads_l, child_ws
        )

        # Backward Right (inference) — params/grads at aligned offset
        var gi_r_t = LayoutTensor[
            dtype, Layout.row_major(BATCH, Self.RIGHT_IN), MutAnyOrigin
        ](gi_r_ptr)
        ...
        Self.Right.backward_gpu_inference[BATCH, dtype](
            ctx, gi_r_t, gr_t, pr, sr, cr, grads_r, child_ws
        )
