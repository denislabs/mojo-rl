...
struct FusedConv2DActivation[
    ...
    fn backward_dW_kernel_2x2[
        ...
        for k_tile in range(num_k_tiles):
            ...
            if gk0 < Self.col_size and bki0 < K_TOTAL:
                ...
                b_smem[b_r0, b_c0] = cache[bb0, gk0 * Self.spatial_out + bs0]
            else:
                b_smem[b_r0, b_c0] = 0
            ...
            if gk1 < Self.col_size and bki1 < K_TOTAL:
                ...
                b_smem[b_r1, b_c1] = cache[bb1, gk1 * Self.spatial_out + bs1]
            else:
                b_smem[b_r1, b_c1] = 0
        ...
        comptime if is_nvidia_gpu():
            ...
            for k_tile in range(num_k_tiles):
                ...
                if gk < Self.col_size and bki < K_TOTAL:
                    ...
                    b_smem[br, bc] = cache[b_idx, gk * Self.spatial_out + s_idx]
                else:
                    b_smem[br, bc] = 0
    ...
    fn vjp_gpu[
        ...
        comptime if has_nvidia_gpu_accelerator():
            ...
            # dcol = W.T @ masked_grad → (col_size, K_TOTAL)
            # then col2im: dcol → grad_input
            # masked_grad is still in grad_input ptr (OC, K_TOTAL)

            # Transpose W: (OC, col_size) → W_T (col_size, OC) — tiny
            ...
            # Reuse col_reshaped_buf for dcol output (col_size, K_TOTAL)
            ...
            # max_matmul: dcol = W_T @ masked_grad
            max_matmul[target="gpu"](dcol, w_t, grad_reshaped, ctx)

            # col2im gather: one thread per input element, reads from dcol
            # dcol[c_k, b*S + s] already has the gradient contribution
            var total_dx = BATCH * Self.IN_DIM
            var grid_dx = (total_dx + TPB - 1) // TPB

            @always_inline
            fn col2im_gather_wrapper(
                grad_input: LayoutTensor[
                    dtype,
            ...
            ctx.enqueue_function[
                col2im_gather_wrapper, col2im_gather_wrapper
            ](
                grad_input,
                dcol,
                grid_dim=(grid_dx,),
