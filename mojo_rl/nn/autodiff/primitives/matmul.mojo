...
struct MatMul[in_dim: Int, out_dim: Int](DiffOp):
    ...
    def vjp_gpu[
        ...
        comptime if has_nvidia_gpu_accelerator():
            # NVIDIA: use max_matmul for both dx and dW — same kernel as
            # forward, ensuring forward/backward consistency for any dtype.
            ...
            # max_matmul doesn't support transpose_a, so we explicitly
            # transpose cache: (BATCH, in_dim) → (in_dim, BATCH), then
            # call standard matmul: (in_dim, BATCH) @ (BATCH, out_dim).
            ...
            # Apple: use hand-written 2x2 tiled kernels
