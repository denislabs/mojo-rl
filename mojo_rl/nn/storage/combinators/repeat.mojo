...
struct Repeat[N: Int, Inner: Module, shared: Bool = False](Module):
    ...
    def make_cpu() raises -> Self:
        ...
        for _ in range(Self.N):
            r.children.append(Self.Inner.make_cpu())
        return r^
    ...
    def make_gpu(ctx: DeviceContext) raises -> Self:
        var r = Self()
        for _ in range(Self.N):
            r.children.append(Self.Inner.make_gpu(ctx))
        return r^
    ...
    def forward[
        ...
        comptime if Self.N == 1:
            ...
            comptime for i in range(Self.N):
                comptime if i == 0:
                    self.children[0].forward[target, B](
                        TensorRefs[Self.Inner.ARITY].of1(inputs[0]),
                        self.act[0], ctx,
                    )
                elif i == Self.N - 1:
                    self.children[Self.N - 1].forward[target, B](
                        TensorRefs[Self.Inner.ARITY].of1(self.act[Self.N - 2]),
                        out, ctx,
                    )
                else:
                    self.children[i].forward[target, B](
                        TensorRefs[Self.Inner.ARITY].of1(self.act[i - 1]),
                        self.act[i], ctx,
                    )
            ...
            comptime for j in range(Self.N):
                ...
                comptime if i == Self.N - 1:
                    self.children[Self.N - 1].vjp[target, B](
                        TensorRefs[Self.Inner.ARITY].of1(self.act[Self.N - 2]),
                        grad_output,
                        TensorRefs[Self.Inner.ARITY].of1(self.grd[Self.N - 2]),
                        ctx,
                    )
                elif i == 0:
                    self.children[0].vjp[target, B](
                    ...
                    self.children[i].vjp[target, B](
                        TensorRefs[Self.Inner.ARITY].of1(self.act[i - 1]),
                        self.grd[i],
                        TensorRefs[Self.Inner.ARITY].of1(self.grd[i - 1]), ctx,
                    )
