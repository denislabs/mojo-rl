"""LSTMCell[IN_, HIDDEN] — PyTorch-equivalent LSTM cell for nn.
...
(see `examples/nn/lstm/`). This matches the legacy `mojo_rl.nn` LSTMCell
and the nn `GRUCell` math conventions.

Parameters are nn `Param` fields, so `for_each_param` / `zero_grad`
work and the cell composes with nn `Adam` / checkpointing. The cell
...
# ──────────────────────────────────────────────────────────────────────
# GPU kernels (separate W_ih / W_hh / b buffers — nn Param layout).
# ──────────────────────────────────────────────────────────────────────
...
struct LSTMCell[IN_: Int, HIDDEN: Int](Module):
    ...
    # the old pure-`*_auto` overrides. nn Adam / zero_grad / checkpoint
    # work unchanged.
    # ------------------------------------------------------------------
    ...
    def forward[
        ...
        raise Error(
            ...
            "(see examples/nn/lstm), not Module.forward"
        )
    ...
    def vjp[
        ...
        raise Error(
            ...
            "(see examples/nn/lstm), not Module.vjp"
        )
    ...
    def vjp_param_grads[
        ...
        raise Error(
            "LSTMCell is recurrent — use step_backward "
            "(see examples/nn/lstm), not Module.vjp_param_grads"
        )
