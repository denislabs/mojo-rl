...
package and the new `mojo_rl/nn/` package. Two corrections from the
initial gap analysis are baked in:

- **RL-specific surfaces live in `mojo_rl/deep_agents/`, not `nn/`.**
  ...
  `deep_agents/primitives/` and `deep_agents/loss/`. They are NOT nn
  gaps. PPO-discrete is similarly unblocked via `deep_agents/ppo/blocks/`,
  so the legacy `CategoricalLogProbOp` / `RatioOp` / `ClipSurrogateOp`
  DiffOps stay out of nn.
...
When deciding "nn or deep_agents?" for a missing nn layer:

- **Mathy + policy-agnostic** (BN, Conv2D, Dropout, MinMaxNorm, SimNorm,
  Mish, Sigmoid, Flatten) → `nn/primitives/`
- **Encodes an RL distribution or loss shape** (stochastic actors,
  log-probs, PPO clip, squashed-Gaussian) → `deep_agents/primitives/`
  or `deep_agents/loss/`

## Performance convention: `max_matmul` everywhere

Any nn primitive with a matmul-shaped inner kernel routes through
...
- **Conv2D Apple-batched single-sgemm.** Legacy nn
  ...
  nn currently does BATCH separate sgemm calls. Gain depends on
  BATCH and the constant-time sgemm setup overhead. Port when a CNN
  agent moves to deep_agents and benchmarks show it matters.
...
- **None at landing.** Phase 2 / 4 / 5 GPU paths are now in tree
  ...
  over overlapping output windows) — matches the nn / deep_agents
  no-atomics convention. See `test_nn_gpu_parity.mojo` for the
  CPU/GPU parity gate.
...
- [x] `tests/nn/test_mish_op.mojo` + `test_sigmoid_op.mojo` — analytic parity
...
`Sequential` with no new code in nn or callers.
...
- [x] `primitives/dropout.mojo` — first nn leaf with non-param runtime
      ...
      train forward (no on-device counter, since nn doesn't do CUDA
...
block they need available in nn.

## Phase 3 — *dropped*

PPO-discrete ops (`CategoricalLogProbOp`, `RatioOp`, `ClipSurrogateOp`)
do not belong in nn under the allocation rule. They are absorbed by
`deep_agents/ppo/blocks/` (already exists) and
`deep_agents/primitives/stochastic_categorical.mojo`. Renumber later
...
encoder). First cross-cutting running-stats story for nn.
...
Unlocks CNN agents on nn. Largest single chunk.
...
Exit: deep_agents DQN / PPO CNN configs can drop their legacy `nn`
imports — every Module they need is in nn.

## Phase 6 — Hand-fused kernels (perf-only)

Likely deferred indefinitely. nn1's bet was AutoFused; nn's bet is
...
P5 only when a CNN agent moves to deep_agents.
P6/P7 on demand.
...
- nn ElementOp template: `mojo_rl/nn/primitives/elementwise.mojo`
- nn ElementOp ladder: `mojo_rl/nn/primitives/ops/`
- nn Module trait: `mojo_rl/nn/core/module.mojo`
- Existing ElementOp parity tests:
  `tests/nn/test_swish_op.mojo`, `tests/nn/test_elementwise_*_parity.mojo`
- RL-side already-ported layers:
  `mojo_rl/deep_agents/primitives/`, `mojo_rl/deep_agents/loss/`
