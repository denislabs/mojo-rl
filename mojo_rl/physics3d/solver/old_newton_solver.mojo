...
struct OldNewtonSolver(ConstraintSolver):
    ...
    def solve_gpu[
        ...
        for c in range(nc):
            ...
            comptime ws_imp_n_on = solver_ws_idx + 13 * MC
            comptime ws_diag_n_on = solver_ws_idx + 14 * MC
            var imp_on = rebind[Scalar[DTYPE]](
                workspace[env, ws_imp_n_on + c]
            )
            var diag_on = rebind[Scalar[DTYPE]](
                workspace[env, ws_diag_n_on + c]
            )
            var R_n_c = (Scalar[DTYPE](1.0) - imp_on) / imp_on * diag_on
        ...
        for c in range(nc):
            ...
            for i in range(NV):
                Jn[i] = rebind[Scalar[DTYPE]](
                    workspace[env, ws_J_n_idx + c * NV + i]
                )
                Jt1[i] = rebind[Scalar[DTYPE]](
                    workspace[env, ws_Jt1_idx + c * NV + i]
                )
                Jt2[i] = rebind[Scalar[DTYPE]](
                    workspace[env, ws_Jt2_idx + c * NV + i]
                )
            ...
            if cs == 1:  # QUADRATIC: standard rank-1 updates
                ...
                var jar_n_c = rebind[Scalar[DTYPE]](
                    workspace[env, ws_jar_n_idx + c]
                )
                var jar_t1_c = rebind[Scalar[DTYPE]](
                    workspace[env, ws_jar_t1_idx + c]
                )
                var jar_t2_c = rebind[Scalar[DTYPE]](
                ...
                var h_t1t1 = (
                    Dm * mu_c * mu_c * jar_t1_c * jar_t1_c / T2_safe
                    + Dm
                    * mu_c
                    * s
                    / T_safe
                    * (jar_t1_c * jar_t1_c / T2_safe - Scalar[DTYPE](1.0))
                )
                var h_t2t2 = (
                    Dm * mu_c * mu_c * jar_t2_c * jar_t2_c / T2_safe
                    + Dm
                    * mu_c
                    * s
                    / T_safe
                    * (jar_t2_c * jar_t2_c / T2_safe - Scalar[DTYPE](1.0))
                )
                var h_t1t2 = (
                    (Dm * mu_c * mu_c + Dm * mu_c * s / T_safe)
                    * jar_t1_c
                    * jar_t2_c
                    / T2_safe
                )
        ...
        for _iter in range(NEWTON_ITER_GPU):
            ...
            for i in range(NV):
                ...
                for c in range(nc):
                    ...
                    if cs == 0:
                        continue
                    var fn_c = rebind[Scalar[DTYPE]](
                        workspace[env, ws_fn_idx + c]
                    )
                    var ft1_c = rebind[Scalar[DTYPE]](
                        workspace[env, ws_ft1_idx + c]
                    )
                    var ft2_c = rebind[Scalar[DTYPE]](
                        workspace[env, ws_ft2_idx + c]
                    )
                    g -= (
                        rebind[Scalar[DTYPE]](
            ...
            if state_changed:
                ...
                for c in range(nc):
                    ...
                    var Jn2 = InlineArray[Scalar[DTYPE], V_SIZE](
                        uninitialized=True
                    )
                    var Jt1_2 = InlineArray[Scalar[DTYPE], V_SIZE](
                        uninitialized=True
                    )
                    var Jt2_2 = InlineArray[Scalar[DTYPE], V_SIZE](
                        uninitialized=True
                    )
                    for i in range(NV):
                        Jn2[i] = rebind[Scalar[DTYPE]](
                            workspace[env, ws_J_n_idx + c * NV + i]
                        )
                        Jt1_2[i] = rebind[Scalar[DTYPE]](
                            workspace[env, ws_Jt1_idx + c * NV + i]
                        )
                        Jt2_2[i] = rebind[Scalar[DTYPE]](
                            workspace[env, ws_Jt2_idx + c * NV + i]
                        )
                    if cs == 1:
                        for i in range(NV):
                        ...
                        var T2_s = T_s * T_s
                        var h_t1t1 = (
                            Dm * mu_c * mu_c * jar_t1_c * jar_t1_c / T2_s
                            + Dm
                            * mu_c
                            * s
                            / T_s
                            * (jar_t1_c * jar_t1_c / T2_s - Scalar[DTYPE](1.0))
                        )
                        var h_t2t2 = (
                            Dm * mu_c * mu_c * jar_t2_c * jar_t2_c / T2_s
                            + Dm
                            * mu_c
                            * s
                            / T_s
                            * (jar_t2_c * jar_t2_c / T2_s - Scalar[DTYPE](1.0))
                        )
                        var h_t1t2 = (
                            (Dm * mu_c * mu_c + Dm * mu_c * s / T_s)
                            * jar_t1_c
                            * jar_t2_c
                            / T2_s
                        )
                        for i in range(NV):
                            for j in range(NV):
                                H[i * NV + j] += (
                                    Dm * Jn2[i] * Jn2[j]
                                    + h_nt1
                                    * (Jn2[i] * Jt1_2[j] + Jt1_2[i] * Jn2[j])
                                    + h_nt2
                                    * (Jn2[i] * Jt2_2[j] + Jt2_2[i] * Jn2[j])
