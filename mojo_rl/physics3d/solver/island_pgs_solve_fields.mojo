...
This is a VERBATIM copy of the fields PGS solver `_contact_solve_env_fields`
(constraints/contact_solve_fields.mojo — itself the golden-validated fields
...
reuses the shared contact_solve_fields helpers (init/normal-precompute/
warmstart/jacobian rows) directly.

Serialized per env (one thread per env on GPU) like the other fields solvers.
Same signature family as `solve_contacts_fields` so callers can swap solvers.
"""
...
# === BODY (copied verbatim from contact_solve_fields _contact_solve_env_fields
# + _contact_solve_fields_kernel + solve_contacts_fields, renamed, with island
# insertions A-D) ===
@always_inline
def _island_pgs_solve_env_fields[
    DTYPE: DType,
    NQ: Int,
    NV: Int,
    NBODY: Int,
