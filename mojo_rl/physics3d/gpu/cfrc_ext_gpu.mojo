...
def compute_cfrc_ext_fields[
    ...
    """Per-field cfrc_ext: identical arithmetic to `compute_cfrc_ext_gpu`,
    but body_mass / body_parent come from the packed `ModelFields.bodies`
    tensor `[NBODY, MODEL_BODY_SIZE]` instead of the legacy `[1, MODEL_SIZE]`
    model slab (offset `model_body_offset(i) + IDX` -> record `bodies[i, IDX]`).
    The contact/xipos/cfrc_ext I/O is still the hook state slab.

    Args:
        ctx: GPU device context.
        states_buf: State buffer [BATCH_SIZE, STATE_SIZE].
        bodies_buf: ModelFields.bodies device buffer [NBODY, MODEL_BODY_SIZE].
    """
    var states = LayoutTensor[
        DTYPE, Layout.row_major(BATCH_SIZE, STATE_SIZE), MutAnyOrigin
    ](states_buf)
