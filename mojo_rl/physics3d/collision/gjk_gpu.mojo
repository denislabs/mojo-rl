"""GPU-compatible GJK/EPA for mesh collision.

Same algorithms as gjk.mojo but reads mesh vertices from LayoutTensor model
buffer instead of List. All other geom support functions are identical since
they don't access mesh data.
"""
...
def _support_mesh_gpu[
    ...
    mesh_vert_buf_off: Int,
    num_verts: Int,
) -> InlineArray[Scalar[DTYPE], 3]:
    """Support point on mesh reading vertices from GPU model buffer."""
    ...
    for i in range(num_verts):
        var off = mesh_vert_buf_off + i * 3
        var vx = rebind[Scalar[DTYPE]](model[0, off + 0])
        var vy = rebind[Scalar[DTYPE]](model[0, off + 1])
        var vz = rebind[Scalar[DTYPE]](model[0, off + 2])
...
def _support_gpu[
    ...
    """Unified support function for GPU — reads mesh verts from model buffer."""
    if geom_type == GEOM_SPHERE:
        ...
        return _support_mesh_gpu[DTYPE, MODEL_SIZE](
            dir_x,
            dir_y,
            dir_z,
            pos_x,
...
def _minkowski_support_gpu[
    ...
    var s1 = _support_gpu[DTYPE, MODEL_SIZE](
        type1,
        p1x,
        p1y,
        p1z,
    ...
    var s2 = _support_gpu[DTYPE, MODEL_SIZE](
        type2,
        p2x,
        p2y,
        p2z,
...
def gjk_epa_gpu[
    ...
    """GPU GJK distance + EPA penetration depth between two convex shapes."""
    # ===== GJK Phase =====
    ...
    var s = _minkowski_support_gpu[DTYPE, MODEL_SIZE](
        type1,
        p1x,
        p1y,
        p1z,
    ...
    for _ in range(GJK_MAX_ITERATIONS):
        ...
        var sn = _minkowski_support_gpu[DTYPE, MODEL_SIZE](
            type1,
            p1x,
            p1y,
            p1z,
    ...
    var s_fwd = _minkowski_support_gpu[DTYPE, MODEL_SIZE](
        type1,
        p1x,
        p1y,
        p1z,
