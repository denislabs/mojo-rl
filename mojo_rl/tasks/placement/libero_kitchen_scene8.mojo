"""`libero_kitchen_scene8`'s device placement table — GENERATED, DO NOT EDIT.

Regenerate with:  pixi run gen-placement-tables
CI checks it with: pixi run gen-placement-tables --check

From `mojo_rl/tasks/families/libero_kitchen_scene8.family`,
`mojo_rl/tasks/scenes/libero_kitchen_scene8.xml` and forward kinematics on it.
2 free slots, 4 regions, 0 of them moving.
...
struct LiberoKitchenScene8Placement(PlacementTable):
    ...
    def region_has_rect(r: Int) -> Bool:
        ...
        if r == 2:
            return True  # kitchen_table_moka_pot_right_init_region
        return True  # kitchen_table_moka_pot_left_init_region
