"""Pong pixel-obs trajectory buffer for LeWM offline training.
...
struct PongBuffer(Movable, LeWMBuffer):
    ...
    @staticmethod
    def load(path: String) raises -> PongBuffer:
        ...
        if len(data) < PONG_BUFFER_HEADER_BYTES:
            raise Error("PongBuffer.load: file too small for header")

        # Magic
        for i in range(4):
            if data[i] != UInt8(ord(PONG_BUFFER_MAGIC[byte=i])):
                raise Error("PongBuffer.load: bad magic")

        var version = _read_uint32_le(data, 4)
        if version != PONG_BUFFER_VERSION:
            raise Error("PongBuffer.load: unsupported version")
        ...
        if (
            ...
            raise Error(
                "PongBuffer.load: shape mismatch — expected"
        ...
        if len(data) < expected_size:
            raise Error("PongBuffer.load: file truncated")

        var buf = PongBuffer(capacity=n_frames)
