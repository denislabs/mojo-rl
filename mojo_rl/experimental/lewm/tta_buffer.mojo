...
struct TTAWindowBuffer[BATCH: Int, T: Int, IMG_DIM: Int, ACT: Int](Movable):
    ...
    ](mut self, b: Int, src: UnsafePointer[Scalar[DT], so]):
    ...
    def fill[
        ...
        self,
        pix_out: UnsafePointer[Scalar[DT], po],
        act_out: UnsafePointer[Scalar[DT], ao],
    ) -> Bool:
        ...
        for b in range(Self.BATCH):
            ...
            for t in range(Self.T):
                ...
                for i in range(Self.IMG_DIM):
                    pix_out[fdst + i] = self.frames[fsrc + i]
                var asrc = (s * Self.T + slot) * Self.ACT
                var adst = (b * Self.T + t) * Self.ACT
                for i in range(Self.ACT):
                    act_out[adst + i] = self.actions[asrc + i]
        return True
