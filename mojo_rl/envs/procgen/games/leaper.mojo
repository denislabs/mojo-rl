...
struct LeaperGame(Copyable, Movable, ProcgenGame):
    # ─── ProcgenGame conformance glue (see games/procgen_env.mojo) ──────
    comptime AssetsT = LeaperAssets
    comptime DEFAULT_DIST = DIST_EASY
    comptime GYM_MAX_STEPS = 500

    @staticmethod
    def load_assets(asset_root: String) raises -> LeaperAssets:
        return LeaperAssets(asset_root)

    @staticmethod
    def make(assets: ArcPointer[LeaperAssets], dist_mode: Int) -> Self:
    ...
    def pg_render_obs(self, assets: LeaperAssets) -> List[UInt8]:
        return self.render_obs(assets)

    def pg_render_obs_train(
        self, assets: LeaperAssets, res: Int, ss: Int
    ) -> List[UInt8]:
        return self.render_obs(assets, res, ss)

    def pg_render(self, assets: LeaperAssets, res: Int) -> List[UInt8]:
