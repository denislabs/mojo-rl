"""`BossfightEnv` — bossfight as a Procgen benchmark environment.

Wraps `BossfightGame` with `LevelScheduler`. 64×64×3 RGB observation, 15-action
discrete space (0..8 move; 9..14 fire/special). Assets shared read-only via
`ArcPointer[BossfightAssets]`, passed into the renderer. Mirrors `PlunderEnv`.
...
from .bossfight import BossfightGame, BossfightAssets, DIST_EASY
from .maze_env import StepResult


struct BossfightEnv(Copyable, Movable):
    ...
    var game: BossfightGame
    var assets: ArcPointer[BossfightAssets]
    ...
    def __init__(
        ...
        self.game = BossfightGame(dist_mode)
        self.assets = assets
