"""`StarpilotEnv` — starpilot as a Procgen benchmark environment.

Wraps `StarpilotGame` with `LevelScheduler`. 64×64×3 RGB observation, 15-action
discrete space (0..8 move; 9..14 fire/special). Assets shared read-only via
`ArcPointer[StarpilotAssets]`, passed into the renderer. Mirrors `ChaserEnv`.
...
from .starpilot import StarpilotGame, StarpilotAssets, DIST_EASY
from .maze_env import StepResult


struct StarpilotEnv(Copyable, Movable):
    ...
    var game: StarpilotGame
    var assets: ArcPointer[StarpilotAssets]
    ...
    def __init__(
        ...
        self.game = StarpilotGame(dist_mode)
        self.assets = assets
