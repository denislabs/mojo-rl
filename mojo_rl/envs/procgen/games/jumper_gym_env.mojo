"""`JumperGymEnv` — `BoxDiscreteActionEnv` adapter for training jumper.

Wraps `JumperEnv` for the deep_agents off-policy discrete driver: normalized
...
from .jumper import DIST_EASY, JumperAssets


@fieldwise_init
struct JumperState(State):
    var index: Int
...
@fieldwise_init
struct JumperAction(Action):
    var value: Int


struct JumperGymEnv[DTYPE: DType = DT](
    BoxDiscreteActionEnv & Movable & ImplicitlyDeletable
):
    comptime dtype = Self.DTYPE
    comptime StateType = JumperState
    comptime ActionType = JumperAction
    ...
    def __init__(
        ...
        self = JumperGymEnv[Self.DTYPE](
            ArcPointer(JumperAssets(asset_root)), rand_seed, num_levels, start_level, dist_mode
        )

    def __init__(
        out self, assets: ArcPointer[JumperAssets], rand_seed: Int = 0, num_levels: Int = 1,
        start_level: Int = 0, dist_mode: Int = DIST_EASY,
    ):
        self.inner = JumperEnv(assets, rand_seed, num_levels, start_level, dist_mode)
        self.steps = 0
    ...
    def reset(mut self) -> JumperState:
        _ = self.reset_obs_list()
        return JumperState(0)

    def step(
        mut self, action: JumperAction, verbose: Bool = False
    ) -> Tuple[JumperState, Scalar[Self.DTYPE], Bool]:
        var res = self.step_obs(action.value)
        return (JumperState(0), res[1], res[2])

    def get_state(self) -> JumperState:
        return JumperState(0)
    ...
    def action_from_index(self, action_idx: Int) -> JumperAction:
        return JumperAction(action_idx)
