"""`NinjaGymEnv` — `BoxDiscreteActionEnv` adapter for training ninja.

Wraps `NinjaEnv` for the deep_agents off-policy discrete driver: normalized
...
from .ninja import DIST_EASY, NinjaAssets


@fieldwise_init
struct NinjaState(State):
    var index: Int
...
@fieldwise_init
struct NinjaAction(Action):
    var value: Int


struct NinjaGymEnv[DTYPE: DType = DT](
    BoxDiscreteActionEnv & Movable & ImplicitlyDeletable
):
    comptime dtype = Self.DTYPE
    comptime StateType = NinjaState
    comptime ActionType = NinjaAction
    ...
    def __init__(
        ...
        self = NinjaGymEnv[Self.DTYPE](
            ArcPointer(NinjaAssets(asset_root)), rand_seed, num_levels, start_level, dist_mode
        )

    def __init__(
        out self, assets: ArcPointer[NinjaAssets], rand_seed: Int = 0, num_levels: Int = 1,
        start_level: Int = 0, dist_mode: Int = DIST_EASY,
    ):
        self.inner = NinjaEnv(assets, rand_seed, num_levels, start_level, dist_mode)
        self.steps = 0
    ...
    def reset(mut self) -> NinjaState:
        _ = self.reset_obs_list()
        return NinjaState(0)

    def step(
        mut self, action: NinjaAction, verbose: Bool = False
    ) -> Tuple[NinjaState, Scalar[Self.DTYPE], Bool]:
        var res = self.step_obs(action.value)
        return (NinjaState(0), res[1], res[2])
    ...
    def action_from_index(self, action_idx: Int) -> NinjaAction:
        return NinjaAction(action_idx)
