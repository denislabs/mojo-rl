"""`StarpilotGymEnv` — `BoxDiscreteActionEnv` adapter for training starpilot.

Wraps `StarpilotEnv` for the deep_agents off-policy discrete driver: emits a
normalized `3×84×84` NCHW float observation (Nature-CNN input; the canonical
64×64 obs stays on `StarpilotEnv` for eval/human) and a 15-action discrete space.
...
selection (train/test split) is handled by the wrapped `StarpilotEnv` scheduler.
...
from .starpilot import DIST_EASY, StarpilotAssets


@fieldwise_init
struct StarpilotState(State):
    var index: Int
...
@fieldwise_init
struct StarpilotAction(Action):
    var value: Int


struct StarpilotGymEnv[DTYPE: DType = DT](
    BoxDiscreteActionEnv & Movable & ImplicitlyDeletable
):
    comptime dtype = Self.DTYPE
    comptime StateType = StarpilotState
    comptime ActionType = StarpilotAction
    ...
    comptime MAX_STEPS = 1000  # base timeout (finish spawns at 500, drifts to ship)

    var inner: StarpilotEnv
    var steps: Int
    ...
    def __init__(
        ...
        self = StarpilotGymEnv[Self.DTYPE](
            ArcPointer(StarpilotAssets(asset_root)),
            rand_seed,
            num_levels,
            start_level,
            dist_mode,
    ...
    def __init__(
        out self,
        assets: ArcPointer[StarpilotAssets],
        rand_seed: Int = 0,
        num_levels: Int = 1,
        start_level: Int = 0,
        dist_mode: Int = DIST_EASY,
    ...
    # --- Env ---
    def reset(mut self) -> StarpilotState:
        _ = self.reset_obs_list()
        return StarpilotState(0)

    def step(
        mut self, action: StarpilotAction, verbose: Bool = False
    ) -> Tuple[StarpilotState, Scalar[Self.DTYPE], Bool]:
        var res = self.step_obs(action.value)
        return (StarpilotState(0), res[1], res[2])

    def get_state(self) -> StarpilotState:
        return StarpilotState(0)
    ...
    # --- DiscreteActionEnv ---
    def action_from_index(self, action_idx: Int) -> StarpilotAction:
        return StarpilotAction(action_idx)
