"""`FruitbotGymEnv` — `BoxDiscreteActionEnv` adapter for training fruitbot.

Wraps `FruitbotEnv` for the deep_agents off-policy discrete driver: emits a
normalized `3×84×84` NCHW float observation (Nature-CNN input; the canonical
64×64 obs stays on `FruitbotEnv` for eval/human) and a 15-action discrete space.
...
selection (train/test split) is handled by the wrapped `FruitbotEnv` scheduler.
...
from .fruitbot import DIST_EASY, FruitbotAssets


@fieldwise_init
struct FruitbotState(State):
    var index: Int
...
@fieldwise_init
struct FruitbotAction(Action):
    var value: Int


struct FruitbotGymEnv[DTYPE: DType = DT](
    BoxDiscreteActionEnv & Movable & ImplicitlyDeletable
):
    comptime dtype = Self.DTYPE
    comptime StateType = FruitbotState
    comptime ActionType = FruitbotAction
    ...
    comptime MAX_STEPS = 1000  # scroll episode length

    var inner: FruitbotEnv
    var steps: Int
    ...
    def __init__(
        ...
        self = FruitbotGymEnv[Self.DTYPE](
    ...
    # --- Env ---
    def reset(mut self) -> FruitbotState:
        _ = self.reset_obs_list()
        return FruitbotState(0)

    def step(
        mut self, action: FruitbotAction, verbose: Bool = False
    ) -> Tuple[FruitbotState, Scalar[Self.DTYPE], Bool]:
        var res = self.step_obs(action.value)
        return (FruitbotState(0), res[1], res[2])

    def get_state(self) -> FruitbotState:
        return FruitbotState(0)
    ...
    # --- DiscreteActionEnv ---
    def action_from_index(self, action_idx: Int) -> FruitbotAction:
        return FruitbotAction(action_idx)
