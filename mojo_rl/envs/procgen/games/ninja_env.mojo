"""`NinjaEnv` — ninja as a Procgen benchmark environment.

Wraps `NinjaGame` with `LevelScheduler`. 64×64×3 RGB observation, 15-action
discrete space (0..8 move+jump). Assets shared read-only via
`ArcPointer[NinjaAssets]`. Mirrors `CoinrunEnv`. See `docs/PROCGEN_CLIMBER_SCOPE.md`.
"""

from std.memory import ArcPointer

from ..core.level_scheduler import LevelScheduler
from .ninja import NinjaGame, NinjaAssets, DIST_EASY
from .maze_env import StepResult


struct NinjaEnv(Copyable, Movable):
    ...
    var game: NinjaGame
    var assets: ArcPointer[NinjaAssets]
    ...
    def __init__(
        out self, assets: ArcPointer[NinjaAssets], rand_seed: Int = 0, num_levels: Int = 0,
        start_level: Int = 0, dist_mode: Int = DIST_EASY,
    ):
        self.scheduler = LevelScheduler(rand_seed, num_levels, start_level)
        self.game = NinjaGame(dist_mode)
        self.assets = assets
