"""`ClimberEnv` — climber as a Procgen benchmark environment.

Wraps `ClimberGame` with `LevelScheduler`. 64×64×3 RGB observation, 15-action
discrete space (0..8 move+jump). Assets shared read-only via
`ArcPointer[ClimberAssets]`. Mirrors `CoinrunEnv`. See `docs/PROCGEN_CLIMBER_SCOPE.md`.
"""

from std.memory import ArcPointer

from ..core.level_scheduler import LevelScheduler
from .climber import ClimberGame, ClimberAssets, DIST_EASY
from .maze_env import StepResult


struct ClimberEnv(Copyable, Movable):
    ...
    var game: ClimberGame
    var assets: ArcPointer[ClimberAssets]
    ...
    def __init__(
        out self, assets: ArcPointer[ClimberAssets], rand_seed: Int = 0, num_levels: Int = 0,
        start_level: Int = 0, dist_mode: Int = DIST_EASY,
    ):
        self.scheduler = LevelScheduler(rand_seed, num_levels, start_level)
        self.game = ClimberGame(dist_mode)
        self.assets = assets
