"""`BossfightGymEnv` — `BoxDiscreteActionEnv` adapter for training bossfight.

Wraps `BossfightEnv` for the deep_agents off-policy discrete driver: emits a
normalized `3×84×84` NCHW float observation (Nature-CNN input; the canonical
64×64 obs stays on `BossfightEnv` for eval/human) and a 15-action discrete space.
`was_terminated()` reports a real terminal (boss killed OR agent hit boss/barrier/
bullet OR flew off-screen) vs. a timeout truncation (`MAX_STEPS`), so value
bootstrapping is correct. Level selection is handled by the wrapped scheduler.

Mirrors `PlunderGymEnv`. See `docs/PROCGEN_BOSSFIGHT_SCOPE.md`.
"""
...
from .bossfight import DIST_EASY, BossfightAssets


@fieldwise_init
struct BossfightState(State):
    var index: Int
...
@fieldwise_init
struct BossfightAction(Action):
    var value: Int


struct BossfightGymEnv[DTYPE: DType = DT](
    BoxDiscreteActionEnv & Movable & ImplicitlyDeletable
):
    comptime dtype = Self.DTYPE
    comptime StateType = BossfightState
    comptime ActionType = BossfightAction
    ...
    comptime MAX_STEPS = 1000  # boss timeout is 4000; deaths usually end sooner

    var inner: BossfightEnv
    var steps: Int
    ...
    def __init__(
        ...
        self = BossfightGymEnv[Self.DTYPE](
            ArcPointer(BossfightAssets(asset_root)),
            rand_seed,
            num_levels,
            start_level,
            dist_mode,
    ...
    def __init__(
        out self,
        assets: ArcPointer[BossfightAssets],
        rand_seed: Int = 0,
        num_levels: Int = 1,
        start_level: Int = 0,
        dist_mode: Int = DIST_EASY,
    ...
    # --- Env ---
    def reset(mut self) -> BossfightState:
        _ = self.reset_obs_list()
        return BossfightState(0)

    def step(
        mut self, action: BossfightAction, verbose: Bool = False
    ) -> Tuple[BossfightState, Scalar[Self.DTYPE], Bool]:
        var res = self.step_obs(action.value)
        return (BossfightState(0), res[1], res[2])

    def get_state(self) -> BossfightState:
        return BossfightState(0)
    ...
    # --- DiscreteActionEnv ---
    def action_from_index(self, action_idx: Int) -> BossfightAction:
        return BossfightAction(action_idx)
    ...
    def step_obs(
        ...
        self.steps += 1
        # Real terminal (win or death) vs. timeout truncation.
