"""`CaveflyerEnv` — caveflyer as a Procgen benchmark environment.

Wraps `CaveflyerGame` with `LevelScheduler`. 64×64×3 RGB observation, 15-action
discrete space (0..8 rotate/thrust; 9..14 fire/special). Assets shared read-only
via `ArcPointer[CaveflyerAssets]`, passed into the renderer. Mirrors `CoinrunEnv`.
...
from .caveflyer import CaveflyerGame, CaveflyerAssets, DIST_EASY
from .maze_env import StepResult


struct CaveflyerEnv(Copyable, Movable):
    ...
    var game: CaveflyerGame
    var assets: ArcPointer[CaveflyerAssets]
    ...
    def __init__(
        ...
        self.game = CaveflyerGame(dist_mode)
        self.assets = assets
