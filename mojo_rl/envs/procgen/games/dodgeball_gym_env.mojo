"""`DodgeballGymEnv` — `BoxDiscreteActionEnv` adapter for training dodgeball.

Wraps `DodgeballEnv` for the deep_agents off-policy discrete driver: normalized
...
from .dodgeball import DIST_EASY, DodgeballAssets


@fieldwise_init
struct DodgeballState(State):
    var index: Int
...
@fieldwise_init
struct DodgeballAction(Action):
    var value: Int


struct DodgeballGymEnv[DTYPE: DType = DT](
    BoxDiscreteActionEnv & Movable & ImplicitlyDeletable
):
    comptime dtype = Self.DTYPE
    comptime StateType = DodgeballState
    comptime ActionType = DodgeballAction
    ...
    def __init__(
        ...
        self = DodgeballGymEnv[Self.DTYPE](
            ArcPointer(DodgeballAssets(asset_root)), rand_seed, num_levels, start_level, dist_mode
        )

    def __init__(
        out self, assets: ArcPointer[DodgeballAssets], rand_seed: Int = 0, num_levels: Int = 1,
        start_level: Int = 0, dist_mode: Int = DIST_EASY,
    ):
        self.inner = DodgeballEnv(assets, rand_seed, num_levels, start_level, dist_mode)
        self.steps = 0
    ...
    def reset(mut self) -> DodgeballState:
        _ = self.reset_obs_list()
        return DodgeballState(0)

    def step(
        mut self, action: DodgeballAction, verbose: Bool = False
    ) -> Tuple[DodgeballState, Scalar[Self.DTYPE], Bool]:
        var res = self.step_obs(action.value)
        return (DodgeballState(0), res[1], res[2])

    def get_state(self) -> DodgeballState:
        return DodgeballState(0)
    ...
    def action_from_index(self, action_idx: Int) -> DodgeballAction:
        return DodgeballAction(action_idx)
