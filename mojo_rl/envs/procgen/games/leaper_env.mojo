"""`LeaperEnv` — leaper as a Procgen benchmark environment.

Wraps `LeaperGame` with `LevelScheduler`. 64×64×3 RGB observation, 15-action
discrete space. Assets shared read-only via `ArcPointer[LeaperAssets]`, passed into
the renderer. Mirrors `ChaserEnv`. See `docs/PROCGEN_LEAPER_SCOPE.md`.
"""

from std.memory import ArcPointer

from ..core.level_scheduler import LevelScheduler
from .leaper import LeaperGame, LeaperAssets, DIST_EASY
from .maze_env import StepResult


struct LeaperEnv(Copyable, Movable):
    ...
    var game: LeaperGame
    var assets: ArcPointer[LeaperAssets]
    ...
    def __init__(
        ...
        self.game = LeaperGame(dist_mode)
        self.assets = assets
