"""`PlunderGymEnv` — `BoxDiscreteActionEnv` adapter for training plunder.

Wraps `PlunderEnv` for the deep_agents off-policy discrete driver: emits a
normalized `3×84×84` NCHW float observation (Nature-CNN input; the canonical
64×64 obs stays on `PlunderEnv` for eval/human) and a 15-action discrete space.
...
selection (train/test split) is handled by the wrapped `PlunderEnv` scheduler.
...
from .plunder import DIST_EASY, PlunderAssets


@fieldwise_init
struct PlunderState(State):
    var index: Int
...
@fieldwise_init
struct PlunderAction(Action):
    var value: Int


struct PlunderGymEnv[DTYPE: DType = DT](
    BoxDiscreteActionEnv & Movable & ImplicitlyDeletable
):
    comptime dtype = Self.DTYPE
    comptime StateType = PlunderState
    comptime ActionType = PlunderAction
    ...
    comptime MAX_STEPS = 1000  # juice loss usually ends episodes sooner (timeout 4000)

    var inner: PlunderEnv
    var steps: Int
    ...
    def __init__(
        ...
        self = PlunderGymEnv[Self.DTYPE](
            ArcPointer(PlunderAssets(asset_root)),
            rand_seed,
            num_levels,
            start_level,
            dist_mode,
    ...
    def __init__(
        out self,
        assets: ArcPointer[PlunderAssets],
        rand_seed: Int = 0,
        num_levels: Int = 1,
        start_level: Int = 0,
        dist_mode: Int = DIST_EASY,
    ...
    # --- Env ---
    def reset(mut self) -> PlunderState:
        _ = self.reset_obs_list()
        return PlunderState(0)

    def step(
        mut self, action: PlunderAction, verbose: Bool = False
    ) -> Tuple[PlunderState, Scalar[Self.DTYPE], Bool]:
        var res = self.step_obs(action.value)
        return (PlunderState(0), res[1], res[2])

    def get_state(self) -> PlunderState:
        return PlunderState(0)
    ...
    # --- DiscreteActionEnv ---
    def action_from_index(self, action_idx: Int) -> PlunderAction:
        return PlunderAction(action_idx)
