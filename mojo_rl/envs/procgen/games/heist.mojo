...
struct HeistGame(Copyable, Movable, ProcgenGame):
    # ─── ProcgenGame conformance glue (see games/procgen_env.mojo) ──────
    comptime AssetsT = HeistAssets
    comptime DEFAULT_DIST = DIST_EASY
    comptime GYM_MAX_STEPS = 500

    @staticmethod
    def load_assets(asset_root: String) raises -> HeistAssets:
        return HeistAssets(asset_root)

    @staticmethod
    def make(assets: ArcPointer[HeistAssets], dist_mode: Int) -> Self:
    ...
    def pg_render_obs(self, assets: HeistAssets) -> List[UInt8]:
        return self.render_obs(assets)

    def pg_render_obs_train(
        self, assets: HeistAssets, res: Int, ss: Int
    ) -> List[UInt8]:
        return self.render_obs(assets, res, ss)

    def pg_render(self, assets: HeistAssets, res: Int) -> List[UInt8]:
