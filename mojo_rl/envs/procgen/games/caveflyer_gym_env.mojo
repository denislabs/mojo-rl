"""`CaveflyerGymEnv` — `BoxDiscreteActionEnv` adapter for training caveflyer.

Wraps `CaveflyerEnv` for the deep_agents off-policy discrete driver: emits a
normalized `3×84×84` NCHW float observation (Nature-CNN input; the canonical
64×64 obs stays on `CaveflyerEnv` for eval/human) and a 15-action discrete space.
`was_terminated()` reports a real terminal (goal reached OR death: obstacle/enemy/
target-touch/cave-wall crash) vs a timeout truncation (`MAX_STEPS`), so value
bootstrapping is correct. Level selection is handled by the wrapped scheduler.

Mirrors `CoinrunGymEnv`. See `docs/PROCGEN_CAVEFLYER_SCOPE.md`.
"""
...
from .caveflyer import DIST_EASY, CaveflyerAssets


@fieldwise_init
struct CaveflyerState(State):
    var index: Int
...
@fieldwise_init
struct CaveflyerAction(Action):
    var value: Int


struct CaveflyerGymEnv[DTYPE: DType = DT](
    BoxDiscreteActionEnv & Movable & ImplicitlyDeletable
):
    comptime dtype = Self.DTYPE
    comptime StateType = CaveflyerState
    comptime ActionType = CaveflyerAction
    ...
    comptime MAX_STEPS = 1000  # caveflyer timeout; deaths usually end sooner

    var inner: CaveflyerEnv
    var steps: Int
    ...
    def __init__(
        ...
        self = CaveflyerGymEnv[Self.DTYPE](
            ArcPointer(CaveflyerAssets(asset_root)),
            rand_seed,
            num_levels,
            start_level,
            dist_mode,
    ...
    def __init__(
        out self,
        assets: ArcPointer[CaveflyerAssets],
        rand_seed: Int = 0,
        num_levels: Int = 1,
        start_level: Int = 0,
        dist_mode: Int = DIST_EASY,
    ...
    # --- Env ---
    def reset(mut self) -> CaveflyerState:
        _ = self.reset_obs_list()
        return CaveflyerState(0)

    def step(
        mut self, action: CaveflyerAction, verbose: Bool = False
    ) -> Tuple[CaveflyerState, Scalar[Self.DTYPE], Bool]:
        var res = self.step_obs(action.value)
        return (CaveflyerState(0), res[1], res[2])

    def get_state(self) -> CaveflyerState:
        return CaveflyerState(0)
    ...
    # --- DiscreteActionEnv ---
    def action_from_index(self, action_idx: Int) -> CaveflyerAction:
        return CaveflyerAction(action_idx)
