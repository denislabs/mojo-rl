"""`HeistGymEnv` — `BoxDiscreteActionEnv` adapter for training heist.

Wraps `HeistEnv` for the deep_agents off-policy discrete driver: normalized
`3×84×84` NCHW float observation + 15-action discrete space. `was_terminated()`
reports a real terminal (reached the gem) vs. a timeout truncation. Heist reward
is sparse (+10 at the exit only) — expect maze-like difficulty. Mirrors
`ChaserGymEnv`. See `docs/PROCGEN_HEIST_SCOPE.md`.
"""
...
from .heist import DIST_EASY, HeistAssets


@fieldwise_init
struct HeistState(State):
    var index: Int
...
@fieldwise_init
struct HeistAction(Action):
    var value: Int


struct HeistGymEnv[DTYPE: DType = DT](
    BoxDiscreteActionEnv & Movable & ImplicitlyDeletable
):
    comptime dtype = Self.DTYPE
    comptime StateType = HeistState
    comptime ActionType = HeistAction
    ...
    comptime MAX_STEPS = 500  # timeout → truncation
    ...
    def __init__(
        ...
        self = HeistGymEnv[Self.DTYPE](
            ArcPointer(HeistAssets(asset_root)),
            rand_seed,
            num_levels,
            start_level,
            dist_mode,
    ...
    # --- Env ---
    def reset(mut self) -> HeistState:
        _ = self.reset_obs_list()
        return HeistState(0)

    def step(
        mut self, action: HeistAction, verbose: Bool = False
    ) -> Tuple[HeistState, Scalar[Self.DTYPE], Bool]:
        var res = self.step_obs(action.value)
        return (HeistState(0), res[1], res[2])
    ...
    # --- DiscreteActionEnv ---
    def action_from_index(self, action_idx: Int) -> HeistAction:
        return HeistAction(action_idx)
