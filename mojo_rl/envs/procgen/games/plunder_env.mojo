"""`PlunderEnv` — plunder as a Procgen benchmark environment.

Wraps `PlunderGame` with `LevelScheduler`. 64×64×3 RGB observation, 15-action
discrete space (0..8 move; 9..14 fire/special). Assets shared read-only via
`ArcPointer[PlunderAssets]`, passed into the renderer. Mirrors `ChaserEnv`.
...
from .plunder import PlunderGame, PlunderAssets, DIST_EASY
from .maze_env import StepResult


struct PlunderEnv(Copyable, Movable):
    ...
    var game: PlunderGame
    var assets: ArcPointer[PlunderAssets]
    ...
    def __init__(
        ...
        self.game = PlunderGame(dist_mode)
        self.assets = assets
