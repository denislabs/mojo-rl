"""`BigfishEnv` — bigfish as a Procgen benchmark environment.

Wraps `BigfishGame` with `LevelScheduler` (train/test level selection). 64×64×3 RGB
observation, 15-action discrete space. Assets shared read-only via
`ArcPointer[BigfishAssets]`, passed into the renderer. Mirrors `ChaserEnv`.
...
from .bigfish import BigfishGame, BigfishAssets, DIST_EASY
from .maze_env import StepResult


struct BigfishEnv(Copyable, Movable):
    ...
    var game: BigfishGame
    var assets: ArcPointer[BigfishAssets]
    ...
    def __init__(
        ...
        self.game = BigfishGame(dist_mode)
        self.assets = assets
