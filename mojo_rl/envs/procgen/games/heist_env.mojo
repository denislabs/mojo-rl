"""`HeistEnv` — heist as a Procgen benchmark environment.

Wraps `HeistGame` (one level) with `LevelScheduler` (train/test level selection):
each `reset()` samples a `current_level_seed` and regenerates the level. 64×64×3
RGB observation, 15-action discrete space (0..8 move incl diagonals; 9..14 no-op).
Assets shared read-only via `ArcPointer[HeistAssets]`, passed into the renderer.
Mirrors `ChaserEnv`. See `docs/PROCGEN_HEIST_SCOPE.md`.
"""

from std.memory import ArcPointer

from ..core.level_scheduler import LevelScheduler
from .heist import HeistGame, HeistAssets, DIST_EASY
from .maze_env import StepResult


struct HeistEnv(Copyable, Movable):
    ...
    var game: HeistGame
    var assets: ArcPointer[HeistAssets]
