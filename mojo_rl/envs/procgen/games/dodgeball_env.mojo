"""`DodgeballEnv` — dodgeball as a Procgen benchmark environment.

Wraps `DodgeballGame` with `LevelScheduler`. 64×64×3 RGB observation, 15-action
discrete space (0..8 move+jump). Assets shared read-only via
`ArcPointer[DodgeballAssets]`. Mirrors `CoinrunEnv`. See `docs/PROCGEN_CLIMBER_SCOPE.md`.
"""

from std.memory import ArcPointer

from ..core.level_scheduler import LevelScheduler
from .dodgeball import DodgeballGame, DodgeballAssets, DIST_EASY
from .maze_env import StepResult


struct DodgeballEnv(Copyable, Movable):
    ...
    var game: DodgeballGame
    var assets: ArcPointer[DodgeballAssets]
    var current_level_seed: Int

    def __init__(
        out self, asset_root: String, rand_seed: Int = 0, num_levels: Int = 0,
    ...
    def __init__(
        out self, assets: ArcPointer[DodgeballAssets], rand_seed: Int = 0, num_levels: Int = 0,
        start_level: Int = 0, dist_mode: Int = DIST_EASY,
    ):
        self.scheduler = LevelScheduler(rand_seed, num_levels, start_level)
        self.game = DodgeballGame(dist_mode)
        self.assets = assets
    ...
    def step(mut self, action: Int) -> StepResult:
        ...
        return StepResult(
            self.game.render_obs(self.assets[]), reward, self.game.done, self.game.level_complete
        )
