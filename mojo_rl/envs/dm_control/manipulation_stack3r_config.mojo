"""`dm_control` `manipulation/stack_3_bricks_random_order_features` config.

Port of `bricks.py::Stack` with three bricks, a fixed base and — the new part —
`randomize_order=True`.

    observation = desired_order(3) + robot(42) + 3 x brick(13)           (84)
    reward      = mean over the two stacked pairs of a stud-to-hole shaping
...
comptime TARGET_HEIGHT: Int = 3  # `_stack(target_height=None)` -> num_bricks
...
struct Stack3RandomConfig(Phyics3dEnvConfig):
    ...
    def custom_extract_obs_cpu[
        ...
        """`desired_order` (3), robot (42), then the bricks in REFERENCE order.

        ⚠⚠ `desired_order` LEADS. It is a TASK observable and composer emits
        those before any entity's, so it is the first three numbers — not the
        last, and not adjacent to the brick blocks it refers to.

        ⚠⚠ THE BRICK BLOCKS ARE EMITTED THROUGH `sigma`. Reference brick `r`
        is our physical brick `sigma(r)`, so `block(sigma(0))` comes first even
        though that may be the brick our model keeps at qpos 16. Emitting them
        in physical order compiles, runs, and hands the policy three shuffled
        13-vectors that disagree with `desired_order`.
        """
        try:
            var order = read_order[DTYPE, NQ, NV, NBODY, MAX_CONTACTS, NSITE](
                d
            )
            for i in range(N_BRICKS):
    ...
    def compute_reward_and_done_cpu[
        ...
        """`Stack.get_reward` — the MEAN over `target_height - 1` pairs.

        ⚠ `pairs = zip(order[:-1], order[1:])` and the mean is over PAIRS, so
        with three bricks it is two terms averaged, not a sum and not the
        worst. Each pair is the lower brick's STUDS against the upper brick's
        HOLES, through `sigma`.
        """
        ...
        return (
            Scalar[DTYPE](total / Float64(TARGET_HEIGHT - 1)),
            False,
        )
    ...
    def custom_reset_cpu[
        ...
        ⚠ The order is drawn HERE rather than in the full hook because it must
        be in `Data.meta` before anything reads an observation, and because a
        reset that failed the IK would otherwise leave a stale order behind.
        """
        ...
        # `random_state.choice(3, size=3, replace=False)` — a permutation.
        # Fisher-Yates on [0, 1, 2]; the reference's bit stream cannot be
        # reproduced, so what matters is that every permutation is reachable
        # (the gate injects its own orders rather than comparing draws).
        var order = InlineArray[Int, N_BRICKS](fill=0)
        for i in range(N_BRICKS):
            order[i] = i
    ...
    def custom_reset_full_cpu[
        ...
        # ── 1. place all three, in REFERENCE index order ────────────────
        # ⚠ `PropPlacer` walks `self._props`, which is `self._bricks`, i.e.
        # reference order — and the contact-disabling pass makes that visible:
        # at step r every brick NOT YET PLACED is invisible to collision, and
        # "not yet placed" is defined by that order, not by ours.
        for r in range(N_BRICKS):
            var phys = sigma[r]
            var ignore = List[Int]()
            ...
            if phys == FIXED_BRICK:
                ...
                if not rf.success:
                    raise Error(
                        "stack_3_random: the base brick found no clear pose in "
                        + String(rf.attempts)
                        + " attempts"
                    )
            else:
        ...
        # ── 2. settle every free brick at once ──────────────────────────
        # ⚠ ALL of them, not one at a time: the reference's tolerance is a max
        # over every prop joint, so the scene settles when the LAST brick does.
        var dofs = List[Int]()
        ...
        # ⚠ THE BODY CLASSES DO NOT DEPEND ON `sigma`. dm_control asks whether
        # a body's top-level body carries a freejoint, and in OUR model that is
        # a fixed fact: `FIXED_BRICK` never has one. So the fixed brick is
        # always BODY_FIXED and the other two always BODY_FREE, whatever the
        # episode's order — the relabeling moves which LOGICAL brick sits
        # there, not which physical one is welded.
        ...
        if not res.success:
            raise Error(
                "stack_3_random: the TCP initializer exhausted "
                + String(res.samples)
                + " samples ("
