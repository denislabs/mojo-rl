...
struct SystemCursor(Indexer, Intable, TrivialRegisterPassable):
    ...
    comptime SYSTEM_CURSOR_TEXT = Self(1)
    """Text selection. Usually an I-beam."""
    comptime SYSTEM_CURSOR_WAIT = Self(2)
    """Wait. Usually an hourglass or watch or spinning ball."""
    comptime SYSTEM_CURSOR_CROSSHAIR = Self(3)
    """Crosshair."""
    comptime SYSTEM_CURSOR_PROGRESS = Self(4)
    """Program is busy but still interactive. Usually it's WAIT with an arrow."""
    comptime SYSTEM_CURSOR_NWSE_RESIZE = Self(5)
    """Double arrow pointing northwest and southeast."""
    comptime SYSTEM_CURSOR_NESW_RESIZE = Self(6)
    """Double arrow pointing northeast and southwest."""
    comptime SYSTEM_CURSOR_EW_RESIZE = Self(7)
    """Double arrow pointing west and east."""
    comptime SYSTEM_CURSOR_NS_RESIZE = Self(8)
    """Double arrow pointing north and south."""
    comptime SYSTEM_CURSOR_MOVE = Self(9)
    """Four pointed arrow pointing north, south, east, and west."""
    comptime SYSTEM_CURSOR_NOT_ALLOWED = Self(10)
    """Not permitted. Usually a slashed circle or crossbones."""
    comptime SYSTEM_CURSOR_POINTER = Self(11)
    """Pointer that indicates a link. Usually a pointing hand."""
    comptime SYSTEM_CURSOR_NW_RESIZE = Self(12)
    """Window resize top-left. This may be a single arrow or a double arrow like NWSE_RESIZE."""
    comptime SYSTEM_CURSOR_N_RESIZE = Self(13)
    """Window resize top. May be NS_RESIZE."""
    comptime SYSTEM_CURSOR_NE_RESIZE = Self(14)
    """Window resize top-right. May be NESW_RESIZE."""
    comptime SYSTEM_CURSOR_E_RESIZE = Self(15)
    """Window resize right. May be EW_RESIZE."""
    comptime SYSTEM_CURSOR_SE_RESIZE = Self(16)
    """Window resize bottom-right. May be NWSE_RESIZE."""
    comptime SYSTEM_CURSOR_S_RESIZE = Self(17)
    """Window resize bottom. May be NS_RESIZE."""
    comptime SYSTEM_CURSOR_SW_RESIZE = Self(18)
    """Window resize bottom-left. May be NESW_RESIZE."""
    comptime SYSTEM_CURSOR_W_RESIZE = Self(19)
...
struct MouseWheelDirection(Indexer, Intable, TrivialRegisterPassable):
    ...
    comptime MOUSEWHEEL_FLIPPED = Self(1)
...
struct MouseButtonFlags(Intable, TrivialRegisterPassable):
    ...
    comptime BUTTON_MIDDLE = Self(2)
    comptime BUTTON_RIGHT = Self(3)
    comptime BUTTON_X1 = Self(4)
...
def has_mouse() raises -> Bool:
    ...
    return _get_dylib_function[lib, "SDL_HasMouse", def() -> Bool]()()
...
def get_mice(
    ...
    ret = _get_dylib_function[
        lib,
        "SDL_GetMice",
        def(count: Ptr[c_int, MutAnyOrigin]) -> Ptr[MouseID, MutAnyOrigin],
    ]()(count)
    if not ret:
...
def get_mouse_name_for_id(
    ...
    ret = _get_dylib_function[
        ...
        def(instance_id: MouseID) -> Ptr[c_char, ImmutAnyOrigin],
    ]()(instance_id)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
def get_mouse_focus() raises -> Ptr[Window, MutAnyOrigin]:
    ...
    return _get_dylib_function[
        lib, "SDL_GetMouseFocus", def() -> Ptr[Window, MutAnyOrigin]
    ]()()
...
def warp_mouse_in_window(
    ...
    return _get_dylib_function[
        ...
        def(window: Ptr[Window, MutAnyOrigin], x: c_float, y: c_float) -> None,
    ]()(window, x, y)
...
def warp_mouse_global(x: c_float, y: c_float) raises:
    ...
    ret = _get_dylib_function[
        lib, "SDL_WarpMouseGlobal", def(x: c_float, y: c_float) -> Bool
    ]()(x, y)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
def set_window_relative_mouse_mode(
    ...
    ret = _get_dylib_function[
        ...
        def(window: Ptr[Window, MutAnyOrigin], enabled: Bool) -> Bool,
    ]()(window, enabled)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
def capture_mouse(enabled: Bool) raises:
    ...
    ret = _get_dylib_function[
        lib, "SDL_CaptureMouse", def(enabled: Bool) -> Bool
    ]()(enabled)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
def create_cursor(
    ...
    hot_x: c_int,
    hot_y: c_int,
) raises -> Ptr[Cursor, MutAnyOrigin]:
...
def create_color_cursor(
    ...
    hot_x: c_int,
    hot_y: c_int,
    out ret: Ptr[Cursor, MutAnyOrigin],
) raises:
...
def create_system_cursor(
    ...
    ret = _get_dylib_function[
        ...
        def(id: SystemCursor) -> Ptr[Cursor, MutAnyOrigin],
    ]()(id)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
def set_cursor(cursor: Ptr[Cursor, MutAnyOrigin]) raises:
    ...
    ret = _get_dylib_function[
        lib, "SDL_SetCursor", def(cursor: Ptr[Cursor, MutAnyOrigin]) -> Bool
    ]()(cursor)
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
def get_cursor() raises -> Ptr[Cursor, MutAnyOrigin]:
    ...
    return _get_dylib_function[
        lib, "SDL_GetCursor", def() -> Ptr[Cursor, MutAnyOrigin]
    ]()()
...
def get_default_cursor() raises -> Ptr[Cursor, MutAnyOrigin]:
    ...
    return _get_dylib_function[
        lib, "SDL_GetDefaultCursor", def() -> Ptr[Cursor, MutAnyOrigin]
    ]()()
...
def destroy_cursor(cursor: Ptr[Cursor, MutAnyOrigin]) raises -> None:
    ...
    return _get_dylib_function[
        lib,
        "SDL_DestroyCursor",
        def(cursor: Ptr[Cursor, MutAnyOrigin]) -> None,
    ]()(cursor)
...
def show_cursor() raises:
    ...
    ret = _get_dylib_function[lib, "SDL_ShowCursor", def() -> Bool]()()
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
def hide_cursor() raises:
    ...
    ret = _get_dylib_function[lib, "SDL_HideCursor", def() -> Bool]()()
    if not ret:
        raise Error(String(unsafe_from_utf8_ptr=get_error()))
...
def cursor_visible() raises -> Bool:
    ...
    return _get_dylib_function[lib, "SDL_CursorVisible", def() -> Bool]()()
