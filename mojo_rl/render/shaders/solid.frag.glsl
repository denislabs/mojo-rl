#version 450
// GLSL twin of gpu_shaders.mojo:SOLID_FRAGMENT_MSL — keep the two in step.
...
void main() {
    ...
    // Light first, texture second — GL_MODULATE; see the MSL twin.
    vec4 base_color = obj_color;
    float shadow_factor = compute_shadow(world_pos);
    vec3 color = mj_shade(N, V, world_pos, base_color.rgb, mat_specular,
                          mat_shininess, mat_emission, shadow_factor);
    if (has_texture > 0.5) {
        vec2 uv = frag_uv * tex_params.xy;
        if (tex_params.z > 0.5) {
            uv = mj_cube_uv(local_pos);
        }
        vec4 tex_color = texture(obj_texture, uv);
